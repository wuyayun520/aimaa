...
class AimaaCoinProduct {
  final String amountStr;
  final String productId;
  final double price;
  int get amount => int.tryParse(amountStr) ?? 0;
  const AimaaCoinProduct(this.amountStr, this.productId, this.price);
}


class AimaaCoinProducts {
  static const List<AimaaCoinProduct> all = [
    AimaaCoinProduct('32', 'Aimaa', 0.99),
    AimaaCoinProduct('60', 'Aimaa1', 1.99),
    AimaaCoinProduct('96', 'Aimaa2', 2.99),
    AimaaCoinProduct('155', 'Aimaa4', 4.99),
    AimaaCoinProduct('189', 'Aimaa5', 5.99),
    AimaaCoinProduct('359', 'Aimaa9', 9.99),
    AimaaCoinProduct('729', 'Aimaa19', 19.99),
    AimaaCoinProduct('1869', 'Aimaa49', 49.99),
    AimaaCoinProduct('2389', 'Aimaa59', 59.99),
    AimaaCoinProduct('3799', 'Aimaa99', 99.90),
  ];
}
...
class _WalletScreenState extends State<WalletScreen> {
  ...
  List<AimaaCoinProduct> get _coinProducts => AimaaCoinProducts.all;
  ...
  Future<void> _loadCoins() async {
    ...
    int coins = prefs.getInt('aimaaCoins') ?? 
                prefs.getInt('aimaaCoins') ?? 
                prefs.getInt('aimaaCoins') ?? 0;
    debugPrint('Loading Aimaa coins: $coins');
      setState(() {
        _coins = coins;
        if (!_isInitialized) {
          _isInitialized = true;
      ...
      if (prefs.getInt('aimaaCoins') != null || prefs.getInt('aimaaDiamonds') != null) {
        await prefs.setInt('aimaaCoins', coins);
      }
  }

  Future<void> _saveCoins(int amount) async {
    final prefs = await SharedPreferences.getInstance();
    debugPrint('Saving Aimaa coins: current=$_coins, adding=$amount, new=${_coins + amount}');
    debugPrint('Call stack: ${StackTrace.current}');
    setState(() {
      _coins += amount;
    });
    await prefs.setInt('aimaaCoins', _coins);
  ...
  Widget _buildProductCard(AimaaCoinProduct item, String priceStr, bool isSelected) {
