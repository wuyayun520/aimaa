...
class AimaaChatScreen extends StatefulWidget {
  final String userId;
  final String userName;
  final String userAvatar;

  const AimaaChatScreen({
    super.key,
  ...
  @override
  State<AimaaChatScreen> createState() => _AimaaChatScreenState();
}

class _AimaaChatScreenState extends State<AimaaChatScreen> {
  List<_AimaaChatMessage> _messages = [];
  ...
  Future<String> _getMediaDirectory() async {
    ...
    final mediaDir = Directory('${dir.path}/aimaa_chat_media');
  ...
  Future<void> _loadMessages() async {
    final dir = await getApplicationDocumentsDirectory();
    final file = File('${dir.path}/aimaa_chat_history_${widget.userId}.json');
    if (await file.exists()) {
      try {
        ...
        setState(() {
          _messages = jsonList.map((e) => _AimaaChatMessage.fromJson(e)).toList();
        });
  ...
  void _sendMessage() {
    ...
    setState(() {
        _messages.add(_AimaaChatMessage(
        text: text,
        isMe: true,
        time: _getTimeStamp(),
          type: _AimaaChatMessageType.text,
      ));
    });
  ...
  Future<void> _sendImage(File imageFile) async {
    try {
      ...
      final fileName = 'aimaa_image_${DateTime.now().millisecondsSinceEpoch}.jpg';
      ...
      setState(() {
        _messages.add(_AimaaChatMessage(
          ...
          isMe: true,
          time: _getTimeStamp(),
          type: _AimaaChatMessageType.image,
        ));
      });
      _scrollToBottom();
      _saveMessages();
  ...
  Future<void> _sendVoice(String audioPath, Duration duration) async {
    try {
      final mediaDir = await _getMediaDirectory();
      final fileName = 'aimaa_voice_${DateTime.now().millisecondsSinceEpoch}.m4a';
      ...
      setState(() {
        _messages.add(_AimaaChatMessage(
          ...
          isMe: true,
          time: _getTimeStamp(),
          type: _AimaaChatMessageType.audio,
        ));
      });
      _scrollToBottom();
      _saveMessages();
    ...
    try {
      if (_isRecording) {
        ...
        if (await _voiceRecorder.hasPermission()) {
          ...
          final filePath = '${dir.path}/aimaa_voice_${DateTime.now().millisecondsSinceEpoch}.m4a';
...
enum _AimaaChatMessageType { text, image, audio }

class _AimaaChatMessage {
  final String? text;
  ...
  final bool isMe;
  final String time;
  final _AimaaChatMessageType type;
  ...
  static _AimaaChatMessage fromJson(Map<String, dynamic> json) => _AimaaChatMessage(
    text: json['text'],
    ...
    isMe: json['isMe'] ?? true,
    time: json['time'] ?? '',
    type: _AimaaChatMessageType.values.firstWhere(
      (e) => e.name == json['type'],
      orElse: () => _AimaaChatMessageType.text,
    ),
  );
}

class _AimaaChatBubble extends StatefulWidget {
  final _AimaaChatMessage message;
  final void Function(File file)? onImageTap;
  const _AimaaChatBubble({required this.message, this.onImageTap});

  @override
  State<_AimaaChatBubble> createState() => _AimaaChatBubbleState();
}

class _AimaaChatBubbleState extends State<_AimaaChatBubble> {
  ...
  void initState() {
    super.initState();
    if (widget.message.type == _AimaaChatMessageType.audio) {
  ...
  Future<void> _togglePlayPause() async {
    try {
      ...
      final dir = await getApplicationDocumentsDirectory();
      final absPath = '${dir.path}/aimaa_chat_media/${msg.audioPath}';
  ...
  Widget build(BuildContext context) {
    ...
    if (msg.type == _AimaaChatMessageType.audio && msg.audioPath != null) {
    ...
    if (msg.type == _AimaaChatMessageType.image && msg.imagePath != null) {
      return Align(
        alignment: msg.isMe ? Alignment.centerRight : Alignment.centerLeft,
        child: GestureDetector(
          onTap: () async {
            final dir = await getApplicationDocumentsDirectory();
            final absPath = '${dir.path}/aimaa_chat_media/${msg.imagePath}';
                ...
                ClipRRect(
                  ...
                  child: FutureBuilder<Directory>(
                    ...
                    builder: (context, snapshot) {
                      ...
                      final absPath = '${snapshot.data!.path}/aimaa_chat_media/${msg.imagePath}';
                      return Image.file(
                        File(absPath),
                        width: 160,
                        height: 160,
...
class _AimaaChatInputBar extends StatelessWidget {
