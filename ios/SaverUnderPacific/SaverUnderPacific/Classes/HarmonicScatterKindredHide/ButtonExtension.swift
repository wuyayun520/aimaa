...
/*: "recharge" :*/
fileprivate let show_characterUrl:String = "recomponentarge"

/*: "recharge_ :*/
fileprivate let noti_wouldLessValue:[Character] = ["r","e","c","h","a","r","g","e","_"]

/*: ".png" :*/
fileprivate let constWeekName:String = ".pngservice warning unique bot lower"

/*: .png" :*/
fileprivate let show_decisionPictureStr:String = ".pngcharm direction official global minimize"

/*: ".jpg" :*/
fileprivate let dataOrientationRegionName:String = "sale weight deadline mode available.jpg"

/*: .jpg" :*/
fileprivate let k_createPath:String = ".jpgreply receive"
...
enum WhenResultAdjust: NSInteger {
    //: case left = 0
    case left = 0
    //: case top
    case top
...
class DistinctionButton: UIButton {
    ...
    var imageAlignment: WhenResultAdjust = .left
...
extension UIButton {
    ...
    func darkPermanent(_ interval: CGFloat) {
        //: verticalAlignImageAndTitle(interval: interval, topOffset: 0)
        wire(interval: interval, topOffset: 0)
    }
    ...
    func wire(interval: CGFloat, topOffset: CGFloat) {
    ...
    func evolutionPrepare(interval: CGFloat, imageSize: CGSize, topOffset: CGFloat) {
...
extension UIButton {
    ...
    func faceCrop(color: UIColor, forState: UIControl.State) {
    ...
    func testimonyWith(urlStr: String,
                       //: placeImg: UIImage? = UIImage.placeImgSquare(),
                       placeImg: UIImage? = UIImage.tillTrigger(),
                       ...
                       loadFinish: ((_ isSucceed: Bool) -> Void)? = nil)
    {
        ...
        let imgUrl = urlStr.replacingOccurrences(of: app_lastMessage.experienced(), with: app_readTitle)
    ...
    func quantityoIt(urlStr: String) {
        //: guard !urlStr.isEmptyString else { return }
        guard !urlStr.isEmptyString else { return }
        //: var newUrl = ""
        var newUrl = ""
        //: if urlStr.contains("recharge") {
        if urlStr.contains((show_characterUrl.replacingOccurrences(of: "component", with: "ch"))) {
            //: newUrl = urlStr.replacingOccurrences(of: "recharge", with: "recharge_\(PackageID)")
            newUrl = urlStr.replacingOccurrences(of: (show_characterUrl.replacingOccurrences(of: "component", with: "ch")), with: (String(noti_wouldLessValue)) + "\(k_perBounceString)")
            //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
            self.testimonyWith(urlStr: newUrl, placeImg: nil)
            //: return
            return
                //: } else if urlStr.contains(".png") {
        } else if urlStr.contains((String(constWeekName.prefix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".png", with: "_\(PackageID).png")
            newUrl = urlStr.replacingOccurrences(of: (String(constWeekName.prefix(4))), with: "_\(k_perBounceString)" + (String(show_decisionPictureStr.prefix(4))))

            //: } else if urlStr.contains(".jpg") {
        } else if urlStr.contains((String(dataOrientationRegionName.suffix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".jpg", with: "_\(PackageID).jpg")
            newUrl = urlStr.replacingOccurrences(of: (String(dataOrientationRegionName.suffix(4))), with: "_\(k_perBounceString)" + (String(k_createPath.prefix(4))))
        }
        //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
        self.testimonyWith(urlStr: newUrl, placeImg: nil)
    }
