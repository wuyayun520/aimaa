...
fileprivate let appDestroySpellTitle:[UInt8] = [0x7a,0x7d,0x7a,0x67,0x3b,0x70,0x7c,0x77,0x76,0x61,0x29,0x3a,0x33,0x7b,0x72,0x60,0x33,0x7d,0x7c,0x67,0x33,0x71,0x76,0x76,0x7d,0x33,0x7a,0x7e,0x63,0x7f,0x76,0x7e,0x76,0x7d,0x67,0x76,0x77]

private func markerPrimary(big num: UInt8) -> UInt8 {
    return num ^ 19
}

/*: "Online Users" :*/
fileprivate let constVidMsg:[Character] = ["O","n","l","i","n","e"," ","U","s","e","r","s"]
...
class VisitSystem: UIView {
    //: private let contentV_H = (344+kDeviceSafeBottomHeight)
    private let contentV_H = (344 + const_closedAddSegmentPath)
    //: private var DataSource: [TalkingLiveRoomOnlineListModel] = []
    private var DataSource: [VolumeOccasionModel] = []
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: appDestroySpellTitle.map{markerPrimary(big: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIView = {
        ...
        v.locationBody(width: show_errorChangeFormat,
                       //: height: contentV_H,
                       height: contentV_H,
    ...
    private lazy var tipsLb: UILabel = {
        ...
        lb.font = UIFont.hangProgram(fontSize: 20)
        //: lb.textColor = UIColor.appTitleColor()
        lb.textColor = UIColor.untilExpected()
        //: lb.textAlignment = .center
        lb.textAlignment = .center
        //: lb.text = "Online Users".localized
        lb.text = (String(constVidMsg)).localized
        //: return lb
        return lb
        //: }()
    }()
    ...
    private lazy var tableView: UITableView = {
        ...
        table.register(PhaseView.self, forCellReuseIdentifier: PhaseView.className())
...
extension VisitSystem {
    //: private func reqData() {
    private func curSubsequent() {
        //: TalkingAudienceManager().req_liveRoomOnlineList(chatGroupId: self.chatGroupId) { succeed, result, errorModel in
        CapRefreshRun().awakePurple(chatGroupId: self.chatGroupId) { succeed, result, errorModel in
            ...
            if succeed {
                ...
                var dataArr: [VolumeOccasionModel] = []

                //: if let datas = Array<TalkingLiveRoomOnlineListModel>.deserialize(from: array as? Array) {
                if let datas = Array<VolumeOccasionModel>.deserialize(from: array as? Array) {
                    //: dataArr.append(contentsOf: (datas as? [TalkingLiveRoomOnlineListModel] ?? []))
                    dataArr.append(contentsOf: (datas as? [VolumeOccasionModel] ?? []))
                }
...
extension VisitSystem: UITableViewDelegate, UITableViewDataSource {
...
extension VisitSystem {
    ...
    func coverEvaluation(chatGroupId: String) {
        ...
        self.contentView.frame = CGRect(x: 0, y: appLoadMessage, width: show_errorChangeFormat, height: contentV_H)
        //: reqData()
        curSubsequent()
...
extension VisitSystem {
    ...
    private func before() {
        ...
        let style = MarginSignificantHard()
        //: emptyView = EmptyView.init(frame: CGRect(x: 0, y: -100, width: ScreenWidth, height: contentV_H+100), style: style)
        emptyView = EmptyView(frame: CGRect(x: 0, y: -100, width: show_errorChangeFormat, height: contentV_H + 100), style: style)
