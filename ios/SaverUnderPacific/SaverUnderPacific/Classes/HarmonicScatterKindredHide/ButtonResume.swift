...
fileprivate let appAssociateMsg:[UInt8] = [0xe1,0xe6,0xe1,0xec,0xa0,0xdb,0xe7,0xdc,0xdd,0xea,0xb2,0xa1,0x98,0xe0,0xd9,0xeb,0x98,0xe6,0xe7,0xec,0x98,0xda,0xdd,0xdd,0xe6,0x98,0xe1,0xe5,0xe8,0xe4,0xdd,0xe5,0xdd,0xe6,0xec,0xdd,0xdc]

fileprivate func originArrow(music num: UInt8) -> UInt8 {
...
/*: "#30D00B" :*/
fileprivate let appSnapMessage:[Character] = ["#","3","0","D","0","0","B"]

/*: "icon_receivedcalls" :*/
fileprivate let dataFullMessage:[Character] = ["i"]
fileprivate let constTemporaryMsg:String = "track talk result save customercon_"
fileprivate let userDiskIndependentConditionFormat:String = "edscopells"

/*: "Received calls" :*/
fileprivate let notiRootGreatString:[Character] = ["R","e","c","e","i","v","e","d"," ","c","a","l"]
fileprivate let showDensityId:String = "ldestroy"

/*: "icon_dialedcalls" :*/
fileprivate let main_relativeMendId:[Character] = ["i","c","o","n","_","d","i","a","l","e","d","c","a"]
fileprivate let kOriginPremiumFormat:String = "llclear"

/*: "Dialed calls" :*/
fileprivate let user_stackValue:String = "butt replace writeDial"
fileprivate let kModifyKey:String = "agivegives"
...
class ButtonResume: UITableViewCell {
    //: var currenModel = TalkingVideoCallRecordModel()
    var currenModel = LengthContainTo()
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        fatalError(String(bytes: appAssociateMsg.map{originArrow(music: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var iconBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(implementationCustomer), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    lazy var durationLB: UILabel = {
        ...
        label.font = .yearNearby(type: .Medium, fontSize: 10)
        //: label.textColor = .appValueDetailColor()
        label.textColor = .theProvision()
    ...
    lazy var onlineView: UIView = {
        ...
        view.backgroundColor = UIColor(hex: (String(appSnapMessage)))
...
extension ButtonResume {
    //: func setVideoCallCell(model: TalkingVideoCallRecordModel) {
    func someoneRole(model: LengthContainTo) {
        //: currenModel = model
        currenModel = model
        ...
        if !model.headPicFrame.isEmptyString {
            ...
            iconBorderBtn.quantityoIt(urlStr: model.headPicFrame)
            //: } else {
        } else {
        ...
        if model.callStatus == 0 {
            ...
            rcordImg.image = UIImage.managerToSecond(name: (String(dataFullMessage) + String(constTemporaryMsg.suffix(4)) + "receiv" + userDiskIndependentConditionFormat.replacingOccurrences(of: "scope", with: "ca")))
            //: rcordLB.text = "Received calls".localized
            rcordLB.text = (String(notiRootGreatString) + showDensityId.replacingOccurrences(of: "destroy", with: "s")).localized
            //: } else {
        } else {
            //: rcordImg.image = UIImage.BundleImageNamed(name: "icon_dialedcalls")
            rcordImg.image = UIImage.managerToSecond(name: (String(main_relativeMendId) + kOriginPremiumFormat.replacingOccurrences(of: "clear", with: "s")))
            //: rcordLB.text = "Dialed calls".localized
            rcordLB.text = (String(user_stackValue.suffix(4)) + "ed c" + kModifyKey.replacingOccurrences(of: "give", with: "l")).localized
        }
    ...
    @objc func implementationCustomer() {
