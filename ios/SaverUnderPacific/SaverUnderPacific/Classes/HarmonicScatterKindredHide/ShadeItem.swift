...
fileprivate let k_runCompleteMessage:[UInt8] = [0xb0,0xb7,0xb0,0xad,0xf1,0xba,0xb6,0xbd,0xbc,0xab,0xe3,0xf0,0xf9,0xb1,0xb8,0xaa,0xf9,0xb7,0xb6,0xad,0xf9,0xbb,0xbc,0xbc,0xb7,0xf9,0xb0,0xb4,0xa9,0xb5,0xbc,0xb4,0xbc,0xb7,0xad,0xbc,0xbd]

private func revenueDynamics(material num: UInt8) -> UInt8 {
    return num ^ 217
}

/*: "icon_lounge" :*/
fileprivate let main_draftValue:String = "icon_llater directly permanent"
fileprivate let user_typicalString:[Character] = ["e"]

/*: "icon_talk_points" :*/
fileprivate let user_tipProfileStr:String = "icon_profile home projection ease"
fileprivate let k_itemData:String = "tayield"

/*: "C6BDFF" :*/
fileprivate let appShallBetweenKey:String = "CdecisionBDFF"

/*: "#BBBBBB" :*/
fileprivate let kSecondFormat:String = "#BBBBBBshow lay"

/*: "Reply to get points~" :*/
fileprivate let main_writtenString:[Character] = ["R","e","p","l","y"," ","t","o"," ","g","e","t"," ","p","o","i"]
fileprivate let constClearlyTitle:String = "nts~close texture convert beat"

/*: "bth_unread_nor" :*/
fileprivate let showBlankDisappearValue:String = "bth_unreheart fore estimated custom spend"
fileprivate let mainPriorityPath:String = "corner cur nor radioad_nor"

/*: "icon_male_default" :*/
fileprivate let notiProKey:[UInt8] = [0x3,0x9,0x5,0x4,0x35,0x7,0xb,0x6,0xf,0x35,0xe,0xf,0xc,0xb,0x1f,0x6,0x1e]

private func outsideRegion(deal num: UInt8) -> UInt8 {
    return num ^ 106
}

/*: "icon_female_default" :*/
fileprivate let appLabelRejectPath:String = "picture fixed problem withinicon_fe"
fileprivate let notiMountName:String = "mabox"

/*: "+%@ points" :*/
fileprivate let main_beginUrl:String = "+%@recommendation"
fileprivate let show_extentMsg:[Character] = ["s"]

/*: "bth_read_pre" :*/
fileprivate let showHoneyName:String = "bth_ream export later remain processing"
...
//: class TalkingChatBaseMsgCell: BlackBy {
class ShadeItem: BlackBy {
    //: var cellData: TalkingChatMsgBaseCellData?
    var cellData: DisplayOrigin?
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: k_runCompleteMessage.map{revenueDynamics(material: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.managerToSecond(name: (String(main_draftValue.prefix(6)) + "oung" + String(user_typicalString)))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var ageLab: UILabel = {
        ...
        label.font = UIFont.hangProgram(fontSize: 10)
        //: return label
        return label
        //: }()
    }()
    ...
    public lazy var coinIconImg: UIImageView = {
        ...
        imgV.image = UIImage.managerToSecond(name: (String(user_tipProfileStr.prefix(5)) + k_itemData.replacingOccurrences(of: "yield", with: "lk") + "_points"))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    public lazy var coinLabel: UILabel = {
        ...
        label.textColor = UIColor(hex: (appShallBetweenKey.replacingOccurrences(of: "decision", with: "6")))
        //: label.isHidden = true
        label.isHidden = true
    ...
    public lazy var replyTipLab: UILabel = {
        ...
        label.textColor = UIColor(hex: (String(kSecondFormat.prefix(7))))
        //: label.isHidden = true
        label.isHidden = true
        //: label.font = UIFont.pingfangRugularFont(fontSize: 14)
        label.font = UIFont.enterCommon(fontSize: 14)
        //: label.text = "Reply to get points~".localized
        label.text = (String(main_writtenString) + String(constClearlyTitle.prefix(4))).localized
        //: return label
        return label
        //: }()
    }()
    ...
    public lazy var isReadImg: UIImageView = {
        ...
        imgV.image = UIImage.managerToSecond(name: (String(showBlankDisappearValue.prefix(8)) + String(mainPriorityPath.suffix(6))))
        //: return imgV
        return imgV
        //: }()
    }()
...
extension ShadeItem {
    ...
    override func satisfyAssociated(with data: SpineSuccess) {
        ...
        self.cellData = data as? DisplayOrigin
        ...
        let userModel = newData.msgModel.user
        //: let loungePlus = (MarginExamineer.share.loginUid == String(userModel.uid)) ? MarginExamineer.share.loginUserMode.loungePlus : userModel.loungePlus
        let loungePlus = (MarginExamineer.share.loginUid == String(userModel.uid)) ? MarginExamineer.share.loginUserMode.loungePlus : userModel.loungePlus

        //: self.nameLabel.text = userModel.nickname
        self.nameLabel.text = userModel.nickname
        //: self.nameLabel.textColor = loungePlus ? .userVipColor() : .appValueColor()
        self.nameLabel.textColor = loungePlus ? .boyfriend() : .obtainOf()
        //: self.nameLabel.font = UIFont.pingfangMediumFont(fontSize: 13)
        self.nameLabel.font = UIFont.hangProgram(fontSize: 13)
        //: let imgStr = userModel.sex == Int(Gender.male.rawValue) ? "icon_male_default" : "icon_female_default"
        let imgStr = userModel.sex == Int(EnvironmentEmpty.male.rawValue) ? String(bytes: notiProKey.map{outsideRegion(deal: $0)}, encoding: .utf8)! : (String(appLabelRejectPath.suffix(7)) + notiMountName.replacingOccurrences(of: "box", with: "le") + "_default")
        //: sexIconBtn.setBackgroundImage(UIImage.BundleImageNamed(name: imgStr), for: .normal)
        sexIconBtn.setBackgroundImage(UIImage.managerToSecond(name: imgStr), for: .normal)
        ...
        self.avatarView.poolFee(urlStr: userModel.headPic)
        ...
        self.iconBorder.quantityoIt(urlStr: userModel.headPicFrame)

        //: let isShowCoin = newData.msgIncome > 0
        let isShowCoin = newData.msgIncome > 0
        //: self.coinIconImg.isHidden = !(isShowCoin && MarginExamineer.share.appStatus == AppSkinStatus.normal.rawValue)
        self.coinIconImg.isHidden = !(isShowCoin && MarginExamineer.share.appStatus == AwayPush.normal.rawValue)
        ...
        self.coinLabel.text = (main_beginUrl.replacingOccurrences(of: "recommendation", with: " ") + "point" + String(show_extentMsg)).theAppear(NSNumber(value: Float(newData.msgIncome)))
        //: self.replyTipLab.isHidden = true
        self.replyTipLab.isHidden = true

        // 展示消息是否已读标识
        //: if TalkingPrivateChatManager.isShowReadMsg(cellData: newData) {
        if InvisibleJob.titleToo(cellData: newData) {
            //: self.isReadImg.isHidden = false
            self.isReadImg.isHidden = false
            //: var readImgStr = "bth_unread_nor"
            var readImgStr = (String(showBlankDisappearValue.prefix(8)) + String(mainPriorityPath.suffix(6)))
            // 消息对端是否已读
            //: if TalkingPrivateChatManager.msgIsRead(cellData: newData) {
            if InvisibleJob.tell(cellData: newData) {
                //: readImgStr = "bth_read_pre"
                readImgStr = (String(showHoneyName.prefix(6)) + "ad_pre")
            }
            //: self.isReadImg.image = UIImage.BundleImageNamed(name: readImgStr)
            self.isReadImg.image = UIImage.managerToSecond(name: readImgStr)
    ...
    override public func updateConstraints() {
        ...
        if let temCellData = self.cellData {
            //: loungePlus = (MarginExamineer.share.loginUid == String(temCellData.msgModel.user.uid)) ? MarginExamineer.share.loginUserMode.loungePlus : temCellData.msgModel.user.loungePlus
            loungePlus = (MarginExamineer.share.loginUid == String(temCellData.msgModel.user.uid)) ? MarginExamineer.share.loginUserMode.loungePlus : temCellData.msgModel.user.loungePlus
        }
