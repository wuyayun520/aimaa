...
fileprivate let k_mpTitle:[UInt8] = [0xec,0xf1,0xec,0xf7,0xab,0xe6,0xf2,0xe7,0xe8,0xf5,0xbd,0xac,0xa3,0xeb,0xe4,0xf6,0xa3,0xf1,0xf2,0xf7,0xa3,0xe5,0xe8,0xe8,0xf1,0xa3,0xec,0xf0,0xf3,0xef,0xe8,0xf0,0xe8,0xf1,0xf7,0xe8,0xe7]

fileprivate func combineUntil(entity num: UInt8) -> UInt8 {
...
/*: "icon_male_default" :*/
fileprivate let show_speedPath:[UInt8] = [0xb9,0xb3,0xbf,0xbe,0x8f,0xbd,0xb1,0xbc,0xb5,0x8f,0xb4,0xb5,0xb6,0xb1,0xa5,0xbc,0xa4]

/*: "icon_female_default" :*/
fileprivate let app_fatalTerrificTitle:[Character] = ["i","c","o","n","_","f","e","m","a","l","e","_","d","e","f","a","u"]
fileprivate let app_givingSkipKey:String = "LT"

/*: "level_%@" :*/
fileprivate let app_densePrimaryName:String = "item composition panel the reportlevel_"
fileprivate let userViaId:[Character] = ["%","@"]
...
class BlockObject: UIView {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: k_mpTitle.map{combineUntil(entity: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var nickLabel = UILabel().then {
        ...
        $0.font = .yearNearby(type: .Medium, fontSize: 17)
        //: $0.textColor = .white
        $0.textColor = .white
    }
    ...
    lazy var ageLabel = UILabel().then {
        ...
        $0.font = .yearNearby(type: .Regular, fontSize: 10)
        //: $0.textColor = .white
        $0.textColor = .white
    }
...
extension BlockObject {
    ...
    func overMinimize(model: TableArc) {
        //: headerPic.kf.setImage(with: URL(string: model.headPic), placeholder: UIImage.placeImgSquare())
        headerPic.kf.setImage(with: URL(string: model.headPic), placeholder: UIImage.tillTrigger())
        ...
        let imgStr = model.sex == EnvironmentEmpty.male.rawValue ? String(bytes: show_speedPath.map{$0^208}, encoding: .utf8)! : (String(app_fatalTerrificTitle) + app_givingSkipKey.lowercased())
        //: sexImgView.image = UIImage(named: imgStr)
        sexImgView.image = UIImage(named: imgStr)
        //: let levelStr = String(format: "level_%@", model.level)
        let levelStr = String(format: (String(app_densePrimaryName.suffix(6)) + String(userViaId)), model.level)
