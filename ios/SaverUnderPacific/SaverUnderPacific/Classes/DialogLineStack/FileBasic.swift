...
class FileBasic: UICollectionViewFlowLayout {

    //: var sectionArr = [GiftDataType]()  
    var sectionArr = [DistanceCould]()  // collection有多少个分区
    //: var hotPage = 0                    
    var hotPage = 0                    // hot页数
    ...
    private var GiftItemHeight  = Int(appDistanceCountTitle/2) // 每个礼物的高度
    ...
    override func prepare() {
        ...
        layoutContentSize = CGSize(width: Int(show_errorChangeFormat-10) * totalPage, height: 2*GiftItemHeight)
    }
    ...
    override func layoutAttributesForItem(at indexPath: IndexPath) -> UICollectionViewLayoutAttributes? {
        ...
        if indexPath.section == DistanceCould.Vip.rawValue {
            //: page += hotPage
            page += hotPage
        //: } else if indexPath.section == GiftDataType.Bags.rawValue {
        } else if indexPath.section == DistanceCould.Bags.rawValue {
        ...
        let itemWidth = Int(show_errorChangeFormat-10) / GiftItemColumn
        //: let x = page * Int(ScreenWidth-10) + pageCol * itemWidth
        let x = page * Int(show_errorChangeFormat-10) + pageCol * itemWidth
