...
/*: "Select a date" :*/
fileprivate let user_zoneId:String = "Selerandom long"
fileprivate let k_libraryKey:String = "universal list integrity cartoon movement date"
...
protocol TweenBlue: NSObjectProtocol {
    //: func pickDateView(year: Int, month: Int, day: Int)
    func chapterDown(year: Int, month: Int, day: Int)
}

//: class TalkingDateView: TalkingBasePickView, UIPickerViewDataSource, UIPickerViewDelegate {
class CoordinatorProcessing: EventCombine, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    weak var delegate: TweenBlue?

    //: override func initPickView() {
    override func calculateInvisible() {
        //: super.initPickView()
        super.calculateInvisible()

        //: titleString = "Select a date".localized
        titleString = (String(user_zoneId.prefix(4)) + "ct a" + String(k_libraryKey.suffix(5))).localized
        ...
        provisionStatement(year: defaultYear!, month: defaultMonth!, day: defaultDay!)
    }

    //: func setDefaultDate(year: Int, month: Int, day: Int) {
    func provisionStatement(year: Int, month: Int, day: Int) {
        //: currentYear = year
        currentYear = year
        //: if year == defaultYear {
        if year == defaultYear {
...
//        }
    ...
    override func beforeTerrific(button: UIButton) {
        //: super.confirmClick(button: button)
        super.beforeTerrific(button: button)
        //: refreshPickViewData()
        coalitionFor()
        ...
        if let day = selectDay {
            ...
            delegate?.chapterDown(year: selectYear!, month: selectMonth!, day: day)
            //: } else {
        } else {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: 0)
            delegate?.chapterDown(year: selectYear!, month: selectMonth!, day: 0)
        }
    }
        ...
        if isShowDay {
            ...
            if component == 0 {
                ...
                if currentYear == yearSum + minShowYear - 1 {
                    ...
                    return noChallenge(year: year, month: month)
                }
                //: } else {
            } else {
                //: return self.yearSum
    ...
    func noChallenge(year: Int, month: Int) -> Int {
