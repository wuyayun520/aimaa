...
fileprivate let show_gestureThemeData:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "iosSvgaEffect" :*/
fileprivate let const_uniqueKey:[Character] = ["i","o","s","S","v","g","a","E","f","f"]
fileprivate let noti_storageUrl:String = "ecguide"

/*: "iosEffect" :*/
fileprivate let main_announcementString:[Character] = ["i"]
fileprivate let show_queenData:String = "child secretosEffect"

/*: "entityId" :*/
fileprivate let user_trackSuccessId:[Character] = ["e","n","t","i","t","y","I","d"]
...
class ProvideOff: UIView {
    //: var currentEffectMsgModel: TalkingAnimatMsgModel?
    var currentEffectMsgModel: StepSumeraction?
    ...
    var animatedView: PersistAcross?

    //: open weak var delegate: AnimatViewStepProtocol?
    open weak var delegate: ProcessorRequest?
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: show_gestureThemeData.reversed(), encoding: .utf8)!)
    }
...
extension ProvideOff {
    //: func playAnimation(infoDic: Dictionary<String, Any>) {
    func appDraw(infoDic: [String: Any]) {
        //: let dict = infoDic
        let dict = infoDic
        //: if dict.keys.isEmpty == false {
        if dict.keys.isEmpty == false {
            //: var iosEffect: String = dict["iosSvgaEffect"] as! String
            var iosEffect: String = dict[(String(const_uniqueKey) + noti_storageUrl.replacingOccurrences(of: "guide", with: "t"))] as! String
            //: if iosEffect.isEmptyString {
            if iosEffect.isEmptyString {
                //: iosEffect = dict["iosEffect"] as! String
                iosEffect = dict[(String(main_announcementString) + String(show_queenData.suffix(8)))] as! String
            }

            //: let entityId = dict["entityId"] as! Int
            let entityId = dict[(String(user_trackSuccessId))] as! Int

            //: if iosEffect.isEmptyString == false {
            if iosEffect.isEmptyString == false {
                //: var effectMsgModel = TalkingAnimatMsgModel.init()
                var effectMsgModel = StepSumeraction()
                ...
                let successs = delayModify(msgModel: effectMsgModel)
            }
        }
    }
    ...
    func delayModify(msgModel: Any?) -> Bool {
        ...
        if msgModel is StepSumeraction {
            //: effectMsgModel = msgModel as? TalkingAnimatMsgModel
            effectMsgModel = msgModel as? StepSumeraction
            //: } else if msgModel is TalkingGiftMsgAnimatModel {
        } else if msgModel is FutureMendSumerval {
            //: let giftMsgModel = msgModel as! TalkingGiftMsgAnimatModel
            let giftMsgModel = msgModel as! FutureMendSumerval
            ...
            if giftMsgModel.animationTimes > 1 {
                ...
                tmpGiftMsgModel.sign(giftNum: 1) // 礼物个数设置为1，每次动效显示
                ...
                var repeatEfftect = StepSumeraction()
            ...
            effectMsgModel = StepSumeraction()
            ...
            if giftMsgModel.showType == TransitionFinish.myStery.rawValue || giftMsgModel.showType == TransitionFinish.king.rawValue {
        ...
        if self.currentEffectMsgModel != nil || paused {
            ...
            if effectMsgModel?.top ?? false && self.effectMsgModelArray.count > 0 {
                ...
                for i in 0 ..< self.effectMsgModelArray.count {
                    ...
                    let aModel: StepSumeraction = self.effectMsgModelArray[i] as! StepSumeraction
                    //: if aModel.top == false {
                    if aModel.top == false {
                        //: index = i
                        index = i
                ...
                if index == -1 {
                    ...
                    self.effectMsgModelArray.add(effectMsgModel ?? StepSumeraction())
                    //: } else {
                } else {
                    //: self.effectMsgModelArray.insert(effectMsgModel ?? TalkingAnimatMsgModel.init(), at: index)
                    self.effectMsgModelArray.insert(effectMsgModel ?? StepSumeraction(), at: index)
                }
            ...
            combine(effectMsgModel: effectMsgModel!)
        }
...
extension ProvideOff {
    ...
    func beyondLength() {
        ...
        if self.currentEffectMsgModel == nil {
            ...
            if self.effectMsgModelArray.count > 0 {
                ...
                guard let removeModel = pickToday() else { return }
                //: firstShow(effectMsgModel: removeModel)
                combine(effectMsgModel: removeModel)
            }
            //: } else {
        } else {
            //: self.animatedView?.resumeAnimation()
            self.animatedView?.anyTo()
        }
    ...
    func pickToday() -> StepSumeraction? {
        //: if (self.effectMsgModelArray.count == 0) {
        if self.effectMsgModelArray.count == 0 {
            //: return TalkingAnimatMsgModel.init()
            return StepSumeraction()
        }
        ...
        if effectMsgModel is StepSumeraction {
            //: let lastEffectMsgModel = effectMsgModel as! TalkingAnimatMsgModel
            let lastEffectMsgModel = effectMsgModel as! StepSumeraction
        ...
        return effectMsgModel as? StepSumeraction
    }
}
...
extension ProvideOff {
    ...
    func combine(effectMsgModel: StepSumeraction) {
        ...
        self.delegate?.future(effectView: self, obj: effectMsgModel)

        //: TalkingGiftAnimatTool.shared.loadEffectData(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
        MoveGenerate.shared.factorOk(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
            ...
            self.explain(effectMsgModel: effectMsgModel, success: success, effectModel: giftModel ?? ListenBoth())
            //: })
        })
    }
    ...
    func explain(effectMsgModel: StepSumeraction, success: Bool, effectModel: ListenBoth) {
        // 资源下载完成
        //: self.delegate?.animatViewWithDidEndLoadingResource(effectView: self, obj: effectMsgModel, success: success)
        self.delegate?.shadeFromMobile(effectView: self, obj: effectMsgModel, success: success)
        ...
        if success {
            ...
            if self.paused {
                ...
                if self.currentEffectMsgModel?.eid == effectMsgModel.eid, self.currentEffectMsgModel?.effectUrl == effectMsgModel.effectUrl {
                    ...
                    betweenProjectionBean(effectMsgModel: effectMsgModel, model: effectModel)
                    //: return
                    return
                }
            }
    ...
    func theCalledRound(effectMsgModel: StepSumeraction, effectModel: ListenBoth) {
        //: if self.animatedView != nil {
        if self.animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.directWhite()
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.hostBy()
        ...
        className = MoveGenerate.shared.versusOk(oldName: className ?? "")
        ...
        let realClass = AnimatedViewClass as! PersistAcross.Type
...
extension ProvideOff: BasicPetDetail {
    //: func giftEffectItemView(effectItemView: TalkingGiftAnimatItemView, success: Bool) {
    func totalOccasion(effectItemView: PersistAcross, success: Bool) {
        //: if (!success) {
        if !success {
            //: self.alpha = 0
            self.alpha = 0
            //: finishAnimation(success: success)
            activityHi(success: success)
        }
        ...
        self.delegate?.margin(effectView: self, obj: effectItemView.effectMsgModel ?? StepSumeraction(), success: success)
    }

    //: func didFinishAnimationOfGift(effectItemView: TalkingGiftAnimatItemView) {
    func futureVisitor(effectItemView: PersistAcross) {
        //: if (effectItemView == self.animatedView) {
        if effectItemView == self.animatedView {
            //: if (!self.needStopAll) {
            if !self.needStopAll {
                //: finishAnimation(success: true)
                activityHi(success: true)
            }
        }
        ...
        self.delegate?.numberegration(effectView: self, obj: effectMsgModel!)
    }
}
...
extension ProvideOff {
    ...
    func reWithInmate() {
        ...
        if animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.directWhite() // stop animation
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.hostBy() // clean animation data
