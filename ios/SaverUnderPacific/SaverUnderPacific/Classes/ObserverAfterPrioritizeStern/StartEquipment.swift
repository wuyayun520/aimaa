...
fileprivate let noti_holdUrl:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_chat_miss" :*/
fileprivate let main_techniqueCurrentlyName:[Character] = ["i","c","o","n","_","c","h","a"]
fileprivate let k_manualString:String = "near tensiont_miss"

/*: "icon_chat_yes" :*/
fileprivate let user_firmlyMistValue:[Character] = ["i","c","o","n","_","c","h","a","t","_","y","e"]
fileprivate let show_underFeedData:String = "associate"
...
class StartEquipment: ShadeItem {
//    var transBlock: (() -> Void)?

    //: var textData: ExceptionKindLocationCreate?
    var textData: ExceptionKindLocationCreate?
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        fatalError(String(bytes: noti_holdUrl.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var quoteView: MechanismPosition = {
        //: let quoteView = TalkingMsgQuoteView()
        let quoteView = MechanismPosition()
        //: quoteView.isHidden = false
        quoteView.isHidden = false
        //: quoteView.addTarget(self, action: #selector(quoteBtnClick), for: .touchUpInside)
        quoteView.addTarget(self, action: #selector(mapRe), for: .touchUpInside)
        //: return quoteView
        return quoteView
        //: }()
    }()
...
//                self.messageData?.translateStatus != .MsgTranslate_succeed && self.messageData?.msgModel.msgInfo.callCmd.count ?? 0 <= 0 && temTextData.msgModel.msgInfo.messageType != .MessageTypeWelcome && "\(temTextData.msgModel.user.uid)" != MovementStructure.getXiaoMiID()) { // 对方发的 && 开启了翻译功能 && 消息未翻译成功
...
//            }
            ...
            if temTextData.msgModel.quoteMsgInfo.blue() {
            ...
            if self.textData?.translateStatus == .MsgTranslate_translating {
                ...
                self.transStatusImagV.image = UIImage.managerToSecond(name: (String(main_techniqueCurrentlyName) + String(k_manualString.suffix(6))))
                ...
                self.transStatusImagV.image = UIImage.managerToSecond(name: (String(user_firmlyMistValue) + show_underFeedData.replacingOccurrences(of: "associate", with: "s")))
            }
    ...
    func nearInstead() {
        ...
        let containerTap = UITapGestureRecognizer(target: self, action: #selector(scaleOn(tap:)))
        ...
        self.container.addGestureRecognizer(containerTap)
        //: let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == MovementStructure.getXiaoMiID())
        let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == MovementStructure.uniqueAbove())
        //: if isOfficial == false {
        if isOfficial == false {
            //: addLongTagGes()
            start()
...
//            }
    ...
    func start() {
        ...
        let longPressGr = UILongPressGestureRecognizer(target: self, action: #selector(featurePut(tap:)))
        //: self.container.addGestureRecognizer(longPressGr)
        self.container.addGestureRecognizer(longPressGr)
    }

    //: @objc func TouchCellMessageAction(tap: UITapGestureRecognizer?) {
    @objc func scaleOn(tap: UITapGestureRecognizer?) {
    ...
    @objc func featurePut(tap: UILongPressGestureRecognizer) {
        //: if tap.state == .ended {
        if tap.state == .ended {
            //: return
            return
...
//    }
    ...
    @objc func mapRe() {
        ...
        self.delegate?.appearBroadcast(cell: self)
    }
}
