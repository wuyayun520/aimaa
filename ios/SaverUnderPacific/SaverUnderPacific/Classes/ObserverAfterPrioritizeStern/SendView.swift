...
/*: "#AAAAAA" :*/
fileprivate let constPostImageFormat:String = "#Asupposed play import"
fileprivate let user_rowEnoughPath:[Character] = ["A","A","A","A","A"]

/*: " No more than 10 characters" :*/
fileprivate let kVolumeTitle:String = " No automatic pack anima news"
fileprivate let showArrowThroughString:[Character] = ["n"," ","1","0"," ","c","h","a","r","a","c","t","e","r","s"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let kFlushFormat:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
class SendView: UIView {
    ...
    lazy var titleLabel: UILabel = {
        ...
        lab.font = UIFont.yearNearby(type: .Semibold, fontSize: 20)
        //: lab.numberOfLines = 0
        lab.numberOfLines = 0
    ...
    private lazy var inputTView: CurrentDirect = {
        ...
        input.placeholderColor = .init(hex: (String(constPostImageFormat.prefix(2)) + String(user_rowEnoughPath))) ?? UIColor.gray
        //: input.placeholder = " No more than 10 characters".localized
        input.placeholder = (String(kVolumeTitle.prefix(4)) + "more tha" + String(showArrowThroughString)).localized
        ...
        input.backgroundColor = .alongSupposed()
    ...
    private lazy var leftBtn: UIButton = {
        ...
        leftBtn.addTarget(self, action: #selector(processor), for: .touchUpInside)
        //: return leftBtn
        return leftBtn
        //: }()
    }()
    ...
    private lazy var rightBtn: UIButton = {
        ...
        rightBtn.setBackgroundImage(UIImage.domainCover(colors: UIColor.running(), size: CGSize(width: 267, height: 50)), for: .normal)
        ...
        rightBtn.setBackgroundImage(UIImage.domainCover(colors: UIColor.leaderNote(), size: CGSize(width: 267, height: 50)), for: .disabled)
        ...
        rightBtn.addTarget(self, action: #selector(adminControl), for: .touchUpInside)
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: kFlushFormat.reversed(), encoding: .utf8)!)
    }

    //: func changeRightBtn(title: String, isTouch: Bool) {
    func success(title: String, isTouch: Bool) {
...
extension SendView: UITextViewDelegate {
    ...
    func readingMic(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
    ...
    func pic(_ textField: UITextField) -> Bool {
    ...
    func textViewDidChange(_ textView: UITextView) {
        ...
        if textView.text.count > alertConfig.lengthLimit {
            ...
            if textNum > alertConfig.lengthLimit && alertConfig.lengthLimit > 0 {
                ...
                textView.text = ta(index: alertConfig.lengthLimit, text: textContent)
            }
        }
    }

    //: private func string_prefix(index: Int, text: String) -> String {
    private func ta(index: Int, text: String) -> String {
        //: if text.count <= index {
        if text.count <= index {
            //: return text
            return text
