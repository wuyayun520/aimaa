...
/*: "/TXCache" :*/
fileprivate let data_calculateName:String = "track radio section weekly example/TXCache"
...
class EqualPublisher: NSObject {
    ...
    open weak var delegate: HideWrite?
    //: var playStatus: VideoPlayerStatus?
    var playStatus: EventMap?
    //: static let shared = TalkingVideoPlayerManager()
    static let shared = EqualPublisher()
    ...
    lazy var player: TXVodPlayer = {
        ...
        let cachePath = NSString(format: "%@%@", NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!, (String(data_calculateName.suffix(8)))) as String
    ...
    lazy var timer: Timer = {
        ...
        let timer = Timer.scheduledTimer(timeInterval: 0.25, target: self, selector: #selector(bold), userInfo: nil, repeats: true)
...
extension EqualPublisher {
    //: func playerWithUrlAndVideoView(url: String, view: UIView) {
    func roundHost(url: String, view: UIView) {
        //: let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let newUrl = url.replacingOccurrences(of: app_lastMessage.experienced(), with: app_readTitle)
    ...
    @objc func bold() {
        ...
        if duration() > 0 {
            ...
            self.delegate?.agentMust(player: self, progress: CGFloat(currentTime() / duration()))
        }
    }

    //: func setRenderMode(renderMode: EnumTypeRenderMode) {
    func against(renderMode: SegmentCommand) {
    ...
    func area(bEnable: Bool) {
        //: var isMute = bEnable
        var isMute = bEnable
        //: if TalkingSocketManager.shared.pacific == true {
        if MaterialCurrency.shared.pacific == true { // 音视频通话时静音
            //: isMute = true
            isMute = true
        }
    ...
    func pan() {
        ...
        if self.playStatus != .Pausing {
            ...
            self.delegate?.manage(player: self, status: self.playStatus!)
        }
    }
...
extension EqualPublisher: TXVodPlayListener {
    ...
    func onPlayEvent(_ player: TXVodPlayer!, event EvtID: Int32, withParam param: [AnyHashable: Any]!) {
        ...
        case PLAY_EVT_PLAY_PROGRESS.rawValue:
            ...
            self.delegate?.manualBy(player: self, duration: Int(duration), currentTime: Int(currentTime))
