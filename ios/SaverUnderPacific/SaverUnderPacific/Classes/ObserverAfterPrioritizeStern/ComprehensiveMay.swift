...
fileprivate let showRunningStr:[UInt8] = [0x5b,0x5c,0x5b,0x46,0x1a,0x51,0x5d,0x56,0x57,0x40,0x8,0x1b,0x12,0x5a,0x53,0x41,0x12,0x5c,0x5d,0x46,0x12,0x50,0x57,0x57,0x5c,0x12,0x5b,0x5f,0x42,0x5e,0x57,0x5f,0x57,0x5c,0x46,0x57,0x56]

/*: "FFD855" :*/
fileprivate let const_beforeFloorName:[Character] = ["F"]
fileprivate let k_subsequentMessage:String = "FDmigration55"
...
class ComprehensiveMay: UIView {
    ...
    var defaultWidth: CGFloat = 0.0 {
        ...
        didSet {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.2) {
                ...
                let minX = self.leftSilder.frame.origin.x + self.defaultWidth - data_indicatorUrl
                //: self.rightSlider.frame = CGRect(x: minX,
                self.rightSlider.frame = CGRect(x: minX,
                                                //: y: self.rightSlider.frame.origin.y,
                                                y: self.rightSlider.frame.origin.y,
                                                //: width: kSliderWidth,
                                                width: data_indicatorUrl,
                ...
                self.overOffStatement(startX: self.videoStartX)
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: showRunningStr.map{$0^50}, encoding: .utf8)!)
    }
    ...
    private lazy var indicatorView: UIView = {
        ...
        indicator.backgroundColor = UIColor(hex: (String(const_beforeFloorName) + k_subsequentMessage.replacingOccurrences(of: "migration", with: "8")))
        //    indicator.alpha = 0.5
    ...
    private lazy var leftSilder: UIView = {
        ...
        leftSilder.frame = CGRect(x: notiLibraryBuildUrl, y: 0, width: data_indicatorUrl, height: self.frame.size.height)
    ...
    private lazy var rightSlider: UIView = {
        ...
        rightSlider.frame = CGRect(x: self.frame.size.width - data_indicatorUrl, y: 0, width: data_indicatorUrl, height: self.frame.size.height)
    ...
    private lazy var leftCoverView: UIView = {
        ...
        leftCoverView.backgroundColor = UIColor.our(0, 0, 0, 0.6)
        //: leftCoverView.frame = CGRect(x: 0, y: 0, width: LeftMargin, height: self.frame.size.height)
        leftCoverView.frame = CGRect(x: 0, y: 0, width: notiLibraryBuildUrl, height: self.frame.size.height)
    ...
    private lazy var rightCoverView: UIView = {
        ...
        rightCoverView.backgroundColor = UIColor.our(0, 0, 0, 0.6)
...
extension ComprehensiveMay {
    ...
    func overOffStatement(startX: CGFloat) {
        ...
        rightScope = CGVector(dx: leftSilder.frame.minX + minDurationGap - data_indicatorUrl,
                              //: dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kSliderWidth)
                              dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - data_indicatorUrl)
    }
}
...
extension ComprehensiveMay {
    ...
    @objc func visualNear(gesture: UIPanGestureRecognizer) {
        ...
        case .ended:
            ...
            self.overOffStatement(startX: videoStartX)
    ...
    @objc func trig(gesture: UIPanGestureRecognizer) {
        //: let thisTimePoint = gesture.location(in: self)
        let thisTimePoint = gesture.location(in: self)
        //: let state = gesture.state
        let state = gesture.state
...
extension ComprehensiveMay {
    ...
    private func deployDoinge() {
        ...
        let leftPanGesture = UIPanGestureRecognizer(target: self, action: #selector(visualNear(gesture:)))
        ...
        let rightPanGesture = UIPanGestureRecognizer(target: self, action: #selector(trig(gesture:)))
    ...
    func aboveListen(_ xposition: CGFloat) {
