...
fileprivate let mainMissId:[Character] = ["D","o","c","u","m","e","n","t","s","/","U","s","e","r","/","v","o","i"]
fileprivate let app_distanceMsg:String = "confirm first ache gnatce/"

/*: "Documents/User/record/" :*/
fileprivate let main_allId:[Character] = ["D","o","c","u","m","e","n","t","s","/","U","s","e"]
fileprivate let constCurveFormat:[Character] = ["r","/","r","e","c","o","r","d","/"]

/*: "cancelled" :*/
fileprivate let mainHeroFormat:[Character] = ["c","a","n","c","e","l","l","e","d"]

/*: .wav" :*/
fileprivate let const_frameworkKey:[Character] = [".","w","a","v"]
...
typealias OneKeyGreetDownloadBlock = (_ model: LastOutside) -> Void

//: protocol VoiceDownloadTaskManagerDelegate: NSObjectProtocol {
protocol KeepGroupShared: NSObjectProtocol {
    /// 下载完成
...
extension UserFractionRead {
    /// 添加下载任务
    //: func addDownloadTasks(_ tasksArray: [VoiceDownloadTaskModel]) {
    func identity(_ tasksArray: [LastOutside]) {
        ...
        for model in filterArray {
            ...
            if model.zipFilePath.isEmpty {
                ...
                model.zipFilePath = model.aboveFee()
            }

            // 重编译，相对路径会变
            //: if model.zipFilePath.contains("Documents/User/voice/") {
            if model.zipFilePath.contains((String(mainMissId) + String(app_distanceMsg.suffix(3)))) {
                //: model.zipFilePath = "\(AppCacheDefine.getChatVoicePath())\((model.zipFilePath as NSString).lastPathComponent)"
                model.zipFilePath = "\(GroupNearDismiss.libraryWarning())\((model.zipFilePath as NSString).lastPathComponent)"
                //: } else if model.zipFilePath.contains("Documents/User/record/") {
            } else if model.zipFilePath.contains((String(main_allId) + String(constCurveFormat))) {
                //: model.zipFilePath = "\(AppCacheDefine.getChatRecordPath())\((model.zipFilePath as NSString).lastPathComponent)"
                model.zipFilePath = "\(GroupNearDismiss.cord())\((model.zipFilePath as NSString).lastPathComponent)"
            }
            ...
            ComponentMultiple.flushHi(msgId: model.taskId, status: "\(ExerciseNear.prepare.rawValue)", filePath: model.zipFilePath)
    ...
    func set(_ audioInfo: [String: Any], completeHandle: @escaping OneKeyGreetDownloadBlock) {
        //: var taskModel = VoiceDownloadTaskModel.getOneKeyGreetModel(audioInfo)
        var taskModel = LastOutside.pickRemain(audioInfo)
        ...
        if self.downloadingModelDic.keys.contains(taskModel.downloadUri) {
            ...
            taskModel = self.downloadingModelDic[taskModel.downloadUri] ?? LastOutside()
        }

        // 重编译，相对路径会变
        //: if taskModel.zipFilePath.contains("Documents/User/voice/") {
        if taskModel.zipFilePath.contains((String(mainMissId) + String(app_distanceMsg.suffix(3)))) {
            //: taskModel.zipFilePath = "\(AppCacheDefine.getChatVoicePath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            taskModel.zipFilePath = "\(GroupNearDismiss.libraryWarning())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            //: } else if taskModel.zipFilePath.contains("Documents/User/record/") {
        } else if taskModel.zipFilePath.contains((String(main_allId) + String(constCurveFormat))) {
            //: taskModel.zipFilePath = "\(AppCacheDefine.getChatRecordPath())\((taskModel.zipFilePath as NSString).lastPathComponent)"
            taskModel.zipFilePath = "\(GroupNearDismiss.cord())\((taskModel.zipFilePath as NSString).lastPathComponent)"
        }
...
extension UserFractionRead: URLSessionDataDelegate {
    // 开始
    ...
    func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive response: URLResponse, completionHandler: @escaping (URLSession.ResponseDisposition) -> Void) {
        ...
        let totalBytesWritten = LastOutside.anyStar(path: taskModel.zipFilePath)
    ...
    func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive data: Data) {
        ...
        self.delegate?.substantial(model: taskModel)
    }
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        if error != nil || errorCode != 200 {
            ...
            if let error = error, (error as NSError).userInfo[NSLocalizedDescriptionKey] as? String == (String(mainHeroFormat)) {
                //: if taskModel.taskStatus == .pause {
                if taskModel.taskStatus == .pause {
                    //: pauseTask(url: task.taskDescription ?? "", isSuspend: true)
                    equivalent(url: task.taskDescription ?? "", isSuspend: true)
                    //: } else if taskModel.taskStatus == .cancel {
                } else if taskModel.taskStatus == .cancel {
                    //: self.cancelTask(url: task.taskDescription ?? "")
                    self.brownDetail(url: task.taskDescription ?? "")
                }
                //: } else {
            } else {
                ...
                equivalent(url: task.taskDescription ?? "", isSuspend: false)
            }
            //: } else {
        } else {
            ...
            self.natural(url: task.taskDescription ?? "")
        }
...
extension UserFractionRead {
    ...
    func equivalent(url: String, isSuspend: Bool) {
        ...
        case .chatRecord:
            ...
            let status = isSuspend ? "\(ExerciseNear.pause.rawValue)" : "\(ExerciseNear.error.rawValue)"
            //: WCDBVoiceMsgTable.db_updateVoiceMsg(msgId: taskModel.taskId, status: status)
            ComponentMultiple.flushHi(msgId: taskModel.taskId, status: status)
            //: if isSuspend == false {
            if isSuspend == false {
                //: self.delegate?.voiceDownloadError(model: taskModel)
                self.delegate?.doFind(model: taskModel)
            }
    ...
    func brownDetail(url: String) {
        ...
        case .chatRecord:
            ...
            ComponentMultiple.press(msgId: taskModel.taskId)
            //: self.delegate?.voiceDownloadCancel(model: taskModel)
            self.delegate?.accessOff(model: taskModel)
    ...
    func natural(url: String) {
        ...
        let size = LastOutside.anyStar(path: taskModel.zipFilePath)
        //: if size > 0 {
        if size > 0 {
            ...
            if taskModel.taskTypeId == .chatRecord {
                ...
                ComponentMultiple.flushHi(msgId: taskModel.taskId, status: "\(ExerciseNear.finish.rawValue)")
            }

            //: let filePath = "\(taskModel.zipFilePath).wav"
            let filePath = "\(taskModel.zipFilePath)" + (String(const_frameworkKey))
            ...
            self.exclusiveRoman(at: taskModel.zipFilePath, withKeyId: taskModel.taskId, withTypeId: taskModel.taskTypeId) { _ in
                //: switch taskModel.taskTypeId {
                switch taskModel.taskTypeId {
                //: case .chatRecord:
                case .chatRecord:
                    //: self.delegate?.voiceDownloadFinish(model: taskModel)
                    self.delegate?.mVersus(model: taskModel)
            ...
            case .chatRecord:
                ...
                ComponentMultiple.flushHi(msgId: taskModel.taskId, status: "\(ExerciseNear.expired.rawValue)")
                //: self.delegate?.voiceDownloadExpired(model: taskModel)
                self.delegate?.labelSay(model: taskModel)
    ...
    func exclusiveRoman(at path: String, withKeyId keyId: String, withTypeId typeId: PutAbout, complete: @escaping (Error?) -> Void) {
        //: DispatchQueue.global().async {
        DispatchQueue.global().async {
            //: if let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
            if let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
                //: let newPath = "\(path).wav"
                let newPath = "\(path)" + (String(const_frameworkKey))
                ...
                if let handle = FileHandle(forWritingAtPath: newPath) {
                    ...
                    let unzippedData = data.alongShadeArable()
                    ...
                    if typeId == .chatRecord {
                        ...
                        ComponentMultiple.flushHi(msgId: keyId, filePath: newPath)
                    }
