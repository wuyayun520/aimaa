...
/*: "add_header_cookies" :*/
fileprivate let k_evaluateStr:[Character] = ["a","d","d","_","h","e","a"]
fileprivate let userWireClubValue:String = "der_csucceed assert drawing"
...
public extension UIImageView {
    ...
    @objc func poolFee(urlStr: String,
    ...
    internal func chest(urlStr: String,
                        //: placeImg: UIImage = UIImage.placeImgSquare(),
                        placeImg: UIImage = UIImage.tillTrigger(),
    ...
    @objc func isBoth(urlStr: String) {
        //: let imgUrl = urlStr.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let imgUrl = urlStr.replacingOccurrences(of: app_lastMessage.experienced(), with: app_readTitle)
        //: self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.giftPlaceHolderImage())
        self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.everythingDownTiti())
    }
    ...
    @objc func alwaysFor(urlStr: String, completion: ((_ image: UIImage?) -> Void)? = nil) {
        // 允许请求携带cookie
        ...
        if var urlComponents = URLComponents(string: urlStr),
           ...
           let currURL = URL(string: PresentLocal.share.serverUrlStr)
        {
        ...
        let downloader = ImageDownloader(name: (String(k_evaluateStr) + String(userWireClubValue.prefix(5)) + "ookies"))
        ...
        self.kf.setImage(with: URL(string: newImgUrl), placeholder: UIImage.tillTrigger(), options: optionsInfo) { result in
            //: if completion != nil {
            if completion != nil {
                //: let image = try? result.get().image
                let image = try? result.get().image
