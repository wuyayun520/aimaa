...
#import "UIImage+Promise.h"

//: @implementation UIImage (Compress)
@implementation UIImage (Promise)

//: - (NSData *)compressedImageData {
- (NSData *)prepare {

    //: UIImage *image = self.wcTimelineCompress;
    UIImage *image = self.contextFor;
...
- (UIImage *)menu {
    ...
    return [UIImage imageWithData:[self prepare]];
}
...
- (UIImage *)enable:(Boolean)isSession {
    //: CGSize size = [self wxImageSize:isSession];
    CGSize size = [self reduce:isSession];
    //: UIImage *reImage = [self resizedImage:size];
    UIImage *reImage = [self constructTrain:size];
...
- (CGSize)reduce:(Boolean)isSession {
...
+ (NSData *)tillTier:(UIImage *)image pushIn:(NSInteger)fileSize{
    ...
    while (outIMGData.length>(fileSize*1024)) {
        ...
        thumIMG = [thumIMG pic:scalesize plain:NO];

        //: outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
        outIMGData = [thumIMG re:(fileSize*1024)];
    }
...
- (UIImage*)trigger:(CGSize)targetSize{
...
- (UIImage *)pic:(CGSize)targetSize plain:(BOOL)isScale{
...
- (UIImage *)constructTrain:(CGSize)newSize {
