...
fileprivate let showBecomeUrl:[UInt8] = [0x19,0x1e,0x19,0x4,0x58,0x13,0x1f,0x14,0x15,0x2,0x4a,0x59,0x50,0x18,0x11,0x3,0x50,0x1e,0x1f,0x4,0x50,0x12,0x15,0x15,0x1e,0x50,0x19,0x1d,0x0,0x1c,0x15,0x1d,0x15,0x1e,0x4,0x15,0x14]

private func errInput(drawer num: UInt8) -> UInt8 {
    return num ^ 112
}
...
class CombineToCurrency: UIView {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: showBecomeUrl.map{errInput(drawer: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var progressView: DelayPlain = {
        ...
        let v = DelayPlain(frame: self.frame, progressBgColor: self.progressBgColor, progressPlayedColor: self.progressPlayedColor, progressCachedColor: self.progressCachedColor)
    ...
    private lazy var panGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(agentIt(gesture:)))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(pageListener(gesture:)))
        //: return v
        return v
        //: }()
    }()
...
extension CombineToCurrency {
    //: @objc func panGesture(gesture: UIPanGestureRecognizer) {
    @objc func agentIt(gesture: UIPanGestureRecognizer) {
        // 移动的距离
    ...
    @objc func pageListener(gesture: UIPanGestureRecognizer) {
...
extension CombineToCurrency {
    ...
    private func beforeOddProcessing() {
