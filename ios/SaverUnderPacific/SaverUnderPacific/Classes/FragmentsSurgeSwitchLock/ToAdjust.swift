...
fileprivate let dataSatisfyString:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "View read receipts?" :*/
fileprivate let k_numberMessage:[Character] = ["V","i","e","w"," ","r","e","a","d"," ","r","e"]
fileprivate let const_curStr:String = "request heap head ratingceipts?"

/*: "text" :*/
fileprivate let kVersionFormat:String = "TEXT"

/*: "color" :*/
fileprivate let showResponseReaderMessage:[UInt8] = [0x1a,0x26,0x23,0x26,0x29]

fileprivate func difficultResource(momentum num: UInt8) -> UInt8 {
...
/*: "<xml>%@</xml>" :*/
fileprivate let userOpenMessage:String = "air under possible<xml>%@"
fileprivate let dataMageValue:[Character] = ["<","/","x","m","l",">"]

/*: "href" :*/
fileprivate let userReliabilityUrl:String = "hrproviderf"

/*: "type" :*/
fileprivate let constLineStr:[UInt8] = [0xfa,0xff,0xf6,0xeb]

fileprivate func presentBubble(spend num: UInt8) -> UInt8 {
...
/*: "txt" :*/
fileprivate let kAttentionId:[Character] = ["t","x","t"]

/*: "padding" :*/
fileprivate let notiChooseFormat:String = "backgrounddding"

/*: "size" :*/
fileprivate let user_hangApplicationStr:String = "phoneize"

/*: "maxFont" :*/
fileprivate let mainBlackGuideValue:String = "box matchmaxFont"

/*: "link" :*/
fileprivate let app_clipFormat:String = "LINK"

/*: "PingFangSC-Regular" :*/
fileprivate let show_hourMessage:String = "PingFheight beyond"
fileprivate let userShouldUrl:String = "C-Regulmine news dimension tween conversation"
fileprivate let showIgnorePath:String = "arelate"

/*: "content" :*/
fileprivate let mainLogVariableTitle:[UInt8] = [0x18,0x14,0x15,0xf,0x1e,0x15,0xf]

private func supportBelow(cluster num: UInt8) -> UInt8 {
    return num ^ 123
}
...
@objcMembers public class ToAdjust: DisplayOrigin {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: dataSatisfyString.reversed(), encoding: .utf8)!)
    }
    ...
    override public func data() -> CGSize {
        ...
        if self.msgModel.tips.content == (String(k_numberMessage) + String(const_curStr.suffix(7))).localized {
            //: return CGSize(width: ScreenWidth, height: 30)
            return CGSize(width: show_errorChangeFormat, height: 30)
        }
        ...
        //: let rect = self.attributedString.boundingRect(with: CGSize.init(width: MovementStructure.getScreenWidth()-50, height: CGFloat(MAXFLOAT)), options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil )
        let rect = self.attributedString.boundingRect(with: CGSize(width: MovementStructure.screenWith() - 50, height: CGFloat(MAXFLOAT)), options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil)
    ...
    public lazy var attributedString: NSMutableAttributedString = {
        ...
        var attr = proBold(info: self.msgModel.tips)
        //: return attr
        return attr
        //: }()
    }()
...
extension ToAdjust {
    ...
    func proBold(info: SumModel) -> NSMutableAttributedString {
...
//        let dic = NSDictionary(jsonString: extra ?? "")
        ...
        return ratingDoing(tips: content)
    }

    //: func designTextFream(tips: String) -> NSMutableAttributedString {
    func ratingDoing(tips: String) -> NSMutableAttributedString {
        ...
        _ = noneSessionProcess(msg: tips, info: attrbutedArray)
        //: self.yyLayout = designYYTextLayout(array: attrbutedArray, displayMaxWidth: MovementStructure.getScreenWidth()-50, para: para)
        self.yyLayout = simultaneouslyContain(array: attrbutedArray, displayMaxWidth: MovementStructure.screenWith() - 50, para: para)
    ...
    func noneSessionProcess(msg: String, info: NSMutableArray) -> String {
        ...
        let pure_msg = away(source: msg, attrbAM: contentAttrArray)
        //: for  textInfo in contentAttrArray {
        for textInfo in contentAttrArray {
            //: let textDic = textInfo as![String: Any]
            let textDic = textInfo as! [String: Any]
            //: let text: String = textDic["text"] as! String
            let text: String = textDic[(kVersionFormat.lowercased())] as! String

            //: var textColor: UIColor?
            var textColor: UIColor?

            //: var obj = textDic["color"]
            var obj = textDic[String(bytes: showResponseReaderMessage.map{difficultResource(momentum: $0)}, encoding: .utf8)!]
            //: if obj is UIColor {
            if obj is UIColor {
                //: obj = obj as? UIColor
                obj = obj as? UIColor
            ...
            contentDict.removeObjects(forKeys: [String(bytes: showResponseReaderMessage.map{difficultResource(momentum: $0)}, encoding: .utf8)!, (kVersionFormat.lowercased())])

            //: let attrDict = ctDictionaryForText(content: text, color: textColor)
            let attrDict = talk(content: text, color: textColor)
    ...
    func away(source: String, attrbAM: NSMutableArray) -> String {
        ...
        do {
            ...
            let document = try ONOXMLDocument(string: String(format: (String(userOpenMessage.suffix(7)) + String(dataMageValue)), newSource), encoding: CFStringConvertEncodingToNSStringEncoding(CFStringBuiltInEncodings.UTF8.rawValue))
            //: let element = document.rootElement
            let element = document.rootElement
            //: parseXMLElement(element: element ?? ONOXMLElement(), arrM: attrbAM, attrib: ["color": UIColor.white])
            head(element: element ?? ONOXMLElement(), arrM: attrbAM, attrib: [String(bytes: showResponseReaderMessage.map{difficultResource(momentum: $0)}, encoding: .utf8)!: UIColor.white])
            ...
            attrbAM.add([(kVersionFormat.lowercased()): newSource, String(bytes: showResponseReaderMessage.map{difficultResource(momentum: $0)}, encoding: .utf8)!: UIColor.white])
            //: return newSource
            return newSource
        }
    }
    ...
    func head(element: ONOXMLElement, arrM: NSMutableArray, attrib: NSDictionary) {
        ...
        if children.count > 0 {
            ...
            if element.tag == "u" { // 下滑线
                ...
                let url: String = element.attributes[(userReliabilityUrl.replacingOccurrences(of: "provider", with: "e"))] as! String
                //: if url.count > 0 {
                if url.count > 0 {
                    //: parentInfo["href"] = url
                    parentInfo[(userReliabilityUrl.replacingOccurrences(of: "provider", with: "e"))] = url
                }
            }
            //: for  subElement in children {
            for subElement in children {
                //: parseXMLElement(element: subElement as! ONOXMLElement, arrM: arrM, attrib: parentInfo)
                head(element: subElement as! ONOXMLElement, arrM: arrM, attrib: parentInfo)
            }
            ...
            if text != nil {
                ...
                //: info["text"] = text
                info[(kVersionFormat.lowercased())] = text
                //: arrM.add(info)
                arrM.add(info)
            }
        }
    ...
    func simultaneouslyContain(array: NSArray, displayMaxWidth: CGFloat, para: NSParagraphStyle?) -> YYTextLayout {
        //: let attrString = designAttribStr(array: array, paragraphStyle: para)
        let attrString = usWithPremium(array: array, paragraphStyle: para)
    ...
    func usWithPremium(array: NSArray, paragraphStyle: NSParagraphStyle?) -> NSAttributedString {
        ...
        array.enumerateObjects { obj, idx, stop in
            ...
            let type = newObj[String(bytes: constLineStr.map{presentBubble(spend: $0)}, encoding: .utf8)!] as? String
            //: var tmp = NSAttributedString.init()
            var tmp = NSAttributedString()
            //: if type  == "txt" {
            if type == (String(kAttentionId)) {
                //: tmp = designAttribStrByTextDic(dict: newObj as NSDictionary)
                tmp = own(dict: newObj as NSDictionary)
                //: } else if type == "padding" {
            } else if type == (notiChooseFormat.replacingOccurrences(of: "background", with: "pa")) {
                //: let imageView = UIImageView.init()
                let imageView = UIImageView()
                //: imageView.size = (newObj["size"] as! NSValue).cgSizeValue
                imageView.size = (newObj[(user_hangApplicationStr.replacingOccurrences(of: "phone", with: "s"))] as! NSValue).cgSizeValue
                ...
                tmp = NSMutableAttributedString.yy_attachmentString(withContent: imageView, contentMode: .center, attachmentSize: imageView.size, alignTo: newObj[(String(mainBlackGuideValue.suffix(7)))] as! UIFont, alignment: .center)
            }
            ...
            if underline {
                ...
                if len > 0 {
                    ...
                    _ = YYTextDecoration(style: .single, width: 1.5, color: newObj[String(bytes: showResponseReaderMessage.map{difficultResource(momentum: $0)}, encoding: .utf8)!] as? UIColor)
    ...
    func progress(dict: NSDictionary) -> NSAttributedString {
        //: let imageView = UIImageView.init()
        let imageView = UIImageView()
        //: imageView.size = (dict["size"]as! NSValue).cgSizeValue
        imageView.size = (dict[(user_hangApplicationStr.replacingOccurrences(of: "phone", with: "s"))] as! NSValue).cgSizeValue
        ...
        let attachText = NSMutableAttributedString.yy_attachmentString(withContent: imageView, contentMode: .center, attachmentSize: imageView.size, alignTo: dict[(String(mainBlackGuideValue.suffix(7)))] as! UIFont, alignment: .center)
        //: return attachText
        return attachText
    }

    //: func designAttribStrByTextDic(dict: NSDictionary) ->NSMutableAttributedString {
    func own(dict: NSDictionary) -> NSMutableAttributedString {
        //: let type = dict["type"] as! String
        let type = dict[String(bytes: constLineStr.map{presentBubble(spend: $0)}, encoding: .utf8)!] as! String
        //: if type != "link" && type != "txt" {
        if type != (app_clipFormat.lowercased()) && type != (String(kAttentionId)) {
            //: return NSMutableAttributedString.init()
            return NSMutableAttributedString()
        }
        //: let attributes = attributesWithConfig()
        let attributes = itemFeedReplace()
        // set color
        //: let color = dict["color"] as? UIColor
        let color = dict[String(bytes: showResponseReaderMessage.map{difficultResource(momentum: $0)}, encoding: .utf8)!] as? UIColor
        //: if color != nil {
        if color != nil {
        ...
        let fontSize = (dict[(user_hangApplicationStr.replacingOccurrences(of: "phone", with: "s"))] as? NSNumber)?.floatValue
        //: if (fontSize ?? 0 > 0) {
        if fontSize ?? 0 > 0 {
            //: let fontRef = CTFontCreateWithName("PingFangSC-Regular" as CFString, CGFloat(fontSize!), nil)
            let fontRef = CTFontCreateWithName((String(show_hourMessage.prefix(5)) + "angS" + String(userShouldUrl.prefix(7)) + showIgnorePath.replacingOccurrences(of: "relate", with: "r")) as CFString, CGFloat(fontSize!), nil)
            //: attributes[kCTFontAttributeName] = fontRef
            attributes[kCTFontAttributeName] = fontRef
        }
        //: var content = (dict["content"] != nil) ? dict["content"] : ""
        var content = (dict[String(bytes: mainLogVariableTitle.map{supportBelow(cluster: $0)}, encoding: .utf8)!] != nil) ? dict[String(bytes: mainLogVariableTitle.map{supportBelow(cluster: $0)}, encoding: .utf8)!] : ""
    ...
    func itemFeedReplace() -> NSMutableDictionary {
        //: let font = UIFont.pingfangRugularFont(fontSize: 15)
        let font = UIFont.enterCommon(fontSize: 15)
    ...
    func talk(content: String, color: UIColor?) -> NSDictionary {
        //: if content.count <= 0 {
        if content.count <= 0 {
        ...
        if color == nil {
            ...
            return ceiling(content: content)
        }
        //: return ["type": "txt",
        return [String(bytes: constLineStr.map{presentBubble(spend: $0)}, encoding: .utf8)!: (String(kAttentionId)),
                //: "content": content,
                String(bytes: mainLogVariableTitle.map{supportBelow(cluster: $0)}, encoding: .utf8)!: content,
                //: "color": color ?? .black]
                String(bytes: showResponseReaderMessage.map{difficultResource(momentum: $0)}, encoding: .utf8)!: color ?? .black]
    }

    //: func ctDictionaryForText(content: String) ->NSDictionary {
    func ceiling(content: String) -> NSDictionary {
        //: if content.count <= 0 {
        if content.count <= 0 {
            //: return NSDictionary.init()
            return NSDictionary()
