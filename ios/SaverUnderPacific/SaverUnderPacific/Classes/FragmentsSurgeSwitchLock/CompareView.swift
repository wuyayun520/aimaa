...
fileprivate let mainFunctionalName:[Character] = ["b","t","n","_","m","e","_","b","a","c","k","_"]
fileprivate let user_voiceAgainStr:String = "covisitinue"

/*: "init(coder:) has not been implemented" :*/
fileprivate let constRateContinuingKey:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
class CompareView: UIView {
    ...
    private var alertConfig: CustomIcon
    ...
    private lazy var leftBtn: UIButton = {
        ...
        leftBtn.layer.borderColor = UIColor.systemRequest().cgColor
        //: leftBtn.layer.borderWidth = 1
        leftBtn.layer.borderWidth = 1
        //: leftBtn.addTarget(self, action: #selector(leftClick), for: .touchUpInside)
        leftBtn.addTarget(self, action: #selector(playerFor), for: .touchUpInside)
        //: return leftBtn
        return leftBtn
        //: }()
    }()
    ...
    private lazy var rightBtn: UIButton = {
        ...
        let image = UIImage.managerToSecond(name: alertConfig.rightBackgroundImage).withTintColor(.systemRequest())
        //: rightBtn.setBackgroundImage(image, for: .normal)
        rightBtn.setBackgroundImage(image, for: .normal)
        //: rightBtn.addTarget(self, action: #selector(rightClick), for: .touchUpInside)
        rightBtn.addTarget(self, action: #selector(situationWritten), for: .touchUpInside)
        //: return rightBtn
        return rightBtn
        //: }()
    }()
         ...
         config: CustomIcon)
    {
        ...
        if leftBtnTitle == nil && leftBtnAttributedTitle == nil {
            ...
            alertConfig.rightBackgroundImage = (String(mainFunctionalName) + user_voiceAgainStr.replacingOccurrences(of: "visit", with: "nt"))
        }
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: constRateContinuingKey.reversed(), encoding: .utf8)!)
    }

    //: func changeRightBtn(title: String, isTouch: Bool) {
    func bagBy(title: String, isTouch: Bool) {
        ...
        if !isTouch {
            ...
            rightBtn.setBackgroundImage(UIImage.managerToSecond(name: alertConfig.rightBackgroundImagefalse), for: .normal)
            //: } else {
        } else {
            //: let image = UIImage.BundleImageNamed(name: alertConfig.rightBackgroundImage).withTintColor(.appThemeColor())
            let image = UIImage.managerToSecond(name: alertConfig.rightBackgroundImage).withTintColor(.systemRequest())
