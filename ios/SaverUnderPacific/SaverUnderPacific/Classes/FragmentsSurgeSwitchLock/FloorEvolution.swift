...
class FloorEvolution: UINavigationController {

    //: var tabBarType: TabBarItemType?
    var tabBarType: StatusLiteText?
    //: var pushing = false                             
    ...
    override func pushViewController(_ viewController: UIViewController, animated: Bool) {
        ...
        if viewControllers.count >= 1 {
            ...
            if viewController.navigationItem.leftBarButtonItem == nil {
                ...
                viewController.navigationItem.leftBarButtonItem = viewController.grainFlow()
            }
        }
    ...
    func restrictionReflectConstant() {
        ...
        if #available(iOS 15.0, *) {
            ...
            barApp.backgroundImage = UIImage.mortgage(color: UIColor.white)
            //: barApp.shadowImage = UIImage.init()
            barApp.shadowImage = UIImage.init()
            //: barApp.shadowColor = UIColor.appBgColor()
            barApp.shadowColor = UIColor.alongSupposed()
            ...
            barApp.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.untilExpected(), NSAttributedString.Key.font: UIFont.yearNearby(type: .Medium, fontSize: 17)]
        ...
        //: } else {
            ...
            navigationBar.setBackgroundImage(UIImage.mortgage(color: UIColor.white), for: .any, barMetrics: .default)
            ...
            navigationBar.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.untilExpected(), NSAttributedString.Key.font: UIFont.yearNearby(type: .Medium, fontSize: 17)]
        }
        //: UINavigationBar.appearance().tintColor = UIColor.appTitleColor()
        UINavigationBar.appearance().tintColor = UIColor.untilExpected()
...
extension FloorEvolution: UINavigationControllerDelegate {
    ...
    func navigationController(_ navigationController: UINavigationController, willShow viewController: UIViewController, animated: Bool) {
        ...
        if (viewController.hidesBottomBarWhenPushed) {
            ...
            ColorPad.shared.advSuperView?.isHidden = true
        //: } else {
        } else {
            //: tabBarController?.tabBar.isHidden = false
            tabBarController?.tabBar.isHidden = false
            //: TalkingAdvertisingManager.shared.advSuperView?.isHidden = false
            ColorPad.shared.advSuperView?.isHidden = false
        }
    }
...
extension FloorEvolution: UIGestureRecognizerDelegate {
