...
fileprivate let notiMortgageString:[UInt8] = [0x87,0x8c,0x87,0x92,0x46,0x81,0x8d,0x82,0x83,0x90,0x58,0x47,0x3e,0x86,0x7f,0x91,0x3e,0x8c,0x8d,0x92,0x3e,0x80,0x83,0x83,0x8c,0x3e,0x87,0x8b,0x8e,0x8a,0x83,0x8b,0x83,0x8c,0x92,0x83,0x82]

fileprivate func depressCould(stage num: UInt8) -> UInt8 {
...
/*: "all" :*/
fileprivate let data_lightData:String = "automaticl"
...
class StageView: ShadeItem {
    //: var giftData: TalkingGroupChatGiftMsgCellData?
    var giftData: MultiLastSine?
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: notiMortgageString.map{depressCould(stage: $0)}, encoding: .utf8)!)
    }
...
extension StageView {
    ...
    override func satisfyAssociated(with data: SpineSuccess) {
        ...
        self.giftData = data as? MultiLastSine
        ...
        self.giftImgView.poolFee(urlStr: imgPreview)
        //: self.giftContentLab.attributedText = giftData.attributedString
        self.giftContentLab.attributedText = giftData.attributedString
        //: if giftData.msgModel.toUser.uid == "all" { // 送礼所有人
        if giftData.msgModel.toUser.uid == (data_lightData.replacingOccurrences(of: "automatic", with: "al")) { // 送礼所有人
            //: self.toUserImgView.image = UIImage.BundleImageNamed(name: giftData.msgModel.toUser.headPic)
            self.toUserImgView.image = UIImage.managerToSecond(name: giftData.msgModel.toUser.headPic)
            //: } else {
        } else {
            //: self.toUserImgView.setUrlImage(urlStr: giftData.msgModel.toUser.headPic)
            self.toUserImgView.poolFee(urlStr: giftData.msgModel.toUser.headPic)
        }
    }
