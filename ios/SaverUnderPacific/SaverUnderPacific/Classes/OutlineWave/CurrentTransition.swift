...
fileprivate let notiImmediateValue:[UInt8] = [0x67,0x60,0x67,0x7a,0x26,0x6d,0x61,0x6a,0x6b,0x7c,0x34,0x27,0x2e,0x66,0x6f,0x7d,0x2e,0x60,0x61,0x7a,0x2e,0x6c,0x6b,0x6b,0x60,0x2e,0x67,0x63,0x7e,0x62,0x6b,0x63,0x6b,0x60,0x7a,0x6b,0x6a]

/*: "ButtonHeadArray" :*/
fileprivate let appStackData:String = "generally speaker upon busy aDrop"
fileprivate let user_insertTextFormat:String = "erMenuCmarker assistant even"
fileprivate let show_meStr:[Character] = ["e","l","l"]

/*: "roomId" :*/
fileprivate let show_cutAppearanceTitle:String = "searcher translate finish light mutualroomId"

/*: "type" :*/
fileprivate let noti_unlessData:[Character] = ["t","y","p","e"]

/*: "page" :*/
fileprivate let dataFragmentVeryMsg:String = "statusge"

/*: "icon_chats_pcr" :*/
fileprivate let dataPathName:String = "task gray thread professional enableicon_"
fileprivate let notiChooseStr:String = "_pcrpermission execute media operation calculation"

/*: "All Numbers" :*/
fileprivate let constApplicationData:String = "All Nunumb could circle mini"
fileprivate let notiFixedKey:String = "mbecons"

/*: "all" :*/
fileprivate let appExistTimingData:[Character] = ["a","l","l"]

/*: "Recent Gifts" :*/
fileprivate let app_ariaRegulateTitle:String = "Recent Gmultiple against combined called"
fileprivate let main_clusterId:String = "ifred"
...
class CurrentTransition: UIView {
    ...
    public var triangleHeight = Config.AgainstInfo.triangleHeight
    /**
     *  调整使下拉优先 当向下偏转屏幕距离足够，优先向下偏转
     */
    //: public var adjustPullDown = Config.DropMenuConfig.adjustPullDown
    public var adjustPullDown = Config.AgainstInfo.adjustPullDown
    ...
    fileprivate lazy var contentView: UIView = self.squareByTier()
    //: fileprivate lazy var mTable: UITableView = self.makeTableView()
    fileprivate lazy var mTable: UITableView = self.suspendTo()
    //: fileprivate var anchorRect = CGRect.zero
    fileprivate var anchorRect = CGRect.zero

    //: static let shared = DropDownMemberMenuView(frame: .zero)
    static let shared = CurrentTransition(frame: .zero)
    ...
    init(frame: CGRect, anchorView: UIView, roomID: String) {
        ...
        super.init(frame: frame)
        //: let window = MovementStructure.getWindow()
        let window = MovementStructure.levelScientific()
        //: self.roomID = roomID
        self.roomID = roomID
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: notiImmediateValue.map{$0^14}, encoding: .utf8)!)
    }

    //: private func makeContentView() -> UIView {
    private func squareByTier() -> UIView {
        //: let view  = UIView()
        let view = UIView()
        //: return view
        return view
    ...
    private func suspendTo() -> UITableView {
    ...
    private func constructMake() {
        ...
        mTable.register(ButtonHeadArray.self, forCellReuseIdentifier: (String(appStackData.suffix(4)) + "Memb" + String(user_insertTextFormat.prefix(7)) + String(show_meStr)))
    ...
    func even() {
        ...
        var dict = [String: Any]()
        //: dict["roomId"] = roomID
        dict[(String(show_cutAppearanceTitle.suffix(6)))] = roomID
        //: dict["type"] = "3"
        dict[(String(noti_unlessData))] = "3"
        //: dict["page"] = String(pageIndex)
        dict[(dataFragmentVeryMsg.replacingOccurrences(of: "status", with: "pa"))] = String(pageIndex)
        ...
        CommonLog.share.isDay(params: dict, completion: { array in
            ...
            if self.pageIndex == 0 {
                ...
                let model = ReduceModel()
                //: model.headPic = "icon_chats_pcr"
                model.headPic = (String(dataPathName.suffix(5)) + "chats" + String(notiChooseStr.prefix(4)))
                //: model.nickname = "All Numbers".localized
                model.nickname = (String(constApplicationData.prefix(6)) + notiFixedKey.replacingOccurrences(of: "con", with: "r")).localized
                //: model.uid = "all"
                model.uid = (String(appExistTimingData))
...
extension CurrentTransition: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: (String(appStackData.suffix(4)) + "Memb" + String(user_insertTextFormat.prefix(7)) + String(show_meStr)), for: indexPath) as! ButtonHeadArray
        ...
        if recentmenuArray.count > 0 && indexPath.section == 0 {
            ...
            cellModel = recentmenuArray[indexPath.row] as! ReduceModel
            //: }else {
        } else {
            //: cellModel = menuArray[indexPath.row] as! TalkingChatRoomMemberModel
            cellModel = menuArray[indexPath.row] as! ReduceModel
        }
        //: cell.setDropMemberMenuCell(model: cellModel)
        cell.reserveDivide(model: cellModel)
        //: return cell
        return cell
    }
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        menuDelegate?.onicialUponTriggerLast(model: cellModel)
    }

    //: func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        //: let headeView = UILabel.init(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: 23))
        let headeView = UILabel(frame: CGRect(x: 0, y: 0, width: show_errorChangeFormat, height: 23))
        //: headeView.backgroundColor = UIColor.appBgColor()
        headeView.backgroundColor = UIColor.alongSupposed()
        //: let titleLB = UILabel.init()
        let titleLB = UILabel()
        //: titleLB.frame = CGRect(x: 3, y: 5, width: 100, height: 14)
        titleLB.frame = CGRect(x: 3, y: 5, width: 100, height: 14)
        //: titleLB.font = .pingfangFont(type: .Medium, fontSize: 12)
        titleLB.font = .yearNearby(type: .Medium, fontSize: 12)
        //: titleLB.textColor = .appValueDetailColor()
        titleLB.textColor = .theProvision()
        //: if recentmenuArray.count > 0 && section == 0 {
        if recentmenuArray.count > 0 && section == 0 {
            //: titleLB.text = "Recent Gifts".localized
            titleLB.text = (String(app_ariaRegulateTitle.prefix(8)) + main_clusterId.replacingOccurrences(of: "red", with: "ts")).localized
            //: }else {
        } else {
            //: titleLB.text = "All Numbers".localized
            titleLB.text = (String(constApplicationData.prefix(6)) + notiFixedKey.replacingOccurrences(of: "con", with: "r")).localized
        }
