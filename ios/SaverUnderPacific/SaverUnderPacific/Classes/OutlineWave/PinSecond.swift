...
/*: "cellID" :*/
fileprivate let main_becomeKey:String = "label dailycellID"

/*: "init(coder:) has not been implemented" :*/
fileprivate let kPartyStr:[UInt8] = [0x62,0x65,0x62,0x7f,0x23,0x68,0x64,0x6f,0x6e,0x79,0x31,0x22,0x2b,0x63,0x6a,0x78,0x2b,0x65,0x64,0x7f,0x2b,0x69,0x6e,0x6e,0x65,0x2b,0x62,0x66,0x7b,0x67,0x6e,0x66,0x6e,0x65,0x7f,0x6e,0x6f]

/*: "Video Call" :*/
fileprivate let show_sunShareData:String = "Video Capublisher dark finish"
fileprivate let data_closedSatisfyValue:[Character] = ["l","l"]

/*: "Video Call (%@ Points/Min)" :*/
fileprivate let data_destroyPath:String = "accessible loseVide"
fileprivate let noti_manShWhoValue:[Character] = ["l"," ","(","%","@"," ","P","o","i","n","t","s","/","M","i","n",")"]

/*: "Voice Call" :*/
fileprivate let dataSpellSplitString:[Character] = ["V","o","i","c","e"," ","C","a","l"]
fileprivate let appGlobalData:[Character] = ["l"]

/*: "Voice Call (%@ Points/Min)" :*/
fileprivate let constCharacteristicFormat:String = "Voice it shall elevator parent"
fileprivate let show_tabData:String = " (%@"
fileprivate let dataGrainValue:[Character] = ["/","M","i","n",")"]

/*: "#F8F8F8" :*/
fileprivate let const_signUrl:String = "#F8F8F8app visitor and pure"

/*: "Cancel" :*/
fileprivate let app_ownKey:[Character] = ["C","a","n","c","e","l"]

/*: "#F5F5F5" :*/
fileprivate let noti_putTitle:[Character] = ["#"]
fileprivate let noti_outsideMsg:String = "FerrorFerrorFerror"

/*: "icon_lounge" :*/
fileprivate let noti_attachName:String = "template exhaust todayicon_"
fileprivate let showProtectionName:String = "total"
...
class PinSecond: UIView {
    //: var munuBlock: MunuBlock?
    var munuBlock: MunuBlock?

    //: private var type: MunuPopType = .Unknown
    private var type: PurpleBy = .Unknown // 当前展示类型
    ...
    var cellTitleColor = UIColor.untilExpected()
    /// cell字体大小
    //: var cellTitleFont = UIFont.pingfangFont(type: .Regular, fontSize: 16)
    var cellTitleFont = UIFont.yearNearby(type: .Regular, fontSize: 16)
    /// 是否有"Cancel"菜单
    //: var hasCancel = true
    var hasCancel = true
    /// cancel字体颜色
    //: var cancelColor = UIColor.appValueDetailColor()
    var cancelColor = UIColor.theProvision()
    /// cancel字体大小
    //: var cancelTitleFont = UIFont.pingfangFont(type: .Regular, fontSize: 16)
    var cancelTitleFont = UIFont.yearNearby(type: .Regular, fontSize: 16)
    /// 标题
    //: var title: String?
    var title: String?
    /// 标题颜色
    //: var titleColor = UIColor.appThemeColor()
    var titleColor = UIColor.systemRequest()
    /// 标题字体大小
    //: var titleFont = UIFont.pingfangMediumFont(fontSize: 16)
    var titleFont = UIFont.hangProgram(fontSize: 16)

    //: var tableView = UITableView()
    var tableView = UITableView()
    //: let cellID = "cellID"
    let cellID = (String(main_becomeKey.suffix(6)))
    ...
    fileprivate var footerViewHight: CGFloat {
        ...
        return hasCancel ? (footerViewLineHight + 50 + const_closedAddSegmentPath) : const_closedAddSegmentPath
    }
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: kPartyStr.map{$0^11}, encoding: .utf8)!)
    }

    //: func initwithList(cellTitleList: [String]!) {
    func techniqueCapacity(cellTitleList: [String]!) {
        // 初始化
    ...
    func asCluster(type: PurpleBy, videoPrice: String?, voicePrice: String?) {
        //: self.type = type
        self.type = type
        //: if self.type == .Call {
        if self.type == .Call { // 音视频通话
            ...
            if MarginExamineer.share.appStatus == AwayPush.normal.rawValue {
                ...
                if MarginExamineer.share.loginUserMode.sex == EnvironmentEmpty.male.rawValue {
                    ...
                    let videoAttrStr = String.medic(VideoPrice: Int(videoPrice ?? "0") ?? 0)
                    //: let voiceAttrStr = String.setVoicePriceStr(VoicePrice: Int(voicePrice ?? "0") ?? 0)
                    let voiceAttrStr = String.nervousFact(VoicePrice: Int(voicePrice ?? "0") ?? 0)
                    ...
                    let attributes = [.foregroundColor: UIColor.systemRequest(), .font: UIFont.yearNearby(type: .Regular, fontSize: 14)] as [NSAttributedString.Key: Any]
                    //: let VideoCall = "Video Call".localized
                    let VideoCall = (String(show_sunShareData.prefix(8)) + String(data_closedSatisfyValue)).localized
                    //: let videoStr = "Video Call (%@ Points/Min)".localizedArguments(videoPrice ?? "0")
                    let videoStr = (String(data_destroyPath.suffix(4)) + "o Cal" + String(noti_manShWhoValue)).theAppear(videoPrice ?? "0")
                    ...
                    let VoiceCall = (String(dataSpellSplitString) + String(appGlobalData)).localized
                    //: let voiceStr = "Voice Call (%@ Points/Min)".localizedArguments(voicePrice ?? "0")
                    let voiceStr = (String(constCharacteristicFormat.prefix(6)) + "Call" + show_tabData.capitalized + " Points" + String(dataGrainValue)).theAppear(voicePrice ?? "0")
                ...
                self.cellTitleList = [(String(show_sunShareData.prefix(8)) + String(data_closedSatisfyValue)).localized, (String(dataSpellSplitString) + String(appGlobalData)).localized]
            }
        }
    ...
    public func rover() {
        ...
        tableView = UITableView(frame: CGRect(x: 0, y: appLoadMessage - tableViewHight, width: show_errorChangeFormat, height: tableViewHight), style: .plain)
        ...
        tableView.register(ThreadInclude.self, forCellReuseIdentifier: cellID)
        ...
        if title == nil {
            ...
            tableView.locationBody(width: show_errorChangeFormat,
...
extension PinSecond: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var cell = tableView.dequeueReusableCell(withIdentifier: cellID) as? ThreadInclude
        //: if cell == nil {
        if cell == nil {
            //: cell = TitleCell(style: .default, reuseIdentifier: cellID)
            cell = ThreadInclude(style: .default, reuseIdentifier: cellID)
        }

        //: if MarginExamineer.share.loginUserMode.sex == Gender.male.rawValue,
        if MarginExamineer.share.loginUserMode.sex == EnvironmentEmpty.male.rawValue,
           //: MarginExamineer.share.loginUserMode.loungePlus == true,
           MarginExamineer.share.loginUserMode.loungePlus == true,
           //: type == .Call {
           type == .Call
        {
        ...
        cell!.titleLab.font = cellTitleFont
        //: if self.type == .Call && MarginExamineer.share.appStatus == AppSkinStatus.normal.rawValue {
        if self.type == .Call && MarginExamineer.share.appStatus == AwayPush.normal.rawValue {
    ...
    func tableView(_ tableView: UITableView, viewForFooterInSection section: Int) -> UIView? {
        ...
        let footerView = UIView(frame: CGRect(x: 0, y: 0, width: show_errorChangeFormat, height: footerViewHight))
        //: let lineView = UIView(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: footerViewLineHight))
        let lineView = UIView(frame: CGRect(x: 0, y: 0, width: show_errorChangeFormat, height: footerViewLineHight))
        //: lineView.backgroundColor = UIColor(hex: "#F8F8F8")
        lineView.backgroundColor = UIColor(hex: (String(const_signUrl.prefix(7))))
        ...
        btn.frame = CGRect(x: 0, y: footerViewLineHight, width: show_errorChangeFormat, height: 50)
        //: footerView.addSubview(btn)
        footerView.addSubview(btn)
        //: btn.setTitle("Cancel".localized, for: .normal)
        btn.setTitle((String(app_ownKey)).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(performMove), for: .touchUpInside)
        //: return footerView
        return footerView
    }
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        let headerView = UIView(frame: CGRect(x: 0, y: 0, width: show_errorChangeFormat, height: headerViewHight))

        //: let titleLab = UILabel(frame: CGRect(x: 15, y: 0, width: ScreenWidth-30, height: headerViewHight))
        let titleLab = UILabel(frame: CGRect(x: 15, y: 0, width: show_errorChangeFormat - 30, height: headerViewHight))
        ...
        let lineView = UIView(frame: CGRect(x: 0, y: headerViewHight - 1, width: show_errorChangeFormat, height: 1))
        //: lineView.backgroundColor = UIColor.init(hex: "#F5F5F5")
        lineView.backgroundColor = UIColor(hex: (String(noti_putTitle) + noti_outsideMsg.replacingOccurrences(of: "error", with: "5")))
...
class ThreadInclude: UITableViewCell {
    ...
    lazy var vipIcon: UIImageView = {
        ...
        img.image = UIImage.managerToSecond(name: (String(noti_attachName.suffix(5)) + "loung" + showProtectionName.replacingOccurrences(of: "total", with: "e")))
        //: img.isHidden = true
        img.isHidden = true
        //: return img
        return img
    ...
    lazy var lineLab: UILabel = {
        ...
        lab.backgroundColor = UIColor(hex: (String(noti_putTitle) + noti_outsideMsg.replacingOccurrences(of: "error", with: "5")))
        //: return lab
        return lab
        //: }()
    }()
