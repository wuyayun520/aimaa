...
class CurrentFrom: EasyChainSensor {
    ...
    lazy var usePhotoView: EmptyChain = {
        //: let v = TalkingSendIntimateInfoView(frame: CGRect.zero, type: .intimatePhoto)
        let v = EmptyChain(frame: CGRect.zero, type: .intimatePhoto)
        //: return v
        return v
        //: }()
    }()
...
extension CurrentFrom {
    //: func crushBtnTool(modle: TalkingMomentModel) {
    func topic(modle: StatusModel) {
        ...
        if modle.sex == nil {
            ...
            //: bottomView.isHidden = modle.sex==MarginExamineer.share.loginUserMode.sex
            bottomView.isHidden = modle.sex == MarginExamineer.share.loginUserMode.sex
        }
