...
/*: "title" :*/
fileprivate let constMakeThirdStr:String = "tpicktle"

/*: "content" :*/
fileprivate let data_accuracyString:String = "conteastern"
fileprivate let app_averageMsg:[Character] = ["t"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let notiDialogTingTitle:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "img" :*/
fileprivate let data_operateConditionMsg:[Character] = ["i","m","g"]

/*: "jumpKey" :*/
fileprivate let dataNumberryFormat:[Character] = ["j","u","m","p","K","e","y"]
...
@objcMembers public class SearchPublish: DisplayOrigin {
    // 数据
    //: var extraJson: JSON {
    var extraJson: JSON {
    ...
    public lazy var titleAttrStr: NSMutableAttributedString = {
        ...
        let attributes = [.foregroundColor: UIColor.untilExpected(), .font: UIFont.yearNearby(type: .Medium, fontSize: 17)] as [NSAttributedString.Key: Any]
        //: let attributeStr = NSMutableAttributedString(string: extraJson["title"].stringValue)
        let attributeStr = NSMutableAttributedString(string: extraJson[(constMakeThirdStr.replacingOccurrences(of: "pick", with: "i"))].stringValue)
    ...
    public lazy var contentAttrStr: NSMutableAttributedString = {
        ...
        let attributes = [.foregroundColor: UIColor.obtainOf(), .font: UIFont.yearNearby(type: .Regular, fontSize: 14)] as [NSAttributedString.Key: Any]
        //: let attributeStr = NSMutableAttributedString(string: extraJson["content"].stringValue)
        let attributeStr = NSMutableAttributedString(string: extraJson[(data_accuracyString.replacingOccurrences(of: "eastern", with: "en") + String(app_averageMsg))].stringValue)
        //: attributeStr.addAttributes(attributes, range: NSRange(location: 0, length: attributeStr.length))
        attributeStr.addAttributes(attributes, range: NSRange(location: 0, length: attributeStr.length))
        //: return attributeStr
        return attributeStr
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: notiDialogTingTitle.reversed(), encoding: .utf8)!)
    }
    ...
    override public func data() -> CGSize {
        ...
        let height = self.composition(0) - 20
    ...
    override public func composition(_ width: CGFloat) -> CGFloat {
        ...
        if extraJson[(String(data_operateConditionMsg))].stringValue.count > 0 {
        ...
        if extraJson[(constMakeThirdStr.replacingOccurrences(of: "pick", with: "i"))].stringValue.count > 0 {
        ...
        if extraJson[(data_accuracyString.replacingOccurrences(of: "eastern", with: "en") + String(app_averageMsg))].stringValue.count > 0 {
            //: height += 4
            height += 4
        ...
        if extraJson[(String(dataNumberryFormat))].stringValue.count > 0 {
            //: jumpHeight = 35
            jumpHeight = 35
            //: height += jumpHeight
            height += jumpHeight
