...
/*: "#BAFBFF" :*/
fileprivate let kYieldTitle:String = "distance route underly design#BAFBFF"
...
class SpecifyBlue: NSObject {
    ...
    func itemSpell(model: ReplaceHold) -> ReplaceHold {
        //: self.model = model
        self.model = model
        /// 普通文本消息
        //: if self.model.messageType == 1 {
    ...
    func tweenOfInstruction() {
        //: let paraStyle = paragraphStyle()
        let paraStyle = noSection()
        ...
        let name = may(text: firstStr, font: UIFont.hangProgram(fontSize: 14), color: UIColor(hex: (String(kYieldTitle.suffix(7))))!)
        // 内容
        //: let content = getAttributed(text: self.model.content, font: UIFont.pingfangMediumFont(fontSize: 14), color: UIColor.white)
        let content = may(text: self.model.content, font: UIFont.hangProgram(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        ...
        botApp(attribText: self.model.msgAttribText)
    }
    ...
    func ready() {
        ...
        let content = may(text: self.model.transContent, font: UIFont.hangProgram(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        //: name.yy_paragraphStyle = paraStyle
        name.yy_paragraphStyle = paraStyle
    ...
    func noSection() -> NSMutableParagraphStyle {
    ...
    func may(text: String, font: UIFont, color: UIColor) -> NSMutableAttributedString {
...
//            attribute.yy_writingDirection = [3]
        ...
        attribute.yy_paragraphStyle = noSection()
        //: return attribute
        return attribute
    }
    ...
    func botApp(attribText: NSMutableAttributedString) {
        // 距离左边8  距离右边也为8
        //: let maxWidth = VideoMsgTableViewWidth - 36
        let maxWidth = show_layerValue - 36
