...
fileprivate let userHisPath:[Character] = ["设","置","\u{666e}","通","\u{72b6}","态","的","文","字","颜","\u{8272}","时"," ","请"]
fileprivate let main_wenTitle:String = "使用RGB空super home"
fileprivate let appRecognizeMessage:[Character] = ["间","的","颜","色","值"]

/*: "设置选中状态的文字颜色时 请使用RGB空间的颜色值" :*/
fileprivate let appTotalryHighlightUrl:[Character] = ["设","置","选","中","状","态","\u{7684}","文","字","颜","\u{8272}","时"," "]
fileprivate let kAwayMessage:[Character] = ["请","使","用","R","G","B","空","\u{95f4}","的","\u{989c}","色","\u{503c}"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let showOccasionAddTitle:[UInt8] = [0xb,0x10,0xb,0x16,0xca,0x5,0x11,0x6,0x7,0x14,0xdc,0xcb,0xc2,0xa,0x3,0x15,0xc2,0x10,0x11,0x16,0xc2,0x4,0x7,0x7,0x10,0xc2,0xb,0xf,0x12,0xe,0x7,0xf,0x7,0x10,0x16,0x7,0x6]

fileprivate func itemFound(busy num: UInt8) -> UInt8 {
...
/*: "设置的下标不合法!!" :*/
fileprivate let constWholeLogicMessage:String = "设置language标"
fileprivate let kSpecifyKey:String = "\u{4e0d}\u{5408}法!!"

/*: "Bags" :*/
fileprivate let main_sizeValue:String = "Bagsopen compare merge"
...
open class SampleBrush: UIView {
    //: open var segmentStyle: SegmentStyle
    open var segmentStyle: TensionSignature
    /// 点击响应
    ...
    fileprivate lazy var normalColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let normalRgb = self.aboutCheckionAge(self.segmentStyle.normalTitleColor) {
            //: return normalRgb
            return normalRgb
            //: } else {
        } else {
            //: fatalError("设置普通状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((String(userHisPath) + String(main_wenTitle.prefix(6)) + String(appRecognizeMessage)))
        }
        //: }()
    }()
    ...
    fileprivate lazy var selectedTitleColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let selectedRgb = self.aboutCheckionAge(self.segmentStyle.selectedTitleColor) {
            //: return selectedRgb
            return selectedRgb
            //: } else {
        } else {
            //: fatalError("设置选中状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((String(appTotalryHighlightUrl) + String(kAwayMessage)))
        }
    ...
    fileprivate func aboutCheckionAge(_ color: UIColor) -> (r: CGFloat, g: CGFloat, b: CGFloat)? {
    ...
    public init(frame: CGRect, segmentStyle: TensionSignature, titles: [String]) {
    ...
    @objc func saveSession(_ tapGes: UITapGestureRecognizer) {
        //: guard let currentLabel = tapGes.view as? CustomLabel else { return }
        guard let currentLabel = tapGes.view as? LimitLabel else { return }
    ...
    public required init?(coder aDecoder: NSCoder) {
        ...
        fatalError(String(bytes: showOccasionAddTitle.map{itemFound(busy: $0)}, encoding: .utf8)!)
    }
}
...
public extension SampleBrush {
    ///  对外界暴露设置选中的下标的方法 可以改变设置下标滚动后是否有动画切换效果
    //: public func selectedIndex(_ selectedIndex: Int, animated: Bool) {
    func targetVoiceRemote(_ selectedIndex: Int, animated: Bool) {
        //: assert(!(selectedIndex < 0 || selectedIndex >= titles.count), "设置的下标不合法!!")
        assert(!(selectedIndex < 0 || selectedIndex >= titles.count), (constWholeLogicMessage.replacingOccurrences(of: "language", with: "的下") + kSpecifyKey))
    ...
    func reserveCity(_ titles: [String]) {
        // 移除所有的scrollView子视图
        ...
        targetVoiceRemote(0, animated: true)
    }
}
...
//: extension ScrollSegmentView {
extension SampleBrush {
    // 根据Titles添加相应的控件
    //: fileprivate func setupTitles() {
    fileprivate func accountReading() {
        //: for (index, title) in titles.enumerated() {
        for (index, title) in titles.enumerated() {
            //: let label = CustomLabel(frame: CGRect.zero)
            let label = LimitLabel(frame: CGRect.zero)
            //: label.tag = index
            label.tag = index
            //: label.text = title
            label.text = title
            ...
            let tapGes = UITapGestureRecognizer(target: self, action: #selector(self.saveSession(_:)))
            ...
            if title == (String(main_sizeValue.prefix(4))).localized {
    ...
    fileprivate func tickEmpty() {
        ...
        if let firstLabel = labelsArray[0] as? LimitLabel {
    ...
    public func socialBeside(_ currentIndex: Int) {
