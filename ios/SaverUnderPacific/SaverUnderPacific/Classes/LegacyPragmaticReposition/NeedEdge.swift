...
fileprivate let const_reportTitle:[UInt8] = [0x4d,0x4a,0x4d,0x50,0xc,0x47,0x4b,0x40,0x41,0x56,0x1e,0xd,0x4,0x4c,0x45,0x57,0x4,0x4a,0x4b,0x50,0x4,0x46,0x41,0x41,0x4a,0x4,0x4d,0x49,0x54,0x48,0x41,0x49,0x41,0x4a,0x50,0x41,0x40]

/*: "move" :*/
fileprivate let showProcedureMsg:[Character] = ["m","o","v","e"]
...
class NeedEdge: UIView {
    ...
    var danumuMsgArray: [ChapterEquallyPush] = []
    //: var danumuItemViewArray: [TalkingGameFloatingScreenItemView] = []
    var danumuItemViewArray: [ResourceOrigin] = []

    //: let TapAction: PublishSubject<TalkingGameFloatingScreenModel?> = PublishSubject()
    let TapAction: PublishSubject<ChapterEquallyPush?> = PublishSubject()
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: const_reportTitle.map{$0^36}, encoding: .utf8)!)
    }
    ...
    func listener(_ danumuMsgModel: ChapterEquallyPush?) {
    ...
    @objc func spot(_ tapGesture: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(spot(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension NeedEdge: UIGestureRecognizerDelegate {
    ...
    func under(for danumuMsgModel: ChapterEquallyPush?, isFirst: Bool) {
        //: let itemView = TalkingGameFloatingScreenItemView(model: danumuMsgModel)
        let itemView = ResourceOrigin(model: danumuMsgModel)
        ...
        var frame = CGRect(x: show_errorChangeFormat, y: isFirst ? 7 : 50, width: itemView.widthValue, height: 24)
        //: var endFrame = frame
        var endFrame = frame
        ...
        if InputCloseInfo.shared.direction == .rightToLeft {
            ...
            endFrame.origin.x = show_errorChangeFormat
        }
        //: addSubview(itemView)
        addSubview(itemView)
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (String(showProcedureMsg)))
...
//    }
    ...
    func totaleraction(_ isFirst: Bool) {
        ...
        if danumuMsgArray.count > 0 {
            ...
            if let model {
                ...
                under(for: model, isFirst: isFirst)
