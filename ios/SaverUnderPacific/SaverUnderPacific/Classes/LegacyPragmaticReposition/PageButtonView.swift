...
fileprivate let appOddPoolValue:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_chats_banner_subtract" :*/
fileprivate let notiAdditionFormat:String = "icreadyn"
fileprivate let constSubsequentTitle:String = "send searcher something_banne"
fileprivate let kCanStr:String = "trprivacyct"

/*: "icon_chats_banner_subtract_pre" :*/
fileprivate let k_sinkData:String = "popularon"
fileprivate let showBrokenAllowSomeoneId:String = "s_babad identity"
fileprivate let mainShId:String = "sunobodyr"
fileprivate let kBroadConfirmId:[Character] = ["a","c","t","_","p","r","e"]

/*: "bannerList" :*/
fileprivate let notiHereId:[Character] = ["b","a","n","n","e","r"]
fileprivate let app_blueMsg:[Character] = ["L","i","s","t"]

/*: "JXBannerCellID" :*/
fileprivate let noti_flexibleTitle:String = "perceptionX"
fileprivate let userActiveFirmlyName:[Character] = ["B","a","n","n","e","r","C","e","l","l","I","D"]

/*: "#FFFFFF" :*/
fileprivate let const_painterPath:[Character] = ["#","F","F","F","F","F","F"]
...
protocol IsolateCrucialSmall: NSObject {
    //: func changeBannerViewFream(isMiniView: Bool)
    func third(isMiniView: Bool)
}
...
class PageButtonView: UIView {
    ...
    open weak var delegate: IsolateCrucialSmall?
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: appOddPoolValue.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var bannerListData: [RomanModel] = //: return Array<RomanModel>()
        .init()
    //: }()
    ...
    private lazy var closeBtn: UIButton = {
        ...
        btn.setImage(UIImage.managerToSecond(name: (notiAdditionFormat.replacingOccurrences(of: "ready", with: "o") + "_chats" + String(constSubsequentTitle.suffix(6)) + "r_sub" + kCanStr.replacingOccurrences(of: "privacy", with: "a"))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_chats_banner_subtract_pre"), for: .selected)
        btn.setImage(UIImage.managerToSecond(name: (k_sinkData.replacingOccurrences(of: "popular", with: "ic") + "_chat" + String(showBrokenAllowSomeoneId.prefix(4)) + "nner_" + mainShId.replacingOccurrences(of: "nobody", with: "bt") + String(kBroadConfirmId))), for: .selected)
...
extension PageButtonView {
    /// banner
    //: func reqBanner() {
    func kind() {
        //: SocialRequestManager.init().req_adBanner(position: 3) { [weak self] succeed, result, errorModel in
        BetweenReplace().markImpression(position: 3) { [weak self] succeed, result, errorModel in
            ...
            let array = JSON(result[(String(notiHereId) + String(app_blueMsg))] ?? []).arrayValue
...
extension PageButtonView {
    ...
    @objc private func live() {
        ...
        self.delegate?.third(isMiniView: isMiniView)
    }

    //: func resetView(isMiniView: Bool) {
    func anyTargetBeauty(isMiniView: Bool) {
        //: if isMiniView {
        if isMiniView {
...
extension PageButtonView: JXBannerDelegate, JXBannerDataSource {
    ...
    func jxBanner(_ banner: JXBannerType) -> (JXBannerCellRegister) {
        ...
        return JXBannerCellRegister(type: JXBannerCell.self, reuseIdentifier: (noti_flexibleTitle.replacingOccurrences(of: "perception", with: "J") + String(userActiveFirmlyName)))
    }
    ...
    func jxBanner(_ banner: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
        ...
        tempCell.imageView.chest(urlStr: url, placeImg: UIImage.tillTrigger())
                  ...
                  builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder
    {
        ...
        pageControl.activeColor = UIColor(hex: (String(const_painterPath)))!
