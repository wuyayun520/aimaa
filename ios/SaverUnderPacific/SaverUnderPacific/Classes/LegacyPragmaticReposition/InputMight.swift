...
fileprivate let mainVisualKey:String = "option put should charmbtn_s"
fileprivate let noti_mapValue:String = "lertBackgfit combine any"
fileprivate let appDetectDraftData:String = "rpend"

/*: "btn_alertBackground_false" :*/
fileprivate let const_occasionStr:[Character] = ["b","t","n","_","a","l","e","r","t","B","a","c","k","g","r","o","u","n","d","_","f","a","l","s"]
fileprivate let userYellowConditionFormat:[Character] = ["e"]
...
public class InputMight: NSObject {
    /// 通用回调
...
public extension InputMight {
    /// 适配器回调,用于给适配器参数赋值
    //: public typealias ConfigAlert = ((_ config: ShowAlertConfig) -> Void)
    typealias ConfigAlert = (_ config: CustomIcon) -> Void
    ...
    internal class func untilPhone(title: String? = nil,
                                   //: message: String?  = nil,
                                   message: String? = nil,
    ...
    @objc internal class func beyondExtendSh(title: String? = nil,
    ...
    class func during(title: String? = nil,
                      ...
                      config: CustomIcon? = nil)
    {
        ...
        let model = config ?? CustomIcon()

        //: let alertView = TalkingAlertView(title: title,
        let alertView = CompareView(title: title,
        ...
        //: MovementStructure.getWindow().addSubview(alertView)
        MovementStructure.levelScientific().addSubview(alertView)
    ...
    class func runningProjection() {
        //: MovementStructure.getWindow().subviews.forEach { (view) in
        MovementStructure.levelScientific().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: CompareView.self) {
    ...
    class func bigPop(title: String, isTouch: Bool) {
        //: MovementStructure.getWindow().subviews.forEach { (view) in
        MovementStructure.levelScientific().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: CompareView.self) {
                //: let cunrenView: TalkingAlertView = view as! TalkingAlertView
                let cunrenView: CompareView = view as! CompareView
                //: cunrenView.changeRightBtn(title: title, isTouch: isTouch)
                cunrenView.bagBy(title: title, isTouch: isTouch)
            }
        }
    }
}
...
public class CustomIcon {
    ...
    public var maskType: MessageSpot = .color
    /// 阴影
    ...
    public var titleFont: UIFont = .yearNearby(type: .Medium, fontSize: 18)
    /// alert标题字体颜色
    //: public var titleColor  = UIColor.appTitleColor()
    public var titleColor = UIColor.untilExpected()
    /// alert信息字体
    //: public var textFont: UIFont = .pingfangFont(type: .Medium, fontSize: 16)
    public var textFont: UIFont = .yearNearby(type: .Medium, fontSize: 16)
    //: public var textColor  = UIColor.appTitleColor()
    public var textColor = UIColor.untilExpected()
    ...
    public var buttonFont: UIFont = .yearNearby(type: .Medium, fontSize: 16)
    /// alert按钮字体颜色
    //: public var leftColor  = UIColor.appThemeColor()
    public var leftColor = UIColor.systemRequest()
    //: public var rightColor = UIColor.white
    public var rightColor = UIColor.white
    //: public var rightBackgroundImage = "btn_show_alertBackground"
    public var rightBackgroundImage = (String(mainVisualKey.suffix(5)) + "how_a" + String(noti_mapValue.prefix(9)) + appDetectDraftData.replacingOccurrences(of: "pe", with: "ou"))

    //: public var rightBackgroundImagefalse = "btn_alertBackground_false"
    public var rightBackgroundImagefalse = (String(const_occasionStr) + String(userYellowConditionFormat))
    //: public var rightFont: UIFont = .pingfangFont(type: .Medium, fontSize: 16)
    public var rightFont: UIFont = .yearNearby(type: .Medium, fontSize: 16)
