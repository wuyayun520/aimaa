...
fileprivate let userHangTitle:[UInt8] = [0xb1,0xb6,0xb1,0xac,0xf0,0xbb,0xb7,0xbc,0xbd,0xaa,0xe2,0xf1,0xf8,0xb0,0xb9,0xab,0xf8,0xb6,0xb7,0xac,0xf8,0xba,0xbd,0xbd,0xb6,0xf8,0xb1,0xb5,0xa8,0xb4,0xbd,0xb5,0xbd,0xb6,0xac,0xbd,0xbc]

/*: "Save successfully" :*/
fileprivate let data_exampleEnjoyKey:String = "Sarecording announcement schedule lost"
fileprivate let dataFeedPath:[Character] = ["v","e"," ","s","u","c","c","e","s","s","f","u","l","l","y"]
...
// __CLOSE_PRINT__
//
//  QuantityegrityViewController.swift
//  AbroadTalking
//
...
class QuantityegrityViewController: CurrentFrom {
    ...
    init(momentModel: StatusModel, index: Int, type: BoxPool) {
        ...
        if InputCloseInfo.shared.direction == .rightToLeft {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.3) {
                ...
                self.collecView.contentOffset = CGPoint(x: curren * Int(show_errorChangeFormat), y: 0)
    ...
    init(images: [UIImage], index: Int, type: BoxPool) {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: myType = type
        myType = type
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: userHangTitle.map{$0^216}, encoding: .utf8)!)
    }
...
//        }
    ...
    override func languageChallenge() {
        //: super.publishDeleteButtonClick()
        super.languageChallenge()
    ...
    lazy var collecView: UICollectionView = {
        ...
        flayout.itemSize = CGSize(width: show_errorChangeFormat, height: appLoadMessage)
        ...
        view.frame = CGRect(x: 0, y: 0, width: show_errorChangeFormat, height: appLoadMessage)
        ...
        view.contentOffset = CGPoint(x: pageIndex * Int(show_errorChangeFormat), y: 0)
        //: if myType == .normal {
        if myType == .normal {
            //: view.contentSize = CGSize(width: (currenModel.pic.count )*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (currenModel.pic.count) * Int(show_errorChangeFormat), height: 0)
            //: } else {
        } else {
            //: view.contentSize = CGSize(width: (photos.count)*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (photos.count) * Int(show_errorChangeFormat), height: 0)
        }
        //: view.register(TalkingMomentPhotosPreviewCell.self, forCellWithReuseIdentifier: TalkingMomentPhotosPreviewCell.className())
        view.register(RequestHide.self, forCellWithReuseIdentifier: RequestHide.className())
...
//    lazy var downloadBtn: DistinctionButton = {
//        let btn = DistinctionButton.init()
...
extension QuantityegrityViewController {
...
//        MultiplePoint.func__openAlbumServiceWithBlock(true) { [weak self] (isOpen: Bool) in
//            if isOpen {
//                guard let self = self else {return}
//                guard  self.pageIndex >= 0 && self.pageIndex < self.currenModel.pic.count  else { return }
//                let model: AreaModel = self.currenModel.pic[self.pageIndex]
...
//    }
    ...
    func isShapeToday(image: UIImage) {
        ...
        PHPhotoLibrary.shared().performChanges({
            ...
            if isSuccess {
                ...
                DispatchQueue.main.async {
                    ...
                    self.nearEasy(showMsg: (String(data_exampleEnjoyKey.prefix(2)) + String(dataFeedPath)).localized)
                }
                //: } else {
            } else {
...
extension QuantityegrityViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    //: func hideTopView() {
    func exceptPing() {
...
//        downloadBtn.isHidden = isHideTopView
    ...
    func deliveryBlind() {
        //: if  currenModel.sex==MarginExamineer.share.loginUserMode.sex {
        if currenModel.sex == MarginExamineer.share.loginUserMode.sex {
            //: return
            return
        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell: RequestHide = collectionView.dequeueReusableCell(withReuseIdentifier: RequestHide.className(), for: indexPath) as! RequestHide
        //: if myType == .normal {
        if myType == .normal {
            //: let model: TalkingMomentPhotoModel = currenModel.pic[indexPath.row]
            let model: AreaModel = currenModel.pic[indexPath.row]
            //: if model.imgUrl?.count ?? 0 > 1 {
            if model.imgUrl?.count ?? 0 > 1 {
                //: cell.configUrl(url: model.imgUrl! )
                cell.title(url: model.imgUrl!)
                //: }else {
            } else {
                //: cell.configImage(image: model.imagePic ?? UIImage.init())
                cell.pool(image: model.imagePic ?? UIImage())
            }

            //: } else {
        } else {
            //: cell.configImage(image: photos[indexPath.row])
            cell.pool(image: photos[indexPath.row])
        }
    ...
    func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        ...
        let tcell: RequestHide = cell as! RequestHide
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let currentIndex = Int(offSetWidth / show_errorChangeFormat)
        //: if myType == .normal {
        if myType == .normal {
