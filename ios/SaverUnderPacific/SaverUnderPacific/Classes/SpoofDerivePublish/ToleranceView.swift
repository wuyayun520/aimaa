...
fileprivate let mainHeadHardPath:[UInt8] = [0x74,0x73,0x74,0x69,0x35,0x7e,0x72,0x79,0x78,0x6f,0x27,0x34,0x3d,0x75,0x7c,0x6e,0x3d,0x73,0x72,0x69,0x3d,0x7f,0x78,0x78,0x73,0x3d,0x74,0x70,0x6d,0x71,0x78,0x70,0x78,0x73,0x69,0x78,0x79]

/*: "gift_bg_dm" :*/
fileprivate let noti_presentationPath:[Character] = ["g","i","f","t","_","b","g","_","d","m"]

/*: "#FFF4B5" :*/
fileprivate let constPlusMsg:[Character] = ["#","F","F","F","4","B","5"]

/*: "won " :*/
fileprivate let mainChapterPath:[Character] = ["w","o","n"," "]

/*: " coins in " :*/
fileprivate let appTickValue:[Character] = [" ","c","o","i","n","s"]
fileprivate let appPlainFormat:String = " in false deem home ago"

/*: "just sent" :*/
fileprivate let dataEvolutionData:[Character] = ["j","u","s","t"," ","s","e","n"]
fileprivate let kCurrentData:String = "significant"

/*: "#4120C1" :*/
fileprivate let userSignValue:String = "#4120C1hang strip pressure spend reserve"
...
class ToleranceView: UIView {
    /// 礼物model
    //: private var giftModel = SocialBroadcastModel()
    private var giftModel = ReduceResult()
    ...
    override init(frame: CGRect) {
        ...
        activityEducateMinimumCorner()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: mainHeadHardPath.map{$0^29}, encoding: .utf8)!)
    }
    ...
    private lazy var bgView: UIImageView = {
        ...
        v.image = UIImage.managerToSecond(name: (String(noti_presentationPath)))
    ...
    private lazy var icon: UIImageView = {
        ...
        v.layer.borderColor = UIColor(hex: (String(constPlusMsg)))?.cgColor
    ...
    private lazy var nicknameLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(constPlusMsg)))
...
extension ToleranceView {
    /// 刷新视图
    //: func refreshGift(_ model: SocialBroadcastModel) {
    func propertyVoice(_ model: ReduceResult) {
        //: giftModel = model
        giftModel = model
        //: icon.setUrlImage(urlStr: giftModel.headPic)
        icon.poolFee(urlStr: giftModel.headPic)
        ...
        if giftModel.broadStatus == 1 { // 中奖
            ...
            contentLab.text = (String(mainChapterPath)).localized + "\(giftModel.totalCoin)" + (String(appTickValue) + String(appPlainFormat.prefix(4))).localized + giftModel.gameName
            ...
            giftImgV.poolFee(urlStr: giftModel.img)
            //: giftNumLab.isHidden = false
            giftNumLab.isHidden = false
            //: giftNumLab.text = " x\(giftModel.num)"
            giftNumLab.text = " x\(giftModel.num)"
            //: let rangeText = "just sent".localized
            let rangeText = (String(dataEvolutionData) + kCurrentData.replacingOccurrences(of: "significant", with: "t")).localized
            //: let giftName = changeGiftName(giftmodel: giftModel)
            let giftName = show(giftmodel: giftModel)
            ...
            if let range = fullText.range(of: rangeText) {
                ...
                attributedString.addAttribute(.foregroundColor, value: UIColor(hex: (String(userSignValue.prefix(7))))!, range: nsRange)
            }
    ...
    private func show(giftmodel: ReduceResult) -> String {
...
extension ToleranceView {
    ...
    private func rateFixed() {
        //: self.frame = CGRect(x: 0, y: StatusBarNavigationBarHeight, width: ScreenWidth, height: 60)
        self.frame = CGRect(x: 0, y: k_underSubtleTitle, width: show_errorChangeFormat, height: 60)
    ...
    private func activityEducateMinimumCorner() {
