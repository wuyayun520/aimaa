...
fileprivate let main_informationMessage:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "SHBDropMenuCell" :*/
fileprivate let user_maxTranslateString:String = "SHBDroinvisible anima flexible promise pure"
fileprivate let notiTailData:String = "pMenuCellmaster phone produce beneath"

/*: "title" :*/
fileprivate let userInvitationData:String = "plustle"
...
public class LostImpact: UIView {
    //: var menuDelegate: DropDownMenuViewDelegate?
    var menuDelegate: InputComponent?
    ...
    var menuBgColor: UIColor? {
        ...
        //: }
    } // = Config.AgainstInfo.menuBgColor
    // 线条颜色
    //: var lineColor = Config.MenuCellConfig.lineColor
    var lineColor = Config.BorderPlatform.lineColor
    // cell高度
    //: public var menuCellHeight = Config.MenuCellConfig.menuCellHeight
    public var menuCellHeight = Config.BorderPlatform.menuCellHeight
    /**
     *  table最大高度限制
     *  默认：5 * cellHeight
     */
    //: var menuMaxHeight = Config.MenuCellConfig.menuMaxHeight
    var menuMaxHeight = Config.BorderPlatform.menuMaxHeight
    /**
     *  小三角高度
     *  45°等腰三角形
     */
    ...
    public var menuStyle = Config.initialMenuStyle ?? ModelAdd.MenuDarkStyle {
        //: didSet {
        didSet {
            //: switch menuStyle {
            switch menuStyle {
...
//        menuView.refreshUI()
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: main_informationMessage.reversed(), encoding: .utf8)!)
    }

    //: private func makeContentView() -> UIView {
    private func afterMeasure() -> UIView {
        //: let view  = UIView()
        let view = UIView()
        //: return view
        return view
    ...
    private func quantityryFormat() -> UITableView {
    ...
    private func cluster() {
        ...
        mTable.register(TrackQueryer.self, forCellReuseIdentifier: (String(user_maxTranslateString.prefix(6)) + String(notiTailData.prefix(9))))
        //: contentView.addSubview(mTable)
        contentView.addSubview(mTable)
    }

    //: private func refreshUI() {
    private func sizeBounce() {
        //: drawmTableFrame()
        joinBy()
        //: mTable.reloadData()
        mTable.reloadData()
...
extension LostImpact: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: (String(user_maxTranslateString.prefix(6)) + String(notiTailData.prefix(9))), for: indexPath) as! TrackQueryer
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        selectionAction?(indexPath.row, cellModel.title ?? (userInvitationData.replacingOccurrences(of: "plus", with: "ti")))
        //: animateRemoveView()
        qualityMomentum()

        //: menuDelegate?.didClickSelectedRow(index: indexPath.row, title: cellModel.title ?? "")
        menuDelegate?.roundDepth(index: indexPath.row, title: cellModel.title ?? "")
    }
}
