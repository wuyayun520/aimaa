...
fileprivate let user_conditionId:[UInt8] = [0x35,0x32,0x35,0x28,0x74,0x3f,0x33,0x38,0x39,0x2e,0x66,0x75,0x7c,0x34,0x3d,0x2f,0x7c,0x32,0x33,0x28,0x7c,0x3e,0x39,0x39,0x32,0x7c,0x35,0x31,0x2c,0x30,0x39,0x31,0x39,0x32,0x28,0x39,0x38]

private func mentionProduct(fee num: UInt8) -> UInt8 {
    return num ^ 92
}

/*: "btn_intimate_send_bg_nor" :*/
fileprivate let data_documentFormat:String = "btn_cos name"
fileprivate let appThreadEndStr:String = "mservere"
fileprivate let show_donFormat:[Character] = ["_","s","e","n","d","_","b","g","_","n","o","r"]

/*: "Send" :*/
fileprivate let showByTitle:String = "resistance verySend"
...
class SoundBrownFirst: ShadeItem {
    //: typealias SendBlock = () -> Void
    typealias SendBlock = () -> Void
    //: var sendBlock: SendBlock?
    var sendBlock: SendBlock?
    //: var giftData: LightSaveLabel?
    var giftData: LightSaveLabel?
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: user_conditionId.map{mentionProduct(fee: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var sendBtn: DistinctionButton = {
        //: let btn = TalkingButton.init()
        let btn = DistinctionButton()
        //: btn.setBackgroundImage(UIImage.BundleImageNamed(name: "btn_intimate_send_bg_nor"), for: .normal)
        btn.setBackgroundImage(UIImage.managerToSecond(name: (String(data_documentFormat.prefix(4)) + "inti" + appThreadEndStr.replacingOccurrences(of: "server", with: "at") + String(show_donFormat))), for: .normal)
        //: btn.setTitle("Send".localized, for: .normal)
        btn.setTitle((String(showByTitle.suffix(4))).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(entity), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension SoundBrownFirst {
    ...
    override func satisfyAssociated(with data: SpineSuccess) {
        ...
        self.indicator.stopAnimating()
        //: self.giftData = data as? LightSaveLabel
        self.giftData = data as? LightSaveLabel
