...
/*: "http://stat. :*/
fileprivate let appSpreadReflectId:String = "http:/constraint deem until close pool"
fileprivate let app_mediaId:String = "density"

/*: .com" :*/
fileprivate let showMessageErrorData:String = "drawing confirm step ad.com"

/*: "http://" :*/
fileprivate let appBroadMessage:[Character] = ["h","t","t","p",":","/","/"]

/*: "https://" :*/
fileprivate let user_amName:String = "httinteraction"
fileprivate let main_directFlexKey:String = "://"

/*: "http://m. :*/
fileprivate let main_sunValue:String = "http:drawing compose prepare could pattern"
fileprivate let data_guideTitle:String = "zz monster texture//m."

/*: "http:// :*/
fileprivate let mainNormInputMessage:String = "behavior trackhttp://"

/*: "3,90" :*/
fileprivate let userOfString:String = "3,9maximum"

/*: "1990-01-01" :*/
fileprivate let dataDiscountFaceYesterdayId:String = "1990-"
fileprivate let appCosyPath:String = "0master-0master"

/*: "MM-dd-yyyy" :*/
fileprivate let noti_rollTextFormat:String = "man restore replace mysteryMM-dd-"
fileprivate let noti_userKey:String = "dimensiondimensiondimensiondimension"
...
class ObtainMomentum: NSObject {
    ...
    var requestType: QueryerReflectGraphic?
    ...
    override init() {
        ...
        self.requestServer = PresentLocal.share.serverUrlStr
        //: self.showErrorStatusBar = true
        self.showErrorStatusBar = true
    }

    //: func paramsContainBinaryObject() -> Bool {
    func operateMemory() -> Bool {
...
struct UniformProcess: HandyJSON {
    ...
    var CSConfig = TranslateModel() // 私聊官方客服提示消息模型
    ...
    var reportDomain: String {
        ...
        if statDomain.isEmptyString {
            ...
            return (String(appSpreadReflectId.prefix(6)) + "/stat" + app_mediaId.replacingOccurrences(of: "density", with: ".")) + "\(app_readTitle)" + (String(showMessageErrorData.suffix(4)))
            //: } else {
        } else {
            //: if !statDomain.hasPrefix("http://") && !statDomain.hasPrefix("https://") {
            if !statDomain.hasPrefix((String(appBroadMessage))) && !statDomain.hasPrefix((user_amName.replacingOccurrences(of: "interaction", with: "ps") + main_directFlexKey.capitalized)) {
                //: return "http://".appending(statDomain)
                return (String(appBroadMessage)).appending(statDomain)
                //: }else {
            } else {
                //: return statDomain
                return statDomain
    ...
    var urlH5Domain: String {
        ...
        if urlDomain.isEmpty {
            ...
            return (String(main_sunValue.prefix(5)) + String(data_guideTitle.suffix(4))) + "\(app_readTitle)" + (String(showMessageErrorData.suffix(4)))
            //: } else {
        } else {
            //: return "http://\(urlDomain)"
            return (String(mainNormInputMessage.suffix(7))) + "\(urlDomain)"
        }
    }
...
public class FoundSystem: NSObject, HandyJSON {
    ...
    var vipSetting = [ShouldViaBlack]() // vip皮肤
    //: var gameShowBit = 0
...
@objcMembers public class HideBasic: NSObject, HandyJSON {
    //: required public override init() {
    override public required init() {}
    ...
    var interest: [BlueName]?
    //: var aboutMe: Array<UserSeleteTagModel>?
    var aboutMe: [BlueName]?
    ...
    var callPopupSetting = (userOfString.replacingOccurrences(of: "maximum", with: "0")) // 表示来电弹窗配置，如”3,90”表示3s后弹第一次，后续90s，返回”0,0”表示不弹窗
    //: var callTabSwitch = 0
    ...
    public lazy var age: String = {
        ...
        let birthdayDate = NSDate.drawingTo(self.birthday ?? (dataDiscountFaceYesterdayId.capitalized + appCosyPath.replacingOccurrences(of: "master", with: "1")), dateFormat: (String(noti_rollTextFormat.suffix(6)) + noti_userKey.replacingOccurrences(of: "dimension", with: "y")))
        //: guard let currentYear = NSDate.getCurrentDateComponents().year else { return "0" }
        guard let currentYear = NSDate.toleranceMeasure().year else { return "0" }
...
public struct BlueName: HandyJSON, Equatable {
    //: var tag_id: String?
    var tag_id: String?
    //: var tag_name = ""
    var tag_name = ""
    ...
    func noAdd(compareTo: BlueName) -> Bool {
        //: return
        return
...
struct DetailFilter: HandyJSON {
    //: var aboutMe: Array<UserTagTypeModel>!
    var aboutMe: [NumberervalModel]!
    //: var interest: Array<UserTagTypeModel>!
    var interest: [NumberervalModel]!
}

//: struct UserTagTypeModel: HandyJSON {
struct NumberervalModel: HandyJSON {
    //: var name = ""
    var name = ""
    //: var name_ar = ""
    var name_ar = ""
...
struct TranslateModel: HandyJSON {
...
struct ShouldViaBlack: HandyJSON {
...
struct PrimaryAdjust: HandyJSON {
    //: var sendImg = 10
    var sendImg = 10 // 发送图片
    //: var sendVideo = 10
