...
fileprivate let user_brushFormat:[UInt8] = [0xaf,0xa8,0xaf,0xb2,0xee,0xa5,0xa9,0xa2,0xa3,0xb4,0xfc,0xef,0xe6,0xae,0xa7,0xb5,0xe6,0xa8,0xa9,0xb2,0xe6,0xa4,0xa3,0xa3,0xa8,0xe6,0xaf,0xab,0xb6,0xaa,0xa3,0xab,0xa3,0xa8,0xb2,0xa3,0xa2]

private func ondPrepare(disk num: UInt8) -> UInt8 {
    return num ^ 198
}
...
protocol UntilStatus: NSObjectProtocol {
    ///
...
class AwayView: UIView {
    ...
    var cell_delegate: UntilStatus?

    //: var model: TalkingMatchResultModel?
    var model: DifficultRangeSensorSubtleOption?
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: user_brushFormat.map{ondPrepare(disk: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(duringStrategyChart(pan:)))
        //: v.delegate = self
        v.delegate = self
        //: return v
        return v
...
extension AwayView: UIGestureRecognizerDelegate {
    ...
    @objc func duringStrategyChart(pan: UIPanGestureRecognizer) {
        //: switch pan.state {
        switch pan.state {
        //: case .began:
        case .began:
        ...
        case .changed:
            ...
            if InputCloseInfo.shared.direction == .rightToLeft {
                ...
                if movePoint.x > show_errorChangeFormat / 4 {
                    ...
                    self.cell_delegate?.buttWithMap(cell: self, forMovePoint: self.currentPoint)
    ...
    func disableSelected() {
        ...
        self.cell_delegate?.densityNative(cell: self)
    }
    ...
    func enablelyTranslation(direction: AdjustSort) {
        //: switch (direction) {
        switch direction {
        //: case .Left:
        case .Left:
