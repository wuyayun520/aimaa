...
/*: "serialQueue" :*/
fileprivate let show_followStr:String = "slow permanent infrastructureserialQu"
fileprivate let dataComplyStr:[Character] = ["e","u","e"]

/*: "fromUids" :*/
fileprivate let notiRangeKey:[UInt8] = [0xed,0xf9,0xf6,0xf4,0xdc,0xf0,0xeb,0xfa]

fileprivate func directNow(normally num: UInt8) -> UInt8 {
...
let constDiskBoxTitle = 50

//: @objcMembers
@objcMembers
//: public class DBUserInfoManager: NSObject {
public class ProgressAssociated: NSObject {
    ...
    private static var _instance: ProgressAssociated?
    //: class func shared() -> DBUserInfoManager {
    class func betweenConsumption() -> ProgressAssociated {
        //: guard _instance != nil else {
        guard _instance != nil else {
            //: _instance = DBUserInfoManager()
            _instance = ProgressAssociated()
            // 添加退出账号通知
            //: NotificationCenter.default.addObserver(_instance!,
            NotificationCenter.default.addObserver(_instance!,
                                                   //: selector: #selector(accountLogout),
                                                   selector: #selector(provide),
    ...
    @objc private func provide() {
        //: DBUserInfoManager.destroy()
        ProgressAssociated.enablece()
    }
}
...
extension ProgressAssociated {
    /// 获取用户消息
    /// - Parameters:
    ///   - userIdsArr: 用户ids
    ///   - completion: 回调
    //: class func cache_getUserInfo(with userIdsArr: [String], completion: @escaping ([DocumentLevelPurple]?, Error?) -> Void) {
    class func recommendFor(with userIdsArr: [String], completion: @escaping ([DocumentLevelPurple]?, Error?) -> Void) {
        // 数量过多，wcdb的where语句会因过长而报错，所以做分页
        //: var newUserIdsArr = userIdsArr
        var newUserIdsArr = userIdsArr
        //: if DBUserInfoManager.shared().needReqUidsSet != nil {
        if ProgressAssociated.betweenConsumption().needReqUidsSet != nil { // 合并去重
            //: newUserIdsArr = Array(DBUserInfoManager.shared().needReqUidsSet!.union(userIdsArr))
            newUserIdsArr = Array(ProgressAssociated.betweenConsumption().needReqUidsSet!.union(userIdsArr))
            //: DBUserInfoManager.shared().needReqUidsSet = nil
            ProgressAssociated.betweenConsumption().needReqUidsSet = nil
        }
        ...
        let pageSize = constDiskBoxTitle
        //: repeat {
        repeat {
            ...
            per(with: subArr, completion: completion)
            //: location += len
            location += len
    ...
    class func ting(with userArr: [String], completion: @escaping ([[String: Any]]?, Error?) -> Void) {
        ...
        let serialQueue = DispatchQueue(label: (String(show_followStr.suffix(8)) + String(dataComplyStr)))
        ...
        serialQueue.async {
            ...
            for item in finalArray {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                    ...
                    PriorMedia.lineSquare(toUids: requestUidsStr) { _, list in
                        //: defer { sema.signal() }
                        defer { sema.signal() }
    ...
    public class func triggerReady(message: V2TIMMessage) -> ComponentMultiple {
        //: let key = TUISocialChatManager.checkPrimaryKey(withMsgModel: message)
        let key = IndependentObserve.run(withMsgModel: message)
        //: var cacheWrap = WCDBVoiceMsgTable.db_getVoiceMsg(with: key)
        var cacheWrap = ComponentMultiple.goldRecommend(with: key)
        //: if cacheWrap == nil {
        if cacheWrap == nil {
            //: let info = TUISocialChatManager.packageDict(withTXMessage: message)
            let info = IndependentObserve.distanceContext(withTXMessage: message)
            //: cacheWrap = WCDBVoiceMsgTable.db_inserVoiceMsgToDB(info)
            cacheWrap = ComponentMultiple.liteAction(info)
        }
        //: return cacheWrap!
        return cacheWrap!
    }
...
extension ProgressAssociated {
    ...
    //: @discardableResult
    @discardableResult
    //: class func cache_getCachedUserInfo(targetId: String) -> DocumentLevelPurple? {
    class func betweenPolicy(targetId: String) -> DocumentLevelPurple? {
        //: return WCDBUserInfoTable.db_getUserInfo(with: targetId)
        return TakeOval.bodyEx(with: targetId)
    }

    /// 更新数据库【用户模型】，并发送通知
    /// - Parameter userInfo: 用户信息model
    //: class func cache_updateMsgUserInfoAndPostNotif(with userInfo: DocumentLevelPurple?) {
    class func informationMini(with userInfo: DocumentLevelPurple?) {
        ...
        let model = ProgressAssociated.betweenPolicy(targetId: userInfo.uid)
        // 自定义字段需要赋值，防止被覆盖
        ...
        TakeOval.value(userInfo)
        //: DispatchQueue.main.async {
        DispatchQueue.main.async {
            //: NotificationCenter.default.post(name: USER_INFO_MSG_LIST_NOTIFICATION, object: [userInfo.uid: userInfo])
            NotificationCenter.default.post(name: kWindowTitle, object: [userInfo.uid: userInfo])
        }
    }
    ...
    class func frame(with targetId: String, intimate: Int, currTime: Int) {
        //: guard let model = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId) else { return }
        guard let model = ProgressAssociated.betweenPolicy(targetId: targetId) else { return }
    ...
    class func vertical(with targetId: String, time: Int) {
        //: guard let model = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId) else { return }
        guard let model = ProgressAssociated.betweenPolicy(targetId: targetId) else { return }
    ...
    class func dailyWithinQuit(with targetUid: String) {
        //: WCDBUserInfoTable.db_deleteUserInfo(with: targetUid)
        TakeOval.replyFire(with: targetUid)
        //: DBUserInfoManager.shared().reqedUidsSet.remove(targetUid)
        ProgressAssociated.betweenConsumption().reqedUidsSet.remove(targetUid)
    }
}
...
extension ProgressAssociated {
    ...
    ///   - userIdsArr: 用户ids
    ///   - completion: 回调
    //: private class func inner_getUserInfo(with userIdsArr: [String], completion: @escaping ([DocumentLevelPurple]?, Error?) -> Void) {
    private class func per(with userIdsArr: [String], completion: @escaping ([DocumentLevelPurple]?, Error?) -> Void) {
        //: let userModels = WCDBUserInfoTable.db_getUserInfos(with: userIdsArr)
        let userModels = TakeOval.untilLike(with: userIdsArr)
        ...
        for uid in userIdsArr where !userModels.contains(where: { $0.uid == uid }) {
            ...
            if ProgressAssociated.betweenConsumption().reqedUidsSet.contains(uid) == false {
        ...
        if !noDBUidArr.isEmpty {
            ...
            ProgressAssociated.sayMy(with: noDBUidArr)
        }
    }
    ...
    private class func sayMy(with userIdsArr: [String]) {
        ...
        ProgressAssociated.betweenConsumption().reqedUidsSet = ProgressAssociated.betweenConsumption().reqedUidsSet.union(userIdsArr)

        // 请求用户消息列表
        //: TalkingChatRequestTool.req_refreshMsgUserListInfo(param: ["fromUids": requestUidsStr]) { succeed, dataArr in
        PriorMedia.launchDownSound(param: [String(bytes: notiRangeKey.map{directNow(normally: $0)}, encoding: .utf8)!: requestUidsStr]) { succeed, dataArr in
            //: guard succeed else {
            guard succeed else { // 失败还原数据
                //: DBUserInfoManager.shared().reqedUidsSet.subtract(userIdsArr)
                ProgressAssociated.betweenConsumption().reqedUidsSet.subtract(userIdsArr)
                //: return
                return
            }
            // 更新数据库
            //: DBUserInfoManager.updateMsgUserInfoTable(userInfoArr: dataArr)
            ProgressAssociated.capacityFor(userInfoArr: dataArr)
            // 发送通知
            //: var retInfoDict: [String: DocumentLevelPurple] = [:]
            var retInfoDict: [String: DocumentLevelPurple] = [:]
            ...
            if !retInfoDict.isEmpty {
                ...
                DispatchQueue.main.async {
                    ...
                    NotificationCenter.default.post(name: kWindowTitle, object: retInfoDict)
                }
            }
        }
    }
    ...
    //: private class func updateMsgUserInfoTable(userInfoArr: [DocumentLevelPurple]) {
    private class func capacityFor(userInfoArr: [DocumentLevelPurple]) {
        //: for aWrap in userInfoArr {
        for aWrap in userInfoArr {
            //: WCDBUserInfoTable.db_updateUserInfo(aWrap)
            TakeOval.value(aWrap)
        }
    }
}
