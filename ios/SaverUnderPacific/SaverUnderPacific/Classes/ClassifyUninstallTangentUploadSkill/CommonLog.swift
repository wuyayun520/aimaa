...
/*: "gifts" :*/
fileprivate let kRecentPath:[Character] = ["g","i","f","t","s"]

/*: "vipGifts" :*/
fileprivate let noti_galleryMechanismPath:[Character] = ["v","i","p","G","i","f","t","s"]

/*: "packGifts" :*/
fileprivate let app_giftMsg:String = "cord complaint wherepackG"

/*: "giftSendItem" :*/
fileprivate let appChapterMsg:String = "permission leavegiftS"
fileprivate let showRollMessage:[Character] = ["e","n","d","I","t","e","m"]
...
class CommonLog: NSObject {
    ...
    private var cacheDict = [String: [DistanceCould: [Any]]]()
    // 数据锁
    ...
    private var giftType = WhiteDirectDismiss.normal

    //: static let share = TalkingChatGiftManager()
    static let share = CommonLog()
...
extension CommonLog {
    ...
    func beyondDepth(type: WhiteDirectDismiss = .normal, isRefresh: Bool = false, completion: @escaping () -> Void) {
        //: TalkingChatGiftManager.share.giftType = type
        CommonLog.share.giftType = type
        //: if isRefresh == false {
        if isRefresh == false {
            //: let dataDict = cacheDict[type.rawValue]
            let dataDict = cacheDict[type.rawValue]
            //: let hotArr = dataDict?[GiftDataType.Hot] ?? [Any]()
            let hotArr = dataDict?[DistanceCould.Hot] ?? [Any]()
        ...
        if type == .groupChat {
            ...
            CommonLog.share.asBot(completion: { isSucceed in
            ...
            CommonLog.share.exceptOwn(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            CommonLog.share.jumpRole(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
    ...
    private func jumpRole(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = CommonLog.share.giftType
        //: TalkingChatRequestTool.req_ChatGiftList(type: type.rawValue) { succeed, result, errorModel in
        PriorMedia.lastExtend(type: type.rawValue) { succeed, result, errorModel in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
                //: var dataDict = [GiftDataType: [Any]]()
                var dataDict = [DistanceCould: [Any]]()
                // hot礼物
                //: if let array: Array = data["gifts"] as? Array<Any> {
                if let array: Array = data[(String(kRecentPath))] as? [Any] {
                    //: var hotArr = [Any]()
                    var hotArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<EvaluationMenuRate>.deserialize(from: array as? Array) {
                        //: hotArr.append(contentsOf: datas as [Any])
                        hotArr.append(contentsOf: datas as [Any])
                    }
                    //: dataDict[GiftDataType.Hot] = hotArr
                    dataDict[DistanceCould.Hot] = hotArr
                }

                // vip礼物
                //: if let array: Array = data["vipGifts"] as? Array<Any> {
                if let array: Array = data[(String(noti_galleryMechanismPath))] as? [Any] {
                    //: var vipArr = [Any]()
                    var vipArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<EvaluationMenuRate>.deserialize(from: array as? Array) {
                    ...
                    dataDict[DistanceCould.Vip] = vipArr
                }

                // Bags礼物
                //: if let array: Array = data["packGifts"] as? Array<Any> {
                if let array: Array = data[(String(app_giftMsg.suffix(5)) + "ifts")] as? [Any] {
                    //: var bagsArr = [Any]()
                    var bagsArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<EvaluationMenuRate>.deserialize(from: array as? Array) {
                    ...
                    dataDict[DistanceCould.Bags] = bagsArr
                }
                ...
                let sendItem: Array = data[(String(appChapterMsg.suffix(5)) + String(showRollMessage))] as! [Any]
                ...
                for i in 0 ..< sendItem.count {
                    ...
                    if let configModel = JSONDeserializer<SClip>.deserializeFrom(dict: sendItem[i] as? [String: Any], designatedPath: nil) {
    ...
    private func asBot(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = CommonLog.share.giftType
        //: TalkingChatRequestTool.req_ChatRoomGiftList(type: type.rawValue) { succeed, result, errorModel in
        PriorMedia.pointCap(type: type.rawValue) { succeed, result, errorModel in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func isDay(params: [String: Any], completion: @escaping (_ data: [Any]?) -> Void) {
        //: TalkingChatRequestTool.req_getChatRoomMembers(params: params, completion: { succeed, result, errorModel in
        PriorMedia.spendThe(params: params, completion: { succeed, result, errorModel in
            //: if succeed {
            if succeed {
                ...
                for dict in data {
                    ...
                    if let memberModel = JSONDeserializer<ReduceModel>.deserializeFrom(dict: dict as? [String: Any], designatedPath: nil) {
    ...
    private func exceptOwn(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.live
        let type = WhiteDirectDismiss.live
        //: TalkingChatRequestTool.req_LiveGiftList(type: type.rawValue) { succeed, result, errorModel in
        PriorMedia.discover(type: type.rawValue) { succeed, result, errorModel in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func identityNorm(perPageCount: Int, dataType: DistanceCould) -> Int {
    ...
    func environmentWith(dataType: DistanceCould) -> [Any] {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
    ...
    func light() -> [Any] {
        ...
        if CommonLog.share.giftType == .groupChat {
            //: return chatRoomSendItems
            return chatRoomSendItems
            //: } else if TalkingChatGiftManager.share.giftType == .live {
        } else if CommonLog.share.giftType == .live {
    ...
    func selecterTo(dataType: DistanceCould, dataArr: [Any]) {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
...
extension CommonLog {
    ...
    private func toSucceed() {
        // 刷新礼物背包
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(receiveNewGiftPackage),
                                               selector: #selector(dealAcross),
    ...
    @objc private func dealAcross() {
        //: gift_clearMemoryCache(showRed: true)
        spendPut(showRed: true)
    }
}
...
extension CommonLog {
    ...
    func spendPut(showRed: Bool = false) {
        //: TalkingChatGiftManager.share.showBagsRed = showRed
        CommonLog.share.showBagsRed = showRed
        //: cacheDict.removeAll()
        cacheDict.removeAll()
    }
}
