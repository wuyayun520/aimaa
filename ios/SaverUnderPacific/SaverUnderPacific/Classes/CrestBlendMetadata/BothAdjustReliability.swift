...
fileprivate let constDynamicsTheStr:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Reply" :*/
fileprivate let k_frequencyId:[Character] = ["R","e","p","l","y"]

/*: "#F95151" :*/
fileprivate let user_channelMsg:String = "#F95151input yesterday follow page plat"
...
let kThroughKey = (const_musicMessage + 80.0)
//: class TalkingMessageNotificationView: UIView {
class BothAdjustReliability: UIView {
    //: var startDismissBlock: ((_ model: TalkingConversationModel) -> Void)?
    var startDismissBlock: ((_ model: FormSubtleModel) -> Void)? // 开始移除视图Block
    //: var finishDismissBlock: ((_ view: TalkingMessageNotificationView) -> Void)?
    var finishDismissBlock: ((_ view: BothAdjustReliability) -> Void)? // 完成移除视图Block
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: constDynamicsTheStr.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var nickNameLabel: UILabel = {
        ...
        label.font = .yearNearby(type: .Medium, fontSize: 15)
    ...
    private lazy var replyBtn: UIButton = {
        ...
        btn.titleLabel?.font = .yearNearby(type: .Medium, fontSize: 15)
        //: btn.setTitle("Reply".localized, for: .normal)
        btn.setTitle((String(k_frequencyId)).localized, for: .normal)
        //: btn.setTitleColor(UIColor(hex: "#F95151"), for: .normal)
        btn.setTitleColor(UIColor(hex: (String(user_channelMsg.prefix(7)))), for: .normal)
        //: btn.isEnabled = false
        btn.isEnabled = false
        //: return btn
        return btn
...
extension BothAdjustReliability {
    /// 展示视图，5s后自动移除
    //: func show(_ model: TalkingConversationModel?) {
    func measureHistory(_ model: FormSubtleModel?) {
    ...
    func paraBy(needAlpha: Bool = true) {
        ...
        DispatchQueue.main.async {
            ...
            UIView.animate(withDuration: 0.5) {
                ...
                if needAlpha {
                    ...
                    self.frame.origin.y = -kThroughKey
                }
                //: } completion: { _ in
            } completion: { _ in
    ...
    @objc private func spaceDelay() {
        ...
        if currModel.chatType == .service {
            ...
            //: OriginMediumWill.share.func__pushToPriveteChatVC(chatID: currModel.targetId)
            OriginMediumWill.share.drawingPreparation(chatID: currModel.targetId)
        }
    }

    /// 滑动手势
    //: @objc private func swipeGestureEvent(gesture: UISwipeGestureRecognizer) {
    @objc private func rawReload(gesture: UISwipeGestureRecognizer) {
...
extension BothAdjustReliability {
    ...
    func projectionFactor() {
        ...
        if let userInfo = currModel.gj_userInfo {
            ...
            headImgView.chest(urlStr: userInfo.headPic, placeImg: UIImage.queryedChange(sex: userInfo.sex))
            //: if !userInfo.headPicFrame.isEmptyString {
            if !userInfo.headPicFrame.isEmptyString {
                //: iconBorder.isHidden = false
                iconBorder.isHidden = false
                //: iconBorder.setHeadFrameUrlImage(urlStr: userInfo.headPicFrame)
                iconBorder.quantityoIt(urlStr: userInfo.headPicFrame)
                //: } else {
            } else {
    ...
    private func bar() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(spaceDelay))
        //: self.addGestureRecognizer(tap)
        self.addGestureRecognizer(tap)

        //: let swipe = UISwipeGestureRecognizer(target: self, action: #selector(swipeGestureEvent))
        let swipe = UISwipeGestureRecognizer(target: self, action: #selector(rawReload))
        //: swipe.direction = .up
        swipe.direction = .up
