...
fileprivate let showSubjectAdjustmentStr:[UInt8] = [0xd4,0xd9,0xd4,0xdf,0x93,0xce,0xda,0xcf,0xd0,0xdd,0xa5,0x94,0x8b,0xd3,0xcc,0xde,0x8b,0xd9,0xda,0xdf,0x8b,0xcd,0xd0,0xd0,0xd9,0x8b,0xd4,0xd8,0xdb,0xd7,0xd0,0xd8,0xd0,0xd9,0xdf,0xd0,0xcf]

fileprivate func discloseNervous(ignore num: UInt8) -> UInt8 {
...
/*: "#AAAAAA" :*/
fileprivate let userEnhanceId:[Character] = ["#","A","A","A","A","A","A"]

/*: "btn_chatsettings_unchoice" :*/
fileprivate let dataListenerCharmContinuingFormat:String = "everything coordinate now projection tradebtn_ch"
fileprivate let constNetworkStr:[Character] = ["a","t","s","e","t","t","i","n","g","s","_","u","n","c"]
fileprivate let user_amName:[Character] = ["h","o","i","c","e"]

/*: "btn_chatsettings_choiced" :*/
fileprivate let showNothingFormat:String = "thumb justbtn_chat"
fileprivate let noti_substantialStr:String = "mustetti"
fileprivate let userToolAdvertisingTitle:String = "himageiced"

/*: "Top" :*/
fileprivate let notiHomeGesturePath:String = "storage half participation exportTop"

/*: "Save" :*/
fileprivate let notiVersusDestroyModelMessage:[Character] = ["S","a","v","e"]

/*: "Cancel" :*/
fileprivate let noti_refugeePhoneInName:String = "cur"
fileprivate let showSwitcherFormat:String = "apauseel"
...
class GreatView: UIView {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: showSubjectAdjustmentStr.map{discloseNervous(ignore: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIView = {
        ...
        let height = actualWidth(w: 475) + const_closedAddSegmentPath
        //: let v = UIView(frame: CGRect(x: 0, y: ScreenHeight-height, width: ScreenWidth, height: height))
        let v = UIView(frame: CGRect(x: 0, y: appLoadMessage - height, width: show_errorChangeFormat, height: height))
        //: v.backgroundColor = .white
        v.backgroundColor = .white
        //: v.Corner(width: v.width, height: v.height, corners: [.topLeft, .topRight], cornerRadii: .init(width: 15, height: 15))
        v.locationBody(width: v.width, height: v.height, corners: [.topLeft, .topRight], cornerRadii: .init(width: 15, height: 15))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var textBgView: UIView = {
        ...
        v.backgroundColor = UIColor.alongSupposed()
    ...
    private lazy var textView: YYTextView = {
        ...
        tv.textColor = UIColor.untilExpected()
        //: tv.font = UIFont.pingfangFont(type: .Regular, fontSize: 16)
        tv.font = UIFont.yearNearby(type: .Regular, fontSize: 16)
    ...
    private lazy var limitLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(userEnhanceId)))
        //: return lab
        return lab
        //: }()
    }()
    ...
    private lazy var topBtn: DistinctionButton = {
        ...
        btn.setImage(UIImage.managerToSecond(name: (String(dataListenerCharmContinuingFormat.suffix(6)) + String(constNetworkStr) + String(user_amName))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "btn_chatsettings_choiced"), for: .selected)
        btn.setImage(UIImage.managerToSecond(name: (String(showNothingFormat.suffix(8)) + noti_substantialStr.replacingOccurrences(of: "must", with: "s") + "ngs_c" + userToolAdvertisingTitle.replacingOccurrences(of: "image", with: "o"))), for: .selected)
        ...
        btn.setTitle((String(notiHomeGesturePath.suffix(3))).localized, for: .normal)
        //: btn.spaceBetweenTitleAndImage = 6
        btn.spaceBetweenTitleAndImage = 6
        //: btn.addTarget(self, action: #selector(topButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(constraintDoing), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var saveBtn: UIButton = {
        ...
        btn.setTitle((String(notiVersusDestroyModelMessage)).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(createFull), for: .touchUpInside)
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        btn.setTitle((noti_refugeePhoneInName.replacingOccurrences(of: "cur", with: "C") + showSwitcherFormat.replacingOccurrences(of: "pause", with: "nc")).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(cell), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension GreatView: YYTextViewDelegate {
...
extension GreatView {
    ...
    @objc private func constraintDoing() {
    ...
    func behindScreen(_ content: String, isTop: Int, toUid: String) {
        //: targetId = toUid
        targetId = toUid
...
extension GreatView {
    ...
    private func poolCalculateWeekDimension() {
        //: frame = CGRect(x: 0, y: 0, width: ScreenWidth, height: ScreenHeight)
        frame = CGRect(x: 0, y: 0, width: show_errorChangeFormat, height: appLoadMessage)
