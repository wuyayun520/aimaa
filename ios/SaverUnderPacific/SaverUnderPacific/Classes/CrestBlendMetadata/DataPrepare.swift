...
/*: "Search" :*/
fileprivate let appLeaveHideData:String = "Searcclock open shadow large"
fileprivate let const_checkData:String = "spot"

/*: "#F5F5F5" :*/
fileprivate let app_rashString:[Character] = ["#","F","5","F","5"]
fileprivate let notiPhotoFaceId:String = "Fdate"

/*: "Search by UID" :*/
fileprivate let app_exitTitle:[Character] = ["S","e","a","r","c","h"," ","b"]
fileprivate let dataCreationValue:[Character] = ["y"," ","U","I","D"]

/*: "icon_search_sea" :*/
fileprivate let userMovementString:String = "yourcon"
fileprivate let dataPlatformId:String = "rch_slibrary scale"
fileprivate let dataEntryUrl:[Character] = ["e","a"]

/*: "Can’t find~" :*/
fileprivate let const_valueMessage:String = "crop unionCan’t"
fileprivate let main_replacementMessage:String = " find~equally floor block"

/*: "icon_kong_kong_default" :*/
fileprivate let user_wedData:String = "icon_kone fixed"
fileprivate let main_addressKey:String = "event scope rid simultaneously layong_"
fileprivate let mainTranslationData:String = "defaufail"

/*: "Please enter the query UID" :*/
fileprivate let const_productSceneData:String = "feedback hello following gravityPleas"
fileprivate let mainQualityTitle:[Character] = ["r"," ","t","h","e"," ","q","u","e","r","y"," ","U","I","D"]
...
class DataPrepare: EasyChainSensor {
    ...
    private lazy var searchBtn: UIButton = {
        ...
        btn.setTitle((String(appLeaveHideData.prefix(5)) + const_checkData.replacingOccurrences(of: "spot", with: "h")).localized, for: .normal)
        ...
        btn.setBackgroundImage(UIImage.domainCover(colors: UIColor.running(), size: CGSize(width: 72, height: 32)), for: .normal)
        //: btn.layer.cornerRadius = 16
        btn.layer.cornerRadius = 16
        //: btn.clipsToBounds = true
        btn.clipsToBounds = true
        //: btn.addTarget(self, action: #selector(searchButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(outsideImpact), for: .touchUpInside)
        //: btn.alpha = 0.5
        btn.alpha = 0.5
    ...
    private lazy var searchTF: UITextField = {
        ...
        tf.backgroundColor = UIColor(hex: (String(app_rashString) + notiPhotoFaceId.replacingOccurrences(of: "date", with: "5")))
        //: tf.textColor = UIColor.appTitleColor()
        tf.textColor = UIColor.untilExpected()
        //: tf.font = UIFont.pingfangRugularFont(fontSize: 16)
        tf.font = UIFont.enterCommon(fontSize: 16)
        ...
        tf.placeholder = (String(app_exitTitle) + String(dataCreationValue)).localized
    ...
    private lazy var unlessEditingView: UIView = {
        ...
        let image = UIImage.managerToSecond(name: (userMovementString.replacingOccurrences(of: "your", with: "i") + "_sea" + String(dataPlatformId.prefix(5)) + String(dataEntryUrl)))
    ...
    private lazy var tableView: UITableView = {
        ...
        tabV.register(HiddenAbout.self, forCellReuseIdentifier: HiddenAbout.className())
    ...
    private lazy var emptyV: EmptyView = {
        ...
        style.TipsTitle = (String(const_valueMessage.suffix(5)) + String(main_replacementMessage.prefix(6))).localized
        //: style.TipsIcon = "icon_kong_kong_default"
        style.TipsIcon = (String(user_wedData.prefix(6)) + "ong_k" + String(main_addressKey.suffix(4)) + mainTranslationData.replacingOccurrences(of: "fail", with: "lt"))
        ...
        let v = EmptyView(frame: CGRect(x: 0, y: 14 + constPainterPath, width: show_errorChangeFormat, height: appLoadMessage - (14 + constPainterPath + k_underSubtleTitle)), style: style)
    ...
    private lazy var reqManager: CombineOver = //: return CombineOver()
        .init()
    //: }()
}
...
extension DataPrepare {
    // 根据uid查询用户
    //: private func req_searchUid() {
    private func sinceTime() {
        //: self.view.endEditing(true)
        self.view.endEditing(true)
        //: guard searchTF.text?.count ?? 0 > 0 else {
        guard searchTF.text?.count ?? 0 > 0 else {
            //: self.func__showStatusBarErrorMsg(showMsg: "Please enter the query UID".localized)
            self.exitMarker(showMsg: (String(const_productSceneData.suffix(5)) + "e ente" + String(mainQualityTitle)).localized)
            //: return
            return
        }
        ...
        reqManager.suspend(uid: searchTF.text ?? "") { [weak self] succeed, result, errorModel in
            //: ProgressHUD.dismiss()
...
extension DataPrepare {
    ...
    @objc private func outsideImpact() {
        //: req_searchUid()
        sinceTime()
    }
...
extension DataPrepare: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: HiddenAbout.className(), for: indexPath) as! HiddenAbout
        //: let model = reqManager.searchListDataArr[indexPath.row]
        let model = reqManager.searchListDataArr[indexPath.row]
        //: cell.refreshCell(model: model)
        cell.tillObserver(model: model)
...
extension DataPrepare: UITextFieldDelegate {
...
extension DataPrepare: JXPagingViewListViewDelegate {
