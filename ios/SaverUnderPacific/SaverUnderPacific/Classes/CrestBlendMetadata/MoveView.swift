...
/*: "#ffffff" :*/
fileprivate let dataMasterId:[Character] = ["#","f","f","f","f","f","f"]

/*: "/dist/loungePlus/index.html" :*/
fileprivate let appFatalDeployUrl:String = "/dist/spine then"
fileprivate let notiLibraryMsg:String = "lus/iintro move flag should heap"

/*: "https://" :*/
fileprivate let mainDetailRedPath:[Character] = ["h","t","t","p","s",":","/","/"]

/*: "http://" :*/
fileprivate let appTrigAdjustPath:[Character] = ["h","t","t","p",":","/","/"]
...
class MoveView: UIView {
    ...
    let Broadcast_TextColor = (String(dataMasterId))
    //: let Broadcast_ContentFont: UIFont = UIFont.pingfangMediumFont(fontSize: 12)
    let Broadcast_ContentFont: UIFont = .hangProgram(fontSize: 12)
    ...
    convenience init(model: LibraryTitleure?) {
        //: self.init()
        self.init()
    ...
    private func line() {
        ...
        if danumuMsgModel.content.count > 0 {
            ...
            var mutableAtt = parse.purchaseBy(withFontSize: 12, insert: "", atIndex: 0)
            // 限制字符长度
    ...
    func beforeWillAccount(withUrl url: String) {
        ...
        self.backgroundImageView.chest(urlStr: url) { [weak self] result, image in
            ...
            if let image {
                ...
                if image.size.height > self.Broadcast_HeightOfBackgroundImageView {
                    ...
                    newImage = image.frequencyThroughGenerate(with: CGSize(width: targetWidth, height: self.Broadcast_HeightOfBackgroundImageView))
                }
            }
    ...
    var danumuMsgModel: LibraryTitleure? {
        //: didSet {
        didSet {
            //: configModel()
            line()
    ...
    var totalTime: CGFloat {
        ...
        return (show_errorChangeFormat + widthValue) * Broadcast_SpendTime / show_errorChangeFormat
    }

    //: var totalTimeOfLine: CGFloat {
    var totalTimeOfLine: CGFloat {
        //: return widthValue * Broadcast_LineSpendTime / ScreenWidth
        return widthValue * Broadcast_LineSpendTime / show_errorChangeFormat
    }

    //: var hide2ShowTime: CGFloat {
    var hide2ShowTime: CGFloat {
        //: return widthValue * Broadcast_SpendTime / ScreenWidth
        return widthValue * Broadcast_SpendTime / show_errorChangeFormat
    }
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(taWithAverage(_:)))
        //: return t
        return t
        //: }()
    }()
...
extension MoveView {
    ...
    @objc func taWithAverage(_ tapGesture: UITapGestureRecognizer) {
        ...
        //: case "1": // 个人中心
        case "1": // 个人中心
            //: OriginMediumWill.share.func__pushToUserDetailVC(uid: self.danumuMsgModel?.jumpKey)
            OriginMediumWill.share.petAcrossAm(uid: self.danumuMsgModel?.jumpKey)

        //: case "2": // 直播间
        case "2": // 直播间
            //: if let uid = self.danumuMsgModel?.jumpKey {
            if let uid = self.danumuMsgModel?.jumpKey {
                //: OriginMediumWill.share.func_audiencePushToLiveRoomVC(uid: uid, enterType: .unknown)
                OriginMediumWill.share.dailyEnable(uid: uid, enterType: .unknown)
            }
        ...
        case "3": // H5页面
            ...
            if let url = self.danumuMsgModel?.jumpKey {
                ...
                if url.contains((String(appFatalDeployUrl.prefix(6)) + "loungeP" + String(notiLibraryMsg.prefix(5)) + "ndex.html")) {
                    //: OriginMediumWill.share.func__pushToSubscribePageWebVC()
                    OriginMediumWill.share.direction()
                    //: return
                    return
                }
                //: let httpsUrl = MarginExamineer.share.appConfigMode.urlH5Domain+url
                let httpsUrl = MarginExamineer.share.appConfigMode.urlH5Domain + url
                //: let httpUrl = httpsUrl.replacingOccurrences(of: "https://", with: "http://")
                let httpUrl = httpsUrl.replacingOccurrences(of: (String(mainDetailRedPath)), with: (String(appTrigAdjustPath)))
                //: OriginMediumWill.share.func__pushToWebVC(urlStr: httpUrl)
                OriginMediumWill.share.anyCalled(urlStr: httpUrl)
            }
