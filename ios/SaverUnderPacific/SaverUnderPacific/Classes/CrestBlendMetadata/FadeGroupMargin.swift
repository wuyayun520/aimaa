...
fileprivate let app_subjectAgePath:[UInt8] = [0x70,0x75,0x70,0x7b,0x2f,0x6a,0x76,0x6b,0x6c,0x79,0x41,0x30,0x27,0x6f,0x68,0x7a,0x27,0x75,0x76,0x7b,0x27,0x69,0x6c,0x6c,0x75,0x27,0x70,0x74,0x77,0x73,0x6c,0x74,0x6c,0x75,0x7b,0x6c,0x6b]

fileprivate func largeAbove(album num: UInt8) -> UInt8 {
...
/*: "icon_rank_coin" :*/
fileprivate let main_medalMsg:[UInt8] = [0x21,0x2b,0x27,0x26,0x17,0x3a,0x29,0x26,0x23,0x17,0x2b,0x27,0x21,0x26]

private func densityResult(host num: UInt8) -> UInt8 {
    return num ^ 72
}

/*: "icon_lounge" :*/
fileprivate let app_providerName:String = "icon_response have pad"
fileprivate let user_localUrl:String = "secondounge"

/*: "icon_rank_coin2" :*/
fileprivate let kStyleUrl:[Character] = ["i","c","o","n","_","r","a","n","k","_","c","o"]
fileprivate let data_everythingId:String = "inmagnitude"
...
class FadeGroupMargin: UITableViewCell {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: app_subjectAgePath.map{largeAbove(album: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var rankLabel = UILabel().then {
        ...
        $0.textColor = UIColor.theProvision()
        //: $0.font = .pingfangFont(type: .Semibold, fontSize: 20)
        $0.font = .yearNearby(type: .Semibold, fontSize: 20)
    }
    ...
    lazy var coinIconBtn = DistinctionButton().then {
        //: $0.setImage(UIImage.BundleImageNamed(name: "icon_rank_coin"), for: .normal)
        $0.setImage(UIImage.managerToSecond(name: String(bytes: main_medalMsg.map{densityResult(host: $0)}, encoding: .utf8)!), for: .normal)
        //: $0.setTitleColor(UIColor.white, for: .normal)
        $0.setTitleColor(UIColor.white, for: .normal)
        //: $0.titleLabel?.font = .pingfangFont(type: .Medium, fontSize: 15)
        $0.titleLabel?.font = .yearNearby(type: .Medium, fontSize: 15)
        ...
        $0.setBackgroundImage(UIImage.domainCover(colors: UIColor.running(), size: CGSize(width: actualWidth(w: 80), height: actualWidth(w: 33))), for: .normal)
    ...
    lazy var loungeImgV = UIImageView().then {
        ...
        $0.image = UIImage.managerToSecond(name: (String(app_providerName.prefix(5)) + user_localUrl.replacingOccurrences(of: "second", with: "l")))
    }
}
...
extension FadeGroupMargin {
    //: func updateRankCell(model: SocialRankItemModel, index: Int) {
    func strip(model: BoldModel, index: Int) {
        ...
        headPic.poolFee(urlStr: model.headPic)
    ...
    func reportHeap(type: Int) {
        //: let imageStr = type == 0 ? "icon_rank_coin":"icon_rank_coin2"
        let imageStr = type == 0 ? String(bytes: main_medalMsg.map{densityResult(host: $0)}, encoding: .utf8)! : (String(kStyleUrl) + data_everythingId.replacingOccurrences(of: "magnitude", with: "2"))
        //: coinIconBtn.setImage(UIImage.BundleImageNamed(name: imageStr), for: .normal)
        coinIconBtn.setImage(UIImage.managerToSecond(name: imageStr), for: .normal)
    }
}
