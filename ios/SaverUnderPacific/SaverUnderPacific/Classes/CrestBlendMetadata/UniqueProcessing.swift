...
fileprivate let show_enterValue:[Character] = ["A","p","p","R","e","s"]
fileprivate let show_placeMessage:String = "ourcetrigger"
fileprivate let k_happyString:String = "mend used defineionKey"

/*: ".zip" :*/
fileprivate let data_mountData:String = "child post.zip"

/*: "pic" :*/
fileprivate let show_clipData:String = "pihome"

/*: "nor" :*/
fileprivate let const_nextValue:String = "nothere"

/*: "svga" :*/
fileprivate let constCrucialData:[Character] = ["s","v","g","a"]

/*: "mp3" :*/
fileprivate let notiOriginalAriaName:String = "mentionp3"

/*: "model" :*/
fileprivate let kArrayTerrificStr:[Character] = ["m","o","d","e","l"]

/*: "language" :*/
fileprivate let show_willMsg:String = "llistenngulisten"
fileprivate let kNoPath:String = "GE"

/*: "video" :*/
fileprivate let showPinMsg:String = "chapterdeo"

/*: "@2x.png" :*/
fileprivate let user_warnStr:String = "perception quote broad access@2x.png"

/*: "png" :*/
fileprivate let appSearchId:[Character] = ["p","n","g"]

/*: "mp4" :*/
fileprivate let app_todayWeightMsg:[Character] = ["m","p","4"]
...
private let appImplementationValue = (String(show_enterValue) + show_placeMessage.replacingOccurrences(of: "trigger", with: "s") + "Vers" + String(k_happyString.suffix(6)))

//: enum SVGAType: String {
enum SignaturePut: String {
...
class UniqueProcessing: NSObject {
    //: static let `default` = SVGAEffectTool()
    static let `default` = UniqueProcessing()
    ...
    private lazy var AppResPath: String = {
        ...
        let res_Dir = (docuPath as NSString).appendingPathComponent(data_shapeValue)
        //: let version = Defaults.string(forKey: AppResourcesVersionKey) ?? ""
        let version = user_findBoundMessage.string(forKey: appImplementationValue) ?? ""
        //: if version == AppVersion, FileManager.default.fileExists(atPath: res_Dir) {
        if version == show_prepareMsg, FileManager.default.fileExists(atPath: res_Dir) {
            //: return res_Dir
            return res_Dir
        }
        //: let path = PodspecBundle.bundle.path(forResource: PodspecName, ofType: ".zip") ?? ""
        let path = WhenNorm.bundle.path(forResource: data_shapeValue, ofType: (String(data_mountData.suffix(4)))) ?? ""
        ...
        let zipSuc = SSZipArchive.unzipFile(atPath: path,
                                            ...
                                            password: data_shapeValue,
                                            //: progressHandler: nil)
                                            progressHandler: nil)
        //: if zipSuc {
        if zipSuc {
            //: Defaults.set(AppVersion, forKey: AppResourcesVersionKey)
            user_findBoundMessage.set(show_prepareMsg, forKey: appImplementationValue)
            //: return res_Dir
            return res_Dir
        }
        //: return ""
    ...
    private lazy var imageResPath: String = //: return (AppResPath as NSString).appendingPathComponent("pic")
        (AppResPath as NSString).appendingPathComponent((show_clipData.replacingOccurrences(of: "home", with: "c")))
    //: }()
    ...
    private lazy var norImageResPath: String = {
        ...
        return (imageResPath as NSString).appendingPathComponent((const_nextValue.replacingOccurrences(of: "there", with: "r")))
        //: }()
    }()
    ...
    private lazy var svgaResPath: String = //: return (AppResPath as NSString).appendingPathComponent("svga")
        (AppResPath as NSString).appendingPathComponent((String(constCrucialData)))
    //: }()
    ...
    private lazy var mp3ResPath: String = //: return (AppResPath as NSString).appendingPathComponent("mp3")
        (AppResPath as NSString).appendingPathComponent((notiOriginalAriaName.replacingOccurrences(of: "mention", with: "m")))
    //: }()
    ...
    public lazy var modelResPath: String = //: return (AppResPath as NSString).appendingPathComponent("model")
        (AppResPath as NSString).appendingPathComponent((String(kArrayTerrificStr)))
    //: }()
    ...
    public lazy var languagePath: String = //: return (AppResPath as NSString).appendingPathComponent("language")
        (AppResPath as NSString).appendingPathComponent((show_willMsg.replacingOccurrences(of: "listen", with: "a") + kNoPath.lowercased()))
    //: }()
    ...
    public lazy var videoPath: String = //: return (AppResPath as NSString).appendingPathComponent("video")
        (AppResPath as NSString).appendingPathComponent((showPinMsg.replacingOccurrences(of: "chapter", with: "vi")))
    //: }()
}

//: extension SVGAEffectTool {
extension UniqueProcessing {
    /// 直接获取图片
    //: func getImage(name: String) -> UIImage? {
    func random(name: String) -> UIImage? {
        //: let languagePath = (languageImageResPath as NSString).appendingPathComponent(name).appending("@2x.png")
        let languagePath = (languageImageResPath as NSString).appendingPathComponent(name).appending((String(user_warnStr.suffix(7))))
        ...
        let path = (norImageResPath as NSString).appendingPathComponent(name).appending((String(user_warnStr.suffix(7))))
    ...
    func actionAm(name: String, type: String = (notiOriginalAriaName.replacingOccurrences(of: "mention", with: "m"))) -> String {
    ...
    func automobile(name: String, type: String = (String(app_todayWeightMsg))) -> String {
