...
fileprivate let main_assetString:String = "UserDenormally quit standard partner properly"
fileprivate let appLengthAddressMsg:String = "TagMpaint seek"

/*: "UserDetailTagInterestCellID" :*/
fileprivate let appFitAgoSlowName:String = "UserDcan reading schedule accelerate"
fileprivate let data_futureFormat:String = "crush theme charm selection resultlTag"
fileprivate let constStorageUrl:[Character] = ["I","n","t","e","r","e","s","t","C","e","l","l","I","D"]

/*: "AIHobbyCellID" :*/
fileprivate let app_directionBlindMessage:String = "AIHobbmanual empty else"
...
class StrategySpine: EasyChainSensor {
    ...
    private var tableHeightModel = AmongModel() // 记录分区高度
    ...
    private lazy var tableView: UITableView = {
        ...
        tableView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: userBeyondAgainstStr, right: 0)
        ...
        tableView.register(InstructionTransition.self, forCellReuseIdentifier: InstructionTransition.className())
        //: tableView.register(TalkingUserDetailTagCell.self, forCellReuseIdentifier: "UserDetailTagMeCellID")
        tableView.register(AngleOriginal.self, forCellReuseIdentifier: (String(main_assetString.prefix(6)) + "tail" + String(appLengthAddressMsg.prefix(4)) + "eCellID"))
        //: tableView.register(TalkingUserDetailTagCell.self, forCellReuseIdentifier: "UserDetailTagInterestCellID")
        tableView.register(AngleOriginal.self, forCellReuseIdentifier: (String(appFitAgoSlowName.prefix(5)) + "etai" + String(data_futureFormat.suffix(4)) + String(constStorageUrl)))
        //: tableView.register(TalkingUserDetailGiftCell.self, forCellReuseIdentifier: TalkingUserDetailGiftCell.className())
        tableView.register(NaturalView.self, forCellReuseIdentifier: NaturalView.className())
        //: tableView.register(TalkingUserDetailTagCell.self, forCellReuseIdentifier: "AIHobbyCellID")
        tableView.register(AngleOriginal.self, forCellReuseIdentifier: (String(app_directionBlindMessage.prefix(6)) + "yCellID"))
        //: return tableView
        return tableView
        //: }()
    }()
...
extension StrategySpine: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cellType: WithoutFirst = self.sectionArr[indexPath.section]
        //: switch cellType {
        switch cellType {
        //: case .profileType:
        case .profileType:
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: InstructionTransition.className(), for: indexPath) as! InstructionTransition
            //: cell.configWithDataModel(userModel: userInfoModel)
            cell.embrace(userModel: userInfoModel)
            //: self.setProfileCellHeight(signAttribute: cell.signLab.attributedText ?? NSAttributedString.init(string: ""))
            self.detail(signAttribute: cell.signLab.attributedText ?? NSAttributedString(string: ""))
            //: return cell
            return cell
        ...
        case .aboutMeType:
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: (String(main_assetString.prefix(6)) + "tail" + String(appLengthAddressMsg.prefix(4)) + "eCellID"), for: indexPath) as! AngleOriginal
            //: cell.configTagCellWithDataModel(type: TUserDetailCellType.aboutMeType, userModel: userInfoModel)
            cell.monsterAgile(type: WithoutFirst.aboutMeType, userModel: userInfoModel)
        ...
        case .intersetsType:
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: (String(appFitAgoSlowName.prefix(5)) + "etai" + String(data_futureFormat.suffix(4)) + String(constStorageUrl)), for: indexPath) as! AngleOriginal
            //: cell.configTagCellWithDataModel(type: TUserDetailCellType.intersetsType, userModel: userInfoModel)
            cell.monsterAgile(type: WithoutFirst.intersetsType, userModel: userInfoModel)
            //: cell.collectionLayoutSubviewBlock = {[weak self] collectH in
            cell.collectionLayoutSubviewBlock = { [weak self] collectH in
                //: guard let self = self else { return }
                guard let self = self else { return }
        ...
        case .giftType:
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: NaturalView.className(), for: indexPath) as! NaturalView
...
extension StrategySpine {
    //: private func setProfileCellHeight(signAttribute: NSAttributedString) {
    private func detail(signAttribute: NSAttributedString) {
        ...
        var labelH = signAttribute.boundingRect(with: CGSize(width: show_errorChangeFormat - 58, height: CGFloat(MAXFLOAT)), options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil).size.height
    ...
    private func againAmong() -> CGFloat {
...
extension StrategySpine: JXPagingViewListViewDelegate {
...
extension StrategySpine {
    ...
    func section(_ giftArr: [WhenCould],
                 //: _ sectionArr: [TUserDetailCellType],
                 _ sectionArr: [WithoutFirst],
                 //: _ userModel: TalkingUserInfoModel) {
                 _ userModel: SpecifySmart)
    {
