...
fileprivate let notiRecentExceptionPath:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "#171717" :*/
fileprivate let mainIgnoreStr:[Character] = ["#","1","7","1","7","1","7"]

/*: "icon_cameraoff" :*/
fileprivate let showSlowFormat:String = "icon_cattention range lite"
fileprivate let main_flowMovementTitle:String = "farfar"
...
class SymbolView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        self.backgroundColor = UIColor.our(51, 51, 51, 1)
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: notiRecentExceptionPath.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var bgView: UIImageView = {
        ...
        alphaV.backgroundColor = UIColor(hex: (String(mainIgnoreStr)))
        //: alphaV.alpha = 0.6
        alphaV.alpha = 0.6
        //: v.addSubview(alphaV)
        v.addSubview(alphaV)
    ...
    private lazy var icon: UIImageView = {
        ...
        v.image = UIImage.managerToSecond(name: (String(showSlowFormat.prefix(6)) + "amerao" + main_flowMovementTitle.replacingOccurrences(of: "far", with: "f")))
        //: v.isHidden = true
        v.isHidden = true
        //: return v
        return v
...
extension SymbolView {
    ...
    func tagSchedule(urlStr: String?, isOpenCamera: LibrarySmartTemp) {
...
extension SymbolView: UIGestureRecognizerDelegate {
    /// 拖拽位移手势
    /// - Parameter pan: 手势
    //: @objc private func dragWindow(pan: UIPanGestureRecognizer) {
    @objc private func miniSkirt(pan: UIPanGestureRecognizer) {
        ...
        newCenterX = min(newCenterX, show_errorChangeFormat - self.frame.size.width * 0.5)
        //: newCenterY = max(self.frame.size.height * 0.5, newCenterY)
        newCenterY = max(self.frame.size.height * 0.5, newCenterY)
        //: newCenterY = min(newCenterY, ScreenHeight - self.frame.size.height * 0.5)
        newCenterY = min(newCenterY, appLoadMessage - self.frame.size.height * 0.5)
...
extension SymbolView {
    ...
    private func until() {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(immediate))
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(miniSkirt(pan:)))
        //: pan.delegate = self
        pan.delegate = self
