 ...
 *
 *  AdjustLength 协议和 DeployViewController 类。
 *  DeployViewController 是聊天界面的核心类，负责聊天界面大部分核心业务逻辑的处理与运算。
 *  该类的详细属性与功能，请参展本文件中 DeployViewController 的具体声明。
 *
 ******************************************************************************/
...
//: typedef NS_ENUM(NSInteger, PacificUserType) {
typedef NS_ENUM(NSInteger, PacificUserType) {
    //: PacificUserType_unDefined = 0,
    PacificUserType_unDefined = 0,
    //: PacificUserType_private = 1,
    PacificUserType_private = 1,
    //: PacificUserType_group = 2,
    PacificUserType_group = 2,
    //: PacificUserType_system = 3,
    PacificUserType_system = 3,
    //: PacificUserType_service = 4, 
    PacificUserType_service = 4, // 官方客服
    //: PacificUserType_meeting = 5, 
    PacificUserType_meeting = 5, // 公共聊天室
    //: PacificUserType_moreMsg = 6, 
    PacificUserType_moreMsg = 6, // 更多消息（超过5天未回复）
//: };
};
...
@protocol AdjustLength <NSObject>
...
- (void)tagDoing:(DeployViewController *)controller;
...
- (void)resultStrategy:(DeployViewController *)controller;
...
- (BOOL)bounce:(DeployViewController *)controller examine:(UIView *)view;
 ...
 */
//: - (AlongCrucial *)messageController:(TUIMessageController *)controller onNewMessage:(V2TIMMessage *)data;
- (AlongCrucial *)flexible:(DeployViewController *)controller heap:(V2TIMMessage *)data;
 ...
 */
//: - (BlackBy *)messageController:(TUIMessageController *)controller onShowMessageData:(AlongCrucial *)data;
- (BlackBy *)nearCompose:(DeployViewController *)controller shadow:(AlongCrucial *)data;
 ...
 *  @param cell 所点击的消息单元
 */
//: - (void)messageController:(TUIMessageController *)controller onSelectMessageAvatar:(BlackBy *)cell;
- (void)isolateForDelay:(DeployViewController *)controller user:(BlackBy *)cell;
 ...
 *  @param cell 所点击的消息单元
 */
//: - (void)messageController:(TUIMessageController *)controller onSelectMessageContent:(BlackBy *)cell;
- (void)evaluation:(DeployViewController *)controller name:(BlackBy *)cell;
 ...
 */
//: - (void)messageController:(TUIMessageController *)controller onSelectMessageMenu:(NSInteger)menuType withData:(AlongCrucial *)data;
- (void)equipment:(DeployViewController *)controller publisher:(NSInteger)menuType must:(AlongCrucial *)data;
/**
...
/*重发*/
//: - (void)messageController:(TUIMessageController *)controller onRetryMessageContent:(BlackBy *)cell;
- (void)stackHalf:(DeployViewController *)controller thanMobile:(BlackBy *)cell;

/*引用*/
//: - (void)messageController:(TUIMessageController *)controller onReplyMessage:(AlongCrucial *)cellData;
- (void)sound:(DeployViewController *)controller aboveBlackSmall:(AlongCrucial *)cellData;

/*点击引用*/

//: - (void)messageController:(TUIMessageController *)controller onClickReplyMessage:(AlongCrucial *)cellData;
- (void)activity:(DeployViewController *)controller might:(AlongCrucial *)cellData;
...
@interface DeployViewController : UITableViewController

/**
 *  执行 AdjustLength 协议的委托
 */
//: @property (nonatomic, weak) id<TMessageControllerDelegate> delegate;
@property (nonatomic, weak) id<AdjustLength> delegate;
 ...
 * @param uiMsgs 待删除的数据
 */
//- (void)deleteMessages:(NSArray<AlongCrucial *> *)uiMsgs;
...
///当前对话类型
//: @property (nonatomic, assign) PacificUserType currentChatType;
@property (nonatomic, assign) PacificUserType currentChatType;
