...
fileprivate let noti_onlyMsg:[Character] = ["C","a","c","h","e","G","i","f","t","E","f","f","e","c","t","C","a","c","h","e","K","e","y"]

/*: "下载动画文件" :*/
fileprivate let kCustomerStr:[Character] = ["下","载","动","画","\u{6587}","件"]

/*: "加载本地动画文件" :*/
fileprivate let const_uniformPanTitle:[Character] = ["加","载","本","地","动"]
fileprivate let kThemeUrl:[Character] = ["画","\u{6587}","件"]

/*: "Cache_GiftEffect/" :*/
fileprivate let dataYearData:String = "item disk submit runningCache"
fileprivate let notiCarefulPath:String = "mic party chain awayffect/"

/*: "GJSGVA" :*/
fileprivate let notiPurpleWritingPath:[Character] = ["G","J","S","G","V","A"]

/*: "GJVap" :*/
fileprivate let constKeyStr:String = "GJVapbody input parent play"

/*: "config.json" :*/
fileprivate let app_alongWhoId:[Character] = ["c","o","n","f","i","g",".","j","s"]
fileprivate let userPassShareStr:[Character] = ["o","n"]

/*: "app. :*/
fileprivate let noti_toPath:String = "app.thread equivalent protection"

/*: .com" :*/
fileprivate let data_diskUrl:String = "scene press.com"

/*: "key" :*/
fileprivate let userMissionUrl:[Character] = ["k","e","y"]

/*: "zip" :*/
fileprivate let show_nearOptionStr:String = "zidaily"

/*: " fail to unzip file" :*/
fileprivate let userTitleureUrl:[Character] = [" ","f","a","i","l"," ","t","o"," ","u","n"]
fileprivate let showPoFillStr:String = "ZI"
fileprivate let user_reductionId:String = "p fileequivalent move sufficient"

/*: "RSA 解密失败" :*/
fileprivate let show_accelerateErrorMsg:String = "RSA 解密\u{5931}败"

/*: "fail to unzip script file" :*/
fileprivate let data_lightStr:[Character] = ["f","a","i","l"," ","t","o"," "]
fileprivate let show_outputData:[Character] = ["u","n","z","i","p"," ","s"]
fileprivate let kRemoveMessage:String = "giving areacript "

/*: "gift" :*/
fileprivate let showPracticalFormat:[Character] = ["g","i","f","t"]

/*: "%@%@effect_%ld" :*/
fileprivate let k_gameValue:[Character] = ["%","@","%","@","e","f","f","e","c"]
fileprivate let constIntervalensityPath:String = "t_%ldgirl variable"

/*: "%@%@effect_%ld_unzipTest/" :*/
fileprivate let show_holdStr:String = "%@%@effboyfriend face balance practical"
fileprivate let showPressureStr:String = "property replacement scientific draftect_%"
fileprivate let notiInnerEstimatedMessage:[Character] = ["z","i","p","T","e","s","t","/"]

/*: "%@%@effect_%ld_unzip/" :*/
fileprivate let app_levelValue:[Character] = ["%","@","%","@","e","f","f","e","c","t","_","%","l","d","_","u","n","z"]
fileprivate let app_selectedKey:String = "ip/surrounding chart grain"
...
typealias CompletionBlock = (_ success: Bool, _ effectModel: ListenBoth?) -> Void

//: typealias ErrorBlock = (_ error: NSError?) -> Void
typealias ErrorBlock = (_ error: NSError?) -> Void

//: class TalkingGiftAnimatTool: NSObject {
class MoveGenerate: NSObject {
    //: let AnimatCacheKey  = "CacheGiftEffectCacheKey"
    let AnimatCacheKey = (String(noti_onlyMsg))

    //: @objc static let shared = TalkingGiftAnimatTool()
    @objc static let shared = MoveGenerate()
    ...
    func factorOk(effectMsgModel: StepSumeraction, completion: @escaping CompletionBlock) {
        //: if checkNewVersionWithAnimatMsgModel(model: effectMsgModel) {
        if awayEx(model: effectMsgModel) {
            //: printLog(message: "下载动画文件")
            printLog(message: (String(kCustomerStr)))
            //: downloadAnimat(effectMsgModel: effectMsgModel) { [weak self] error in
            askShare(effectMsgModel: effectMsgModel) { [weak self] error in
                ...
                self.original(effectMsgModel: effectMsgModel, complete: completion)
            }
            //: } else {
        } else {
            //: printLog(message: "加载本地动画文件")
            printLog(message: (String(const_uniformPanTitle) + String(kThemeUrl)))
            //: self.loadLocalAnimatConfig(effectMsgModel: effectMsgModel, complete: completion)
            self.original(effectMsgModel: effectMsgModel, complete: completion)
        }
    }
    ...
    func toleranceOption(fileName: String, model: StepSumeraction) -> String {
        //: let scriptDirectory = fetchScriptDirectory(animatMsgModel: model)
        let scriptDirectory = plusWarning(animatMsgModel: model)
    ...
    func noticeBy() {
        ...
        limited(path: havePrivacy())
    }

    // 动效资源本地的缓存目录路径
    //: func fetchScriptDirectory(animatMsgModel: TalkingAnimatMsgModel) -> String {
    func plusWarning(animatMsgModel: StepSumeraction) -> String {
        //: if animatMsgModel.eid == 0 {
        if animatMsgModel.eid == 0 {
            //: return String.init(format: "%@/%@/", giftEffectDirectory(), animatMsgModel.effectUrl.md5()!)
            return String(format: "%@/%@/", havePrivacy(), animatMsgModel.effectUrl.md5()!)
            //: } else {
        } else {
            //: return String.init(format: "%@/%zd/", giftEffectDirectory(), animatMsgModel.eid)
            return String(format: "%@/%zd/", havePrivacy(), animatMsgModel.eid)
        }
    }

    //: func giftEffectDirectory() -> String {
    func havePrivacy() -> String {
        //: let subPath = "Cache_GiftEffect/"
        let subPath = (String(dataYearData.suffix(5)) + "_GiftE" + String(notiCarefulPath.suffix(6)))
    ...
    func compositionArray(filePath: String) -> String {
    ...
    func recent(model: StepSumeraction) -> Bool {
        //: return !checkNewVersionWithAnimatMsgModel(model: model)
        return !awayEx(model: model)
    }

    //: func getNewAnimatClassName(oldName: String) -> String {
    func versusOk(oldName: String) -> String {
        //: var newName = oldName
        var newName = oldName
        //: if newName.hasPrefix("GJSGVA") {
        if newName.hasPrefix((String(notiPurpleWritingPath))) {
            //: newName = String(describing: TalkingSGVAAnimationView.self)
            newName = String(describing: WantView.self)
            //: } else if newName.hasPrefix("GJVap") {
        } else if newName.hasPrefix((String(constKeyStr.prefix(5)))) {
            //: newName = String(describing: TalkingVapAnimationView.self)
            newName = String(describing: TagView.self)
        }
        //: return newName
        return newName
    }
...
extension MoveGenerate {
    //: func loadLocalAnimatConfig(effectMsgModel: TalkingAnimatMsgModel, complete: @escaping CompletionBlock) {
    func original(effectMsgModel: StepSumeraction, complete: @escaping CompletionBlock) {
        //: let scriptDirectory = fetchScriptDirectory(animatMsgModel: effectMsgModel)
        let scriptDirectory = plusWarning(animatMsgModel: effectMsgModel)
        //: let scriptPath = (scriptDirectory as NSString).appendingPathComponent("config.json")
        let scriptPath = (scriptDirectory as NSString).appendingPathComponent((String(app_alongWhoId) + String(userPassShareStr)))
        ...
        let dic = NSDictionary.surroundingDown(withJsonString: configString!) ?? NSDictionary()
        //: let giftAmodel = TalkingGiftAnimatModel.init()
        let giftAmodel = ListenBoth()
        //: let effectModel = giftAmodel.setAnimatModek(dic: dic)
        let effectModel = giftAmodel.containToFeeCount(dic: dic)
        ...
        className = versusOk(oldName: className)
        //: let clsName = PodspecName + "." + className
        let clsName = data_shapeValue + "." + className
    ...
    func awayEx(model: StepSumeraction) -> Bool {
    ...
    func askShare(effectMsgModel: StepSumeraction, errorBlock: @escaping ErrorBlock) {
        ...
        PriorMedia.monster(giftUrl: effectMsgModel.effectUrl, path: factorEnable(model: effectMsgModel)) { [weak self] t in
            //: guard let self = self else {return}
            guard let self = self else { return }
            //: DispatchQueue.global().async {
            DispatchQueue.global().async {
                //: self.afterDownloadGiftAnimat(effectMsgModel: effectMsgModel, isSucess: t, errorBlock: errorBlock)
                self.following(effectMsgModel: effectMsgModel, isSucess: t, errorBlock: errorBlock)
            }
        }
    }

    //: func afterDownloadGiftAnimat(effectMsgModel: TalkingAnimatMsgModel, isSucess: Bool, errorBlock: @escaping ErrorBlock) {
    func following(effectMsgModel: StepSumeraction, isSucess: Bool, errorBlock: @escaping ErrorBlock) {
        //: if !isSucess {
        if !isSucess {
            //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.DownLoadFailed.rawValue))
            errorBlock(NSError(domain: (String(noti_toPath.prefix(4))) + "\(app_readTitle)" + (String(data_diskUrl.suffix(4))), code: TranslateConvert.DownLoadFailed.rawValue))
            //: return
            return
            ...
            let downloadPath = factorEnable(model: effectMsgModel)
            //: let unzipVerifyDir = unzipVerifyDirectory(model: effectMsgModel)
            let unzipVerifyDir = guide(model: effectMsgModel)
            ...
            if verifyUnzipSucc {
                ...
                for filename in unzippedFiles {
                    //: if filename == "key" {
                    if filename == (String(userMissionUrl)) {
                        // encrypted md5 file
                        //: keyFilePath = (unzipVerifyDir as NSString).appendingPathComponent(filename)
                        keyFilePath = (unzipVerifyDir as NSString).appendingPathComponent(filename)
                        //: } else if (filename as NSString).pathExtension == "zip" {
                    } else if (filename as NSString).pathExtension == (show_nearOptionStr.replacingOccurrences(of: "daily", with: "p")) {
                        // script file
                ...
                printLog(message: (String(userTitleureUrl) + showPoFillStr.lowercased() + String(user_reductionId.prefix(6))))
                //: isFailed = true
                isFailed = true
                //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.UnzipFailed.rawValue))
                errorBlock(NSError(domain: (String(noti_toPath.prefix(4))) + "\(app_readTitle)" + (String(data_diskUrl.suffix(4))), code: TranslateConvert.UnzipFailed.rawValue))
            }
            ...
            if !isFailed {
                ...
                do {
                    ...
                    let md5Data: Data = AssociatedComponent.extraPermanent(NSData(contentsOfFile: keyFilePath) as Data?, might: kBridgeId)
                    ...
                    let md5 = compositionArray(filePath: scriptZipFilePath)
                    ...
                    if decryptMD5 != md5, FileManager.default.fileExists(atPath: keyFilePath) {
                        ...
                        errorBlock(NSError(domain: (String(noti_toPath.prefix(4))) + "\(app_readTitle)" + (String(data_diskUrl.suffix(4))), code: TranslateConvert.VerifyFailed.rawValue))
                    }
                    ...
                    printLog(message: (show_accelerateErrorMsg))
                    //: isFailed = true
                    isFailed = true
                    //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.VerifyFailed.rawValue))
                    errorBlock(NSError(domain: (String(noti_toPath.prefix(4))) + "\(app_readTitle)" + (String(data_diskUrl.suffix(4))), code: TranslateConvert.VerifyFailed.rawValue))
            ...
            if !isFailed {
                ...
                let unzipTmpDir = train(model: effectMsgModel)
                ...
                //: else {
                    ...
                    printLog(message: (String(data_lightStr) + String(show_outputData) + String(kRemoveMessage.suffix(6)) + "file"))
                    //: isFailed = true
                    isFailed = true
                    //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.UnzipFailed.rawValue))
                    errorBlock(NSError(domain: (String(noti_toPath.prefix(4))) + "\(app_readTitle)" + (String(data_diskUrl.suffix(4))), code: TranslateConvert.UnzipFailed.rawValue))
    ...
    func cleanVisual(type: ActionClipList) -> String {
        //: var tmpIdentify = ""
        var tmpIdentify = ""
        //: switch type {
        switch type {
        //: case .Gift:     tmpIdentify =  "gift"
        case .Gift: tmpIdentify = (String(showPracticalFormat))
        //: break
        //: default:
        default:
            //: break
    ...
    func factorEnable(model: StepSumeraction) -> String {
        ...
        let downloadTmpPath = String(format: (String(k_gameValue) + String(constIntervalensityPath.prefix(5))), NSTemporaryDirectory(), cleanVisual(type: model.effectType), model.eid)
        //: return downloadTmpPath
        return downloadTmpPath
    }

    //: func unzipVerifyDirectory(model: TalkingAnimatMsgModel)->String {
    func guide(model: StepSumeraction) -> String {
        ...
        let unzipVerifyDirectory = String(format: (String(show_holdStr.prefix(7)) + String(showPressureStr.suffix(5)) + "ld_un" + String(notiInnerEstimatedMessage)), NSTemporaryDirectory(), cleanVisual(type: model.effectType), model.eid)
        //: return unzipVerifyDirectory
        return unzipVerifyDirectory
    }

    //: func unzipTmpDirectory(model: TalkingAnimatMsgModel)->String {
    func train(model: StepSumeraction) -> String {
        ...
        let unzipTmpDirectory = String(format: (String(app_levelValue) + String(app_selectedKey.prefix(3))), NSTemporaryDirectory(), cleanVisual(type: model.effectType), model.eid)
