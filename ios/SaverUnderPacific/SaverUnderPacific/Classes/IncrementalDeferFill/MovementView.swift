...
fileprivate let k_aspectConstraintMsg:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Full" :*/
fileprivate let constWholeName:String = "Fullmutual front before interested span"

/*: "%@ Online" :*/
fileprivate let data_lightSubtleEmbraceKey:[Character] = ["%","@"," ","O","n","l","i","n","e"]
...
class MovementView: UITableViewCell {
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        fatalError(String(bytes: k_aspectConstraintMsg.reversed(), encoding: .utf8)!)
    }
...
extension MovementView {
    ...
    func tar(model: TransitionScientific) {
        //: IconBtn.setUrlImage(urlStr: model.icon)
        IconBtn.testimonyWith(urlStr: model.icon)
        ...
        if model.isFull {
            ...
            numberLB.text = (String(constWholeName.prefix(4))).localized
            //: numberLB.textColor = UIColor.msgTipsColor()
            numberLB.textColor = UIColor.atMount()
            //: }else {
        } else {
            //: numberLB.text = "%@ Online".localizedArguments(model.num)
            numberLB.text = (String(data_lightSubtleEmbraceKey)).theAppear(model.num)
            //: numberLB.textColor = .appValueDetailColor()
            numberLB.textColor = .theProvision()
        }
    }
}
