...
/*: "reply" :*/
fileprivate let data_priorGrainMsg:[Character] = ["r","e","p","l","y"]
...
extension WriteDestroy {
    //: mutating func caculateCommentItemHeight() {
    mutating func actualBlock() {
        //: var tempContent = ""
        var tempContent = ""
        ...
        if replyUser?.count ?? 0 > 0 {
            ...
            if InputCloseInfo.shared.direction == .leftToRight {
                ...
                tempContent = (String(data_priorGrainMsg)).localized + "\(replyUser!): \(content!)"
                //: } else {
            } else {
                //: tempContent = "\(content!) :\(replyUser!)" + "reply".localized
                tempContent = "\(content!) :\(replyUser!)" + (String(data_priorGrainMsg)).localized
            }
        ...
        let sizeheight = Int(TweenView.multiple(text: tempContent, size: CGSize(width: show_errorChangeFormat - 70, height: CGFloat(MAXFLOAT)), font: UIFont.yearNearby(type: .Regular, fontSize: 16), lineNumber: 0).height)
