...
fileprivate let show_slyTitle:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "More" :*/
fileprivate let userSnapKey:String = "Moreflag projection boyfriend mist prior"

/*: "icon_details_arrow" :*/
fileprivate let userSirMsg:String = "icon_dexperienced click"
fileprivate let show_environmentKey:String = "acapacityow"

/*: "Post" :*/
fileprivate let dataStructureData:String = "terms bit remind disclose methodPost"

/*: "btn_dynamic_stop_nor" :*/
fileprivate let show_sourceString:[Character] = ["b","t","n","_","d","y","n","a","m","i","c"]
fileprivate let noti_equipmentId:String = "_stotake cartoon concrete ark transform"
fileprivate let constStrategyFormat:String = "mission respective train filterp_nor"
...
class BlueComposition: UITableViewCell {
    //: var userId: String = ""
    var userId: String = ""
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: show_slyTitle.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var maskMoreView: UIView = {
        ...
        view.backgroundColor = .our(0, 0, 0, 0.4)
    ...
    lazy var moreButton: DistinctionButton = .init().then {
        //: $0.titleLabel?.font = .pingfangFont(type: .Regular, fontSize: 17)
        $0.titleLabel?.font = .yearNearby(type: .Regular, fontSize: 17)
        //: $0.setTitle("More".localized, for: .normal)
        $0.setTitle((String(userSnapKey.prefix(4))).localized, for: .normal)
        //: $0.setImage(UIImage(named: "icon_details_arrow"), for: .normal)
        $0.setImage(UIImage(named: (String(userSirMsg.prefix(6)) + "etails_" + show_environmentKey.replacingOccurrences(of: "capacity", with: "rr"))), for: .normal)
...
extension BlueComposition {
    ...
    func bot(userModel: SpecifySmart) {
        ...
        strA = [(String(dataStructureData.suffix(4))).localized, str]
        //: colorA = [UIColor.appTitleColor(), UIColor.appValueColor()]
        colorA = [UIColor.untilExpected(), UIColor.obtainOf()]
        //: fontA = [UIFont.pingfangMediumFont(fontSize: 18), UIFont.pingfangMediumFont(fontSize: 16)]
        fontA = [UIFont.hangProgram(fontSize: 18), UIFont.hangProgram(fontSize: 16)]
        ...
        for index in 0 ..< itemCount {
            ...
            if index == 0 {
                ...
                let model: ReceiverRequestModel = (userModel.momentPic?.list)![index]
                ...
                firstImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.tillTrigger())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: firstImgView)
                    only(btn: firstImgView)
                }
                ...
                secondImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.tillTrigger())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: secondImgView)
                    only(btn: secondImgView)
                }
                //: } else {
            } else {
                //: let model: TalkingUserInfoMomentItemModel = (userModel.momentPic?.list)![index]
                ...
                thirdImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.tillTrigger())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: thirdImgView)
                    only(btn: thirdImgView)
                }
            }
        }
    }
    ...
    func only(btn: UIImageView) {
        ...
        let imag = UIImageView(image: UIImage(named: (String(show_sourceString) + String(noti_equipmentId.prefix(4)) + String(constStrategyFormat.suffix(5)))))
...
extension BlueComposition {
    ...
    private func deploy() {
        ...
        let height = (show_errorChangeFormat - actualWidth(w: 15 * 4)) / 3.0
