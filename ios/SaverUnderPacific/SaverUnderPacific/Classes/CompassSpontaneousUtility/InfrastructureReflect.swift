...
fileprivate let main_scientificExceptionMessage:[UInt8] = [0x36,0x31,0x36,0x2b,0x77,0x3c,0x30,0x3b,0x3a,0x2d,0x65,0x76,0x7f,0x37,0x3e,0x2c,0x7f,0x31,0x30,0x2b,0x7f,0x3d,0x3a,0x3a,0x31,0x7f,0x36,0x32,0x2f,0x33,0x3a,0x32,0x3a,0x31,0x2b,0x3a,0x3b]

private func canvasNo(standard num: UInt8) -> UInt8 {
    return num ^ 95
}

/*: "icon_chat_start" :*/
fileprivate let constExactlyTitle:String = "icon_chatout underly lat"
fileprivate let main_introName:String = "_startend busy program equal"

/*: "icon_chat_lock" :*/
fileprivate let app_expressionPath:String = "icon_shall distribution us architecture thumb"
fileprivate let show_listData:[Character] = ["_","l","o","c","k"]

/*: "icon_chat_burned" :*/
fileprivate let appSelectionBlankMessage:String = "icon_hold rid method menu other"
fileprivate let constHeKeyStr:String = "receiverrned"

/*: "icon_chat_burned_shan" :*/
fileprivate let user_changeValue:[Character] = ["i","c","o","n","_","c","h","a"]
fileprivate let user_translationValue:[Character] = ["t","_","b","u","r"]
fileprivate let mainFormalPath:[Character] = ["n","e","d","_","s","h","a","n"]

/*: "Already burned" :*/
fileprivate let k_maleValue:String = "Alreaprice fade automatic"
fileprivate let showSayCoordinatorString:[Character] = ["r","n","e","d"]
...
class InfrastructureReflect: ShadeItem {
    //: var videoData: TalkingChatVideoMsgCellData?
    var videoData: MeasurePaint?
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: main_scientificExceptionMessage.map{canvasNo(standard: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var playImgView: UIImageView = {
        ...
        imgV.image = UIImage.managerToSecond(name: (String(constExactlyTitle.prefix(9)) + String(main_introName.prefix(6))))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    private lazy var lockImgView: UIImageView = {
        ...
        imgV.image = UIImage.managerToSecond(name: (String(app_expressionPath.prefix(5)) + "chat" + String(show_listData)))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    private lazy var burnedImgView: DistinctionButton = {
        ...
        imgV.setBackgroundImage(UIImage.managerToSecond(name: (String(appSelectionBlankMessage.prefix(5)) + "chat_" + constHeKeyStr.replacingOccurrences(of: "receiver", with: "bu"))), for: .normal)
        //: imgV.setImage(UIImage.BundleImageNamed(name: "icon_chat_burned_shan"), for: .normal)
        imgV.setImage(UIImage.managerToSecond(name: (String(user_changeValue) + String(user_translationValue) + String(mainFormalPath))), for: .normal)
        //: imgV.imageAlignment = .top
        imgV.imageAlignment = .top
        //: imgV.setTitle("Already burned".localized, for: .normal)
        imgV.setTitle((String(k_maleValue.prefix(5)) + "dy bu" + String(showSayCoordinatorString)).localized, for: .normal)
        //: imgV.setTitleColor(.white, for: .normal)
        imgV.setTitleColor(.white, for: .normal)
        //: imgV.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 16)
        imgV.titleLabel?.font = UIFont.enterCommon(fontSize: 16)
        //: return imgV
        return imgV
        //: }()
    }()
...
extension InfrastructureReflect {
    //: override func fill(with data: TCommonCellData) {
    override func satisfyAssociated(with data: SpineSuccess) {
        //: super.fill(with: data)
        super.satisfyAssociated(with: data)
        //: self.videoData = data as? TalkingChatVideoMsgCellData
        self.videoData = data as? MeasurePaint
        ...
        self.dutationLab.text = NSDate.funWithEnableicial(totalTime: duration)
        ...
        self.permissionWill(urlStr: videoModel.coverImg, isMosaic: imgIsMosaic)
    }
    ...
    func permissionWill(urlStr: String, isMosaic: Bool) {
        //: guard let newData = self.videoData else { return }
        guard let newData = self.videoData else { return }
        ...
        if !coverImgPath.isEmptyString, isExit { // 加载本地
            ...
            //: if isMosaic {
            if isMosaic {
                //: self.picImgView.image = UIImage.wrap(image, styleOrange: 90)
                self.picImgView.image = UIImage.wrap(image, styleOrange: 90)
                //: } else {
            } else {
            ...
            self.picImgView.alwaysFor(urlStr: urlStr) { [weak self] image in
