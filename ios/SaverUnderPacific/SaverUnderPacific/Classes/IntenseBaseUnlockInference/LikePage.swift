...
fileprivate let show_visibleStr:[UInt8] = [0x20,0x73,0x69,0x20,0x73,0x75,0x74,0x61,0x74,0x53,0x6e,0x6f,0x69,0x74,0x61,0x7a,0x69,0x72,0x6f,0x68,0x74,0x75,0x61,0x20,0x6e,0x6f,0x69,0x74,0x61,0x63,0x6f,0x6c]

/*: "get location failed. error: :*/
fileprivate let mainHiFireMessage:[Character] = ["g","e","t"," ","l","o","c","a","t","i","o","n"," ","f","a","i","l","e","d","."," ","e","r","r"]
fileprivate let noti_bubbleMsg:[Character] = ["o","r",":"]
...
class LikePage: NSObject {
    ...
    var locationType: GestureMedium?

    // singleton
    //: private static var _instance: TalkingLocationManager?
    private static var _instance: LikePage?
    //: class func shared() -> TalkingLocationManager {
    class func ontoData() -> LikePage {
    ...
    func childConversation(block: @escaping Locationblock) {
    ...
    func enabletoAcross(block: @escaping Authblock) {
    ...
    func characterOf() -> GestureMedium {
    ...
    func openWork() -> CLAuthorizationStatus {
        ...
        if #available(iOS 14.0, *) {
            ...
            printLog(message: String(bytes: show_visibleStr.reversed(), encoding: .utf8)! + "\(status.rawValue)")
            //: return status
            return status
            //: } else {
        } else {
...
extension LikePage: CLLocationManagerDelegate {
...
//            guard abs(loction.timestamp.timeIntervalSinceNow) > 5 else { return }
    ...
    func locationManager(_ manager: CLLocationManager, didFailWithError error: Error) {
        ...
        printLog(message: (String(mainHiFireMessage) + String(noti_bubbleMsg)) + "\(error.localizedDescription)")
    }
}
