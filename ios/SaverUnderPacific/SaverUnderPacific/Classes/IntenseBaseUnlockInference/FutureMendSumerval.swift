...
/*: "num" :*/
fileprivate let kRelateString:[Character] = ["n","u","m"]
...
@objc public enum TransitionFinish: Int {
    //: case normal  = 1
    case normal = 1 // 普通礼物
    //: case myStery = 2
    case myStery = 2 // 盲盒礼物
...
struct FutureMendSumerval: HandyJSON {
    //: init() {
    init() {}
    ...
    var animationTimes = 0
    //: var showType = 0
    var showType = 0 /// TransitionFinish
    ...
    mutating func sign(giftNum: Int) {
        //: self.num = giftNum
        self.num = giftNum
    }
}
...
struct KitWhite: HandyJSON {
    ...
    mutating func mapping(mapper: HelpingMapper) {
        ...
        mapper <<<
            //: self.giftNum <-- "num"
            self.giftNum <-- (String(kRelateString))
    }
}
