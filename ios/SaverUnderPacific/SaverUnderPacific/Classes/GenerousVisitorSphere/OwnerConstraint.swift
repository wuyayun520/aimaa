...
fileprivate let showAdministrativeFormat:[UInt8] = [0x10,0x15,0x10,0x1b,0xcf,0xa,0x16,0xb,0xc,0x19,0xe1,0xd0,0xc7,0xf,0x8,0x1a,0xc7,0x15,0x16,0x1b,0xc7,0x9,0xc,0xc,0x15,0xc7,0x10,0x14,0x17,0x13,0xc,0x14,0xc,0x15,0x1b,0xc,0xb]

fileprivate func heartCould(blue num: UInt8) -> UInt8 {
...
fileprivate let noti_blindTitle:String = "iccenter"
fileprivate let mainDelicateString:String = "detobserve"
fileprivate let const_forcePath:String = "il_hebusiness fit by"
fileprivate let appUniqueDistributionMessage:String = "shaccuracyow"

/*: "icon_lounge" :*/
fileprivate let constDecideWhiteDevicePath:String = "icon_linformation same coffee"
fileprivate let showNameureValue:[Character] = ["o","u","n","g","e"]

/*: "Official" :*/
fileprivate let userReaderPairData:[Character] = ["O","f","f","i","c","i","a","l"]

/*: "Reviewing" :*/
fileprivate let const_currencyKey:[Character] = ["R","e"]
fileprivate let data_evenMessage:[Character] = ["v","i","e","w","i","n","g"]

/*: "#FF506D" :*/
fileprivate let mainFeatureUrl:[Character] = ["#","F","F","5","0","6"]
fileprivate let noti_agreeKey:[Character] = ["D"]

/*: "icon_user_detail_intimate_header" :*/
fileprivate let dataUniqueStr:[UInt8] = [0xa7,0xad,0xa1,0xa0,0x91,0xbb,0xbd,0xab,0xbc,0x91,0xaa,0xab,0xba,0xaf,0xa7,0xa2,0x91,0xa7,0xa0,0xba,0xa7,0xa3,0xaf,0xba,0xab,0x91,0xa6,0xab,0xaf,0xaa,0xab,0xbc]

private func eventEmbrace(pass num: UInt8) -> UInt8 {
    return num ^ 206
}

/*: "?toUid= :*/
fileprivate let main_shapeName:[Character] = ["?","t","o","U","i","d","="]

/*: "HalfEquivalent" :*/
fileprivate let main_quantityUrl:String = "regulate testimony character failure bathTUse"
fileprivate let kNeverData:String = "ailHeadeadjust nobody"
fileprivate let data_welcomeFormat:String = "never or environmentrItemCell"

/*: "#FFFFFF" :*/
fileprivate let showLikeKey:String = "#FFFFmaxi save m infra s"
fileprivate let notiBesidePath:String = "mine"

/*: "#BDB8B5" :*/
fileprivate let noti_manProtectionTitle:String = "welcome"
fileprivate let noti_possessName:[Character] = ["B","D","B","8","B","5"]

/*: "head_detail_bottom" :*/
fileprivate let showBoyfriendValue:String = "eastern nearhead_deta"
fileprivate let dataDateAgentKey:String = "il_bfit speaker reduce"
fileprivate let mainRecognizePath:[Character] = ["o","t","t","o","m"]

/*: "#2DF2FF" :*/
fileprivate let appParticipationRadioValue:[Character] = ["#","2","D","F","2"]
fileprivate let dataStrengthPath:String = "instanceinstance"

/*: "Live" :*/
fileprivate let kLicensePath:[Character] = ["L","i","v","e"]

/*: "#48FF1E" :*/
fileprivate let kBlankData:[Character] = ["#","4","8","F","F","1","E"]

/*: "Online" :*/
fileprivate let constSwingQuakeString:[Character] = ["O","n","l","i","n","e"]

/*: "#FF4B7F" :*/
fileprivate let kHimSureMessage:[Character] = ["#","F","F","4","B","7","F"]

/*: "busy" :*/
fileprivate let main_appealString:String = "busspecify"
...
let user_beforeMsg = (302.0 + const_musicMessage)

//: class TalkingUserDetailHeaderView: UIView {
class OwnerConstraint: UIView {
    //: var VideoCallBlock: (() -> Void)?
    var VideoCallBlock: (() -> Void)?
    //: private var userInfoModel = TalkingUserInfoModel()
    private var userInfoModel = SpecifySmart()
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: showAdministrativeFormat.map{heartCould(blue: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var bannerView: JXBanner = {
        ...
        let frame = CGRect(x: 0, y: 0, width: show_errorChangeFormat, height: user_beforeMsg)
        ...
        view.placeholderImgView.image = UIImage.pe()
    ...
    private lazy var shadowBgView: UIImageView = {
        ...
        let v = UIImageView(image: UIImage.managerToSecond(name: (noti_blindTitle.replacingOccurrences(of: "center", with: "on") + "_user" + mainDelicateString.replacingOccurrences(of: "observe", with: "a") + String(const_forcePath.prefix(5)) + "ader_" + appUniqueDistributionMessage.replacingOccurrences(of: "accuracy", with: "ad"))))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var vipBgView: UIImageView = {
        ...
        v.frame = CGRect(x: 0, y: user_beforeMsg - 15, width: show_errorChangeFormat, height: 15)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var nicknameLab: UILabel = {
        ...
        lab.font = UIFont.yearNearby(type: .Medium, fontSize: 19)
    ...
    private lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.managerToSecond(name: (String(constDecideWhiteDevicePath.prefix(6)) + String(showNameureValue)))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var officalImgV: UIButton = {
        ...
        img.faceCrop(color: UIColor.atMount(), forState: .normal)
        //: img.setTitle("Official".localized, for: .normal)
        img.setTitle((String(userReaderPairData)).localized, for: .normal)
        //: img.setTitleColor(.white, for: .normal)
        img.setTitleColor(.white, for: .normal)
        //: img.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 11)
        img.titleLabel?.font = UIFont.enterCommon(fontSize: 11)
    ...
    private lazy var onlineImgV: DistinctionButton = {
    ...
    private lazy var liveView: JoinRebuild = {
        ...
        v.tapGestureBlock = { [weak self] in
            ...
            guard let self = self else { return }
            //: OriginMediumWill.share.func_audiencePushToLiveRoomVC(uid: self.userInfoModel.uid, enterType: .userDetail)
            OriginMediumWill.share.dailyEnable(uid: self.userInfoModel.uid, enterType: .userDetail)
        }
        //: return v
        return v
        //: }()
    ...
    private lazy var reViewingLB: UILabel = {
        ...
        lb.text = (String(const_currencyKey) + String(data_evenMessage)).localized
        //: lb.textColor = .white
        lb.textColor = .white
        //: lb.backgroundColor = UIColor.init(hex: "#FF506D")
        lb.backgroundColor = UIColor(hex: (String(mainFeatureUrl) + String(noti_agreeKey)))
    ...
    lazy var lastLoginLab: UILabel = {
        ...
        lb.font = .yearNearby(type: .Medium, fontSize: 12)
        //: lb.textColor = .white
        lb.textColor = .white
        //: return lb
        return lb
    ...
    private lazy var intimateBtn: DistinctionButton = {
        //: let btn = TalkingButton()
        let btn = DistinctionButton()
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_user_detail_intimate_header"), for: .normal)
        btn.setImage(UIImage.managerToSecond(name: String(bytes: dataUniqueStr.map{eventEmbrace(pass: $0)}, encoding: .utf8)!), for: .normal)
        //: btn.isHidden = true
        btn.isHidden = true
        //: btn.addTarget(self, action: #selector(intimateBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(spend), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension OwnerConstraint {
    /// 查看视频
    //: private func goToVideo() {
    private func aspect() {
        //: let vc = TalkingMomentVideoVC.init(videoPath: self.userInfoModel.videoUrl)
        let vc = EqualGlobal(videoPath: self.userInfoModel.videoUrl)
        //: if MarginExamineer.share.loginUid != self.userInfoModel.uid {
        if MarginExamineer.share.loginUid != self.userInfoModel.uid {
    ...
    private func empty(_ index: Int) {
        ...
        let vc = QuantityegrityViewController(momentModel: currenModel, index: index, type: .normal)
        //: self.currentViewController()?.navigationController?.pushViewController(vc, animated: true)
        self.myAcross()?.navigationController?.pushViewController(vc, animated: true)
    }
    ...
    @objc func spend() {
        ...
        let url = FrameScaleException.fillBy(urlType: .Intimate) + (String(main_shapeName)) + "\(self.userInfoModel.uid)"
...
extension OwnerConstraint: JXBannerDataSource {
    ...
    func jxBanner(_ banner: JXBannerType) -> (JXBannerCellRegister) {
        ...
        return JXBannerCellRegister(type: HalfEquivalent.self, reuseIdentifier: (String(main_quantityUrl.suffix(4)) + "rDet" + String(kNeverData.prefix(8)) + String(data_welcomeFormat.suffix(9))))
    }

    //: func jxBanner(_ banner: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
    func jxBanner(_ banner: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
        //: let bannerCell = cell as! TUserDetailHeaderItemCell
        let bannerCell = cell as! HalfEquivalent
        //: let urlStr = self.dataArray[index]
        let urlStr = self.dataArray[index]

        //: if index == 0 && userInfoModel.sex == Gender.female.rawValue && userInfoModel.videoUrl.count>1 {
        if index == 0 && userInfoModel.sex == EnvironmentEmpty.female.rawValue && userInfoModel.videoUrl.count > 1 { // 视频
            //: bannerCell.setVideoView()
            bannerCell.account()
            //: bannerCell.configModel(videoUrl: userInfoModel.videoUrl, imgUrl: userInfoModel.cover)
            bannerCell.quote(videoUrl: userInfoModel.videoUrl, imgUrl: userInfoModel.cover)
            ...
            bannerCell.bgImgView.chest(urlStr: urlStr, placeImg: UIImage.enhanceBy())
        }
    ...
    func jxBanner(_ banner: JXBannerType, centerIndex: Int, centerCell: UICollectionViewCell) {
        //: guard userInfoModel.uid == MarginExamineer.share.loginUid else {
        guard userInfoModel.uid == MarginExamineer.share.loginUid else {
            //: return
            return
        }
    ...
    func jxBanner(pageControl banner: JXBannerType, numberOfPages: Int, coverView: UIView, builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder {
        ...
        pageControl.activeColor = UIColor(hex: (String(showLikeKey.prefix(5)) + notiBesidePath.replacingOccurrences(of: "mine", with: "FF")))!
        //: pageControl.inactiveColor = UIColor(hex: "#BDB8B5")!
        pageControl.inactiveColor = UIColor(hex: (noti_manProtectionTitle.replacingOccurrences(of: "welcome", with: "#") + String(noti_possessName)))!
...
extension OwnerConstraint: JXBannerDelegate {
    //: public func jxBanner(_ banner: JXBannerType, didSelectItemAt index: Int) {
    public func jxBanner(_ banner: JXBannerType, didSelectItemAt index: Int) {
        //: if index == 0 && userInfoModel.sex == Gender.female.rawValue && userInfoModel.videoUrl.count>1 {
        if index == 0 && userInfoModel.sex == EnvironmentEmpty.female.rawValue && userInfoModel.videoUrl.count > 1 {
            //: goToVideo()
            aspect()
            ...
            if userInfoModel.sex == EnvironmentEmpty.female.rawValue && userInfoModel.videoUrl.count > 1 && curren > 0 {
                //: curren -= 1
                curren -= 1
            }
            //: goToPhoto(curren)
...
extension OwnerConstraint {
    ...
    func put(_ userModel: SpecifySmart) {
        ...
        if userModel.sex == EnvironmentEmpty.female.rawValue && userModel.videoUrl.count > 1 {
        ...
        let normalImg = UIImage.managerToSecond(name: (String(showBoyfriendValue.suffix(9)) + String(dataDateAgentKey.prefix(4)) + String(mainRecognizePath))).withTintColor(.white)
        //: if userModel.loungePlus {
        if userModel.loungePlus {
            //: if let vipModel = MarginExamineer.share.appUserConfigMode.vipSetting.filter({$0.vipSkinId == userModel.vipSkinId}).first {
            if let vipModel = MarginExamineer.share.appUserConfigMode.vipSetting.filter({ $0.vipSkinId == userModel.vipSkinId }).first {
                //: vipBgView.setUrlImage(urlStr: vipModel.vipSkinTop, placeImg: normalImg)
                vipBgView.chest(urlStr: vipModel.vipSkinTop, placeImg: normalImg)
            }
            //: } else {
        } else {
        ...
        self.nicknameLab.textColor = userModel.loungePlus ? .boyfriend() : .white
        ...
        if userModel.intimateUrl.count > 0 {
            ...
            intimateImgV.poolFee(urlStr: userModel.intimateUrl)
        }
        // 在线状态
        ...
        if userModel.livePlayUrl.count > 0 {
            ...
            let image = UIImage.programFront(color: UIColor(hex: (String(appParticipationRadioValue) + dataStrengthPath.replacingOccurrences(of: "instance", with: "F")))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.mightPhoto(), for: .normal)
            onlineImgV.setImage(image.mightPhoto(), for: .normal)
            //: onlineImgV.setTitle("Live".localized, for: .normal)
            onlineImgV.setTitle((String(kLicensePath)).localized, for: .normal)
            ...
            let image = UIImage.programFront(color: UIColor(hex: (String(kBlankData)))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.mightPhoto(), for: .normal)
            onlineImgV.setImage(image.mightPhoto(), for: .normal)
            //: onlineImgV.setTitle("Online".localized, for: .normal)
            onlineImgV.setTitle((String(constSwingQuakeString)).localized, for: .normal)
            ...
            let image = UIImage.programFront(color: UIColor(hex: (String(kHimSureMessage)))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.mightPhoto(), for: .normal)
            onlineImgV.setImage(image.mightPhoto(), for: .normal)
            //: onlineImgV.setTitle("busy".localized, for: .normal)
            onlineImgV.setTitle((main_appealString.replacingOccurrences(of: "specify", with: "y")).localized, for: .normal)
            //: } else {
        } else {
        ...
        // 直播
        //: if MarginExamineer.share.loginUid != userModel.uid &&
        if MarginExamineer.share.loginUid != userModel.uid,
        ...
        if liveView.isHidden == false {
            ...
            liveView.theAngleBehavior(liveUrl: userModel.livePlayUrl)
        }
    }

    /// 添加视图
    //: private func setupSubviews() {
    private func beyondShadow() {
    ...
    private func anFast() {
        ...
        liveView.snp.makeConstraints { make in
            ...
            make.top.equalTo(const_musicMessage + 80)
        ...
        reViewingLB.snp.makeConstraints { make in
            ...
            make.top.equalToSuperview().offset(const_musicMessage + 15)
