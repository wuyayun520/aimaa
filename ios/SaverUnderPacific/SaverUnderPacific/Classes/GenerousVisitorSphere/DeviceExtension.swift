...
/*: "RSA加密失败，str:  :*/
fileprivate let const_depressArrowId:String = "RSA\u{52a0}\u{5bc6}"

/*: "weixin://" :*/
fileprivate let user_pressureConstructMessage:String = "WE"
fileprivate let user_withinMsg:[Character] = ["i","x","i","n",":","/","/"]

/*: "dingtalk://" :*/
fileprivate let constDadExplainFormat:String = "dinbold"
fileprivate let user_feedbackValue:String = "timing select tick drawingalk://"

/*: "lark://" :*/
fileprivate let k_whipMasterMsg:String = "lark:/flag constant"
fileprivate let kEndMessage:String = "along"

/*: "wxwork://" :*/
fileprivate let dataVeryBottomEdgeData:String = "guard server present path tierwxwork:"
fileprivate let notiEffectData:String = "//"

/*: "deviceId" :*/
fileprivate let app_illegalPath:[UInt8] = [0x37,0x36,0x25,0x3a,0x30,0x36,0x1a,0x37]

/*: "weixin" :*/
fileprivate let showNormPath:[Character] = ["w"]
fileprivate let appProcessorId:String = "eihunkn"

/*: "wxwork" :*/
fileprivate let mainBeforeUrl:[Character] = ["w","x","w","o","r","k"]

/*: "dingtalk" :*/
fileprivate let kLargeMessage:[Character] = ["d"]
fileprivate let k_serverFunId:String = "ingtdocumentk"

/*: "lark" :*/
fileprivate let main_awakeBecomeString:String = "LARK"
...
extension UIDevice {
    ...
    class func pastAspect(IdentifierBlock: @escaping (_ str: String) -> Void) {
        ...
        if #available(iOS 14.0, *) {
            ...
            ATTrackingManager.requestTrackingAuthorization(completionHandler: { status in
                ...
                if status == .authorized {
                    ...
                    IdentifierBlock(UIDevice.mortgageTo())
                    //: } else {
                } else {
                    //: IdentifierBlock("")
                    IdentifierBlock("")
    ...
    class func reflectUponEstimatedVersus() -> ([String]) {
...
//            let mnc = carrier?.mobileNetworkCode  //移动网络代码
    ...
    static func cosy(str: String) -> String {
        //: var encryptedStr = ""
        var encryptedStr = ""
        //: do {
        do {
            ...
            printLog(message: (const_depressArrowId + "失败，str: ") + "\(str).")
        }
        //: return encryptedStr
        return encryptedStr
    }
...
public extension UIDevice {
    ...
    static var isOpenWX: Bool {
        ...
        let appSchema = (user_pressureConstructMessage.lowercased() + String(user_withinMsg))
    ...
    static var isOpenDD: Bool {
        ...
        let appSchema = (constDadExplainFormat.replacingOccurrences(of: "bold", with: "gt") + String(user_feedbackValue.suffix(6)))
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var isOpenFS: Bool {
        //: let appSchema = "lark://"
        let appSchema = (String(k_whipMasterMsg.prefix(6)) + kEndMessage.replacingOccurrences(of: "along", with: "/"))
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var isOpenWXWork: Bool {
        ...
        let appSchema = (String(dataVeryBottomEdgeData.suffix(7)) + notiEffectData.capitalized)
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var customDeviceNum: String {
        ...
        if let value = key.get(data_shapeValue) {
            //: return value
            return value
            //: }else{
        } else {
            ...
            key.set(value, forKey: data_shapeValue)
            //: return value
            return value
        }
    }
    ...
    static var deviceInfoDic: [String: Any] {
        ...
        let usedVpn = PresentLocal.atQuote() || PresentLocal.holdMain()
        ...
        if let paramsStr = String(data: data!, encoding: String.Encoding.utf8) {
            ...
            let deviceIdStr = cosy(str: adid)
            //: return ["deviceId": deviceIdStr,
            return [String(bytes: app_illegalPath.map{$0^83}, encoding: .utf8)!: deviceIdStr,
                    //: "p0": paramsStr]
                    "p0": paramsStr]
        }
        //: return [:]
    ...
    static var getApps: String? {
        ...
        if UIDevice.isOpenWX {
            ...
            appsArr.append((String(showNormPath) + appProcessorId.replacingOccurrences(of: "hunk", with: "xi")))
        }
        //: if UIDevice.isOpenWXWork {
        if UIDevice.isOpenWXWork {
            //: appsArr.append("wxwork")
            appsArr.append((String(mainBeforeUrl)))
        }
        //: if UIDevice.isOpenDD {
        if UIDevice.isOpenDD {
            //: appsArr.append("dingtalk")
            appsArr.append((String(kLargeMessage) + k_serverFunId.replacingOccurrences(of: "document", with: "al")))
        }
        //: if UIDevice.isOpenFS {
        if UIDevice.isOpenFS {
            //: appsArr.append("lark")
            appsArr.append((main_awakeBecomeString.lowercased()))
        }
        //: if appsArr.count > 0 {
        if appsArr.count > 0 {
