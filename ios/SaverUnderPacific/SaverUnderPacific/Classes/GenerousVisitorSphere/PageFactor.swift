...
fileprivate let show_typicalAttentionMsg:[UInt8] = [0x8e,0x93,0x8e,0x99,0x4d,0x88,0x94,0x89,0x8a,0x97,0x5f,0x4e,0x45,0x8d,0x86,0x98,0x45,0x93,0x94,0x99,0x45,0x87,0x8a,0x8a,0x93,0x45,0x8e,0x92,0x95,0x91,0x8a,0x92,0x8a,0x93,0x99,0x8a,0x89]

fileprivate func meetResume(player num: UInt8) -> UInt8 {
...
/*: "icon_chat_lock" :*/
fileprivate let appHaveStr:[Character] = ["i","c","o","n","_","c","h","a","t","_","l","o","c","k"]

/*: "icon_chat_burned" :*/
fileprivate let kGreetStr:String = "near promise spread unknown analysisicon_ch"
fileprivate let main_rashKey:String = "spell media whole talkat_bu"

/*: "icon_chat_burned_shan" :*/
fileprivate let notiMenuKey:String = "today top there subsequenticon_c"
fileprivate let constRunProgressString:String = "burnprovide"
fileprivate let noti_beforeStr:[Character] = ["n"]

/*: "Already burned" :*/
fileprivate let showRelatedUntilUrl:String = "Alreamoment start"
...
class PageFactor: ShadeItem {
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        fatalError(String(bytes: show_typicalAttentionMsg.map{meetResume(player: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var lockImagV: UIImageView = {
        ...
        imag.image = UIImage.managerToSecond(name: (String(appHaveStr)))
    ...
    lazy var burnedImagV: DistinctionButton = {
        ...
        imgV.setBackgroundImage(UIImage.managerToSecond(name: (String(kGreetStr.suffix(7)) + String(main_rashKey.suffix(5)) + "rned")), for: .normal)
        //: imgV.setImage(UIImage.BundleImageNamed(name: "icon_chat_burned_shan"), for: .normal)
        imgV.setImage(UIImage.managerToSecond(name: (String(notiMenuKey.suffix(6)) + "hat_" + constRunProgressString.replacingOccurrences(of: "provide", with: "e") + "d_sha" + String(noti_beforeStr))), for: .normal)
        //: imgV.imageAlignment = .top
        imgV.imageAlignment = .top
        //: imgV.setTitle("Already burned".localized, for: .normal)
        imgV.setTitle((String(showRelatedUntilUrl.prefix(5)) + "dy burned").localized, for: .normal)
...
extension PageFactor {
    //: override func fill(with data: TCommonCellData) {
    override func satisfyAssociated(with data: SpineSuccess) {
        //: super.fill(with: data)
        super.satisfyAssociated(with: data)
        //: guard let imageCellData = data as? TalkingChatImageMsgCellData else { return }
        guard let imageCellData = data as? DelayComposition else { return }
        ...
        let intimatePhoto: EaseModel? = imageCellData.msgModel.msgInfo.intimatePhoto
        ...
        if intimatePhoto != nil || imageCellData.msgModel.msgInfo.customSend.isBurn == "1" || Int(imageCellData.msgModel.msgInfo.customSend.giftId) ?? 0 > 0 { // 私密照片
            ...
            translation(data: imageCellData, urlStr: imgUrlStr, isMosaic: imgIsMosaic)

            //: } else {
        } else { // 普通图片
            //: showImageView(data: imageCellData, urlStr: nil, isMosaic: false)
            translation(data: imageCellData, urlStr: nil, isMosaic: false)
        }
    }
    ...
    ///   - urlStr: 图片网络地址
    ///   - isMosaic: 是否需要马赛克
    //: func showImageView(data: TalkingChatImageMsgCellData, urlStr: String?, isMosaic: Bool) {
    func translation(data: DelayComposition, urlStr: String?, isMosaic: Bool) {
        ...
        if data.imageUrl!.count > 0, isExists {
            ...
            DispatchQueue.global().async {
                ...
                DispatchQueue.main.async {
                    ...
                    //: if isMosaic {
                    if isMosaic {
                        //: self.photoView.image = UIImage.wrap(image, styleOrange: 90)
                        self.photoView.image = UIImage.wrap(image, styleOrange: 90)
                        //: } else {
                    } else {
            ...
            nameClose(urlStr: urlStr ?? "", isMosaic: isMosaic)
            //: } else {
        } else { // 本地图片请求网络
            ...
            if data.imageData != nil {
                ...
                self.photoView.poolFee(urlStr: data.imageUrl!)
            }
        }
    }
    ...
    func nameClose(urlStr: String, isMosaic: Bool) {
        //: self.photoView.setCookieToUrlImage(urlStr: urlStr) {[weak self] image in
        self.photoView.alwaysFor(urlStr: urlStr) { [weak self] image in
