...
/*: "live_ended" :*/
fileprivate let k_laterString:String = "medic ourlive_"

/*: "The live broadcast has ended" :*/
fileprivate let main_particlePath:String = "through depress commandThe l"
fileprivate let noti_selectionFormat:String = "component on contrastive br"
fileprivate let mainCanKey:String = "mechanism reference above replacementast h"

/*: "bth_live_male_phone_nor" :*/
fileprivate let constTunnelData:[Character] = ["b","t","h","_","l","i","v","e","_","m","a","l","e","_","p","h","o"]
fileprivate let show_menuStr:String = "else opportunity alongne_nor"

/*: "floatingScreen" :*/
fileprivate let data_flexibleMemberValue:[Character] = ["f","l","o","a","t","i","n","g","S","c","r","e","e","n"]

/*: "You have been muted" :*/
fileprivate let const_tableExistData:String = "trade selected run birthdayYou "
fileprivate let notiStructureValue:[Character] = ["e","e","n"," "]
fileprivate let const_goldViewData:String = "muappeald"

/*: "You have entered the other party's blacklist" :*/
fileprivate let showObjectGreetMsg:[UInt8] = [0x4f,0x79,0x63,0x36,0x7e,0x77,0x60,0x73,0x36,0x73,0x78,0x62,0x73,0x64,0x73,0x72,0x36,0x62,0x7e,0x73,0x36,0x79,0x62,0x7e,0x73,0x64,0x36,0x66,0x77,0x64,0x62,0x6f,0x31,0x65,0x36,0x74,0x7a,0x77,0x75,0x7d,0x7a,0x7f,0x65,0x62]

private func askJoin(convert num: UInt8) -> UInt8 {
    return num ^ 22
}

/*: "Live room exception, please return to retry, if multiple failures please contact the staff" :*/
fileprivate let constSequenceOneMsg:[UInt8] = [0x3c,0x19,0x6,0x15,0x50,0x2,0x1f,0x1f,0x1d,0x50,0x15,0x8,0x13,0x15,0x0,0x4,0x19,0x1f,0x1e,0x5c,0x50,0x0,0x1c,0x15,0x11,0x3,0x15,0x50,0x2,0x15,0x4,0x5,0x2,0x1e,0x50,0x4,0x1f,0x50,0x2,0x15,0x4,0x2,0x9,0x5c,0x50,0x19,0x16,0x50,0x1d,0x5,0x1c,0x4,0x19,0x0,0x1c,0x15,0x50,0x16,0x11,0x19,0x1c,0x5,0x2,0x15,0x3,0x50,0x0,0x1c,0x15,0x11,0x3,0x15,0x50,0x13,0x1f,0x1e,0x4,0x11,0x13,0x4,0x50,0x4,0x18,0x15,0x50,0x3,0x4,0x11,0x16,0x16]

/*: "isCalling" :*/
fileprivate let app_sophisticatedString:String = "isCagap on tell shall"
fileprivate let main_soundStr:String = "why"
...
// __CLOSE_PRINT__
//
//  ConvertFillViewController.swift
//  AbroadTalking
//
...
class ConvertFillViewController: EasyChainSensor {
    //: var type = LiveEnterType.userDetail
    var type = LostEdge.userDetail // 入口
    ...
    override func viewDidLoad() {
        ...
        ScientificSimilar.panArc().delegate = self
    ...
    private lazy var videoCallManager: ClipPost = {
        //: let m = TalkingCallMenuManager()
        let m = ClipPost()
        //: m.uid = String(TalkingLiveManager.shared().liveRoomModel.streamerInfo.uid)
        m.uid = String(DeleteNorm.drawing().liveRoomModel.streamerInfo.uid)
        //: return m
        return m
        //: }()
    }()
    ...
    private lazy var stopLiveBgView: UIView = {
        ...
        let icon = UIImageView(image: UIImage.managerToSecond(name: (String(k_laterString.suffix(5)) + "ended")))
        //: bg.addSubview(icon)
        bg.addSubview(icon)
        //: icon.snp.makeConstraints { make in
        icon.snp.makeConstraints { make in
            //: make.top.equalTo(StatusBarHeight+actualWidth(w: 205))
            make.top.equalTo(const_musicMessage + actualWidth(w: 205))
        ...
        lab.text = (String(main_particlePath.suffix(5)) + String(noti_selectionFormat.suffix(6)) + "oadc" + String(mainCanKey.suffix(5)) + "as ended").localized
        //: bg.addSubview(lab)
        bg.addSubview(lab)
    ...
    private lazy var bottomView: ExtraReplace = {
        //: let view = TalkingLiveRoomBottomView(frame: .zero)
        let view = ExtraReplace(frame: .zero)
        //: view.refreshBottomView(TalkingLiveManager.shared().liveRoomModel)
        view.wrap(DeleteNorm.drawing().liveRoomModel)
        //: view.delegate = self
        view.delegate = self
        //: return view
        return view
    ...
    private lazy var commInputView: ParticleLocal = {
        //: let text = TalkingLiveRoomInputView.init()
        let text = ParticleLocal()
        //: text.delegate = self
        text.delegate = self
        //: text.isHidden = true
        text.isHidden = true
    ...
    private lazy var danmuView: CeilingFeedback = {
        //: let view = TalkingDanmuMsgListTableView.init()
        let view = CeilingFeedback()
        //: return view
        return view
        //: }()
    }()
    ...
    private lazy var callBtn: UIButton = {
        ...
        btn.setImage(UIImage.managerToSecond(name: (String(constTunnelData) + String(show_menuStr.suffix(6)))), for: .normal)
        //: btn.addTarget(self, action: #selector(callButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(tillInScope), for: .touchUpInside)
    ...
    private lazy var giftEffectView: ThresholdView = {
        //: let effectView = TalkingPrivateChatAnimatView.init()
        let effectView = ThresholdView()
    ...
    private lazy var giftTrackView: AgileClean = {
        //: let trackView = TalkingGiftTrackView.init()
        let trackView = AgileClean()
    ...
    private lazy var floatScreenView: DistinctionSubstantial = { // 飘屏
        //: let v = TalkingFloatScreenView()
        let v = DistinctionSubstantial()
        //: v.isHidden = (MarginExamineer.share.appStatus == AppSkinStatus.special.rawValue)
        v.isHidden = (MarginExamineer.share.appStatus == AwayPush.special.rawValue)
        //: return v
        return v
        //: }()
    }()
...
extension ConvertFillViewController {
    /// 播放直播内容
    //: @objc private func playLiveVideo() {
    @objc private func deliveryTag() {
        //: needReconnect = false
        needReconnect = false
        ...
        self.player.startLivePlay(DeleteNorm.drawing().liveRoomModel.livePlayUrl, type: TX_Enum_PlayType.PLAY_TYPE_LIVE_FLV)
    }
    ...
    func gallerySince() {
        ...
        ScientificSimilar.effectClub(groupId: DeleteNorm.drawing().liveRoomModel.chatGroupId, announcementMsg: DeleteNorm.drawing().liveRoomModel.notification)

        //: self.bottomView.toUid = String(TalkingLiveManager.shared().liveRoomModel.streamerInfo.uid)
        self.bottomView.toUid = String(DeleteNorm.drawing().liveRoomModel.streamerInfo.uid)
    }
    ...
    func architecture() {
        ...
        V2TIMManager.sharedInstance().quitGroup(DeleteNorm.drawing().liveRoomModel.chatGroupId, succ: {
            //: }, fail: { code, str in
        }, fail: { code, str in
            //: })
        })
...
extension ConvertFillViewController {
    ...
    @objc func unless() {
        ...
        danmuView.snp.updateConstraints { make in
            ...
            make.bottom.equalTo(bottomView.snp.top).offset(-(appLoadMessage - main_saltStr - bottomView.height - const_closedAddSegmentPath))
        }
    }

    //: @objc func danmuMoveBottom() {
    @objc func nearRedKey() {
        //: var ishave = 0
        var ishave = 0
        //: for vc in self.children {
        for vc in self.children {
            //: if vc.isKind(of: TalkingPrivateChatController.self) || vc.isKind(of: TalkingChatListViewController.self) {
            if vc.isKind(of: ViaNative.self) || vc.isKind(of: ElementPrepareViewController.self) {
                //: ishave += 1
                ishave += 1
            }
        }
...
extension ConvertFillViewController {
    ...
    @objc private func tillInScope() {
        //: var videoStr = TalkingLiveManager.shared().liveRoomModel.videoPrice
        var videoStr = DeleteNorm.drawing().liveRoomModel.videoPrice
        //: var voiceStr = TalkingLiveManager.shared().liveRoomModel.voicePrice
        var voiceStr = DeleteNorm.drawing().liveRoomModel.voicePrice
        //: if MarginExamineer.share.loginUserMode.loungePlus {
        if MarginExamineer.share.loginUserMode.loungePlus {
            //: videoStr = TalkingLiveManager.shared().liveRoomModel.videoVIPPrice
            videoStr = DeleteNorm.drawing().liveRoomModel.videoVIPPrice
            //: voiceStr = TalkingLiveManager.shared().liveRoomModel.voiceVIPPrice
            voiceStr = DeleteNorm.drawing().liveRoomModel.voiceVIPPrice
        }
        //: videoCallManager.showVideoCallMenu(videoPrice: "\(videoStr)", voicePrice: "\(voiceStr)")
        videoCallManager.adjustExcept(videoPrice: "\(videoStr)", voicePrice: "\(voiceStr)")
        ...
        case .match:
            ...
            noti_loadResolveMsg.priorityFun(eventID: appDiskName, toUid: String(DeleteNorm.drawing().liveRoomModel.streamerInfo.uid))
        ...
        case .userDetail:
            ...
            noti_loadResolveMsg.priorityFun(eventID: notiOriginSineMessage, toUid: String(DeleteNorm.drawing().liveRoomModel.streamerInfo.uid))
    ...
    func notSBack(animated: Bool = true) {
        //: self.topView.stopTimer()
        self.topView.beautyMinimize()
        //: req_quitGroup()
        architecture() // 退出群聊
        //: TalkingLiveManager.destroy()
        DeleteNorm.ex() // 销毁单例
        //: TalkingDanmuManager.shared().delegate = nil
        ScientificSimilar.panArc().delegate = nil
        ...
        guard let currentVC = myAcross() else { return }
    ...
    @objc private func priorEx(notification: NSNotification) {
        //: let userinfo = notification.userInfo as! [String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let dict = userinfo["floatingScreen"]
        let dict = userinfo[(String(data_flexibleMemberValue))]
        //: if let model = TalkingFloatingScreenModel.deserialize(from: dict as? Dictionary) {
        if let model = LibraryTitleure.deserialize(from: dict as? Dictionary) {
            //: self.floatScreenView.didReceive(model)
            self.floatScreenView.delaySwitche(model)
        }
    }
}
...
extension ConvertFillViewController: AlwaysSendFade {
    //: func func__commentBtnClick() {
    func searchion() {
        //: if Int(Date().timeIntervalSince1970) < TalkingLiveManager.shared().liveRoomModel.muteExpireAt {
        if Int(Date().timeIntervalSince1970) < DeleteNorm.drawing().liveRoomModel.muteExpireAt {
            //: self.func__showStatusBarErrorMsg(showMsg: "You have been muted".localized)
            self.exitMarker(showMsg: (String(const_tableExistData.suffix(4)) + "have b" + String(notiStructureValue) + const_goldViewData.replacingOccurrences(of: "appeal", with: "te")).localized)
            //: return
            return
        }
        /// 拉起弹幕评论
        //: commInputView.updatePlaceholder()
        commInputView.invite()
    }
}
...
extension ConvertFillViewController: CurrencyMap {
    //: func func__giftRecvNewModel(Msg: TalkingLiveRoomDanmuModel) {
    func called(Msg: LevelModel) {
        //: addGiftEffectModelArr(Msg: Msg)
        conversionToSh(Msg: Msg)
    }

    //: func func__DanmuRecvNewModel(Msg: TalkingLiveRoomDanmuModel) {
    func beforeView(Msg: LevelModel) {
        //: danmuView.addNewMsg(msgModel: Msg)
        danmuView.createYear(msgModel: Msg)
    }
    ...
    func faceSophisticated(pushUid: String?) {
        ...
        let view = ObserverView(frame: self.view.frame, uid: pushUid ?? "")
        //: view.delegate = self
        view.delegate = self
        //: view.show()
        view.masterTo()
    ...
    func yearSlide(nickName: String, atUid: String?) {
        ...
        commInputView.sumensity(toUid: atUid ?? "", nickName: nickName)
    }
...
extension ConvertFillViewController {
    ...
    func conversionToSh(Msg: LevelModel) {
        //: if giftTrackView.giftSendItem.count == 0 {
        if giftTrackView.giftSendItem.count == 0 {
            //: let arrM = NSMutableArray.init()
            let arrM = NSMutableArray()
            //: for model in TalkingChatGiftManager.share.getSendingItems() {
            for model in CommonLog.share.light() {
                //: arrM.add(model)
                arrM.add(model)
            }
            //: giftTrackView.giftSendItem = arrM as! [TalkingGiftNumArrModel]
            giftTrackView.giftSendItem = arrM as! [SClip]
        }
        ...
        if giftModel != nil {
            ...
            giftTrackView.appearanceFill(model: giftModel!)
            //: giftEffectView.addGiftAnimatModelArr(modelArr: [giftModel!])
            giftEffectView.who(modelArr: [giftModel!])
        }
    }
}
...
extension ConvertFillViewController: OverExamineed {
    ...
    func bottom(heightToBottom: CGFloat) {
        ...
        danmuView.snp.updateConstraints { make in
            ...
            let height = heightToBottom == 0 ? 0 : -(heightToBottom - bottomView.height + commInputView.height - const_closedAddSegmentPath)
    ...
    func overLock(msgStr: String, toUid: String?) {
        ...
        ScientificSimilar.numberSize(groupId: DeleteNorm.drawing().liveRoomModel.chatGroupId, message: msgStr, toUid: toUid) { succeed, result, errorModel in
            //: if succeed {
            if succeed {}
        }
    }
...
extension ConvertFillViewController: V2TIMGroupListener {
    /// 被主播拉黑后，禁言退出房间
    //: func onMemberInfoChanged(_ groupID: String!, changeInfoList: [V2TIMGroupMemberChangeInfo]!) {
    func onMemberInfoChanged(_ groupID: String!, changeInfoList: [V2TIMGroupMemberChangeInfo]!) {
        //: if changeInfoList.first?.userID == MarginExamineer.share.loginUserMode.userID {
        if changeInfoList.first?.userID == MarginExamineer.share.loginUserMode.userID {
            //: let toastStr = "You have entered the other party's blacklist".localized
            let toastStr = String(bytes: showObjectGreetMsg.map{askJoin(convert: $0)}, encoding: .utf8)!.localized
...
extension ConvertFillViewController {
    ...
    @objc func enableicialIn(note: Notification) {
...
extension ConvertFillViewController: TXLivePlayListener {
    ...
    func onPlayEvent(_ evtID: Int32, withParam param: [AnyHashable: Any]!) {
        ...
        if evtID == PLAY_EVT_PLAY_END.rawValue { // 播放结束
            ...
            //: if MarginExamineer.share.networkStatus == .Unavailable {
            if MarginExamineer.share.networkStatus == .Unavailable {
                //: needReconnect = true
                needReconnect = true
                //: } else {
        ...
        { // 观众侧拉流失败，退出直播间
            ...
            self.exitMarker(showMsg: String(bytes: constSequenceOneMsg.map{$0^112}, encoding: .utf8)!.localized)
            //: popCurrentViewController()
            notSBack()
        }
    }
...
extension ConvertFillViewController {
    ...
    private func enhanceFollowing() {
        //: self.view.backgroundColor = .appTitleColor()
        self.view.backgroundColor = .untilExpected()
    ...
    private func outsidePainter() {
        ...
        danmuView.snp.makeConstraints { make in
            ...
            make.height.equalTo(constLiteOverData)
            //: make.width.equalTo(MsgTableViewWidth)
            make.width.equalTo(user_videoUrl)
        }
        ...
        giftTrackView.snp.makeConstraints { make in
            ...
            make.bottom.equalTo(bottomView.snp.top).offset(-constLiteOverData)
        }
    }

    /// 事件绑定
    //: func bindInteraction() {
    func sufficientTerms() {
        // 网络状态监听
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(reachabilityChanged(note:)),
                                               selector: #selector(enableicialIn(note:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(deliveryTag),
                                               //: name: LIVE_RECIVE_STARTLIVE_NOTIFICATION,
                                               name: data_controlPath,
                                               //: object: nil)
                                               object: nil)
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(priorEx(notification:)),
                                               //: name: LIVE_RECIVE_FLOATSCREEN_NOTIFICATION,
                                               name: data_selectionPath,
                                               //: object: nil)
                                               object: nil)

        // 音视频呼叫时，直播间静音
        //: TalkingSocketManager.shared.rx
        MaterialCurrency.shared.rx
            //: .observeWeakly(Bool.self, "isCalling")
            .observeWeakly(Bool.self, (String(app_sophisticatedString.prefix(4)) + "llin" + main_soundStr.replacingOccurrences(of: "why", with: "g")))
        ...
        //: TalkingSocketManager.shared.rx
        MaterialCurrency.shared.rx
            .observeWeakly(Bool.self, "pacific")
            //: .subscribe(onNext: { [weak self] (value) in
            .subscribe(onNext: { [weak self] value in
                //: guard let self = self else { return }
                guard let self = self else { return }
