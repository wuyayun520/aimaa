...
/*: "rYMsgType" :*/
fileprivate let constWhatId:String = "selection clearly marginrYMsg"
fileprivate let data_brokenName:String = "beat mechanism reward loadType"

/*: "GJ:CallCustom" :*/
fileprivate let userTranslateName:[Character] = ["G","J",":","C","a","l","l","C"]
fileprivate let userTextMessage:[Character] = ["u","s","t","o","m"]
...
class HeadOptionFragment: NSObject {
    //: private var showView: TalkingMessageNotificationView?
    private var showView: BothAdjustReliability? // 当前View
    //: private var preView: TalkingMessageNotificationView?
    private var preView: BothAdjustReliability? // 旧的View
    ...
    static var shared = HeadOptionFragment()
    ...
    private lazy var msgArr: [FormSubtleModel] = //: return [FormSubtleModel]()
        .init()
    //: }()
}
...
extension HeadOptionFragment {
    /// 处理接收到的TX消息，展示消息通知
    /// - Parameters:
    ///   - msg: V2TIMMessage
    ///   - dataDict: 消息字典
    //: func onRecvNewMessage(msg: V2TIMMessage, dataDict: [String: Any]) {
    func file(msg: V2TIMMessage, dataDict: [String: Any]) {
        ...
        guard scientific() == true || locateSegment() == true else { return }
        //: guard msg.userID != MovementStructure.getCustomerServiceID() && msg.userID != MovementStructure.getXiaoMiID() else {
        guard msg.userID != MovementStructure.cartoonConnection(), msg.userID != MovementStructure.uniqueAbove() else {
            //: return
            return
        }
        //: let contenType = TUISocialChatManager.messageCellType(dataDict)
        let contenType = IndependentObserve.responseVertical(dataDict)
        // 提示消息
        ...
        if contenType == .TXMessageContentType_text, dataDict[(String(constWhatId.suffix(5)) + String(data_brokenName.suffix(4)))] as? String == (String(userTranslateName) + String(userTextMessage)) {
            //: return
            return
        }

        // 2. V2TIMMessage => FormSubtleModel
        ...
        //: if let userInfo: DocumentLevelPurple = DBUserInfoManager.cache_getCachedUserInfo(targetId: model.targetId) {
        if let userInfo: DocumentLevelPurple = ProgressAssociated.betweenPolicy(targetId: model.targetId) {
    ...
    private func deployCount() {
        ...
        let notifView = BothAdjustReliability(frame: .zero)
        //: notifView.show(firstModel)
        notifView.measureHistory(firstModel)
    ...
    private func locateSegment() -> Bool {
        //: guard let currentVC = currentViewController() else { return false }
        guard let currentVC = myAcross() else { return false }
        //: if currentVC.isKind(of: TalkingSocialViewController.self) ||
        if currentVC.isKind(of: TotalegrationNextCommand.self) ||
            //: currentVC.isKind(of: SocialPopularViewController.self) ||
            currentVC.isKind(of: SignatureSegment.self) ||
            //: currentVC.isKind(of: TalkingMomentViewController.self) ||
            currentVC.isKind(of: PerformViewController.self) ||
            //: currentVC.isKind(of: TalkingMeViewController.self) || currentVC.isKind(of: RandomMatchVC.self) {
            currentVC.isKind(of: ProfileViewController.self) || currentVC.isKind(of: PutPromiseViewController.self)
        {
            //: return true
            return true
        }
