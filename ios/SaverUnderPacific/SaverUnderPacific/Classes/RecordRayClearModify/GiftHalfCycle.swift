...
fileprivate let user_tooMessage:[UInt8] = [0xf4,0xf3,0xf4,0xe9,0xb5,0xfe,0xf2,0xf9,0xf8,0xef,0xa7,0xb4,0xbd,0xf5,0xfc,0xee,0xbd,0xf3,0xf2,0xe9,0xbd,0xff,0xf8,0xf8,0xf3,0xbd,0xf4,0xf0,0xed,0xf1,0xf8,0xf0,0xf8,0xf3,0xe9,0xf8,0xf9]

private func compositionGenderPlus(run num: UInt8) -> UInt8 {
    return num ^ 157
}

/*: "bg_medals_podium" :*/
fileprivate let user_entryUrl:String = "bg_medcatch change in receiver"
fileprivate let dataExceptStr:String = "odphoneum"

/*: "icon_rank_first_bg" :*/
fileprivate let userRemovePath:String = "icon_ranksatisfy specify"
fileprivate let kJobTrustStr:[Character] = ["_","f","i","r","s","t","_","b","g"]

/*: "icon_rank_first" :*/
fileprivate let userScaleJoinValue:[Character] = ["i","c","o","n","_","r","a","n","k","_","f","i","r"]
fileprivate let app_detailedUrl:String = "scant"

/*: "icon_ammount_points" :*/
fileprivate let k_cosFemaleFormat:[Character] = ["i","c","o","n","_","a","m","m","o","u","n","t","_"]
fileprivate let appComposeMentionTitle:[Character] = ["p","o","i","n","t","s"]

/*: "icon_lounge" :*/
fileprivate let kFitProgramRemarkStr:String = "icon_attach restore"

/*: "icon_rank_coin" :*/
fileprivate let dataUserLowerPath:[Character] = ["i","c","o","n","_"]
fileprivate let mainCeilingString:[Character] = ["r","a","n","k","_","c","o","i","n"]

/*: "icon_rank_coin2" :*/
fileprivate let notiUniversalIonMsg:String = "icostroke"
fileprivate let showFeatureSaleFormat:[Character] = ["n","2"]

/*: "icon_rank_second" :*/
fileprivate let user_ehKey:String = "some accelerateicon_r"
fileprivate let mainArableMessage:String = "ofee"

/*: "icon_rank_third" :*/
fileprivate let user_labTitle:[Character] = ["i","c","o","n","_","r","a","n"]
fileprivate let k_animaStr:String = "distribution limit prompt behaviork_"
fileprivate let data_qualityTitle:[Character] = ["t","h","i","r","d"]
...
class GiftHalfCycle: UIView {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: user_tooMessage.map{compositionGenderPlus(run: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var medalsView = UIImageView().then {
        ...
        $0.image = UIImage.managerToSecond(name: (String(user_entryUrl.prefix(6)) + "als_p" + dataExceptStr.replacingOccurrences(of: "phone", with: "i")))
    }

    //: lazy var firstView = SocialRankHeaderItemView.init(frame: .zero, rankType: .firstRankType).then {
    lazy var firstView = RegulateOption(frame: .zero, rankType: .firstRankType).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.tag = 0
        $0.tag = 0
        //: let tap = UITapGestureRecognizer.init(target: self, action: #selector(registTapAction))
        let tap = UITapGestureRecognizer(target: self, action: #selector(computeTing))
        //: $0.addGestureRecognizer(tap)
        $0.addGestureRecognizer(tap)
    }

    //: lazy var secondView = SocialRankHeaderItemView.init(frame: .zero, rankType: .secondRankType).then {
    lazy var secondView = RegulateOption(frame: .zero, rankType: .secondRankType).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.tag = 1
        $0.tag = 1
    ...
    lazy var thirdView = RegulateOption(frame: .zero, rankType: .thirdRankType).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.tag = 2
        $0.tag = 2
...
extension GiftHalfCycle {
    //: private func bindInteraction() {
    private func sharedRequire() {}

    //: private func updateSubviewsWithData() {
    private func noneYesterday() {
        //: firstView.updateItemWithModel(model: dataArray[0], type: self.index)
        firstView.directDismiss(model: dataArray[0], type: self.index)
        //: secondView.updateItemWithModel(model: dataArray.count>1 ? dataArray[1]: nil, type: self.index)
        secondView.directDismiss(model: dataArray.count > 1 ? dataArray[1] : nil, type: self.index)
        //: thirdView.updateItemWithModel(model: dataArray.count>2 ? dataArray[2]: nil, type: self.index)
        thirdView.directDismiss(model: dataArray.count > 2 ? dataArray[2] : nil, type: self.index)
    }

    //: @objc func registTapAction(tapGesture: UITapGestureRecognizer) {
    @objc func computeTing(tapGesture: UITapGestureRecognizer) {
...
extension GiftHalfCycle {
    ...
    private func theClear() {
        //: let firstWidth = (ScreenWidth - actualWidth(w: 68)) / 3 + actualWidth(w: 10)
        let firstWidth = (show_errorChangeFormat - actualWidth(w: 68)) / 3 + actualWidth(w: 10)
        //: let itemWidth = (ScreenWidth - actualWidth(w: 68)) / 3 - actualWidth(w: 10)
        let itemWidth = (show_errorChangeFormat - actualWidth(w: 68)) / 3 - actualWidth(w: 10)
...
class RegulateOption: UIView {
    //: enum SRankHeaderItemType: Int {
    enum PointService: Int {
        //: case firstRankType
        case firstRankType
        //: case secondRankType
        case secondRankType
    ...
    public init(frame: CGRect, rankType: PointService) {
        //: super.init(frame: frame)
        super.init(frame: frame)
        //: self.setupItemViews(type: rankType)
        self.starBy(type: rankType)
    }
    ...
    lazy var headPicBgView = UIImageView().then {
        ...
        $0.image = UIImage.managerToSecond(name: (String(userRemovePath.prefix(9)) + String(kJobTrustStr)))
    }
    ...
    lazy var crownImgView = UIImageView().then {
        ...
        $0.image = UIImage.managerToSecond(name: (String(userScaleJoinValue) + app_detailedUrl.replacingOccurrences(of: "scan", with: "s")))
    }
    ...
    lazy var nickLabel = UILabel().then {
        ...
        $0.font = .yearNearby(type: .Medium, fontSize: 18)
        //: $0.textAlignment = .center
        $0.textAlignment = .center
    }

    //: lazy var coinButton = TalkingButton().then {
    lazy var coinButton = DistinctionButton().then {
        //: $0.isUserInteractionEnabled = false
        $0.isUserInteractionEnabled = false
        //: $0.setImage(UIImage.BundleImageNamed(name: "icon_ammount_points"), for: .normal)
        $0.setImage(UIImage.managerToSecond(name: (String(k_cosFemaleFormat) + String(appComposeMentionTitle))), for: .normal)
        //: $0.titleLabel?.font = .pingfangFont(type: .Regular, fontSize: 13)
        $0.titleLabel?.font = .yearNearby(type: .Regular, fontSize: 13)
    ...
    lazy var loungeImgV = UIImageView().then {
        ...
        $0.image = UIImage.managerToSecond(name: (String(kFitProgramRemarkStr.prefix(5)) + "lounge"))
    }
}
...
extension RegulateOption {
    //: func updateItemWithModel(model: SocialRankItemModel?, type: Int) {
    func directDismiss(model: BoldModel?, type: Int) {
        ...
        headPic.chest(urlStr: model!.headPic, placeImg: UIImage.queryedChange(sex: model!.sex))
        ...
        if type == 0 {
            ...
            coinButton.setImage(UIImage.managerToSecond(name: (String(dataUserLowerPath) + String(mainCeilingString))), for: .normal)
            //: } else {
        } else {
            //: coinButton.setImage(UIImage.BundleImageNamed(name: "icon_rank_coin2"), for: .normal)
            coinButton.setImage(UIImage.managerToSecond(name: (notiUniversalIonMsg.replacingOccurrences(of: "stroke", with: "n") + "_rank_coi" + String(showFeatureSaleFormat))), for: .normal)
        }
    }
}
...
//        var topCrownMargin = actualHeight(h: 4)
        ...
        case .firstRankType:
            ...
            crownImgView.image = UIImage.managerToSecond(name: (String(userScaleJoinValue) + app_detailedUrl.replacingOccurrences(of: "scan", with: "s")))
            //: headPic.layer.borderColor = UIColor.RGBA(255, 224, 58, 1).cgColor
            headPic.layer.borderColor = UIColor.our(255, 224, 58, 1).cgColor
...
//            topCrownMargin = actualHeight(h: 10)
        ...
        case .secondRankType:
            ...
            crownImgView.image = UIImage.managerToSecond(name: (String(user_ehKey.suffix(6)) + "ank_sec" + mainArableMessage.replacingOccurrences(of: "fee", with: "nd")))
            //: headPic.layer.borderColor = UIColor.RGBA(182, 199, 254, 1).cgColor
            headPic.layer.borderColor = UIColor.our(182, 199, 254, 1).cgColor
        ...
        case .thirdRankType:
            ...
            crownImgView.image = UIImage.managerToSecond(name: (String(user_labTitle) + String(k_animaStr.suffix(2)) + String(data_qualityTitle)))
            //: headPic.layer.borderColor = UIColor.RGBA(251, 166, 109, 1).cgColor
            headPic.layer.borderColor = UIColor.our(251, 166, 109, 1).cgColor
            //: headPic.layer.cornerRadius = actualWidth(w: 31)
            headPic.layer.cornerRadius = actualWidth(w: 31)
            //: break
        }
