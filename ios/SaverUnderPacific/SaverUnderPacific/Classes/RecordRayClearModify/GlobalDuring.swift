...
fileprivate let appVariableUrl:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "[未知消息]" :*/
fileprivate let kCapturePath:String = "[未知female息]"

/*: "extra" :*/
fileprivate let showTransformId:[Character] = ["e","x","t","r","a"]

/*: "#0084FF" :*/
fileprivate let noti_nothingData:String = "#0"
fileprivate let appOnceOrientationId:String = "084FFhang context package"

/*: "#FFEE4B" :*/
fileprivate let notiPeoplePath:[Character] = ["#","F"]
fileprivate let app_popularKey:[Character] = ["F","E","E","4","B"]

/*: "<at>@[\\S\\s]+?</at>" :*/
fileprivate let userLocalRewardTitle:String = "builder warning pic case<at>@["
fileprivate let show_boundName:String = "broken frame extent check]+?</a"
fileprivate let main_strategyData:[Character] = ["t",">"]

/*: "jumpKey" :*/
fileprivate let k_dismissReplaceId:String = "fee streak gallery charm termsjumpK"
fileprivate let noti_builderString:[Character] = ["e","y"]

/*: "user" :*/
fileprivate let appSectionAdjustmentUrl:String = "USER"

/*: "uid" :*/
fileprivate let showWriteAtId:[UInt8] = [0x64,0x69,0x75]

/*: "#16D073" :*/
fileprivate let noti_tailName:String = "challenge swing educator day#16D073"

/*: "#F95644" :*/
fileprivate let userSaveName:String = "#F95644try honey if"
...
//: @objcMembers public class TalkingGroupChatTextMsgCellData: TalkingChatMsgBaseCellData {
@objcMembers public class GlobalDuring: DisplayOrigin {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: appVariableUrl.reversed(), encoding: .utf8)!)
    }
    ...
    override public var msgModel: EnhanceModel {
        ...
        set {
            ...
            if newValue.msgInfo.content.isEmptyString {
                //: contentStr = "[未知消息]"
                contentStr = (kCapturePath.replacingOccurrences(of: "female", with: "消"))
                //: }else {
            } else {
    ...
    public lazy var contentAttrString: NSMutableAttributedString = {
        ...
        let attr = weightStage(msgInfoModel: self.msgModel.msgInfo)
        //: return attr
        return attr
        //: }()
    }()
    ...
    func weightStage(msgInfoModel: ColumnModel) -> NSMutableAttributedString {
        ...
        guard dic[(String(showTransformId))] is [String: Any] else {
            //: return NSMutableAttributedString.init(string: "")
            return NSMutableAttributedString(string: "")
        }
        ...
        let textColor = (self.direction == .MsgDirectionIncoming) ? UIColor.untilExpected() : .white
        //: let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: "#0084FF")! : UIColor(hex: "#FFEE4B")!
        let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: (noti_nothingData.capitalized + String(appOnceOrientationId.prefix(5))))! : UIColor(hex: (String(notiPeoplePath) + String(app_popularKey)))!
        ...
        let regex = try! NSRegularExpression(pattern: (String(userLocalRewardTitle.suffix(6)) + "\\S\\s" + String(show_boundName.suffix(6)) + String(main_strategyData)), options: [])
        ...
        for (index, value) in rangeArr.enumerated() {
            ...
            if jumpInfo[(String(k_dismissReplaceId.suffix(5)) + String(noti_builderString))].stringValue == (appSectionAdjustmentUrl.lowercased()) {
                //: uid = jumpInfo["uid"].intValue
                uid = jumpInfo[String(bytes: showWriteAtId.reversed(), encoding: .utf8)!].intValue
            }
            //: if uid != -1 {
            if uid != -1 {
                ...
                attr.yy_setTextHighlight(range, color: atColor, backgroundColor: .clear, userInfo: [String(bytes: showWriteAtId.reversed(), encoding: .utf8)!: uid]) { [weak self] containerView, text, range, rect in
    ...
    func cancel() -> NSMutableAttributedString {
        ...
        var color = UIColor(hex: (String(noti_tailName.suffix(7))))!
        //: if self.translateStatus == .MsgTranslate_fail {
        if self.translateStatus == .MsgTranslate_fail {
            //: color = UIColor(hex: "#F95644")!
            color = UIColor(hex: (String(userSaveName.prefix(7))))!
        }
