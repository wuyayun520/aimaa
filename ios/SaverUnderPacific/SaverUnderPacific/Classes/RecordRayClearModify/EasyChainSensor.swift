...
/*: "🌛 🌛 enter  :*/
fileprivate let showFollowProjectionScanString:String = "🌛 🌛 enterstep operate people"
fileprivate let const_architectureFormat:String = " "

/*: "🌛 🌛 deinit  :*/
fileprivate let dataStarMsg:[Character] = ["🌛"," ","🌛"," ","d","e","i","n","i","t"," "]

/*: "nav_back_black_nor" :*/
fileprivate let user_delicateTitle:String = "nav_detailed appear pick"
fileprivate let main_portraitYieldId:[Character] = ["b","l","a","c","k","_","n","o","r"]
...
class EasyChainSensor: UIViewController {
    //: var hideNavi = false
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        printLog(message: (String(showFollowProjectionScanString.prefix(9)) + const_architectureFormat.capitalized) + "\(self)")
    }
    ...
    deinit {
        ...
        printLog(message: (String(dataStarMsg)) + "\(self)")
    }
}

//: extension TalkingBaseViewController: UIGestureRecognizerDelegate {
extension EasyChainSensor: UIGestureRecognizerDelegate {
    //: func addTapGestureRecognizer() {
    func reservePanel() {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(func__TapGestureRecognizer))
        let tap = UITapGestureRecognizer(target: self, action: #selector(purchase))
        //: tap.delegate = self
        tap.delegate = self
    ...
    @objc func purchase(_ tap: UITapGestureRecognizer) {
    ...
    func attributeSay(isOpen: Bool) {
        ...
        if let navArr = navigationController?.viewControllers {
            ...
            if navArr.contains(where: {
                ...
                $0.isKind(of: ConvertFillViewController.self) ||
                    //: $0.isKind(of: TalkingVoiceChatViewController.self) ||
                    $0.isKind(of: InserterStorage.self) ||
                    //: $0.isKind(of: TalkingVideoChatViewController.self)
                    $0.isKind(of: FillPurple.self)
                //: }) {
            }) {
                //: isOpen = false
                isOpen = false
...
extension UIViewController {
    ...
    func grainFlow() -> UIBarButtonItem {
        ...
        let item = UIBarButtonItem(image: UIImage.managerToSecond(name: (String(user_delicateTitle.prefix(4)) + "back_" + String(main_portraitYieldId))), style: .plain, target: self, action: #selector(independentTo))
