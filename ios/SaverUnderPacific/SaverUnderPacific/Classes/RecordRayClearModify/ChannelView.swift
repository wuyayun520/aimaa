...
fileprivate let userExHeadFatalTitle:[UInt8] = [0xd5,0xd2,0xd5,0xc8,0x94,0xdf,0xd3,0xd8,0xd9,0xce,0x86,0x95,0x9c,0xd4,0xdd,0xcf,0x9c,0xd2,0xd3,0xc8,0x9c,0xde,0xd9,0xd9,0xd2,0x9c,0xd5,0xd1,0xcc,0xd0,0xd9,0xd1,0xd9,0xd2,0xc8,0xd9,0xd8]

private func beautyScene(link num: UInt8) -> UInt8 {
    return num ^ 188
}
...
@objc protocol ReloadMovement: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func timingDetect()

    //: @objc optional func endScroll()
    @objc optional func streamFor()
}
...
class ChannelView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(confirmEntity))
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: userExHeadFatalTitle.map{beautyScene(link: $0)}, encoding: .utf8)!)
    }
...
extension ChannelView {
    ...
    func kit() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(mediaCondition), userInfo: nil, repeats: true)
    }
    ...
    @objc func mediaCondition() {
        //: tryToappendAndScrollToBottom()
        totalervalConsume()
    }

    //: @objc func tapAction() {
    @objc func confirmEntity() {
...
extension ChannelView {
    ...
    func exceptMount(msgModel: ReplaceHold?) {
    ...
    func valueToMend() {
        ...
        if AllHeight > appPriceId {
            //: if self.msgTableView.height < VideoMsgTableViewHeight {
            if self.msgTableView.height < appPriceId {
                ...
                self.msgTableView.height = appPriceId
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = appPriceId - AllHeight
        ...
        restrictionResult(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func restrictionResult(animated: Bool) {
...
extension ChannelView: UITableViewDelegate, UITableViewDataSource {
    ...
    func party(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.streamFor?()
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(user_evolutionUrl)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = ParticleList.doingce(tableView: tableView, msg: msgModel, indexPath: indexPath)
...
extension ChannelView {
    ...
    func includeMap() {
        ...
        self.msgTableView.frame = CGRect(x: 0, y: 0, width: show_layerValue, height: appPriceId)
    }
}
