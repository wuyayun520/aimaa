...
fileprivate let data_logName:String = "TSysterefugee appeal title procedure"
fileprivate let userUnlessUrl:String = "sapremium"
fileprivate let data_personalKey:String = "pattern topic_ReuseId"

/*: "init(coder:) has not been implemented" :*/
fileprivate let app_procedureName:[UInt8] = [0x66,0x61,0x66,0x7b,0x27,0x6c,0x60,0x6b,0x6a,0x7d,0x35,0x26,0x2f,0x67,0x6e,0x7c,0x2f,0x61,0x60,0x7b,0x2f,0x6d,0x6a,0x6a,0x61,0x2f,0x66,0x62,0x7f,0x63,0x6a,0x62,0x6a,0x61,0x7b,0x6a,0x6b]

private func replacementCon(tick num: UInt8) -> UInt8 {
    return num ^ 15
}

/*: "icon_talk_notic" :*/
fileprivate let const_buttUrl:[Character] = ["i","c","o","n","_","t","a","l","k","_","n","o"]
fileprivate let appPublisherShouldTitle:[Character] = ["t","i","c"]
...
@objc public protocol CompareMark: NSObjectProtocol {
    /**
     *  长按消息回调
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onLongPressMessage(cell: BlackBy)
    func angleBroad(cell: BlackBy)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onRetryMessage(cell: BlackBy)
    func moment(cell: BlackBy)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessage(cell: BlackBy)
    func totalo(cell: BlackBy)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessageAvatar(cell: BlackBy)
    func fragmentTemp(cell: BlackBy)

    /**
     *  点击消息单元中引用消息
     *  @param cell 委托者，消息单元
     */
    //: func onReplyMessage(cell: BlackBy)
    func appearBroadcast(cell: BlackBy)
}

//: @objcMembers public class BlackBy: TCommonTableViewCell {
@objcMembers public class BlackBy: MediaProjection {
    /**
     *  信息数据类。
     *  存储了该massageCell中所需的信息。包括发送者 ID，发送者头像、信息发送状态、信息气泡图标等。
     *  messageData 详细信息请参考：Section\Chat\CellData\AlongCrucial.h
     */
    //: public var messageData: AlongCrucial?
    public var messageData: AlongCrucial?

    /// 复用cell标识符
    //: @objc public static let TSystemMessageCell_ReuseId = "TSystemMessageCell_ReuseId"
    public static let TSystemMessageCell_ReuseId = (String(data_logName.prefix(6)) + "mMes" + userUnlessUrl.replacingOccurrences(of: "premium", with: "ge") + "Cell" + String(data_personalKey.suffix(8)))

    /**
     *  协议委托
     *  负责实现 CompareMark 协议中的功能。
     */
    //: public weak var delegate: TMessageCellDelegate?
    public weak var delegate: CompareMark?
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: app_procedureName.map{replacementCon(tick: $0)}, encoding: .utf8)!)
    }
    ...
    override public func satisfyAssociated(with data: SpineSuccess) {
        //: super.fill(with: data)
        super.satisfyAssociated(with: data)
        //: let newData: AlongCrucial = data as! AlongCrucial
        let newData: AlongCrucial = data as! AlongCrucial
    ...
    override public func updateConstraints() {
        ...
        let csize = self.messageData?.data()
    ...
    lazy var avatarView: UIImageView = {
        ...
        let tap1 = UITapGestureRecognizer(target: self, action: #selector(usedPurple(recognizer:)))
    ...
    lazy var nameLabel: UILabel = {
        ...
        lb.font = UIFont.hangProgram(fontSize: 13)
        //: lb.textColor = .gray
        lb.textColor = .gray
    ...
    public lazy var container: UIView = {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(underCalled(recognizer:)))
        ...
        let longPress = UILongPressGestureRecognizer(target: self, action: #selector(off(recognizer:)))
    ...
    lazy var retryView: UIImageView = {
        ...
        imagV.image = UIImage.managerToSecond(name: (String(const_buttUrl) + String(appPublisherShouldTitle)))
        ...
        let resendTap = UITapGestureRecognizer(target: self, action: #selector(sparkFace(recognizer:)))
...
extension BlackBy {
    ...
    @objc func usedPurple(recognizer: UIGestureRecognizer) {
        //: self.delegate?.onSelectMessageAvatar(cell: self)
        self.delegate?.fragmentTemp(cell: self)
    }

    //: @objc func onLongPress(recognizer: UIGestureRecognizer) {
    @objc func off(recognizer: UIGestureRecognizer) {
