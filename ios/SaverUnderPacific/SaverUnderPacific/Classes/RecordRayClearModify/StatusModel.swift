...
enum SubsequentInsideAspectNeed: String {
    //: case nones  = "0"
    case nones = "0"
    //: case Photo = "1"
    case Photo = "1"
...
struct ActionModel: HandyJSON {
    ...
    var status: TitleCanvas?
}

///
//: enum MomentVideoStatus: String {
enum TitleCanvas: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension StatusModel {
    ...
    mutating func cornerDelicate(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            textHeight = Int(TweenView.multiple(text: content!, size: CGSize(width: show_errorChangeFormat - 54, height: CGFloat(MAXFLOAT)), font: UIFont.yearNearby(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == SubsequentInsideAspectNeed.Photo.rawValue || momentType == SubsequentInsideAspectNeed.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
