...
fileprivate let const_mediaData:[UInt8] = [0x74,0x73,0x74,0x69,0x35,0x7e,0x72,0x79,0x78,0x6f,0x27,0x34,0x3d,0x75,0x7c,0x6e,0x3d,0x73,0x72,0x69,0x3d,0x7f,0x78,0x78,0x73,0x3d,0x74,0x70,0x6d,0x71,0x78,0x70,0x78,0x73,0x69,0x78,0x79]

private func statusBan(action num: UInt8) -> UInt8 {
    return num ^ 29
}

/*: "icon_chat_start" :*/
fileprivate let notiPadSimilarData:String = "orientation orientation till coordinator windowicon_c"
fileprivate let showFailFastMirrorMessage:[Character] = ["t"]

/*: "#EBEBEB" :*/
fileprivate let constVisualKey:String = "private bounce plus fun#EBEBEB"

/*: "icon_talk_reply_voive" :*/
fileprivate let notiHighlightName:String = "icon_tmedia relation consumption lost"
fileprivate let showDropUrl:[Character] = ["p","l","y","_","v","o","i","v","e"]

/*: "#DCDCDC" :*/
fileprivate let show_symbolMessage:String = "subtle become para#DCDCDC"

/*: "text" :*/
fileprivate let appComplaintMessage:String = "ceilingxt"

/*: "gift" :*/
fileprivate let notiExhaustScriptId:String = "gispec"

/*: "Sent " :*/
fileprivate let user_whatData:[Character] = ["S","e","n","t"," "]

/*: "audio" :*/
fileprivate let app_earnEntryMsg:[Character] = ["a","u","d","i","o"]

/*: "img" :*/
fileprivate let noti_clipLoopStr:[Character] = ["i","m","g"]

/*: "video" :*/
fileprivate let noti_heartValue:String = "VIDEO"
...
class MechanismPosition: UIButton {
    ...
    required init?(coder: NSCoder) {
        ...
        fatalError(String(bytes: const_mediaData.map{statusBan(action: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var playView: UIImageView = {
        ...
        img.image = UIImage.managerToSecond(name: (String(notiPadSimilarData.suffix(6)) + "hat_star" + String(showFailFastMirrorMessage)))
        //: img.isHidden = true
        img.isHidden = true
        //: return img
        return img
    ...
    lazy var audioView: UIView = {
        ...
        v.backgroundColor = UIColor(hex: (String(constVisualKey.suffix(7))))
    ...
    lazy var audioImg: UIImageView = {
        ...
        img.image = UIImage.managerToSecond(name: (String(notiHighlightName.prefix(6)) + "alk_re" + String(showDropUrl)))
        //: return img
        return img
        //: }()
    }()
    ...
    var direction: CommandNorm = .MsgDirectionIncoming {
        //: didSet {
        didSet {
            //: if direction == .MsgDirectionIncoming {
            if direction == .MsgDirectionIncoming {
                //: self.lineView.backgroundColor = UIColor.appValueDetailColor()
                self.lineView.backgroundColor = UIColor.theProvision()
                //: self.nameLab.textColor = UIColor.appValueDetailColor()
                self.nameLab.textColor = UIColor.theProvision()
                //: }else{
            } else {
                //: self.lineView.backgroundColor = UIColor(hex: "#DCDCDC")
                self.lineView.backgroundColor = UIColor(hex: (String(show_symbolMessage.suffix(7))))
                //: self.nameLab.textColor = UIColor(hex: "#DCDCDC")
                self.nameLab.textColor = UIColor(hex: (String(show_symbolMessage.suffix(7))))
            }
        }
    }
    ...
    var quoteModel: LocationModel? {
        ...
        didSet {
            ...
            if quoteModel.renderType == (appComplaintMessage.replacingOccurrences(of: "ceiling", with: "te")) {
                //: self.nameLab.text = quoteModel.sendName + ": " +  quoteModel.renderData.content
                self.nameLab.text = quoteModel.sendName + ": " + quoteModel.renderData.content
                //: }else if quoteModel.renderType == "gift" {
            } else if quoteModel.renderType == (notiExhaustScriptId.replacingOccurrences(of: "spec", with: "ft")) {
                //: self.nameLab.text = quoteModel.sendName + ": " + "Sent ".localized + quoteModel.renderData.giftNameLocal() + " x\(quoteModel.renderData.giftNum)"
                self.nameLab.text = quoteModel.sendName + ": " + (String(user_whatData)).localized + quoteModel.renderData.will() + " x\(quoteModel.renderData.giftNum)"
                //: }else if quoteModel.renderType == "audio" {
            } else if quoteModel.renderType == (String(app_earnEntryMsg)) {
            ...
            } else if quoteModel.renderType == (String(noti_clipLoopStr)) {
                //: self.nameLab.text = quoteModel.sendName + ": "
                self.nameLab.text = quoteModel.sendName + ": "
                ...
                if quoteModel.renderData.previewUrl.count > 0 && isExists {
                    ...
                    self.imgView.poolFee(urlStr: quoteModel.renderData.previewUrl)
                }
                //: spaceRight = 34
                spaceRight = 34
                //: }else if quoteModel.renderType == "video" {
            } else if quoteModel.renderType == (noti_heartValue.lowercased()) {
                //: self.nameLab.text = quoteModel.sendName + ": "
                self.nameLab.text = quoteModel.sendName + ": "
                //: self.imgView.isHidden = false
                self.imgView.isHidden = false
                ...
                self.imgView.poolFee(urlStr: quoteModel.renderData.coverUrl)
                //: spaceRight = 34
                spaceRight = 34
            }
