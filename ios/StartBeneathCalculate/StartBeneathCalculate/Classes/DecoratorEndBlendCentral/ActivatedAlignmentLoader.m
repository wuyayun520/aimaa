...
@implementation ActivatedAlignmentLoader
...
- (void)circleOfCurvature {
    ...
    if (self.isInVC && self.isActive) {
        //: if (self.currentChatType == AboveCellPreviewType_group ||
        if (self.currentChatType == AboveCellPreviewType_group ||
            //: self.currentChatType == AboveCellPreviewType_meeting) {
            self.currentChatType == AboveCellPreviewType_meeting) {
...
- (void)setTargetID:(NSString *)targetID{
    //: _targetID = targetID;
    _targetID = targetID;
    //: if (self.currentChatType == AboveCellPreviewType_meeting) {
    if (self.currentChatType == AboveCellPreviewType_meeting) {
...
- (void)modify {
    ...
    int msgCount = 20;
    //: if (self.currentChatType == AboveCellPreviewType_meeting) { 
    if (self.currentChatType == AboveCellPreviewType_meeting) { // 公共聊天室一次性获取50条，不分页
        //: msgCount = 50;
        msgCount = 50;
    }
    ...
    if (self.currentChatType == AboveCellPreviewType_group ||
        //: self.currentChatType == AboveCellPreviewType_meeting) { 
        self.currentChatType == AboveCellPreviewType_meeting) { // 群聊
        ...
        [[V2TIMManager sharedInstance] getGroupHistoryMessageList:self.targetID count:msgCount lastMsg:self.msgForGet succ:^(NSArray<V2TIMMessage *> *msgs) {
            ...
            //: if (self.currentChatType == AboveCellPreviewType_meeting) { 
            if (self.currentChatType == AboveCellPreviewType_meeting) { // 公共聊天室不分页
                //: self.noMoreMsg = YES;
                self.noMoreMsg = YES;
            }
        ...
        //: } fail:^(int code, NSString *msg) {
            ...
            self.isLoadingMsg = NO;
            //: if ([ListenLayoutDelivery shared].isConnection) { 
            if ([ListenLayoutDelivery shared].isConnection) { //推送点进来会报错
    ...
    //: } else if (self.currentChatType == AboveCellPreviewType_private ||
    } else if (self.currentChatType == AboveCellPreviewType_private ||
               //: self.currentChatType == AboveCellPreviewType_system ||
               self.currentChatType == AboveCellPreviewType_system ||
               //: self.currentChatType == AboveCellPreviewType_service) { 
               self.currentChatType == AboveCellPreviewType_service) { // 单聊
...
- (void)insideInMessage:(NSNotification *)notification
{
    ...
    if (uiMsgs.count > 0) {
        ...
        //: if ((self.tableView.contentSize.height - self.tableView.contentOffset.y) < [AnimateUnaryAllocator getScreenHeight] * 1.5) {
        if ((self.tableView.contentSize.height - self.tableView.contentOffset.y) < [AnimateUnaryAllocator eachHeight] * 1.5) {
...
- (NSMutableArray *)seasonablenessMsg:(NSArray *)msgs
{
    ...
    for (NSInteger k = msgs.count - 1; k >= 0; --k) {
        ...
        // 时间信息
        //: ConnectNativeHandler *dateMsg = [self transSystemMsgFromDate:msg.timestamp];
        ConnectNativeHandler *dateMsg = [self flashBlunt:msg.timestamp];
        // 外部自定义的消息
        ...
        if ([self.delegate respondsToSelector:@selector(plain:bold:)]) {
            ...
            //: if(msg.groupID.length > 0 && ![data isKindOfClass:[ConnectNativeHandler class]]){
            if(msg.groupID.length > 0 && ![data isKindOfClass:[ConnectNativeHandler class]]){
                //: data.showName = YES;
                data.showName = YES;
            }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!data.reuseId) {
        ...
        //: if([data isKindOfClass:[ConnectNativeHandler class]]) {
        if([data isKindOfClass:[ConnectNativeHandler class]]) {
...
- (void)element:(AgoneCellData *)msg {
    ...
    //: @autoreleasepool{} __weak __typeof__(self) weak_self = self;
    @autoreleasepool{} __weak __typeof__(self) weak_self = self;
    //: BOOL isGroup = self.currentChatType == AboveCellPreviewType_meeting || self.currentChatType == AboveCellPreviewType_group;
    BOOL isGroup = self.currentChatType == AboveCellPreviewType_meeting || self.currentChatType == AboveCellPreviewType_group;
    ...
    //: if (isGroup && ![msg isKindOfClass:[ConnectNativeHandler class]]) {
    if (isGroup && ![msg isKindOfClass:[ConnectNativeHandler class]]) {
        //: msg.showName = YES;
        msg.showName = YES;
    }
...
- (void)outerNameStatus:(AgoneCellData *)msg modify:(NSString *)msgId storage:(NSNumber *)income commonStatus:(TMsgStatus)status {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        NSMutableDictionary *tempDic = [ActivatedAlignmentLoader origin:msg];
...
//: - (ConnectNativeHandler *)transSystemMsgFromDate:(NSDate *)date
- (ConnectNativeHandler *)flashBlunt:(NSDate *)date
{
    //: if(_msgForDate == nil || fabs([date timeIntervalSinceDate:_msgForDate.timestamp]) > (5 * 60)) {
    if(_msgForDate == nil || fabs([date timeIntervalSinceDate:_msgForDate.timestamp]) > (5 * 60)) {
        //: ConnectNativeHandler *system = [[ConnectNativeHandler alloc] initWithDirection:BeAdditiveArithmeticMsgDirectionOutgoing];
        ConnectNativeHandler *system = [[ConnectNativeHandler alloc] initWithDirection:BeAdditiveArithmeticMsgDirectionOutgoing];
...
- (BOOL)spell:(AgoneCellData *)cellData {
    //: if (self.currentChatType != AboveCellPreviewType_private) {
    if (self.currentChatType != AboveCellPreviewType_private) {
        //: return NO;
        return NO;
    }
