...
extension ExamReactiveCompatible {
    ...
    @objc func remark(_: UITapGestureRecognizer) {
        ...
        //: case "1": // 个人中心
        case "1": // 个人中心
            //: SmartThen.share.func__pushToUserDetailVC(uid: self.danumuMsgModel?.jumpKey)
            SmartThen.share.numbererest(uid: self.danumuMsgModel?.jumpKey)

        //: case "2": // 直播间
        case "2": // 直播间
            //: if let uid = self.danumuMsgModel?.jumpKey {
            if let uid = self.danumuMsgModel?.jumpKey {
                //: SmartThen.share.func_audiencePushToLiveRoomVC(uid: uid, enterType: .unknown)
                SmartThen.share.simpleAlong(uid: uid, enterType: .unknown)
            }
        ...
        case "3": // H5页面
            ...
            if let url = self.danumuMsgModel?.jumpKey {
                ...
                //: SmartThen.share.func__pushToWebVC(urlStr: httpUrl)
                SmartThen.share.lifeStyle(urlStr: httpUrl)
            }
