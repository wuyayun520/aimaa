...
extension ActivityBuildListener {
    ...
    func primaryCompletion(filePath: String, completion: @escaping (_ succeed: Bool, _ errorMsg: String?) -> Void) {
        ...
        stress(type: 5) { succeed, msg in
            ...
            put.setFinish { [weak self] _, error in
                ...
                TagThen.someRootCompletion(fileUrl: self.cosCredentialJson[(noti_contrastCutName.replacingOccurrences(of: "heavy", with: "f") + String(noti_metalNetworkId.suffix(6)))].stringValue) { succeed, _, errorModel in
    ...
    func stress(type: Int, completion: @escaping (_ isSucceed: Bool, _ errorMsg: String?) -> Void) {
        ...
        TagThen.robot(type: type) { succeed, result, errorModel in
            //: guard succeed else {
            guard succeed else {
