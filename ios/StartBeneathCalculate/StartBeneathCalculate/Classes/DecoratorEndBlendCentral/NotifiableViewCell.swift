...
fileprivate let main_holderPath:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "F5F5F5" :*/
fileprivate let dataExpansionName:[Character] = ["F","5","F","5","F","5"]
...
class NotifiableViewCell: UITableViewCell {
    ...
    var videoView = MigrationVideoCell()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: main_holderPath.reversed(), encoding: .utf8)!)
    }
    ...
    func approximately(isListTableCell: Bool) {
    ...
    lazy var userInfoView: InertGasReactiveCompatible = {
        //: let  view = TalkingMomentUserInfoView.init()
        let view = InertGasReactiveCompatible()
    ...
    lazy var textContentView: PlacementView = {
    ...
    lazy var photoView: ControlView = {
        ...
        let view = ControlView(frame: CGRect(x: 0, y: 0, width: 0, height: 0), collectionViewLayout: flayout)
        //: backView.addSubview(view)
        backView.addSubview(view)
        //: return view
        return view
    ...
    lazy var lineView: UIView = {
        ...
        view.backgroundColor = UIColor(hex: (String(dataExpansionName)))
        //: self.contentView.addSubview(view)
        self.contentView.addSubview(view)
        //: return view
        return view
...
extension NotifiableViewCell {
    //: func configCell(model: TalkingMomentModel) {
    func fallWith(model: AgencyMomentModel) {
        //: cunrrenModel = model
        cunrrenModel = model
        //: userInfoView.isHidden = false
        userInfoView.isHidden = false
        //: userInfoView.configInfo(model: model)
        userInfoView.info(model: model)
        //: photoView.isMyhost = self.isMyhost
        photoView.isMyhost = self.isMyhost
        //: textContentView.configText(text: model.content ?? "", isMomentDetail: model.isMomentDetail ?? false)
        textContentView.isomerization(text: model.content ?? "", isMomentDetail: model.isMomentDetail ?? false)

        //: if model.momentType == MomentType.Photo.rawValue || model.momentType == MomentType.Live.rawValue {
        if model.momentType == MigrationBasicType.Photo.rawValue || model.momentType == MigrationBasicType.Live.rawValue {
...
//            }
            ...
            photoView.orate(model: cunrrenModel)
...
//            }
            ...
            videoView.simulation(model: model.videoInfo!, isTop: model.pinStatus)
        }

        //: bottomView.configModel(model: cunrrenModel)
        bottomView.mild(model: cunrrenModel)
...
//        }
    ...
    @objc func conformTo() {
        ...
        let vc = AssociatedDirectProject(videoPath: cunrrenModel.videoInfo!.videoUrl)
        //: if cunrrenModel.sex != FacultyReactiveCompatible.share.loginUserMode.sex {
        if cunrrenModel.sex != FacultyReactiveCompatible.share.loginUserMode.sex {
    ...
    //: func playVideo() {
    func operatingInstructions() {
        //: if cunrrenModel.momentType == MomentType.Video.rawValue {
        if cunrrenModel.momentType == MigrationBasicType.Video.rawValue {
            //: videoView.setupPlayer()
            videoView.fadeMaximum()
        }
    }
...
extension NotifiableViewCell {
    ...
    private func precedency() {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(conformTo))
