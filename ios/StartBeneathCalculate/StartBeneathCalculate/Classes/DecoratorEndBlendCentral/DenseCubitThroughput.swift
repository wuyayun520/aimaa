...
fileprivate let k_toleranceTitle:[UInt8] = [0xbf,0xb8,0xbf,0xa2,0xfe,0xb5,0xb9,0xb2,0xb3,0xa4,0xec,0xff,0xf6,0xbe,0xb7,0xa5,0xf6,0xb8,0xb9,0xa2,0xf6,0xb4,0xb3,0xb3,0xb8,0xf6,0xbf,0xbb,0xa6,0xba,0xb3,0xbb,0xb3,0xb8,0xa2,0xb3,0xb2]

private func tellThrough(remark num: UInt8) -> UInt8 {
    return num ^ 214
}
...
typealias SlidingHandler = (_ direction: ItinerarySlideDirection) -> Void
...
class DenseCubitThroughput: UIView {
    //: var cropStart: CGFloat {
    var cropStart: CGFloat {
        //: let offset = overlay.startOffset - (LeftMargin - self.contentView.contentOffset.x)
        let offset = overlay.startOffset - (data_conversationName - self.contentView.contentOffset.x)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_toleranceTitle.map{tellThrough(remark: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIScrollView = {
        ...
        while time <= self.duration {
            ...
            let image = self.pack(point: time)
            //: imv.image = image
            imv.image = image
    ...
    lazy var overlay: SubatomicParticleView = {
        ...
        let overlay = SubatomicParticleView(frame: self.bounds, maxCropLen: maxWidth, minCropLen: minWidth)
...
extension DenseCubitThroughput {
    /// 重置位置
    //: func resetPostion() {
    func statin() {
        ...
        self.overlay.displayIn(startX: data_conversationName - x)
    }
    ...
    private func pack(point: CGFloat) -> UIImage {
...
extension DenseCubitThroughput: UIScrollViewDelegate {
    //: func scrollViewDidScroll(_ scrollView: UIScrollView) {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        //: var offset = scrollView.contentOffset
        var offset = scrollView.contentOffset
        //: let minx = LeftMargin - overlay.startOffset
        let minx = data_conversationName - overlay.startOffset
    ...
    func scrollViewWillBeginDragging(_: UIScrollView) {
        ...
        self.overlay.offPlay(isShow: false)
    }
    ...
    func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        ...
        self.overlay.offPlay(isShow: true)
    }
...
extension DenseCubitThroughput {
    ...
    private func likeStage() {
        ...
        self.contentView.snp.makeConstraints { make in
            ...
            make.leading.equalToSuperview().offset(data_conversationName)
