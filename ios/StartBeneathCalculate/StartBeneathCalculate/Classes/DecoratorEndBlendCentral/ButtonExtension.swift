...
/*: "recharge" :*/
fileprivate let appCoreRetName:String = "rradicalhar"
fileprivate let notiGardenRecordMagnitudePath:String = "moment"

/*: "recharge_ :*/
fileprivate let main_agentUrl:[Character] = ["r","e","c","h","a","r","g"]
fileprivate let showPutKey:String = "e_now tap sew name organization"

/*: ".png" :*/
fileprivate let noti_yesUrl:[Character] = [".","p","n","g"]

/*: .png" :*/
fileprivate let mainThenKey:String = "stat under resistance commercial operate.png"

/*: ".jpg" :*/
fileprivate let dataOrientationMessage:[Character] = [".","j","p","g"]

/*: .jpg" :*/
fileprivate let k_premiumOddUrl:String = ".jpgfragment rocket haul open"
...
enum AdvancedImageAlignment: NSInteger {
    //: case left = 0
    case left = 0
    //: case top
    case top
...
class SeparateControlReactiveCompatible: UIButton {
    ...
    var imageAlignment: AdvancedImageAlignment = .left
...
extension UIButton {
    ...
    func example(_ interval: CGFloat) {
        //: verticalAlignImageAndTitle(interval: interval, topOffset: 0)
        underPlace(interval: interval, topOffset: 0)
    }
    ...
    func underPlace(interval: CGFloat, topOffset: CGFloat) {
    ...
    func instSwitchset(interval: CGFloat, imageSize: CGSize, topOffset: CGFloat) {
...
extension UIButton {
    ...
    func askPath(color: UIColor, forState: UIControl.State) {
    ...
    func slash(urlStr: String) {
        //: guard !urlStr.isEmptyString else { return }
        guard !urlStr.isEmptyString else { return }
        //: var newUrl = ""
        var newUrl = ""
        //: if urlStr.contains("recharge") {
        if urlStr.contains((appCoreRetName.replacingOccurrences(of: "radical", with: "ec") + notiGardenRecordMagnitudePath.replacingOccurrences(of: "moment", with: "ge"))) {
            //: newUrl = urlStr.replacingOccurrences(of: "recharge", with: "recharge_\(PackageID)")
            newUrl = urlStr.replacingOccurrences(of: (appCoreRetName.replacingOccurrences(of: "radical", with: "ec") + notiGardenRecordMagnitudePath.replacingOccurrences(of: "moment", with: "ge")), with: (String(main_agentUrl) + String(showPutKey.prefix(2))) + "\(showViewValue)")
            //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
            self.climb(urlStr: newUrl, placeImg: nil)
            //: return
            return
                //: } else if urlStr.contains(".png") {
        } else if urlStr.contains((String(noti_yesUrl))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".png", with: "_\(PackageID).png")
            newUrl = urlStr.replacingOccurrences(of: (String(noti_yesUrl)), with: "_\(showViewValue)" + (String(mainThenKey.suffix(4))))

            //: } else if urlStr.contains(".jpg") {
        } else if urlStr.contains((String(dataOrientationMessage))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".jpg", with: "_\(PackageID).jpg")
            newUrl = urlStr.replacingOccurrences(of: (String(dataOrientationMessage)), with: "_\(showViewValue)" + (String(k_premiumOddUrl.prefix(4))))
        }
        //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
        self.climb(urlStr: newUrl, placeImg: nil)
    }
