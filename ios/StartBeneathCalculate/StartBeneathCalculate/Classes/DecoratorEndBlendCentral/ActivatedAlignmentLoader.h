 ...
 *
 *  RunningReadHalf 协议和 ActivatedAlignmentLoader 类。
 *  ActivatedAlignmentLoader 是聊天界面的核心类，负责聊天界面大部分核心业务逻辑的处理与运算。
 *  该类的详细属性与功能，请参展本文件中 ActivatedAlignmentLoader 的具体声明。
 *
 ******************************************************************************/
...
@class ActivatedAlignmentLoader;
//: @class RatioViewCell;
@class RatioViewCell;
//: @class AgoneCellData;
@class AgoneCellData;
...
//: typedef NS_ENUM(NSInteger, AboveCellPreviewType) {
typedef NS_ENUM(NSInteger, AboveCellPreviewType) {
    //: AboveCellPreviewType_unDefined = 0,
    AboveCellPreviewType_unDefined = 0,
    //: AboveCellPreviewType_private = 1,
    AboveCellPreviewType_private = 1,
    //: AboveCellPreviewType_group = 2,
    AboveCellPreviewType_group = 2,
    //: AboveCellPreviewType_system = 3,
    AboveCellPreviewType_system = 3,
    //: AboveCellPreviewType_service = 4, 
    AboveCellPreviewType_service = 4, // 官方客服
    //: AboveCellPreviewType_meeting = 5, 
    AboveCellPreviewType_meeting = 5, // 公共聊天室
    //: AboveCellPreviewType_moreMsg = 6, 
    AboveCellPreviewType_moreMsg = 6, // 更多消息（超过5天未回复）
//: };
};
...
@protocol RunningReadHalf <NSObject>
...
- (void)rendering:(ActivatedAlignmentLoader *)controller;
...
- (void)disabledRefer:(ActivatedAlignmentLoader *)controller;
...
- (BOOL)block:(ActivatedAlignmentLoader *)controller refShared:(UIView *)view;
 ...
 */
//: - (AgoneCellData *)messageController:(TUIMessageController *)controller onNewMessage:(V2TIMMessage *)data;
- (AgoneCellData *)plain:(ActivatedAlignmentLoader *)controller bold:(V2TIMMessage *)data;
 ...
 */
//: - (RatioViewCell *)messageController:(TUIMessageController *)controller onShowMessageData:(AgoneCellData *)data;
- (RatioViewCell *)controllerLayer:(ActivatedAlignmentLoader *)controller referent:(AgoneCellData *)data;
 ...
 *  @param cell 所点击的消息单元
 */
//: - (void)messageController:(TUIMessageController *)controller onSelectMessageAvatar:(RatioViewCell *)cell;
- (void)of:(ActivatedAlignmentLoader *)controller ask:(RatioViewCell *)cell;
 ...
 *  @param cell 所点击的消息单元
 */
//: - (void)messageController:(TUIMessageController *)controller onSelectMessageContent:(RatioViewCell *)cell;
- (void)fill:(ActivatedAlignmentLoader *)controller handle:(RatioViewCell *)cell;
 ...
 */
//: - (void)messageController:(TUIMessageController *)controller onSelectMessageMenu:(NSInteger)menuType withData:(AgoneCellData *)data;
- (void)screen:(ActivatedAlignmentLoader *)controller namePacket:(NSInteger)menuType intervalermingle:(AgoneCellData *)data;
/**
...
/*重发*/
//: - (void)messageController:(TUIMessageController *)controller onRetryMessageContent:(RatioViewCell *)cell;
- (void)fixedSecure:(ActivatedAlignmentLoader *)controller noSection:(RatioViewCell *)cell;

/*引用*/
//: - (void)messageController:(TUIMessageController *)controller onReplyMessage:(AgoneCellData *)cellData;
- (void)outline:(ActivatedAlignmentLoader *)controller border:(AgoneCellData *)cellData;

/*点击引用*/

//: - (void)messageController:(TUIMessageController *)controller onClickReplyMessage:(AgoneCellData *)cellData;
- (void)effective:(ActivatedAlignmentLoader *)controller before:(AgoneCellData *)cellData;
...
/////////////////////////////////////////////////////////////////////////////////
//
//                         ActivatedAlignmentLoader
//
/////////////////////////////////////////////////////////////////////////////////

/**
 * 腾讯云 TUIKit
 *  【模块名称】ActivatedAlignmentLoader
...
@interface ActivatedAlignmentLoader : UITableViewController

/**
 *  执行 RunningReadHalf 协议的委托
 */
//: @property (nonatomic, weak) id<TMessageControllerDelegate> delegate;
@property (nonatomic, weak) id<RunningReadHalf> delegate;
 ...
 * @param uiMsgs 待删除的数据
 */
//- (void)deleteMessages:(NSArray<AgoneCellData *> *)uiMsgs;
...
/**
 * 批量将V2TIMessage转换成AgoneCellData
 */
...
///当前对话类型
//: @property (nonatomic, assign) AboveCellPreviewType currentChatType;
@property (nonatomic, assign) AboveCellPreviewType currentChatType;
