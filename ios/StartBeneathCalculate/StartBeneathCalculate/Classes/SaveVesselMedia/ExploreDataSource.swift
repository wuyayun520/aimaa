...
/*: "cellID" :*/
fileprivate let const_routinePath:String = "cellIDabsolutely stretch commit"

/*: "init(coder:) has not been implemented" :*/
fileprivate let noti_effTitle:[UInt8] = [0x42,0x45,0x42,0x5f,0x3,0x48,0x44,0x4f,0x4e,0x59,0x11,0x2,0xb,0x43,0x4a,0x58,0xb,0x45,0x44,0x5f,0xb,0x49,0x4e,0x4e,0x45,0xb,0x42,0x46,0x5b,0x47,0x4e,0x46,0x4e,0x45,0x5f,0x4e,0x4f]

private func perchMember(old num: UInt8) -> UInt8 {
    return num ^ 43
}

/*: "Video Call" :*/
fileprivate let mainBlendName:[Character] = ["V","i","d","e","o"," ","C","a","l","l"]

/*: "Video Call (%@ Points/Min)" :*/
fileprivate let show_currencyMsg:String = "Video route available opportunity manage"
fileprivate let show_trafficName:String = "%@ Poinprevious blend counter quick"
fileprivate let appChannelClearlyValue:String = "ts/Min)pan down especial"

/*: "Voice Call" :*/
fileprivate let mainCollectData:[Character] = ["V","o","i","c","e"," ","C","a","l"]
fileprivate let main_mPushMessage:[Character] = ["l"]

/*: "Voice Call (%@ Points/Min)" :*/
fileprivate let app_okFormat:String = "mobile customer adoptVoice"
fileprivate let dataCombineLiftText:String = "recognize glass ex Call "
fileprivate let appInsertId:String = "ease properly(%@ Po"

/*: "#F8F8F8" :*/
fileprivate let show_bulletFormat:String = "evaluate while photo cloud selected#F8F8F8"

/*: "Cancel" :*/
fileprivate let notiLawnAgainMsg:String = "belly from file lastCance"
fileprivate let notiManagerServiceUrl:[Character] = ["l"]

/*: "#F5F5F5" :*/
fileprivate let kPopulationId:String = "board version file phase#F5F5F5"

/*: "icon_lounge" :*/
fileprivate let data_displayTitle:[Character] = ["i","c","o","n","_","l","o","u","n","g","e"]
...
class ExploreDataSource: UIView {
    //: var munuBlock: MunuBlock?
    var munuBlock: MunuBlock?

    //: private var type: MunuPopType = .Unknown
    private var type: AfterPopType = .Unknown // 当前展示类型

    /// cell内容
    //: var cellTitleList = [Any]()
    var cellTitleList = [Any]()
    /// cell字体颜色
    //: var cellTitleColor = UIColor.mottleResult()
    var cellTitleColor = UIColor.mottleResult()
    /// cell字体大小
    //: var cellTitleFont = UIFont.pingfangFont(type: .Regular, fontSize: 16)
    var cellTitleFont = UIFont.recipeDecision(type: .Regular, fontSize: 16)
    /// 是否有"Cancel"菜单
    //: var hasCancel = true
    var hasCancel = true
    /// cancel字体颜色
    //: var cancelColor = UIColor.appValueDetailColor()
    var cancelColor = UIColor.keepRid()
    /// cancel字体大小
    //: var cancelTitleFont = UIFont.pingfangFont(type: .Regular, fontSize: 16)
    var cancelTitleFont = UIFont.recipeDecision(type: .Regular, fontSize: 16)
    /// 标题
    //: var title: String?
    var title: String?
    /// 标题颜色
    //: var titleColor = UIColor.appThemeColor()
    var titleColor = UIColor.crosswise()
    /// 标题字体大小
    //: var titleFont = UIFont.pingfangMediumFont(fontSize: 16)
    var titleFont = UIFont.originally(fontSize: 16)

    //: var tableView = UITableView()
    var tableView = UITableView()
    //: let cellID = "cellID"
    let cellID = (String(const_routinePath.prefix(6)))
    ...
    fileprivate var footerViewHight: CGFloat {
        ...
        return hasCancel ? (footerViewLineHight + 50 + data_cornerMessage) : data_cornerMessage
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_effTitle.map{perchMember(old: $0)}, encoding: .utf8)!)
    }

    //: func initwithList(cellTitleList: [String]!) {
    func footList(cellTitleList: [String]!) {
        // 初始化
    ...
    func detectIncrementalCost(type: AfterPopType, videoPrice: String?, voicePrice: String?) {
        //: self.type = type
        self.type = type
        //: if self.type == .Call {
        if self.type == .Call { // 音视频通话
            ...
            if FacultyReactiveCompatible.share.appStatus == PenConstantTarget.normal.rawValue {
                ...
                if FacultyReactiveCompatible.share.loginUserMode.sex == ATextLiteral.male.rawValue {
                    ...
                    let videoAttrStr = String.policyConnect(VideoPrice: Int(videoPrice ?? "0") ?? 0)
                    //: let voiceAttrStr = String.setVoicePriceStr(VoicePrice: Int(voicePrice ?? "0") ?? 0)
                    let voiceAttrStr = String.marketingCost(VoicePrice: Int(voicePrice ?? "0") ?? 0)
                    ...
                    let attributes = [.foregroundColor: UIColor.crosswise(), .font: UIFont.recipeDecision(type: .Regular, fontSize: 14)] as [NSAttributedString.Key: Any]
                    //: let VideoCall = "Video Call".localized
                    let VideoCall = (String(mainBlendName)).localized
                    //: let videoStr = "Video Call (%@ Points/Min)".localizedArguments(videoPrice ?? "0")
                    let videoStr = (String(show_currencyMsg.prefix(6)) + "Call (" + String(show_trafficName.prefix(7)) + String(appChannelClearlyValue.prefix(7))).extraPropertyLetter(videoPrice ?? "0")
                    ...
                    let VoiceCall = (String(mainCollectData) + String(main_mPushMessage)).localized
                    //: let voiceStr = "Voice Call (%@ Points/Min)".localizedArguments(voicePrice ?? "0")
                    let voiceStr = (String(app_okFormat.suffix(5)) + String(dataCombineLiftText.suffix(6)) + String(appInsertId.suffix(6)) + "ints/Min)").extraPropertyLetter(voicePrice ?? "0")
                ...
                self.cellTitleList = [(String(mainBlendName)).localized, (String(mainCollectData) + String(main_mPushMessage)).localized]
            }
        }
    ...
    public func popSend() {
        ...
        tableView = UITableView(frame: CGRect(x: 0, y: kTopUrl - tableViewHight, width: main_assetUrl, height: tableViewHight), style: .plain)
        ...
        tableView.register(BroadViewCell.self, forCellReuseIdentifier: cellID)
        ...
        if title == nil {
            ...
            tableView.cornerPopular(width: main_assetUrl,
...
extension ExploreDataSource: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        var cell = tableView.dequeueReusableCell(withIdentifier: cellID) as? BroadViewCell
        //: if cell == nil {
        if cell == nil {
            //: cell = TitleCell(style: .default, reuseIdentifier: cellID)
            cell = BroadViewCell(style: .default, reuseIdentifier: cellID)
        }
        ...
        cell!.titleLab.font = cellTitleFont
        //: if self.type == .Call && FacultyReactiveCompatible.share.appStatus == AppSkinStatus.normal.rawValue {
        if self.type == .Call && FacultyReactiveCompatible.share.appStatus == PenConstantTarget.normal.rawValue {
    ...
    func tableView(_: UITableView, viewForFooterInSection _: Int) -> UIView? {
        ...
        let footerView = UIView(frame: CGRect(x: 0, y: 0, width: main_assetUrl, height: footerViewHight))
        //: let lineView = UIView(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: footerViewLineHight))
        let lineView = UIView(frame: CGRect(x: 0, y: 0, width: main_assetUrl, height: footerViewLineHight))
        //: lineView.backgroundColor = UIColor(hex: "#F8F8F8")
        lineView.backgroundColor = UIColor(hex: (String(show_bulletFormat.suffix(7))))
        ...
        btn.frame = CGRect(x: 0, y: footerViewLineHight, width: main_assetUrl, height: 50)
        //: footerView.addSubview(btn)
        footerView.addSubview(btn)
        //: btn.setTitle("Cancel".localized, for: .normal)
        btn.setTitle((String(notiLawnAgainMsg.suffix(5)) + String(notiManagerServiceUrl)).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(conflagration), for: .touchUpInside)
        //: return footerView
        return footerView
    }
    ...
    func tableView(_: UITableView, viewForHeaderInSection _: Int) -> UIView? {
        ...
        let headerView = UIView(frame: CGRect(x: 0, y: 0, width: main_assetUrl, height: headerViewHight))

        //: let titleLab = UILabel(frame: CGRect(x: 15, y: 0, width: ScreenWidth-30, height: headerViewHight))
        let titleLab = UILabel(frame: CGRect(x: 15, y: 0, width: main_assetUrl - 30, height: headerViewHight))
        ...
        let lineView = UIView(frame: CGRect(x: 0, y: headerViewHight - 1, width: main_assetUrl, height: 1))
        //: lineView.backgroundColor = UIColor.init(hex: "#F5F5F5")
        lineView.backgroundColor = UIColor(hex: (String(kPopulationId.suffix(7))))
...
class BroadViewCell: UITableViewCell {
    ...
    lazy var vipIcon: UIImageView = {
        ...
        img.image = UIImage.adName(name: (String(data_displayTitle)))
        //: img.isHidden = true
        img.isHidden = true
        //: return img
        return img
    ...
    lazy var lineLab: UILabel = {
        ...
        lab.backgroundColor = UIColor(hex: (String(kPopulationId.suffix(7))))
        //: return lab
        return lab
        //: }()
    }()
