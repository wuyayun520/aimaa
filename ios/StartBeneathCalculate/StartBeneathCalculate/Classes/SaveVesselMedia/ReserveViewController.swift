...
fileprivate let showGeneralData:[UInt8] = [0x8c,0x91,0x8c,0x97,0x4b,0x86,0x92,0x87,0x88,0x95,0x5d,0x4c,0x43,0x8b,0x84,0x96,0x43,0x91,0x92,0x97,0x43,0x85,0x88,0x88,0x91,0x43,0x8c,0x90,0x93,0x8f,0x88,0x90,0x88,0x91,0x97,0x88,0x87]

fileprivate func peerOdd(document num: UInt8) -> UInt8 {
...
class ReserveViewController: SafetyFactorViewController {
    //: var tranConfig: BasePresentTransition!
    var tranConfig: FoundAnimatedTransitioning!
    //: init() {
    init() {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: self.tranConfig = BasePresentTransition.init(target: self)
        self.tranConfig = FoundAnimatedTransitioning(target: self)
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: showGeneralData.map{peerOdd(document: $0)}, encoding: .utf8)!)
    }
}

//: class BasePresentTransition: NSObject {
class FoundAnimatedTransitioning: NSObject {
    ...
    var frameOfPresentedView = CGRect(x: 0, y: 0, width: main_assetUrl, height: kTopUrl * 0.6)
    ...
    func goldBalance(corners: UIRectCorner, radius: CGFloat) {
...
extension FoundAnimatedTransitioning: UIViewControllerTransitioningDelegate {
    ...
    func presentationController(forPresented presented: UIViewController, presenting: UIViewController?, source _: UIViewController) -> UIPresentationController? {
        ...
        let res = PresentationReactiveCompatible(transition: self, presentedViewController: presented, presentingViewController: presenting)
        //: return res
        return res
    }
}
...
extension FoundAnimatedTransitioning: UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if toVC == targetVC {
            ...
            methodSmartProgress(transitionContext: transitionContext)
            //: } else {
        } else {
            //: dismissTransition(transitionContext: transitionContext)
            revenueContext(transitionContext: transitionContext)
        }
    }

    //: private func presentTransition(transitionContext: UIViewControllerContextTransitioning) {
    private func methodSmartProgress(transitionContext: UIViewControllerContextTransitioning) {
        //: let toVC = transitionContext.viewController(forKey: .to)
        let toVC = transitionContext.viewController(forKey: .to)
    ...
    private func revenueContext(transitionContext: UIViewControllerContextTransitioning) {
...
class PresentationReactiveCompatible: UIPresentationController, UIGestureRecognizerDelegate {
    //: var transition: BasePresentTransition!
    var transition: FoundAnimatedTransitioning!

    //: init(transition: BasePresentTransition, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    init(transition: FoundAnimatedTransitioning, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    ...
    lazy var visualView: UIView = {
        ...
        view.frame = containerView?.bounds ?? CGRect(x: 0, y: 0, width: main_assetUrl, height: kTopUrl)
