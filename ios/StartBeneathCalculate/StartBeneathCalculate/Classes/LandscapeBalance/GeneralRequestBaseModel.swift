...
/*: "http://stat. :*/
fileprivate let mainInformationId:[Character] = ["h","t","t","p",":"]
fileprivate let appPrepareData:[Character] = ["/","/","s","t","a","t","."]

/*: .com" :*/
fileprivate let data_costBeautyKey:[Character] = [".","c","o","m"]

/*: "http://" :*/
fileprivate let mainPicValue:String = "wide currencyhttp:/"
fileprivate let show_hostMessage:String = "/"

/*: "https://" :*/
fileprivate let appEveryoneUrl:[Character] = ["h","t","t","p","s",":","/","/"]

/*: "http://m. :*/
fileprivate let noti_onlyName:String = "http://m.character size phone big"

/*: "http:// :*/
fileprivate let k_mutualPath:[Character] = ["h","t","t","p",":"]
fileprivate let dataSiblingDomainPressUrl:String = "stepstep"

/*: "3,90" :*/
fileprivate let constChallengeName:String = "fully,90"

/*: "1990-01-01" :*/
fileprivate let app_perId:[Character] = ["1","9","9","0","-","0"]
fileprivate let userCapacityFormat:[Character] = ["1","-","0","1"]

/*: "MM-dd-yyyy" :*/
fileprivate let constMixFormat:[Character] = ["M","M","-","d","d"]
fileprivate let noti_calledPath:[Character] = ["-","y","y","y","y"]
...
class AgileCrucialTransformer: NSObject {
    ...
    var requestType: RangeRequestType?
    ...
    override init() {
        ...
        self.requestServer = RocketRecognizeAddrTool.share.serverUrlStr
        //: self.showErrorStatusBar = true
        self.showErrorStatusBar = true
    }

    //: func paramsContainBinaryObject() -> Bool {
    func ownAcrossJournal() -> Bool {
...
struct AvailableBaseResponse: HandyJSON {
    //: var errno: Int!
...
public struct MovementErrorResponse {
    //: let errorCode: Int
    let errorCode: Int
    //: let errorMsg: String
    let errorMsg: String
...
struct ActivityHandyJSON: HandyJSON {
    ...
    var CSConfig = IonPossibilityMeasurable() // 私聊官方客服提示消息模型
    ...
    var reportDomain: String {
        ...
        if statDomain.isEmptyString {
            ...
            return (String(mainInformationId) + String(appPrepareData)) + "\(app_frameName)" + (String(data_costBeautyKey))
            //: } else {
        } else {
            //: if !statDomain.hasPrefix("http://") && !statDomain.hasPrefix("https://") {
            if !statDomain.hasPrefix((String(mainPicValue.suffix(6)) + show_hostMessage.capitalized)) && !statDomain.hasPrefix((String(appEveryoneUrl))) {
                //: return "http://".appending(statDomain)
                return (String(mainPicValue.suffix(6)) + show_hostMessage.capitalized).appending(statDomain)
                //: }else {
            } else {
                //: return statDomain
                return statDomain
    ...
    var urlH5Domain: String {
        ...
        if urlDomain.isEmpty {
            ...
            return (String(noti_onlyName.prefix(9))) + "\(app_frameName)" + (String(data_costBeautyKey))
            //: } else {
        } else {
            //: return "http://\(urlDomain)"
            return (String(k_mutualPath) + dataSiblingDomainPressUrl.replacingOccurrences(of: "step", with: "/")) + "\(urlDomain)"
        }
    }
...
public class RetainModelType: NSObject, HandyJSON {
    ...
    var vipSetting = [ActivityMeasurable]() // vip皮肤
    //: var gameShowBit = 0
    ...
    var tabPageBanner = [LackTransformable]() // V1.6.4 一级页面广告
    //: var needUploadLog = false
    var needUploadLog = false // V1.7.2 是否上传用户日志
    //: var intimateLimit = IntimateLimitModel()
    var intimateLimit = CollectionLimitModel() // 1.7.6 各种操作的亲密度限制
...
@objcMembers public class TargetReactiveCompatible: NSObject, HandyJSON {
    //: required public override init() {
    override public required init() {}
    ...
    var interest: [BeUserTagModel]?
    //: var aboutMe: Array<UserSeleteTagModel>?
    var aboutMe: [BeUserTagModel]?
    ...
    var callPopupSetting = (constChallengeName.replacingOccurrences(of: "fully", with: "3")) // 表示来电弹窗配置，如”3,90”表示3s后弹第一次，后续90s，返回”0,0”表示不弹窗
    //: var callTabSwitch = 0
    ...
    public lazy var age: String = {
        ...
        let birthdayDate = NSDate.remoteFormat(self.birthday ?? (String(app_perId) + String(userCapacityFormat)), dateFormat: (String(constMixFormat) + String(noti_calledPath)))
        //: guard let currentYear = NSDate.getCurrentDateComponents().year else { return "0" }
        guard let currentYear = NSDate.dragWritten().year else { return "0" }
...
struct ViewMeasurable: HandyJSON {
    //: var id: String?
    var id: String?
    //: var url: String?
    var url: String?
...
public struct BeUserTagModel: HandyJSON, Equatable {
    //: var tag_id: String?
    var tag_id: String?
    //: var tag_name = ""
    var tag_name = ""
    ...
    func analogDigitalConverter(compareTo: BeUserTagModel) -> Bool {
        //: return
        return
...
struct AfterMeasurable: HandyJSON {
    //: var aboutMe: Array<UserTagTypeModel>!
    var aboutMe: [CalendarTypeModel]!
    //: var interest: Array<UserTagTypeModel>!
    var interest: [CalendarTypeModel]!
}

//: struct UserTagTypeModel: HandyJSON {
struct CalendarTypeModel: HandyJSON {
    //: var name = ""
    var name = ""
    //: var name_ar = ""
    var name_ar = ""
    ...
    var list: [BeUserTagModel]!
}

/// 官方客服提示消息模型
//: struct CSConfigModel: HandyJSON {
struct IonPossibilityMeasurable: HandyJSON {
...
struct ActivityMeasurable: HandyJSON {
...
struct CollectionLimitModel: HandyJSON {
    //: var sendImg = 10
    var sendImg = 10 // 发送图片
    //: var sendVideo = 10
