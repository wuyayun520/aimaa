...
fileprivate let show_scheduleHeadMagnitudeerValue:[UInt8] = [0x1,0x6,0x1,0x1c,0x40,0xb,0x7,0xc,0xd,0x1a,0x52,0x41,0x48,0x0,0x9,0x1b,0x48,0x6,0x7,0x1c,0x48,0xa,0xd,0xd,0x6,0x48,0x1,0x5,0x18,0x4,0xd,0x5,0xd,0x6,0x1c,0xd,0xc]

private func stateTolerance(bring num: UInt8) -> UInt8 {
    return num ^ 104
}

/*: "all" :*/
fileprivate let noti_bookBrushStr:String = "surel"
...
class FeatureReactiveCompatible: AsyncMasterObserver {
    //: var giftData: TalkingGroupChatGiftMsgCellData?
    var giftData: CompartmentReactiveCompatible?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_scheduleHeadMagnitudeerValue.map{stateTolerance(bring: $0)}, encoding: .utf8)!)
    }
...
extension FeatureReactiveCompatible {
    ...
    override func lineStreetwise(with data: CommonalityCellData) {
        ...
        self.giftData = data as? CompartmentReactiveCompatible
        ...
        self.giftImgView.notFit(urlStr: imgPreview)
        //: self.giftContentLab.attributedText = giftData.attributedString
        self.giftContentLab.attributedText = giftData.attributedString
        //: if giftData.msgModel.toUser.uid == "all" { // 送礼所有人
        if giftData.msgModel.toUser.uid == (noti_bookBrushStr.replacingOccurrences(of: "sure", with: "al")) { // 送礼所有人
            //: self.toUserImgView.image = UIImage.BundleImageNamed(name: giftData.msgModel.toUser.headPic)
            self.toUserImgView.image = UIImage.adName(name: giftData.msgModel.toUser.headPic)
            //: } else {
        } else {
            //: self.toUserImgView.setUrlImage(urlStr: giftData.msgModel.toUser.headPic)
            self.toUserImgView.notFit(urlStr: giftData.msgModel.toUser.headPic)
        }
    }
