...
class FeatureThen: SafetyFactorViewController {
    ...
    private lazy var reqManager: TagThen = //: return TagThen()
        .init()
    //: }()
...
extension FeatureThen {
    ...
    func shade() {
        ...
        ActivityBuildListener.shared.domain(type: 1, coverPath: self.coverPath, videoPath: self.videoPath) { coverUrl, videoUrl, succeed in
            ...
            TagThen.maxAnswer(params: dict) { [weak self] succeed, _, _ in
                //: ProgressHUD.dismiss()
                StrangerProgressHUD.barDismiss()
                //: guard let self = self else { return }
                guard let self = self else { return }
...
extension FeatureThen: UIImagePickerControllerDelegate & UINavigationControllerDelegate {
...
extension FeatureThen: BeAnnouncementObjectProtocol {
    /// 选择图片
    ...
    func fast() {
        ...
        let vc = AssignThen(videoPath: self.videoPath)
        //: vc.modalPresentationStyle = .fullScreen
        vc.modalPresentationStyle = .fullScreen
        //: self.present(vc, animated: true)
        self.present(vc, animated: true)
