...
class GenerationThen: EarthThen {
...
extension GenerationThen {
    ...
    override func lineStreetwise(with data: CommonalityCellData) {
        ...
        guard let tipCellData = data as? AgoThen else { return }
        ...
        if dataLayout != nil {
            ...
            //: let layout = YYTextLayout.init(containerSize: CGSize.init(width: MacroBorderDefineThen.getScreenWidth()-50, height: CGFloat(MAXFLOAT)), text: tipCellData.attributedString)
            let layout = YYTextLayout(containerSize: CGSize(width: MacroBorderDefineThen.areaWidth() - 50, height: CGFloat(MAXFLOAT)), text: tipCellData.attributedString)
