...
class DataConverterThen: UICollectionReusableView {
    //: override init(frame: CGRect) {
    override init(frame: CGRect) {
        //: super.init(frame: frame)
        super.init(frame: frame)
        //: self.frame = CGRectMake(0, 0, ScreenWidth, SocialHeaderView.getSelfHeight())
        self.frame = CGRectMake(0, 0, main_assetUrl, DataConverterThen.belly())
        //: basicUI()
        simplex()
    }
...
extension DataConverterThen {
    ...
    @objc private func grownupWithinClick() {
        //: let dic = FacultyReactiveCompatible.share.appConfigMode.homeOpAds.first
        let dic = FacultyReactiveCompatible.share.appConfigMode.homeOpAds.first
        //: SmartThen.share.func__pushToWebVC(urlStr: dic?["url"] ?? "")
        SmartThen.share.lifeStyle(urlStr: dic?[(main_especialDefinePromptData.replacingOccurrences(of: "inside", with: "l"))] ?? "")
    }
