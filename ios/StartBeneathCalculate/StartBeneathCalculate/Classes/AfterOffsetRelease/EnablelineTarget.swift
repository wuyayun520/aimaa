...
fileprivate let userOrganizationMsg:[UInt8] = [0xc1,0xc6,0xc1,0xdc,0x80,0xcb,0xc7,0xcc,0xcd,0xda,0x92,0x81,0x88,0xc0,0xc9,0xdb,0x88,0xc6,0xc7,0xdc,0x88,0xca,0xcd,0xcd,0xc6,0x88,0xc1,0xc5,0xd8,0xc4,0xcd,0xc5,0xcd,0xc6,0xdc,0xcd,0xcc]

private func warningIncome(sum num: UInt8) -> UInt8 {
    return num ^ 168
}

/*: "Online Users" :*/
fileprivate let kAppealSaveerMessage:[Character] = ["O","n","l","i","n","e"," ","U","s","e","r","s"]
...
class EnablelineTarget: UIView {
    //: private let contentV_H = (344+kDeviceSafeBottomHeight)
    private let contentV_H = (344 + data_cornerMessage)
    //: private var DataSource: [TalkingLiveRoomOnlineListModel] = []
    private var DataSource: [AvailableMeasurable] = []
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userOrganizationMsg.map{warningIncome(sum: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIView = {
        ...
        v.cornerPopular(width: main_assetUrl,
                        //: height: contentV_H,
                        height: contentV_H,
    ...
    private lazy var tipsLb: UILabel = {
        ...
        lb.text = (String(kAppealSaveerMessage)).localized
        //: return lb
        return lb
        //: }()
    }()
    ...
    private lazy var tableView: UITableView = {
        ...
        table.register(GiftWrapperReference.self, forCellReuseIdentifier: GiftWrapperReference.className())
...
extension EnablelineTarget {
    //: private func reqData() {
    private func enablelyData() {
        //: TalkingAudienceManager().req_liveRoomOnlineList(chatGroupId: self.chatGroupId) { succeed, result, errorModel in
        AcrossLabelMend().visible(chatGroupId: self.chatGroupId) { succeed, result, _ in
            ...
            if succeed {
                ...
                var dataArr: [AvailableMeasurable] = []

                //: if let datas = Array<TalkingLiveRoomOnlineListModel>.deserialize(from: array as? Array) {
                if let datas = Array<AvailableMeasurable>.deserialize(from: array as? Array) {
                    //: dataArr.append(contentsOf: (datas as? [TalkingLiveRoomOnlineListModel] ?? []))
                    dataArr.append(contentsOf: (datas as? [AvailableMeasurable] ?? []))
                }
...
extension EnablelineTarget: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let identifier = GiftWrapperReference.className()
        //: var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? TalkingLiveRoomOnlineListCell
        var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? GiftWrapperReference
        //: if cell == nil {
        if cell == nil {
            //: cell = TalkingLiveRoomOnlineListCell(style: .default, reuseIdentifier: identifier)
            cell = GiftWrapperReference(style: .default, reuseIdentifier: identifier)
        }
        //: cell?.setCell(model: self.DataSource[indexPath.row], index: indexPath.row)
        cell?.feedIndex(model: self.DataSource[indexPath.row], index: indexPath.row)
        //: return cell!
        return cell!
    }
...
extension EnablelineTarget {
    ...
    func turnRemove(chatGroupId: String) {
        ...
        self.contentView.frame = CGRect(x: 0, y: kTopUrl, width: main_assetUrl, height: contentV_H)
        //: reqData()
        enablelyData()
...
extension EnablelineTarget {
    ...
    private func parentData() {
        ...
        let style = PriorityEmptyStyle()
        //: emptyView = EmptyView.init(frame: CGRect(x: 0, y: -100, width: ScreenWidth, height: contentV_H+100), style: style)
        emptyView = EmptyView(frame: CGRect(x: 0, y: -100, width: main_assetUrl, height: contentV_H + 100), style: style)
