    ...
    func hologram(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
        ...
        appAdjustData.kitEspecial(model: reqModel) { succeed, result, errorModel in
            ...
            guard succeed == true else {
                ...
                ElasticActiveEffect.pathAge(msg: (String(data_procedureTitle.suffix(7)) + show_handMethodKey) + "\(String(describing: errorModel?.errorMsg)), 参数:\(dict).")
                //: handle(nil, succeed)
                handle(nil, succeed)
                //: return
                return
        ...
        appAdjustData.kitEspecial(model: reqModel) { succeed, result, errorModel in
            ...
            guard succeed == true || errorModel?.errorCode == 405 else { // 验证接口失败，重试接口
                ...
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 2) {
                    ...
                    ElasticActiveEffect.pathAge(msg: (String(show_someoneMsg.prefix(7)) + kBarFormat.capitalized) + "\(String(describing: errorModel?.errorCode)), 参数:\(params).")
    ...
    func circleGenericEntry(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
        ...
        appAdjustData.kitEspecial(model: reqModel) { succeed, result, errorModel in
            ...
            guard succeed == true else {
                ...
                ElasticActiveEffect.pathAge(msg: (String(k_fallName) + constAlongPath.replacingOccurrences(of: "literal", with: "败")) + "\(String(describing: errorModel?.errorMsg)), 参数:\(dict).")
                //: handle(nil, succeed)
                handle(nil, succeed)
                //: return
                return
        ...
        appAdjustData.kitEspecial(model: reqModel) { succeed, result, errorModel in
            ...
            guard succeed == true || errorModel?.errorCode == 405 else { // 验证接口失败，重试接口
                ...
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 3) {
                    ...
                    ElasticActiveEffect.pathAge(msg: (String(appIdenticalKey)) + "\(String(describing: errorModel?.errorCode)), 参数:\(params).")
...
extension CreaterReactiveCompatible {
    ...
    fileprivate func mainVisitor(_ productId: String) {
        ...
        guard SKPaymentQueue.canMakePayments() else {
            ...
            ElasticActiveEffect.pathAge(msg: (String(kItData.suffix(4)) + "\u{8be2}\u{5185}购商" + String(dataTopValue) + String(mainWantTitle.prefix(9))) + "\(productId).")
...
extension CreaterReactiveCompatible: SKProductsRequestDelegate {
    ...
    func productsRequest(_: SKProductsRequest, didReceive response: SKProductsResponse) {
        ...
        guard response.products.count > 0 else {
            ...
            ElasticActiveEffect.pathAge(msg: (String(data_renderLayName.suffix(6)) + mainSuccessfullyFormat + ", pro" + const_gravityValue.replacingOccurrences(of: "to", with: "c") + "s.co" + String(mainFaceAdminAgainstName.prefix(6))))
    ...
    func request(_: SKRequest, didFailWithError error: Error) {
        ...
        ElasticActiveEffect.pathAge(msg: (String(dataStackFormat.suffix(5)) + "内\u{8d2d}商品失败" + String(show_complexMsg.suffix(8))) + "\(error).")
        //: self.completionHandle?( .noProductId, 0, currentPayType)
        self.completionHandle?(.noProductId, 0, currentPayType)
    }
...
extension CreaterReactiveCompatible: SKPaymentTransactionObserver {
    ...
    func paymentQueue(_: SKPaymentQueue, updatedTransactions transactions: [SKPaymentTransaction]) {
        ...
        for transaction in transactions {
            ...
            case .failed: // 交易失败/取消
                ...
                ElasticActiveEffect.pathAge(msg: (String(show_photoData.prefix(6)) + String(main_refuseSingleValue.prefix(5))) + "\(currentPayType)" + (String(noti_wordEarlySafeData)) + "\(String(describing: createOrderId))" + (String(dataEditKey.suffix(6)) + notiErrPauseTitle.capitalized) + "\(transaction.payment.productIdentifier)" + (user_portraitName + String(app_exceptionValue.prefix(5))) + "\(String(describing: transaction.transactionIdentifier)).")
            ...
            case .deferred: // 交易延期
                ...
                ElasticActiveEffect.pathAge(msg: (String(show_charmData)) + "\(currentPayType)" + (String(noti_wordEarlySafeData)) + "\(String(describing: createOrderId))" + (String(dataEditKey.suffix(6)) + notiErrPauseTitle.capitalized) + "\(transaction.payment.productIdentifier)" + (user_portraitName + String(app_exceptionValue.prefix(5))) + "\(String(describing: transaction.transactionIdentifier)).")
            ...
            @unknown default:
                ...
                ElasticActiveEffect.pathAge(msg: (String(const_formationProvideKey.prefix(5)) + String(app_arrowId)) + "\(currentPayType)" + (String(noti_wordEarlySafeData)) + "\(String(describing: createOrderId))" + (String(dataEditKey.suffix(6)) + notiErrPauseTitle.capitalized) + "\(transaction.payment.productIdentifier)" + (user_portraitName + String(app_exceptionValue.prefix(5))) + "\(String(describing: transaction.transactionIdentifier)).")
                //: fatalError(" 未知的交易类型")
    ...
    fileprivate func shadeSelectCheck(_ transactionId: String, _ payType: AttentionType) {
        ...
        guard let receiptStr = getData(transactionId, payType) else {
            ...
            ElasticActiveEffect.pathAge(msg: (mainRatingData + String(app_licenseGroupName) + dataExecutePath.replacingOccurrences(of: "argument", with: "ct") + "ionId:") + "\(transactionId)" + (String(const_outletPendingKey)) + "\(String(describing: createOrderId))" + (String(const_spendMessage) + String(kWrapFormat)) + "\(payType).")
