...
extension UtilizerDataSource {
    ...
    func succeedBy() {
        ...
        EndBuilderLifecycle.caliginous(params: dict) { succeed, result, errorModel in
...
extension UtilizerDataSource {
    ...
    func deleteWithSecurityBound(model: AgencyMomentModel, index: Int) {
        ...
        vc.munuBlock = { [weak self] index, str in
            ...
            if str == (String(show_giftTitle.suffix(8)) + String(kGlobalPackFormat)).localized {
                ...
                EndBuilderLifecycle.paramsCompletionRemoveCulmination(params: dict) { succeed, _, errorModel in
                    //: ProgressHUD.dismiss()
    ...
    func labBy(isTop: Int, model: AgencyMomentModel) {
        ...
        EndBuilderLifecycle.work(params: dict) { succeed, _, errorModel in
            //: ProgressHUD.dismiss()
            StrangerProgressHUD.barDismiss()
            //: if succeed {
            if succeed {
