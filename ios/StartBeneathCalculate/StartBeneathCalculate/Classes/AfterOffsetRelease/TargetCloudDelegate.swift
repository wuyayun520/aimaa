...
extension TargetCloudDelegate {
    ...
    private func zoneSize() {
        ...
        MigrationThen.pathAge(msg: (String(show_againstName) + String(showEscapeLanceListStr.prefix(4))) + "\(logId).")
...
extension TargetCloudDelegate: TRTCCloudDelegate {
    ...
    func onError(_ errCode: TXLiteAVError, errMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        MigrationThen.pathAge(msg: (String(user_phaseData) + String(main_interestFormat) + String(notiQualityName.prefix(5))) + "\(errCode)" + (String(dataFlagDirectReadStr.prefix(3)) + String(kAgoKey.prefix(6))) + "\(String(describing: errMsg)).")
        //: if errMsg != nil {
        if errMsg != nil {
    ...
    func onWarning(_ warningCode: TXLiteAVWarning, warningMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        MigrationThen.pathAge(msg: (String(const_phaseUrl.suffix(5)) + "音频通话S" + noti_aboutRestrictionMsg + String(showFollowingPath) + String(kInnerStr)) + "\(warningCode)" + (String(kStretchPath.suffix(5)) + "ningMsg" + user_valueSocialConfirmFormat.replacingOccurrences(of: "platform", with: ":")) + "\(String(describing: warningMsg)).")
    }

    //: func onEnterRoom(_ result: Int) {
    func onEnterRoom(_ result: Int) {
        //: UploadLogTool.writeLog(msg: "TRTC 音频通话进入房间. result:\(result), logId：\(logId).")
        MigrationThen.pathAge(msg: (String(mainNetworkUrl) + data_turnName.capitalized) + "\(result)" + (String(k_committeeMsg.prefix(6)) + String(notiResponsePath.suffix(2))) + "\(logId).")
        //: if result < 0 {
        if result < 0 { // 加入失败
