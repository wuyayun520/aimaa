...
/*: "Speaker On" :*/
fileprivate let show_countryData:[Character] = ["S","p","e","a","k","e","r"]
fileprivate let constGenerateFixedComplaintTitle:[Character] = [" ","O","n"]

/*: "Speaker Off" :*/
fileprivate let user_disappearTitle:[Character] = ["S","p","e","a","k","e"]
fileprivate let appClothesMsg:String = "balloon three odd hostr Off"

/*: "TRTC 音频通话销毁. logId： :*/
fileprivate let show_againstName:[Character] = ["T","R","T","C"," ","音","频","通","话","销","毁","."," ","l","o"]
fileprivate let showEscapeLanceListStr:String = "gId：local template then identify"

/*: "TRTC 音频通话错误. errCode: :*/
fileprivate let user_phaseData:[Character] = ["T","R","T","C"," ","\u{97f3}","\u{9891}","通","话","错","误","."," ","e","r"]
fileprivate let main_interestFormat:[Character] = ["r"]
fileprivate let notiQualityName:String = "Code:distribution revenue stack"

/*: , errMsg: :*/
fileprivate let dataFlagDirectReadStr:String = ", elock hood"
fileprivate let kAgoKey:String = "rrMsg:video running"

/*: "TRTC 音频通话SDK收到警告. warningCode: :*/
fileprivate let const_phaseUrl:String = "other heightTRTC "
fileprivate let noti_aboutRestrictionMsg:String = "DK收\u{5230}"
fileprivate let showFollowingPath:[Character] = ["警","告","."," ","w","a","r"]
fileprivate let kInnerStr:[Character] = ["n","i","n","g","C","o","d","e",":"]

/*: , warningMsg: :*/
fileprivate let kStretchPath:String = "tip page, war"
fileprivate let user_valueSocialConfirmFormat:String = "platform"

/*: "TRTC 音频通话进入房间. result: :*/
fileprivate let mainNetworkUrl:[Character] = ["T","R","T","C"," ","音","\u{9891}","通","话","进","入","房","间","."," ","r","e","s","u","l","t"]
fileprivate let data_turnName:String = ":"

/*: , logId： :*/
fileprivate let k_committeeMsg:String = ", logIstrike until injury"
fileprivate let notiResponsePath:String = "refer disable clothesd："

/*:  加入了房间" :*/
fileprivate let kBreastPath:String = " 加入了\u{623f}间"

/*:  离开了房间" :*/
fileprivate let user_beforeName:[Character] = [" ","离","\u{5f00}","了","房","\u{95f4}"]
...
private class ThatThen: NSObject {
...
class TargetCloudDelegate: NSObject {
    ...
    private var config = ThatThen() // 音频渲染配置类
    //: private var logId = 0
...
extension TargetCloudDelegate {
    ...
    func at(enable: Bool) {
        ...
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 1) {
            ...
            self.replace(notif: nil)
        }
    }

    /// 释放资源
    //: func shouldReleaseAllResource() {
    func shouldLevelSupport() {
        //: destoryTRTCView()
        zoneSize()
    }
    ...
    func sinceValue(roomId: Int) {
        //: self.handsfreeBtn?.isEnabled = !hasHeadSet()
        self.handsfreeBtn?.isEnabled = !rugged()
    ...
    func poise(isHandsfree: Bool) {
        //: guard hasHeadSet() == false else { return }
        guard rugged() == false else { return }
        ...
        self.handsfreeDesLab?.text = isHandsfree ? (String(show_countryData) + String(constGenerateFixedComplaintTitle)).localized : (String(user_disappearTitle) + String(appClothesMsg.suffix(5))).localized
        //: updateWhenPlayDeviceChanged(isHandsfree: isHandsfree)
        square(isHandsfree: isHandsfree)
    }
}
...
extension TargetCloudDelegate {
    ...
    func totalerrupt() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(replace(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(observerProduct(notif:)),
    ...
    @objc private func sumry(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
    ...
    @objc private func replace(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
    ...
    @objc private func feed(notif: Notification) {
    ...
    @objc func observerProduct(notif: Notification) {
        ...
        case .oldDeviceUnavailable: // 免提
            ...
            self.handsfreeDesLab?.text = (String(show_countryData) + String(constGenerateFixedComplaintTitle)).localized
            //: updateWhenPlayDeviceChanged(isHandsfree: self.config.isHandsfree)
            square(isHandsfree: self.config.isHandsfree)
...
extension TargetCloudDelegate {
    ...
    private func councilChamberEnterDrop() {
    ...
    private func square(isHandsfree: Bool) {
    ...
    private func zoneSize() {
        //: UploadLogTool.writeLog(msg: "TRTC 音频通话销毁. logId：\(logId).")
        ElasticActiveEffect.pathAge(msg: (String(show_againstName) + String(showEscapeLanceListStr.prefix(4))) + "\(logId).")
        //: guard self.config.isGeneral else { return }
        guard self.config.isGeneral else { return }

        //: if self.config.isHandsfree == false {
        if self.config.isHandsfree == false { // 恢复系统扬声器播放声音
            //: updateWhenPlayDeviceChanged(isHandsfree: true)
            square(isHandsfree: true)
        }
...
extension TargetCloudDelegate: TRTCCloudDelegate {
    ...
    func onError(_ errCode: TXLiteAVError, errMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        ElasticActiveEffect.pathAge(msg: (String(user_phaseData) + String(main_interestFormat) + String(notiQualityName.prefix(5))) + "\(errCode)" + (String(dataFlagDirectReadStr.prefix(3)) + String(kAgoKey.prefix(6))) + "\(String(describing: errMsg)).")
        //: if errMsg != nil {
        if errMsg != nil {
    ...
    func onWarning(_ warningCode: TXLiteAVWarning, warningMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        ElasticActiveEffect.pathAge(msg: (String(const_phaseUrl.suffix(5)) + "音频通话S" + noti_aboutRestrictionMsg + String(showFollowingPath) + String(kInnerStr)) + "\(warningCode)" + (String(kStretchPath.suffix(5)) + "ningMsg" + user_valueSocialConfirmFormat.replacingOccurrences(of: "platform", with: ":")) + "\(String(describing: warningMsg)).")
    }

    //: func onEnterRoom(_ result: Int) {
    func onEnterRoom(_ result: Int) {
        //: UploadLogTool.writeLog(msg: "TRTC 音频通话进入房间. result:\(result), logId：\(logId).")
        ElasticActiveEffect.pathAge(msg: (String(mainNetworkUrl) + data_turnName.capitalized) + "\(result)" + (String(k_committeeMsg.prefix(6)) + String(notiResponsePath.suffix(2))) + "\(logId).")
        //: if result < 0 {
        if result < 0 { // 加入失败
