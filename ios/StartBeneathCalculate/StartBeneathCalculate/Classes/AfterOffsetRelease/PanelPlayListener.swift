...
/*: "call_bgm" :*/
fileprivate let constClipValue:String = "call_bgmused keep"

/*: "call_response_bgm" :*/
fileprivate let appOwnFormat:String = "cvisitorll"
fileprivate let main_carrierGriefName:String = "se_bgmprompt stage holder"

/*: "invitePop_call_bgm" :*/
fileprivate let main_sequenceMessage:String = "ibubblevite"
fileprivate let user_scopeValue:String = "_bgmit name spend"
...
enum ReportedTransformable: Int {
    //: case Call
    case Call // 音视频拨打音乐
    //: case Response
...
class PanelPlayListener: NSObject {
    //: var type = CallBGMPlayerType.Call
    var type = ReportedTransformable.Call
    //: var player = AVPlayer()
    var player = AVPlayer()
    //: var filePath = ""
    var filePath = ""
    ...
    class func explain(type: ReportedTransformable) -> PanelPlayListener {
        //: let player = TalkingCallBGMPlayer()
        let player = PanelPlayListener()
        //: player.type = type
        player.type = type
        //: player.getFilePath()
        player.trademark()
...
extension PanelPlayListener {
    ...
    private func withoutFill(notify: Notification) {
    ...
    private func anMerge(notify: Notification) {
    ...
    private func trademark() {
        //: var fileName = "call_bgm"
        var fileName = (String(constClipValue.prefix(8)))
        //: if self.type == .Response {
        if self.type == .Response {
            //: fileName = "call_response_bgm"
            fileName = (appOwnFormat.replacingOccurrences(of: "visitor", with: "a") + "_respon" + String(main_carrierGriefName.prefix(6)))
            //: } else if self.type == .invitePop {
        } else if self.type == .invitePop {
            //: fileName = "invitePop_call_bgm"
            fileName = (main_sequenceMessage.replacingOccurrences(of: "bubble", with: "n") + "Pop_call" + String(user_scopeValue.prefix(4)))
        }
        // 收到通话推送要播放mp3声音，不能放到.zip中
        //: let path = SVGAEffectTool.default.getMp3Path(name: fileName)
        let path = TaEffectTool.default.globalBird(name: fileName)
        //: self.filePath = path
        self.filePath = path
    }
}
...
extension PanelPlayListener: TXVodPlayListener {
