...
class DemonstrateView: UIView {
    ...
    var animatedView: DisabledScenarioList?
...
extension DemonstrateView {
    ...
    func burnLoop(effectMsgModel: GalleryMsgModel, effectModel: AfterAnimatModel) {
        ...
        let realClass = AnimatedViewClass as! DisabledScenarioList.Type
...
extension DemonstrateView: BasicLayoutScope {
    //: func giftEffectItemView(effectItemView: TalkingGiftAnimatItemView, success: Bool) {
    func distanceSuccess(effectItemView: DisabledScenarioList, success: Bool) {
        //: if (!success) {
        if !success {
            //: self.alpha = 0
            self.alpha = 0
    ...
    func roundOut(effectItemView: DisabledScenarioList) {
