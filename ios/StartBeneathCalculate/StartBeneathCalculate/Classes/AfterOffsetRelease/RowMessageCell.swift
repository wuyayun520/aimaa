...
class RowMessageCell: EarthThen {
//    var transBlock: (() -> Void)?
...
//                self.messageData?.translateStatus != .MsgTranslate_succeed && self.messageData?.msgModel.msgInfo.callCmd.count ?? 0 <= 0 && temTextData.msgModel.msgInfo.messageType != .MessageTypeWelcome && "\(temTextData.msgModel.user.uid)" != MacroBorderDefineThen.getXiaoMiID()) { // 对方发的 && 开启了翻译功能 && 消息未翻译成功
...
//            }
    ...
    func instance() {
        ...
        self.container.addGestureRecognizer(containerTap)
        //: let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == MacroBorderDefineThen.getXiaoMiID())
        let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == MacroBorderDefineThen.seethe())
        //: if isOfficial == false {
        if isOfficial == false {
            //: addLongTagGes()
            week()
