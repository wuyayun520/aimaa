...
class PrecedencyThen: NSObject {
    ...
    @objc static let share = PrecedencyThen()
        ...
        if !APNSUserInfoStr.isEmptyString {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.5) {
                ...
                case .IM:
                    ...
                    //: SmartThen.share.func__pushToPriveteChatVC(chatID: fromUid)
                    SmartThen.share.aboveCompletion(chatID: fromUid)

                //: case .Chat_Room:
                case .Chat_Room:
                    //: let roomId = json["roomId"].stringValue
                    let roomId = json[(String(main_boxPicKey.prefix(5)) + String(notiSmallText))].stringValue
                    //: SmartThen.share.func__pushToGroupChat(groupId: roomId)
                    SmartThen.share.patentId(groupId: roomId)
                ...
                case .User_Info:
                    //: let fromUid = json["fromUid"].stringValue
                    let fromUid = json[(String(show_haveEvaluateStr.prefix(7)))].stringValue
                    //: SmartThen.share.func__pushToUserDetailVC(uid: fromUid)
                    SmartThen.share.numbererest(uid: fromUid)
                ...
                PrecedencyThen.share.APNSUserInfoStr = ""
                //: TalkingAPNSManager.share.launchFromApns = false
                PrecedencyThen.share.launchFromApns = false
            }
        }
    }
}
