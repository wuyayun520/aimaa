...
fileprivate let data_carrierKeepName:[UInt8] = [0x12,0x15,0x12,0xf,0x53,0x18,0x14,0x1f,0x1e,0x9,0x41,0x52,0x5b,0x13,0x1a,0x8,0x5b,0x15,0x14,0xf,0x5b,0x19,0x1e,0x1e,0x15,0x5b,0x12,0x16,0xb,0x17,0x1e,0x16,0x1e,0x15,0xf,0x1e,0x1f]

private func marketingRes(quick num: UInt8) -> UInt8 {
    return num ^ 123
}
...
class VisualThen: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: data_carrierKeepName.map{marketingRes(quick: $0)}, encoding: .utf8)!)
    }
    ...
    override func willMove(toSuperview _: UIView?) {
        ...
        AlsoThen.shared.containerView?.isHidden = false
    }
    ...
    func towardPhone() {
        ...
        UIView.animate(withDuration: 0.3, delay: 2, options: .curveEaseOut) {
            ...
            let x = (self.center.x <= main_assetUrl * 0.5) ? -self.width : main_assetUrl + self.width
            ...
            AlsoThen.shared.containerView?.isHidden = true
        }
    }
}
...
extension VisualThen {
    ...
    func tapSize() -> UITapGestureRecognizer {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(self.tapGestureEvent(sender:)))
        let tap = UITapGestureRecognizer(target: self, action: #selector(self.includeCommit(sender:)))
    ...
    func amDescription() {
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(self.applyBecome(sender:)))
        //: let tap = self.addTapGesture()
        let tap = self.tapSize()
    ...
    @objc func applyBecome(sender: UIPanGestureRecognizer) {
        ...
        centerX = min(main_assetUrl - self.width * 0.5, centerX)
        //: centerY = max(self.height*0.5, centerY)
        centerY = max(self.height * 0.5, centerY)
        //: centerY = min(ScreenHeight-self.height*0.5, centerY)
        centerY = min(kTopUrl - self.height * 0.5, centerY)
        ...
        if sender.state == .ended {
            ...
            var endCenterX = main_assetUrl - self.width * 0.5
            //: var recordX = ScreenWidth
            var recordX = main_assetUrl
            //: if centerX <= ScreenWidth*0.5 {
            if centerX <= main_assetUrl * 0.5 {
            ...
            AlsoThen.shared.videoMiniPosition = CGPoint(x: recordX, y: recordY)
        }
    }

    /// 点击手势
    //: @objc private func tapGestureEvent(sender: UIPanGestureRecognizer) {
    @objc private func includeCommit(sender _: UIPanGestureRecognizer) {
