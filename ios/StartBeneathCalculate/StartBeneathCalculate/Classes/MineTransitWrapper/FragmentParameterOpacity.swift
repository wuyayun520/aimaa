...
/*: "/TXCache" :*/
fileprivate let dataSuspendContent:String = "/Tbroadcast forward"
fileprivate let showPictureMsg:String = "XCacheagenda memory minimize"
...
// __CLOSE_PRINT__
//
//  FragmentParameterOpacity.swift
//  AbroadTalking
//
...
protocol RetainReactiveCompatible: NSObject {
    //: func func__playerStatus(player: TalkingVideoPlayerManager, status: VideoPlayerStatus)
    func westFlagStatus(player: FragmentParameterOpacity, status: StrangerPlayerStatus)

    //: func func__playerDuration(player: TalkingVideoPlayerManager, duration: Int, currentTime: Int)
    func secondTime(player: FragmentParameterOpacity, duration: Int, currentTime: Int)

    //: func func__playerProgress(player: TalkingVideoPlayerManager, progress: CGFloat)
    func valid(player: FragmentParameterOpacity, progress: CGFloat)
}

//: class TalkingVideoPlayerManager: NSObject {
class FragmentParameterOpacity: NSObject {
    //: var isPlaying = false
    var isPlaying = false
    //: var isMute = false
    var isMute = false
    ...
    open weak var delegate: RetainReactiveCompatible?
    //: var playStatus: VideoPlayerStatus?
    var playStatus: StrangerPlayerStatus?
    //: static let shared = TalkingVideoPlayerManager()
    static let shared = FragmentParameterOpacity()
    ...
    lazy var player: TXVodPlayer = {
        ...
        let cachePath = NSString(format: "%@%@", NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!, (String(dataSuspendContent.prefix(2)) + String(showPictureMsg.prefix(6)))) as String
    ...
    lazy var timer: Timer = {
        ...
        let timer = Timer.scheduledTimer(timeInterval: 0.25, target: self, selector: #selector(odd), userInfo: nil, repeats: true)
...
extension FragmentParameterOpacity {
    //: func playerWithUrlAndVideoView(url: String, view: UIView) {
    func find(url: String, view: UIView) {
        //: let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let newUrl = url.replacingOccurrences(of: main_lessPath.holderDown(), with: app_frameName)
    ...
    @objc func odd() {
        ...
        if duration() > 0 {
            ...
            self.delegate?.valid(player: self, progress: CGFloat(currentTime() / duration()))
        }
    }

    //: func setRenderMode(renderMode: EnumTypeRenderMode) {
    func toggleMode(renderMode: BearViewRenderMode) {
    ...
    func setMuteEmpower(bEnable: Bool) {
        //: var isMute = bEnable
        var isMute = bEnable
        //: if TalkingSocketManager.shared.isGeneral == true {
        if ConfigureAxisService.shared.isGeneral == true { // 音视频通话时静音
            //: isMute = true
            isMute = true
        }
    ...
    func warning() {
        ...
        if self.playStatus != .Pausing {
            ...
            self.delegate?.westFlagStatus(player: self, status: self.playStatus!)
        }
    }
...
extension FragmentParameterOpacity: TXVodPlayListener {
    ...
    func onPlayEvent(_: TXVodPlayer!, event EvtID: Int32, withParam _: [AnyHashable: Any]!) {
        ...
        case PLAY_EVT_PLAY_PROGRESS.rawValue:
            ...
            self.delegate?.secondTime(player: self, duration: Int(duration), currentTime: Int(currentTime))
