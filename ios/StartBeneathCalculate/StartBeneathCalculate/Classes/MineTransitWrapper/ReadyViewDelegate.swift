...
fileprivate let const_topicKey:[UInt8] = [0x5b,0x5c,0x5b,0x46,0x1a,0x51,0x5d,0x56,0x57,0x40,0x8,0x1b,0x12,0x5a,0x53,0x41,0x12,0x5c,0x5d,0x46,0x12,0x50,0x57,0x57,0x5c,0x12,0x5b,0x5f,0x42,0x5e,0x57,0x5f,0x57,0x5c,0x46,0x57,0x56]

/*: "Post" :*/
fileprivate let show_targetStrokeBronzeUrl:[Character] = ["P","o","s","t"]

/*: "Profile" :*/
fileprivate let dataEffectiveValue:[Character] = ["P","r","o","f","i"]
fileprivate let main_ratingText:[Character] = ["l","e"]

/*: "bth_me_follow_nor" :*/
fileprivate let k_commentText:[Character] = ["b","t","h","_","m","e","_","f","o","l"]
fileprivate let k_bounceKey:String = "low_norbusy sun"

/*: "removeAttentionUid" :*/
fileprivate let kRearValue:[Character] = ["r","e","m","o","v","e","A","t","t","e","n","t","i","o","n","U"]
fileprivate let appDynamicsFormat:[Character] = ["i","d"]

/*: "attentionUid" :*/
fileprivate let showElectBanValue:[Character] = ["a","t","t","e","n","t"]
fileprivate let showLabelMessage:String = "gross pen map outer stepionUid"

/*: "bth_me_follow_pre" :*/
fileprivate let app_pleaseName:[UInt8] = [0x0,0x12,0x6,0xfd,0xb,0x3,0xfd,0x4,0xd,0xa,0xa,0xd,0x15,0xfd,0xe,0x10,0x3]

fileprivate func exceptMilk(reader num: UInt8) -> UInt8 {
...
class ReadyViewDelegate: SafetyFactorViewController {
    //: var uid: String = ""
    var uid: String = ""
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: const_topicKey.map{$0^50}, encoding: .utf8)!)
    }
    ...
    private lazy var navView: GoBarView = { // 导航
    ...
    private lazy var headerView: TrueDataSource = { // banner
        ...
        let v = TrueDataSource(frame: frame)
    ...
    private lazy var bottomView: GateOperationLeft = { // bottom
        //: let v = TalkingUserDetailBottomView(frame: .zero)
        let v = GateOperationLeft(frame: .zero)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var segmentedView: JXSegmentedView = {
        ...
        let segmentedView = JXSegmentedView(frame: CGRect(x: 0, y: show_textNextFormat, width: main_assetUrl, height: 44))
        ...
        segmentedDataSource.titleNormalColor = UIColor.keepRid()
        //: segmentedDataSource.titleSelectedColor = UIColor.mottleResult()
        segmentedDataSource.titleSelectedColor = UIColor.mottleResult()
        ...
        indicator.lineStyle = .normal
        //: indicator.indicatorColor = UIColor.mottleResult()
        indicator.indicatorColor = UIColor.mottleResult()
    ...
    private lazy var pagerView: JXPagingListRefreshView = {
        ...
        view.pinSectionHeaderVerticalOffset = Int(main_sendData)
        //: return view
        return view
        //: }()
    }()
    ...
    private lazy var titleArray: [String] = //: return ["Post".localized, "Profile".localized]
        [(String(show_targetStrokeBronzeUrl)).localized, (String(dataEffectiveValue) + String(main_ratingText)).localized]
    //: }()

    //: private lazy var dataVC: [TalkingBaseViewController] = {
    private lazy var dataVC: [SafetyFactorViewController] = //: return [postVC, profileVC]
        [postVC, profileVC]
    //: }()

    //: private lazy var postVC: TalkingUserDetailPostViewController = {
    private lazy var postVC: SpanViewController = {
        //: let vc = TalkingUserDetailPostViewController()
        let vc = SpanViewController()
        //: return vc
        return vc
        //: }()
    }()
    ...
    private lazy var profileVC: UnderlyingObjectProtocol = {
        //: let vc = TalkingUserDetailProfileViewController()
        let vc = UnderlyingObjectProtocol()
        //: return vc
        return vc
        //: }()
    }()
    ...
    private lazy var followBtn: UIImageView = {
        ...
        img.image = UIImage.adName(name: (String(k_commentText) + String(k_bounceKey.prefix(7))))
        //: img.isUserInteractionEnabled = true
        img.isUserInteractionEnabled = true
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(followButtonClick))
        let tap = UITapGestureRecognizer(target: self, action: #selector(holder))
...
extension ReadyViewDelegate {
    ...
    private func re() {
        ...
        if userInfoModel.isAttention {
            ...
            dict[(String(kRearValue) + String(appDynamicsFormat))] = userInfoModel.uid
            //: isattion = false
            isattion = false
            //: } else {
        } else {
            //: dict["attentionUid"] = userInfoModel.uid
            dict[(String(showElectBanValue) + String(showLabelMessage.suffix(6)))] = userInfoModel.uid
        }
    ...
    func magnitudery() {
        ...
        queue.async(group: group) {
            ...
            PriorityReactiveCompatible.bass(uid: self.uid) { succeed, result, _ in
                //: self.setNoremarlView()
                self.allSet()
                //: if self.uid == FacultyReactiveCompatible.share.loginUid {
                if self.uid == FacultyReactiveCompatible.share.loginUid {
                ...
                if succeed == true {
                    ...
                    if let userModel = JSONDeserializer<SeekGoMeasurable>.deserializeFrom(dict: result as? [String: Any], designatedPath: nil) {
                        ...
                        self.navView.nearView(userModel)
                        //: self.headerView.refreshView(userModel)
                        self.headerView.operateView(userModel)
                        //: self.bottomView.refreshView(userModel)
                        self.bottomView.swallow(userModel)
                        //: self.postVC.refreshPostViewController(userModel)
                        self.postVC.panelController(userModel)
                        // 场景三：男性用户邀请通话弹窗监听
                        ...
                        (LocomoteApplication.shared as! LocomoteApplication).marginPic(uid: userModel.uid, videoUrl: userModel.videoUrl, headPic: userModel.headPic)
                    }
                }
                //: group.leave()
                group.leave()
        ...
        queue.async(group: group) {
            ...
            PriorityReactiveCompatible.listenerActiveCompletion(uid: self.uid) { succeed, result, _ in
                //: if succeed == true {
                if succeed == true {
                    ...
                    for dic in result as! [[String: Any]] {
                        ...
                        if let model = GrossMeasurable.deserialize(from: dic as? Dictionary) {
    ...
    private func awakeNotify() {
        ...
        self.profileVC.enter(giftsArr, profileSectionArr, userInfoModel)
    }
}
...
extension ReadyViewDelegate {
    ...
    private func theme() {
        //: self.followBtn.isHidden = (userInfoModel.uid == FacultyReactiveCompatible.share.loginUid)
        self.followBtn.isHidden = (userInfoModel.uid == FacultyReactiveCompatible.share.loginUid)
        //: let followImgStr = self.userInfoModel.isAttention ? "bth_me_follow_pre":"bth_me_follow_nor"
        let followImgStr = self.userInfoModel.isAttention ? String(bytes: app_pleaseName.map{exceptMilk(reader: $0)}, encoding: .utf8)! : (String(k_commentText) + String(k_bounceKey.prefix(7)))
        //: self.followBtn.image = UIImage.BundleImageNamed(name: followImgStr)
        self.followBtn.image = UIImage.adName(name: followImgStr)
    }
    ...
    private func define() {
        ...
        guard userInfoModel.loungePlus else { return }
        //: guard let vipModel = FacultyReactiveCompatible.share.appUserConfigMode.vipSetting.filter({$0.vipSkinId == userInfoModel.vipSkinId}).first else { return }
        guard let vipModel = FacultyReactiveCompatible.share.appUserConfigMode.vipSetting.filter({ $0.vipSkinId == userInfoModel.vipSkinId }).first else { return }
        ...
        vipCenterView.notFit(urlStr: vipModel.vipSkinCenter, placeImg: nil)
        //: vipBottomView.setUrlImage(urlStr: vipModel.vipSkinBottom, placeImg: nil)
        vipBottomView.notFit(urlStr: vipModel.vipSkinBottom, placeImg: nil)
    }
}
...
extension ReadyViewDelegate: JXSegmentedViewDelegate {
...
extension ReadyViewDelegate: JXPagingViewDelegate {
    ...
    func tableMap(_: JXPagingView, mainTableViewDidScroll scrollView: UIScrollView) {
        //: let offsetY = scrollView.contentOffset.y
        let offsetY = scrollView.contentOffset.y
        //: if  offsetY > StatusBarNavigationBarHeight {
        if offsetY > main_sendData {
            //: self.navView.whiteBgView.alpha = 1
            self.navView.whiteBgView.alpha = 1
            //: self.navView.backButton.tintColor = .mottleResult()
            self.navView.backButton.tintColor = .mottleResult()
            //: self.navView.moreButton.tintColor = .mottleResult()
            self.navView.moreButton.tintColor = .mottleResult()
            //: } else {
        } else {
            //: self.navView.whiteBgView.alpha = min(offsetY/StatusBarNavigationBarHeight, 1)
            self.navView.whiteBgView.alpha = min(offsetY / main_sendData, 1)
            //: let color = self.navView.whiteBgView.alpha > 0.7 ? UIColor.mottleResult():UIColor.white
            let color = self.navView.whiteBgView.alpha > 0.7 ? UIColor.mottleResult() : UIColor.white
...
extension ReadyViewDelegate {
    ...
    private func allSet() {
        ...
        bottomView.snp.makeConstraints { make in
            ...
            make.height.equalTo(const_nearbyMessage)
        }
    }
    ...
    private func ofConstraint() {
        ...
        pagerView.frame = CGRect(x: 0, y: 0, width: main_assetUrl, height: kTopUrl)
