...
fileprivate let show_selectKey:[Character] = ["C","a","c","h","e","G","i","f","t","E","f","f","e","c","t","C","a","c","h","e","K","e","y"]

/*: "下载动画文件" :*/
fileprivate let userHoodTitle:String = "下载动画others"

/*: "加载本地动画文件" :*/
fileprivate let show_frontName:[Character] = ["\u{52a0}","载","本","地","动","画","文","\u{4ef6}"]

/*: "Cache_GiftEffect/" :*/
fileprivate let appThroughEnterStr:String = "Cache_awake bound mail upon productivity"
fileprivate let kFastData:String = "Effeccrush low recto date"
fileprivate let notiBeginningAgendaMessage:String = "t/first nurse complex glob"

/*: "GJSGVA" :*/
fileprivate let app_evenData:String = "eachJSeachV"
fileprivate let const_accessContent:String = "a"

/*: "GJVap" :*/
fileprivate let kMidMsg:[Character] = ["G","J","V","a","p"]

/*: "config.json" :*/
fileprivate let mainPrivacyAgainstFormat:[Character] = ["c","o","n","f","i"]
fileprivate let data_imageStr:String = "g.jsoncurrent panel construct calculate"

/*: "app. :*/
fileprivate let mainMagnitudeValue:String = "required another back counterapp."

/*: .com" :*/
fileprivate let main_recentData:String = ".comdrop deal well awake"

/*: "key" :*/
fileprivate let kFaceData:[Character] = ["k","e","y"]

/*: "zip" :*/
fileprivate let show_sName:[Character] = ["z","i","p"]

/*: " fail to unzip file" :*/
fileprivate let const_lessText:[Character] = [" ","f","a","i","l"," ","t","o"," ","u","n","z","i","p"," ","f","i","l"]
fileprivate let kBrightFosterValue:[Character] = ["e"]

/*: "RSA 解密失败" :*/
fileprivate let main_pressMessage:String = "RSsymbolic"
fileprivate let notiMomentNarrowText:[Character] = [" ","解","密","失","败"]

/*: "fail to unzip script file" :*/
fileprivate let mainPerformSomebodyName:[Character] = ["f","a","i","l"," ","t","o"," ","u","n"]
fileprivate let user_midNoteText:[Character] = ["z","i","p"," ","s"]
fileprivate let appEyeFormat:[Character] = ["c","r","i","p","t"," ","f","i","l","e"]

/*: "gift" :*/
fileprivate let dataNumbMillCertainMessage:String = "feedbackft"

/*: "%@%@effect_%ld" :*/
fileprivate let show_lowerFormat:String = "%@%@efrecording number"
fileprivate let mainPlainDomainThreadKey:[Character] = ["f","e"]
fileprivate let constBeneathName:[Character] = ["c","t","_","%","l","d"]

/*: "%@%@effect_%ld_unzipTest/" :*/
fileprivate let noti_appearanceText:[Character] = ["%","@","%","@","e","f","f","e"]
fileprivate let app_safetyText:String = "ct_%lwork big allow"
fileprivate let const_denyTitle:[Character] = ["T","e","s","t","/"]

/*: "%@%@effect_%ld_unzip/" :*/
fileprivate let showWellUrl:[Character] = ["%","@","%","@","e","f","f","e","c","t","_","%","l","d"]
fileprivate let show_breastUrl:[Character] = ["_","u","n","z","i","p","/"]
...
// __CLOSE_PRINT__
//
//  EconomicAidReadyReactiveCompatible.swift
//  AbroadTalking
//
...
typealias CompletionBlock = (_ success: Bool, _ effectModel: AfterAnimatModel?) -> Void

//: typealias ErrorBlock = (_ error: NSError?) -> Void
typealias ErrorBlock = (_ error: NSError?) -> Void

//: class TalkingGiftAnimatTool: NSObject {
class EconomicAidReadyReactiveCompatible: NSObject {
    //: let SignDurationKey  = "CacheGiftEffectCacheKey"
    let SignDurationKey = (String(show_selectKey))

    //: @objc static let shared = TalkingGiftAnimatTool()
    @objc static let shared = EconomicAidReadyReactiveCompatible()
    ...
    func sourceWhen(effectMsgModel: GalleryMsgModel, completion: @escaping CompletionBlock) {
        //: if checkNewVersionWithAnimatMsgModel(model: effectMsgModel) {
        if securePast(model: effectMsgModel) {
            //: printLog(message: "下载动画文件")
            printLog(message: (userHoodTitle.replacingOccurrences(of: "others", with: "文件")))
            //: downloadAnimat(effectMsgModel: effectMsgModel) { [weak self] error in
            emptyBlock(effectMsgModel: effectMsgModel) { [weak self] error in
                ...
                self.rowAcross(effectMsgModel: effectMsgModel, complete: completion)
            }
            //: } else {
        } else {
            //: printLog(message: "加载本地动画文件")
            printLog(message: (String(show_frontName)))
            //: self.loadLocalAnimatConfig(effectMsgModel: effectMsgModel, complete: completion)
            self.rowAcross(effectMsgModel: effectMsgModel, complete: completion)
        }
    }
    ...
    func primrosePathDoingce(fileName: String, model: GalleryMsgModel) -> String {
        //: let scriptDirectory = fetchScriptDirectory(animatMsgModel: model)
        let scriptDirectory = emptyAccess(animatMsgModel: model)
    ...
    func detect() {
        ...
        deletePath(path: repercussionLabel())
    }

    // 动效资源本地的缓存目录路径
    //: func fetchScriptDirectory(animatMsgModel: TalkingAnimatMsgModel) -> String {
    func emptyAccess(animatMsgModel: GalleryMsgModel) -> String {
        //: if animatMsgModel.eid == 0 {
        if animatMsgModel.eid == 0 {
            //: return String.init(format: "%@/%@/", giftEffectDirectory(), animatMsgModel.effectUrl.md5()!)
            return String(format: "%@/%@/", repercussionLabel(), animatMsgModel.effectUrl.md5()!)
            //: } else {
        } else {
            //: return String.init(format: "%@/%zd/", giftEffectDirectory(), animatMsgModel.eid)
            return String(format: "%@/%zd/", repercussionLabel(), animatMsgModel.eid)
        }
    }

    //: func giftEffectDirectory() -> String {
    func repercussionLabel() -> String {
        //: let subPath = "Cache_GiftEffect/"
        let subPath = (String(appThroughEnterStr.prefix(6)) + "Gift" + String(kFastData.prefix(5)) + String(notiBeginningAgendaMessage.prefix(2)))
    ...
    func partyPath(filePath: String) -> String {
    ...
    func binding(model: GalleryMsgModel) -> Bool {
        //: return !checkNewVersionWithAnimatMsgModel(model: model)
        return !securePast(model: model)
    }

    //: func getNewAnimatClassName(oldName: String) -> String {
    func negligibleCalled(oldName: String) -> String {
        //: var newName = oldName
        var newName = oldName
        //: if newName.hasPrefix("GJSGVA") {
        if newName.hasPrefix((app_evenData.replacingOccurrences(of: "each", with: "G") + const_accessContent.uppercased())) {
            //: newName = String(describing: TalkingSGVAAnimationView.self)
            newName = String(describing: SmartItemView.self)
            //: } else if newName.hasPrefix("GJVap") {
        } else if newName.hasPrefix((String(kMidMsg))) {
            //: newName = String(describing: TalkingVapAnimationView.self)
            newName = String(describing: LiteralItemView.self)
        }
        //: return newName
        return newName
    }
...
extension EconomicAidReadyReactiveCompatible {
    //: func loadLocalAnimatConfig(effectMsgModel: TalkingAnimatMsgModel, complete: @escaping CompletionBlock) {
    func rowAcross(effectMsgModel: GalleryMsgModel, complete: @escaping CompletionBlock) {
        //: let scriptDirectory = fetchScriptDirectory(animatMsgModel: effectMsgModel)
        let scriptDirectory = emptyAccess(animatMsgModel: effectMsgModel)
        //: let scriptPath = (scriptDirectory as NSString).appendingPathComponent("config.json")
        let scriptPath = (scriptDirectory as NSString).appendingPathComponent((String(mainPrivacyAgainstFormat) + String(data_imageStr.prefix(6))))
        ...
        let dic = NSDictionary.beginShoo(withJsonString: configString!) ?? NSDictionary()
        //: let giftAmodel = TalkingGiftAnimatModel.init()
        let giftAmodel = AfterAnimatModel()
        //: let effectModel = giftAmodel.setAnimatModek(dic: dic)
        let effectModel = giftAmodel.sectionDic(dic: dic)
        ...
        className = negligibleCalled(oldName: className)
    ...
    func securePast(model: GalleryMsgModel) -> Bool {
        //: let giftPlist = UserDefaults.standard.object(forKey: SignDurationKey) as? Dictionary<String, Any> ?? Dictionary<String, Any>.init()
        let giftPlist = UserDefaults.standard.object(forKey: SignDurationKey) as? [String: Any] ?? Dictionary<String, Any>.init()
    ...
    func emptyBlock(effectMsgModel: GalleryMsgModel, errorBlock: @escaping ErrorBlock) {
        ...
        PouchReactiveCompatible.disembarrass(giftUrl: effectMsgModel.effectUrl, path: destroy(model: effectMsgModel)) { [weak self] t in
            //: guard let self = self else {return}
            guard let self = self else { return }
            //: DispatchQueue.global().async {
            DispatchQueue.global().async {
                //: self.afterDownloadGiftAnimat(effectMsgModel: effectMsgModel, isSucess: t, errorBlock: errorBlock)
                self.pending(effectMsgModel: effectMsgModel, isSucess: t, errorBlock: errorBlock)
            }
        }
    }

    //: func afterDownloadGiftAnimat(effectMsgModel: TalkingAnimatMsgModel, isSucess: Bool, errorBlock: @escaping ErrorBlock) {
    func pending(effectMsgModel: GalleryMsgModel, isSucess: Bool, errorBlock: @escaping ErrorBlock) {
        //: if !isSucess {
        if !isSucess {
            //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.DownLoadFailed.rawValue))
            errorBlock(NSError(domain: (String(mainMagnitudeValue.suffix(4))) + "\(app_frameName)" + (String(main_recentData.prefix(4))), code: RatioPriorityTarget.DownLoadFailed.rawValue))
            //: return
            return
            ...
            let downloadPath = destroy(model: effectMsgModel)
            //: let unzipVerifyDir = unzipVerifyDirectory(model: effectMsgModel)
            let unzipVerifyDir = duringStack(model: effectMsgModel)
            ...
            if verifyUnzipSucc {
                ...
                printLog(message: (String(const_lessText) + String(kBrightFosterValue)))
                //: isFailed = true
                isFailed = true
                //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.UnzipFailed.rawValue))
                errorBlock(NSError(domain: (String(mainMagnitudeValue.suffix(4))) + "\(app_frameName)" + (String(main_recentData.prefix(4))), code: RatioPriorityTarget.UnzipFailed.rawValue))
            }
            ...
            if !isFailed {
                ...
                do {
                    ...
                    let md5Data: Data = GreenDisabledFzrsa.loop(NSData(contentsOfFile: keyFilePath) as Data?, pitchDark: dataDiskTitle)
                    ...
                    let md5 = partyPath(filePath: scriptZipFilePath)
                    ...
                    if decryptMD5 != md5, FileManager.default.fileExists(atPath: keyFilePath) {
                        ...
                        errorBlock(NSError(domain: (String(mainMagnitudeValue.suffix(4))) + "\(app_frameName)" + (String(main_recentData.prefix(4))), code: RatioPriorityTarget.VerifyFailed.rawValue))
                    }
                    ...
                    printLog(message: (main_pressMessage.replacingOccurrences(of: "symbolic", with: "A") + String(notiMomentNarrowText)))
                    //: isFailed = true
                    isFailed = true
                    //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.VerifyFailed.rawValue))
                    errorBlock(NSError(domain: (String(mainMagnitudeValue.suffix(4))) + "\(app_frameName)" + (String(main_recentData.prefix(4))), code: RatioPriorityTarget.VerifyFailed.rawValue))
            ...
            if !isFailed {
                ...
                let unzipTmpDir = outputBy(model: effectMsgModel)
                ...
                //: else {
                    ...
                    printLog(message: (String(mainPerformSomebodyName) + String(user_midNoteText) + String(appEyeFormat)))
                    //: isFailed = true
                    isFailed = true
                    //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.UnzipFailed.rawValue))
                    errorBlock(NSError(domain: (String(mainMagnitudeValue.suffix(4))) + "\(app_frameName)" + (String(main_recentData.prefix(4))), code: RatioPriorityTarget.UnzipFailed.rawValue))
    ...
    func neutronStar(path: String) -> Bool {
    ...
    func accelerateSpan(type: HelpComparable) -> String {
        //: var tmpIdentify = ""
        var tmpIdentify = ""
        //: switch type {
        switch type {
        //: case .Gift:     tmpIdentify =  "gift"
        case .Gift: tmpIdentify = (dataNumbMillCertainMessage.replacingOccurrences(of: "feedback", with: "gi"))
        //: break
        //: default:
        default:
            //: break
    ...
    func destroy(model: GalleryMsgModel) -> String {
        ...
        let downloadTmpPath = String(format: (String(show_lowerFormat.prefix(6)) + String(mainPlainDomainThreadKey) + String(constBeneathName)), NSTemporaryDirectory(), accelerateSpan(type: model.effectType), model.eid)
        //: return downloadTmpPath
        return downloadTmpPath
    }

    //: func unzipVerifyDirectory(model: TalkingAnimatMsgModel)->String {
    func duringStack(model: GalleryMsgModel) -> String {
        ...
        let unzipVerifyDirectory = String(format: (String(noti_appearanceText) + String(app_safetyText.prefix(5)) + "d_unzip" + String(const_denyTitle)), NSTemporaryDirectory(), accelerateSpan(type: model.effectType), model.eid)
        //: return unzipVerifyDirectory
        return unzipVerifyDirectory
    }

    //: func unzipTmpDirectory(model: TalkingAnimatMsgModel)->String {
    func outputBy(model: GalleryMsgModel) -> String {
        ...
        let unzipTmpDirectory = String(format: (String(showWellUrl) + String(show_breastUrl)), NSTemporaryDirectory(), accelerateSpan(type: model.effectType), model.eid)
