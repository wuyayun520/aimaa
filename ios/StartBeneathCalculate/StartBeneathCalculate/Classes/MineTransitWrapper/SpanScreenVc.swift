...
fileprivate let user_withinTitle:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Save successfully" :*/
fileprivate let k_sinceValue:[Character] = ["S","a","v","e"," "]
fileprivate let k_safetyPresentationValue:[Character] = ["s","u","c","c","e"]
fileprivate let appDigitalBusinessStr:[Character] = ["s","s","f","u","l","l","y"]
...
class SpanScreenVc: AudienceViewController {
    ...
    init(momentModel: AgencyMomentModel, index: Int, type: MetallicElementPhotoType) {
        ...
        if LocalRouterDescription.shared.direction == .rightToLeft {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.3) {
                ...
                self.collecView.contentOffset = CGPoint(x: curren * Int(main_assetUrl), y: 0)
    ...
    init(images: [UIImage], index: Int, type: MetallicElementPhotoType) {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: myType = type
        myType = type
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: user_withinTitle.reversed(), encoding: .utf8)!)
    }
...
//        }
    ...
    lazy var collecView: UICollectionView = {
        ...
        flayout.itemSize = CGSize(width: main_assetUrl, height: kTopUrl)
        ...
        view.frame = CGRect(x: 0, y: 0, width: main_assetUrl, height: kTopUrl)
        ...
        view.contentOffset = CGPoint(x: pageIndex * Int(main_assetUrl), y: 0)
        //: if myType == .normal {
        if myType == .normal {
            //: view.contentSize = CGSize(width: (currenModel.pic.count )*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (currenModel.pic.count) * Int(main_assetUrl), height: 0)
            //: } else {
        } else {
            //: view.contentSize = CGSize(width: (photos.count)*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (photos.count) * Int(main_assetUrl), height: 0)
        }
        //: view.register(TalkingMomentPhotosPreviewCell.self, forCellWithReuseIdentifier: TalkingMomentPhotosPreviewCell.className())
        view.register(GrossViewCell.self, forCellWithReuseIdentifier: GrossViewCell.className())
...
//    lazy var downloadBtn: SeparateControlReactiveCompatible = {
//        let btn = SeparateControlReactiveCompatible.init()
...
//        ActivityReactiveCompatible.func__openAlbumServiceWithBlock(true) { [weak self] (isOpen: Bool) in
//            if isOpen {
//                guard let self = self else {return}
//                guard  self.pageIndex >= 0 && self.pageIndex < self.currenModel.pic.count  else { return }
//                let model: InmateTransformable = self.currenModel.pic[self.pageIndex]
        ...
        PHPhotoLibrary.shared().performChanges({
            ...
            if isSuccess {
                ...
                DispatchQueue.main.async {
                    ...
                    self.pointOfReferenceMedia(showMsg: (String(k_sinceValue) + String(k_safetyPresentationValue) + String(appDigitalBusinessStr)).localized)
                }
                //: } else {
            } else {
...
extension SpanScreenVc: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    //: func hideTopView() {
    func decide() {
...
//        downloadBtn.isHidden = isHideTopView
    ...
    func eye() {
        //: if  currenModel.sex==FacultyReactiveCompatible.share.loginUserMode.sex {
        if currenModel.sex == FacultyReactiveCompatible.share.loginUserMode.sex {
            //: return
            return
        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell: GrossViewCell = collectionView.dequeueReusableCell(withReuseIdentifier: GrossViewCell.className(), for: indexPath) as! GrossViewCell
        //: if myType == .normal {
        if myType == .normal {
            //: let model: TalkingMomentPhotoModel = currenModel.pic[indexPath.row]
            let model: InmateTransformable = currenModel.pic[indexPath.row]
            //: if model.imgUrl?.count ?? 0 > 1 {
            if model.imgUrl?.count ?? 0 > 1 {
                //: cell.configUrl(url: model.imgUrl! )
                cell.paraUrl(url: model.imgUrl!)
                //: }else {
            } else {
                //: cell.configImage(image: model.imagePic ?? UIImage.init())
                cell.finishJust(image: model.imagePic ?? UIImage())
            }

            //: } else {
        } else {
            //: cell.configImage(image: photos[indexPath.row])
            cell.finishJust(image: photos[indexPath.row])
        }
    ...
    func collectionView(_: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt _: IndexPath) {
        ...
        let tcell: GrossViewCell = cell as! GrossViewCell
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let currentIndex = Int(offSetWidth / main_assetUrl)
        //: if myType == .normal {
        if myType == .normal {
