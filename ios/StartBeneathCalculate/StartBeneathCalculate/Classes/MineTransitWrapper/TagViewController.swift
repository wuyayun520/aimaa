...
class TagViewController: SafetyFactorViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        super.viewWillAppear(animated)
        //: IonV2Listener.shared.func__refreshSysMsg()
        IonV2Listener.shared.clog()
        // 公共聊天室
...
extension TagViewController {
    ...
    func mentionContextModel(indexPath _: IndexPath, conversationModel: MultistageRocketConversationModel) {
        ...
        if conversationModel.isPinned { type = .topList }
        //: IonV2Listener.shared.func__removeConversation(targetId: conversationModel.targetId, dataType: type)
        IonV2Listener.shared.nameType(targetId: conversationModel.targetId, dataType: type)
    ...
    func beanScene(conversationModel: MultistageRocketConversationModel) {
        ...
        let isTop = !conversationModel.isPinned
        //: IonV2Listener.shared.func__setConversationTop(convModel: conversationModel, isTop: isTop)
        IonV2Listener.shared.top(convModel: conversationModel, isTop: isTop)
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if curType == .All {
            ...
            if model.chatType == .group { // 群聊
                ...
                //: SmartThen.share.func__pushToWebVC(webViewType: .FAQ)
                SmartThen.share.joinType(webViewType: .FAQ)
                ...
                //: if model.targetId == MacroBorderDefineThen.getXiaoMiID() {
                if model.targetId == MacroBorderDefineThen.seethe() { // 系统消息
                ...
                //: SmartThen.share.func__pushToPriveteChatVC(chatID: model.targetId, isFrom: isFrom) { vc in
                SmartThen.share.aboveCompletion(chatID: model.targetId, isFrom: isFrom) { vc in
...
extension TagViewController {
    ...
    func originalModel(model: MultistageRocketConversationModel) {
        ...
        guard model.onlyOneRoomId.isEmptyString else {
            //: SmartThen.share.func__pushToGroupChat(groupId: model.onlyOneRoomId)
            SmartThen.share.patentId(groupId: model.onlyOneRoomId)
            //: return
            return
        }
