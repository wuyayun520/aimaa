...
fileprivate let kSignalMsg:[Character] = ["i","c","o","n","_","k","o","n","g","_","k","o","n","g","_","d","e","f","a","u","l","t"]

/*: "page" :*/
fileprivate let app_complaintData:[UInt8] = [0x1c,0xd,0xb,0x9]

/*: "uid" :*/
fileprivate let notiBusinessFormat:[UInt8] = [0x7e,0x62,0x6f]

private func chamberSure(contribution num: UInt8) -> UInt8 {
    return num ^ 11
}

/*: "list" :*/
fileprivate let constZoneName:[Character] = ["l","i","s","t"]
...
class SpanViewController: SafetyFactorViewController {
    //: private var pageIndex = 0
    private var pageIndex = 0
    //: private var postDataArr = [TalkingUserInfoMomentItemModel]()
    private var postDataArr = [VisualItemModel]()
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collectionView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: const_nearbyMessage, right: 0)
        //: collectionView.register(TalkingPostItemCell.self, forCellWithReuseIdentifier: TalkingPostItemCell.className())
        collectionView.register(BroadView.self, forCellWithReuseIdentifier: BroadView.className())
    ...
    private lazy var emptyView: UIView = {
        ...
        img.image = UIImage.adName(name: (String(kSignalMsg)))
...
extension SpanViewController {
    ...
    private func version() {
        ...
        let params = [String(bytes: app_complaintData.map{$0^108}, encoding: .utf8)!: pageIndex, String(bytes: notiBusinessFormat.map{chamberSure(contribution: $0)}, encoding: .utf8)!: userInfoModel.uid] as [String: Any]
        //: TalkingMomentRequestTool.req_UserMomentList(params: params) { [weak self] succeed, result, errorModel in
        EndBuilderLifecycle.caliginous(params: params) { [weak self] succeed, result, _ in
            //: guard let self = self else { return }
            guard let self = self else { return }
            ...
            if let resultArr = resultDict[(String(constZoneName))] as? [Any] {
                ...
                for dict in resultArr {
                    ...
                    if let model = AgencyMomentModel.deserialize(from: dict as? Dictionary) {
...
extension SpanViewController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: BroadView.className(), for: indexPath) as! BroadView
    ...
    func collectionView(_: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        var detailModel = AgencyMomentModel()
        ...
        let vc = AcrossMediaDuration(model: detailModel)
...
extension SpanViewController: JXPagingViewListViewDelegate {
...
extension SpanViewController {
    ...
    func panelController(_ userModel: SeekGoMeasurable) {
