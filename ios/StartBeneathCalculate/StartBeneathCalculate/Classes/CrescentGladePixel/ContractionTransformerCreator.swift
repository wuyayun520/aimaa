...
/*: "rYMsgType" :*/
fileprivate let k_sizeMsg:String = "meet offer app lab recordingrYMs"
fileprivate let main_exSomethingStr:[Character] = ["g","T","y","p","e"]

/*: "GJ:CallCustom" :*/
fileprivate let dataSelectId:[Character] = ["G","J",":","C","a","l","l","C","u"]
fileprivate let kSourceClearText:String = "dismissom"
...
// __CLOSE_PRINT__
//
//  ContractionTransformerCreator.swift
//  AbroadTalking
//
...
class ContractionTransformerCreator: NSObject {
    //: private var showView: TalkingMessageNotificationView?
    private var showView: SaverView? // 当前View
    //: private var preView: TalkingMessageNotificationView?
    private var preView: SaverView? // 旧的View
    ...
    static var shared = ContractionTransformerCreator()
    ...
    override private init() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(trenchantThemeAccountingData),
    ...
    /// 清除数据
    //: @objc func clearData() {
    @objc func trenchantThemeAccountingData() {
        //: if self.preView != nil {
        if self.preView != nil {
            //: self.preView?.dismiss()
            self.preView?.angleBy()
        }
        //: if self.showView != nil {
        if self.showView != nil {
            //: self.showView?.dismiss()
            self.showView?.angleBy()
        }
    ...
    private lazy var msgArr: [MultistageRocketConversationModel] = //: return [MultistageRocketConversationModel]()
        .init()
    //: }()
}
...
extension ContractionTransformerCreator {
    /// 处理接收到的TX消息，展示消息通知
    /// - Parameters:
    ///   - msg: V2TIMMessage
    ///   - dataDict: 消息字典
    //: func onRecvNewMessage(msg: V2TIMMessage, dataDict: [String: Any]) {
    func goReport(msg: V2TIMMessage, dataDict: [String: Any]) {
        ...
        guard join() == true || without() == true else { return }
        //: guard msg.userID != AnimateUnaryAllocator.getCustomerServiceID() && msg.userID != AnimateUnaryAllocator.getXiaoMiID() else {
        guard msg.userID != AnimateUnaryAllocator.receiveRemove(), msg.userID != AnimateUnaryAllocator.seethe() else {
            //: return
            return
        }
        //: let contenType = TUISocialChatManager.messageCellType(dataDict)
        let contenType = CivilityReactiveCompatible.prompt(dataDict)
        // 提示消息
        ...
        if contenType == .TXMessageContentType_text, dataDict[(String(k_sizeMsg.suffix(4)) + String(main_exSomethingStr))] as? String == (String(dataSelectId) + kSourceClearText.replacingOccurrences(of: "dismiss", with: "st")) {
            //: return
            return
        }

        // 2. V2TIMMessage => MultistageRocketConversationModel
        ...
        //: if let userInfo: ConsumerPenHandyJSON = DBUserInfoManager.cache_getCachedUserInfo(targetId: model.targetId) {
        if let userInfo: ConsumerPenHandyJSON = DeployMatrixDelegate.biologicalAgent(targetId: model.targetId) {
    ...
    private func west() {
        ...
        let notifView = SaverView(frame: .zero)
        //: notifView.show(firstModel)
        notifView.child(firstModel)
    ...
    private func without() -> Bool {
        //: guard let currentVC = currentViewController() else { return false }
        guard let currentVC = field() else { return false }
        //: if currentVC.isKind(of: TalkingSocialViewController.self) ||
        if currentVC.isKind(of: DigitalWantViewDelegate.self) ||
            ...
            currentVC.isKind(of: ThreeViewController.self) ||
            //: currentVC.isKind(of: TalkingMeViewController.self) || currentVC.isKind(of: RandomMatchVC.self) {
            currentVC.isKind(of: RetainAgoViewController.self) || currentVC.isKind(of: LiteralMatchVc.self)
        {
            //: return true
            return true
        }
