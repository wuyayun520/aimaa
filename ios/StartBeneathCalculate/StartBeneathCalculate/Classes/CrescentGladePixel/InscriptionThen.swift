...
class InscriptionThen: NSObject {
    ...
    static var shared = InscriptionThen()
...
extension InscriptionThen {
    ...
    func goReport(msg: V2TIMMessage, dataDict: [String: Any]) {
        ...
        //: guard msg.userID != MacroBorderDefineThen.getCustomerServiceID() && msg.userID != MacroBorderDefineThen.getXiaoMiID() else {
        guard msg.userID != MacroBorderDefineThen.receiveRemove(), msg.userID != MacroBorderDefineThen.seethe() else {
            //: return
            return
        }
        ...
        if let userInfo: ConsumerPenHandyJSON = InertGasThen.biologicalAgent(targetId: model.targetId) {
    ...
    private func join() -> Bool {
        ...
        if currentVC.isKind(of: AdvancedErrorDelegate.self) ||
            ...
            currentVC.isKind(of: MileageThen.self)
        {
            //: return true
            return true
        }
