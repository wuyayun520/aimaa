...
class MotionViewController: SafetyFactorViewController {
    ...
    var DataSource: [LitePivotalNotification] = []
    //: var limit = 20
    var limit = 20
    ...
    lazy var MainTable: UITableView = {
        ...
        table.register(AutoCustomOperation.self, forCellReuseIdentifier: AutoCustomOperation.className())
    ...
    func flicker() {
        ...
        PouchReactiveCompatible.should(params: dict) { succeed, result, _ in
            ...
            if succeed {
                ...
                var dataArr: [LitePivotalNotification] = []
                //: if let datas = Array<TalkingVideoCallRecordModel>.deserialize(from: array as? Array) {
                if let datas = Array<LitePivotalNotification>.deserialize(from: array as? Array) {
                    //: dataArr.append(contentsOf: (datas as? [TalkingVideoCallRecordModel])!)
                    dataArr.append(contentsOf: (datas as? [LitePivotalNotification])!)
                }
                //: if self.pageIndex == 0 {
                if self.pageIndex == 0 {
...
extension MotionViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let identifier = AutoCustomOperation.className()
        //: var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? TalkingVideoCallTableCell
        var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? AutoCustomOperation
        //: if cell == nil {
        if cell == nil {
            //: cell = TalkingVideoCallTableCell(style: .default, reuseIdentifier: identifier)
            cell = AutoCustomOperation(style: .default, reuseIdentifier: identifier)
        }
        //: let model: TalkingVideoCallRecordModel = self.DataSource[indexPath.row]
        let model: LitePivotalNotification = self.DataSource[indexPath.row]
