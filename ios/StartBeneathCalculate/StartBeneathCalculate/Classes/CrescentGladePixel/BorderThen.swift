...
@objcMembers public class BorderThen: NSObject {
    ...
    private func anyDateIndex(
        ...
        for textInfo in attrArray {
            ...
            if type.lowercased() == String(bytes: kReBorderName.map{applyMatch(via: $0)}, encoding: .utf8)! {
                ...
                //: if self.containImg {
                if self.containImg {
                    //: let width = max(180, MacroBorderDefineThen.getScreenWidth() - 150)
                    let width = max(180, MacroBorderDefineThen.areaWidth() - 150)
