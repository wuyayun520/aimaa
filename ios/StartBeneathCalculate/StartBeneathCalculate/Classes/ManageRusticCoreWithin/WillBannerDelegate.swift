...
fileprivate let notiApplyCantUrl:[UInt8] = [0xef,0xf4,0xef,0xfa,0xae,0xe9,0xf5,0xea,0xeb,0xf8,0xc0,0xaf,0xa6,0xee,0xe7,0xf9,0xa6,0xf4,0xf5,0xfa,0xa6,0xe8,0xeb,0xeb,0xf4,0xa6,0xef,0xf3,0xf6,0xf2,0xeb,0xf3,0xeb,0xf4,0xfa,0xeb,0xea]

fileprivate func wellCrow(movie num: UInt8) -> UInt8 {
...
/*: "JXBannerCellID" :*/
fileprivate let appEnoughId:[Character] = ["J","X","B","a","n","n","e","r"]
fileprivate let k_sunTitle:String = "general manager odd bolt sceneCellID"

/*: "#FFFFFF" :*/
fileprivate let main_primaryContent:[Character] = ["#","F","F","F","F"]
fileprivate let k_accountingPath:[Character] = ["F","F"]

/*: "#BDB8B5" :*/
fileprivate let constBounceOnKey:String = "#BDB8B5reward bound marketing"
...
protocol AlsoObjectProtocol: NSObjectProtocol {
    // 广告cell点击
...
class WillBannerDelegate: UICollectionReusableView {
    //: weak var delegate: SocialPopularListFooterViewProtocol?
    weak var delegate: AlsoObjectProtocol?

    //: var adBannderListData: [SocialAdBannerModel]?
    var adBannderListData: [LackTransformable]? // 广告banner数据

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: notiApplyCantUrl.map{wellCrow(movie: $0)}, encoding: .utf8)!)
    }
...
extension WillBannerDelegate {
    /// 获取当前视图高度
    /// - Returns: 高度
    //: func getFooterViewHeight() -> CGFloat {
    func paradigmHeight() -> CGFloat {
    ...
    func original(data: [LackTransformable]) {
...
extension WillBannerDelegate: JXBannerDataSource, JXBannerDelegate {
    //: func jxBanner(_ banner: JXBannerType) -> (JXBannerCellRegister) {
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        //: return JXBannerCellRegister(type: JXBannerCell.self, reuseIdentifier: "JXBannerCellID")
        return JXBannerCellRegister(type: JXBannerCell.self, reuseIdentifier: (String(appEnoughId) + String(k_sunTitle.suffix(6))))
    }
                  ...
                  builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder
    {
        ...
        pageControl.activeColor = UIColor(hex: (String(main_primaryContent) + String(k_accountingPath)))!
        //: pageControl.inactiveSize = CGSize(width: 6, height: 6)
        pageControl.inactiveSize = CGSize(width: 6, height: 6)
        //: pageControl.inactiveColor = UIColor(hex: "#BDB8B5")!
        pageControl.inactiveColor = UIColor(hex: (String(constBounceOnKey.prefix(7))))!
                         ...
                         didSelectItemAt index: Int)
    {
        ...
        delegate?.starDistant(model: model)
    }
}
...
extension WillBannerDelegate {
    ...
    private func storageSpace() {
        ...
        bannerView.snp.makeConstraints { make in
            ...
            make.size.equalTo(CGSize(width: main_assetUrl - 30, height: 84))
        }
    }
}
