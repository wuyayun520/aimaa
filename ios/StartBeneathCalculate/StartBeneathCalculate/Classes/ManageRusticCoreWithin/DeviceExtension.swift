...
/*: "RSA加密失败，str:  :*/
fileprivate let showWaitAccessName:String = "RSA加密sale fill"
fileprivate let k_stepStr:[Character] = ["\u{5931}","败","\u{ff0c}","s","t","r",":"," "]

/*: "weixin://" :*/
fileprivate let k_temporaryMessage:String = "wound with partner broadcastweixin:"
fileprivate let dataCleanMsg:[Character] = ["/","/"]

/*: "dingtalk://" :*/
fileprivate let kSafetyMixValue:String = "dinbuilder"

/*: "lark://" :*/
fileprivate let k_routeMsg:String = "lark://sale processing decision"

/*: "wxwork://" :*/
fileprivate let userMarchLightContent:String = "wclipwo"

/*: "deviceId" :*/
fileprivate let noti_broadContent:[UInt8] = [0x34,0x35,0x26,0x39,0x33,0x35,0x19,0x34]

/*: "weixin" :*/
fileprivate let constSmartId:[Character] = ["w","e","i","x","i"]
fileprivate let kConKey:String = "instruction"

/*: "wxwork" :*/
fileprivate let appSolutionAlwaysUrl:[Character] = ["w","x","w","o","r","k"]

/*: "dingtalk" :*/
fileprivate let kSaltMessage:String = "DINGTALK"

/*: "lark" :*/
fileprivate let constMinimizePath:String = "lapeer"
...
extension UIDevice {
    ...
    class func identifierWork(IdentifierBlock: @escaping (_ str: String) -> Void) {
        ...
        if #available(iOS 14.0, *) {
            ...
            ATTrackingManager.requestTrackingAuthorization(completionHandler: { status in
                ...
                if status == .authorized {
                    ...
                    IdentifierBlock(UIDevice.minuteId())
                    //: } else {
                } else {
                    //: IdentifierBlock("")
                    IdentifierBlock("")
...
//            let mnc = carrier?.mobileNetworkCode  //移动网络代码
    ...
    static func bitPush(str: String) -> String {
        //: var encryptedStr = ""
        var encryptedStr = ""
        //: do {
        do {
            ...
            printLog(message: (String(showWaitAccessName.prefix(5)) + String(k_stepStr)) + "\(str).")
        }
        //: return encryptedStr
        return encryptedStr
    }
...
public extension UIDevice {
    ...
    static var isOpenWX: Bool {
        ...
        let appSchema = (String(k_temporaryMessage.suffix(7)) + String(dataCleanMsg))
    ...
    static var isOpenDD: Bool {
        ...
        let appSchema = (kSafetyMixValue.replacingOccurrences(of: "builder", with: "g") + "talk://")
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var isOpenFS: Bool {
        //: let appSchema = "lark://"
        let appSchema = (String(k_routeMsg.prefix(7)))
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var isOpenWXWork: Bool {
        ...
        let appSchema = (userMarchLightContent.replacingOccurrences(of: "clip", with: "x") + "rk://")
        //: let appUrl = URL(string: appSchema)!
        let appUrl = URL(string: appSchema)!
        //: if UIApplication.shared.canOpenURL(appUrl) {
        if UIApplication.shared.canOpenURL(appUrl) {
    ...
    static var customDeviceNum: String {
        ...
        if let value = key.get(noti_allFailureUrl) {
            //: return value
            return value
            //: }else{
        } else {
            ...
            key.set(value, forKey: noti_allFailureUrl)
            //: return value
            return value
        }
    }
    ...
    static var deviceInfoDic: [String: Any] {
        ...
        let usedVpn = RocketRecognizeAddrTool.anyMagnitude() || RocketRecognizeAddrTool.contribution()
        ...
        if let paramsStr = String(data: data!, encoding: String.Encoding.utf8) {
            ...
            let deviceIdStr = bitPush(str: adid)
            //: return ["deviceId": deviceIdStr,
            return [String(bytes: noti_broadContent.map{$0^80}, encoding: .utf8)!: deviceIdStr,
                    //: "p0": paramsStr]
                    "p0": paramsStr]
        }
        //: return [:]
    ...
    static var getApps: String? {
        ...
        if UIDevice.isOpenWX {
            ...
            appsArr.append((String(constSmartId) + kConKey.replacingOccurrences(of: "instruction", with: "n")))
        }
        //: if UIDevice.isOpenWXWork {
        if UIDevice.isOpenWXWork {
            //: appsArr.append("wxwork")
            appsArr.append((String(appSolutionAlwaysUrl)))
        }
        //: if UIDevice.isOpenDD {
        if UIDevice.isOpenDD {
            //: appsArr.append("dingtalk")
            appsArr.append((kSaltMessage.lowercased()))
        }
        //: if UIDevice.isOpenFS {
        if UIDevice.isOpenFS {
            //: appsArr.append("lark")
            appsArr.append((constMinimizePath.replacingOccurrences(of: "peer", with: "rk")))
        }
        //: if appsArr.count > 0 {
        if appsArr.count > 0 {
