...
extension FactorChatManager {
    ...
    func oval(isMatch: Bool = false) {
        ...
        if isMatch {
            ...
            MigrationThen.pathAge(msg: (String(userClearSendKey.prefix(5)) + "随机视频" + noti_investKey))
            //: } else {
        } else {
            //: UploadLogTool.writeLog(msg: "TRTC \((infoModel?.videoStage == .Waiting) ? "发起":"收到")视频通话. logId:\(infoModel?.logId ?? 0).")
            MigrationThen.pathAge(msg: (String(userLiftTravelText.suffix(5))) + "\((infoModel?.videoStage == .Waiting) ? "发起" : "收到")" + (showHomeId + " logId" + String(data_cityText)) + "\(infoModel?.logId ?? 0).")
        }
    ...
    private func halfText() {
        ...
        MigrationThen.pathAge(msg: (constAccordingText.replacingOccurrences(of: "draft", with: "TR") + " 视频\u{901a}" + constClotCantFeedbackId + " log" + String(main_homeOnlyFormat)) + "\(infoModel?.logId ?? 0).")
...
extension FactorChatManager: TRTCCloudDelegate {
    ...
    func onError(_ errCode: TXLiteAVError, errMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        MigrationThen.pathAge(msg: (String(app_docContent.prefix(7)) + showBeginningPanelHeritageId + "errCode:") + "\(errCode)" + (String(dataTransactionName) + String(main_lackName.prefix(6))) + "\(String(describing: errMsg)).")
        //: if errMsg != nil {
        if errMsg != nil {
    ...
    func onWarning(_ warningCode: TXLiteAVWarning, warningMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        MigrationThen.pathAge(msg: (dataLibraryMessage.replacingOccurrences(of: "dancing", with: "RT") + " \u{89c6}频通话S" + String(appAnalyzeKey.prefix(5)) + "告. war" + String(constCommunicationMessage.prefix(9))) + "\(warningCode)" + (String(showWorldName.suffix(4)) + dataBadMsg.lowercased() + "Msg:") + "\(String(describing: warningMsg)).")
    ...
    func onEnterRoom(_ result: Int) {
        ...
        MigrationThen.pathAge(msg: (mainBrokenButtonId.replacingOccurrences(of: "replace", with: "TR") + String(showFormationDragName)) + "\(result)" + (String(userVisualData) + String(kInputUrl.suffix(5))) + "\(infoModel?.logId ?? 0).")
        //: if result < 0 {
        if result < 0 { // 加入失败
