...
/*: "mf/call/GetUserInfo" :*/
fileprivate let app_fatherId:String = "mf/cals cancel"
fileprivate let notiSunName:String = "l/GetUsbetween say eastern fatal"
fileprivate let noti_pureUrl:String = "erInfountil committee"

/*: "uid" :*/
fileprivate let dataPropertyStr:[UInt8] = [0xcb,0xd7,0xda]

/*: "TRTC 随机视频匹配中." :*/
fileprivate let userClearSendKey:String = "TRTC gold leading single"
fileprivate let noti_investKey:String = "匹\u{914d}中."

/*: "TRTC  :*/
fileprivate let userLiftTravelText:String = "something portrait flash square pullTRTC "

/*: 视频通话. logId: :*/
fileprivate let showHomeId:String = "视\u{9891}通话."
fileprivate let data_cityText:[Character] = [":"]

/*: "TRTC 视频通话销毁. logId： :*/
fileprivate let constAccordingText:String = "draftTC"
fileprivate let constClotCantFeedbackId:String = "话销\u{6bc1}."
fileprivate let main_homeOnlyFormat:[Character] = ["I","d","："]

/*: "TRTC 视频通话错误. errCode: :*/
fileprivate let app_docContent:String = "TRTC 视频op section strip between quit"
fileprivate let showBeginningPanelHeritageId:String = "通话错\u{8bef}. "

/*: , errMsg: :*/
fileprivate let dataTransactionName:[Character] = [","," ","e"]
fileprivate let main_lackName:String = "rrMsg:stop terms"

/*: "TRTC 视频通话SDK收到警告. warningCode: :*/
fileprivate let dataLibraryMessage:String = "TdancingC"
fileprivate let appAnalyzeKey:String = "DK收到警cur clearly beauty tendency multiple"
fileprivate let constCommunicationMessage:String = "ningCode:income entity pic ready bag"

/*: , warningMsg: :*/
fileprivate let showWorldName:String = "answer seat, wa"
fileprivate let dataBadMsg:String = "RNING"

/*: "TRTC 视频通话进入房间. result: :*/
fileprivate let mainBrokenButtonId:String = "replace"
fileprivate let showFormationDragName:[Character] = ["T","C"," ","视","频","\u{901a}","话","进","入","房","间","."," ","r","e","s","u","l","t",":"]

/*: , logId： :*/
fileprivate let userVisualData:[Character] = [","," ","l"]
fileprivate let kInputUrl:String = "mutual magnitude between demandogId："

/*:  加入了房间" :*/
fileprivate let k_failureContent:String = " "
fileprivate let userVersionName:String = "加\u{5165}了房\u{95f4}"

/*:  离开了房间" :*/
fileprivate let userNothingMessage:[Character] = [" ","离","\u{5f00}","了","房","间"]
...
class TvChatConfig: NSObject {
    ...
    var remoteCameraStatus = ArrayCameraStatus.Normal // 对方摄像头状态
}

//: class TalkingVideoChatManager: NSObject {
class FactorChatManager: NSObject {
    //: var infoModel: TalkingVideoChatModel?
    var infoModel: ControlChatModel? // 视频通话model
    //: var config = TalkingVideoChatConfig()
    var config = TvChatConfig() // 视频渲染配置类
...
extension FactorChatManager {
    /// 开始视频速配（男）
    /// - Parameters:
    ///   - uid: 接收者的id
    ///   - completion: 回调
    //: class func req_callGetUserInfo(uid: String, completion: @escaping FinishBlock) {
    class func designReceive(uid: String, completion: @escaping FinishBlock) {
        //: let reqModel = TalkingRequestModel.init()
        let reqModel = AgileCrucialTransformer()
        //: reqModel.requestPath = "mf/call/GetUserInfo"
        reqModel.requestPath = (String(app_fatherId.prefix(6)) + String(notiSunName.prefix(7)) + String(noti_pureUrl.prefix(6)))
        //: reqModel.requestType = .GET
        reqModel.requestType = .GET
        //: reqModel.params = ["uid": uid]
        reqModel.params = [String(bytes: dataPropertyStr.map{$0^190}, encoding: .utf8)!: uid]
...
extension FactorChatManager {
    ...
    func evaluate(_ frontCamera: Bool) {
        ...
        self.command(front: frontCamera)
    }

    /// 摄像头开启与关闭
    /// - Parameter openCamera: 是否开启
    //: func useCamera(_ openCamera: Bool) {
    func reuse(_ openCamera: Bool) {
        ...
        self.descriptionRef(isOpen: openCamera)
    }
    ...
    func blink() {
        ...
        DispatchQueue.main.async {
            ...
            self.startChange(smallWindow: self.infoModel?.renderPostion != .selfOnBg)
...
extension FactorChatManager {
    ...
    private func command(front: Bool) {
    ...
    private func descriptionRef(isOpen: Bool) {
        //: if isOpen {
        if isOpen {
...
extension FactorChatManager {
    ...
    func ofSet() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(zoneSquare(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(representation(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(blankNotif(notif:)),
    ...
    @objc private func zoneSquare(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
            // 静音
    ...
    @objc private func representation(notif: Notification?) {
        //: if notif == nil {
    ...
    @objc private func blankNotif(notif: Notification) {
    ...
    @objc func the(notif: Notification) {
...
extension FactorChatManager {
    ...
    func oval(isMatch: Bool = false) {
        //: if isMatch {
        if isMatch {
            //: UploadLogTool.writeLog(msg: "TRTC 随机视频匹配中.")
            ElasticActiveEffect.pathAge(msg: (String(userClearSendKey.prefix(5)) + "随机视频" + noti_investKey))
            //: } else {
        } else {
            //: UploadLogTool.writeLog(msg: "TRTC \((infoModel?.videoStage == .Waiting) ? "发起":"收到")视频通话. logId:\(infoModel?.logId ?? 0).")
            ElasticActiveEffect.pathAge(msg: (String(userLiftTravelText.suffix(5))) + "\((infoModel?.videoStage == .Waiting) ? "发起" : "收到")" + (showHomeId + " logId" + String(data_cityText)) + "\(infoModel?.logId ?? 0).")
        }
        //: if self.infoModel?.videoStage == .Waiting {
        if self.infoModel?.videoStage == .Waiting {
            //: startTRTCPreview(smallWindow: false)
            startChange(smallWindow: false)
    ...
    func startChange(smallWindow: Bool) {
    ...
    private func enterWeight() {
        ...
        params.sdkAppId = dataInfoCleanMsg
        //: params.roomId = UInt32(roomId)
        params.roomId = UInt32(roomId)
        //: params.userId = FacultyReactiveCompatible.share.loginUserMode.userID
        params.userId = FacultyReactiveCompatible.share.loginUserMode.userID
        //: params.userSig = FacultyReactiveCompatible.share.appUserConfigMode.uSigTrtc
        params.userSig = FacultyReactiveCompatible.share.appUserConfigMode.uSigTrtc
    ...
    private func halfText() {
        //: UploadLogTool.writeLog(msg: "TRTC 视频通话销毁. logId：\(infoModel?.logId ?? 0).")
        ElasticActiveEffect.pathAge(msg: (constAccordingText.replacingOccurrences(of: "draft", with: "TR") + " 视频\u{901a}" + constClotCantFeedbackId + " log" + String(main_homeOnlyFormat)) + "\(infoModel?.logId ?? 0).")
...
extension FactorChatManager: TRTCCloudDelegate {
    ...
    func onError(_ errCode: TXLiteAVError, errMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        ElasticActiveEffect.pathAge(msg: (String(app_docContent.prefix(7)) + showBeginningPanelHeritageId + "errCode:") + "\(errCode)" + (String(dataTransactionName) + String(main_lackName.prefix(6))) + "\(String(describing: errMsg)).")
        //: if errMsg != nil {
        if errMsg != nil {
            //: self.func__showStatusBarErrorMsg(showMsg: errMsg!)
            self.micromicronProcess(showMsg: errMsg!)
        }
    }
    ...
    func onWarning(_ warningCode: TXLiteAVWarning, warningMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        ElasticActiveEffect.pathAge(msg: (dataLibraryMessage.replacingOccurrences(of: "dancing", with: "RT") + " \u{89c6}频通话S" + String(appAnalyzeKey.prefix(5)) + "告. war" + String(constCommunicationMessage.prefix(9))) + "\(warningCode)" + (String(showWorldName.suffix(4)) + dataBadMsg.lowercased() + "Msg:") + "\(String(describing: warningMsg)).")
        ...
        if self.config.remoteCameraStatus == .Normal {
            ...
            NotificationCenter.default.post(name: show_purchaseFormat, object: self)
        }
    }
    ...
    func onEnterRoom(_ result: Int) {
        ...
        ElasticActiveEffect.pathAge(msg: (mainBrokenButtonId.replacingOccurrences(of: "replace", with: "TR") + String(showFormationDragName)) + "\(result)" + (String(userVisualData) + String(kInputUrl.suffix(5))) + "\(infoModel?.logId ?? 0).")
        //: if result < 0 {
        if result < 0 { // 加入失败
            ...
            ConfigureAxisService.shared.loadBig()
            //: } else {
        } else {
            //: self.config.disconnected = false
            self.config.disconnected = false
    ...
    func onRemoteUserEnterRoom(_ userId: String) {
        ...
        printLog(message: "\(userId)" + (k_failureContent.capitalized + userVersionName))
    }

    //: func onRemoteUserLeaveRoom(_ userId: String, reason: Int) {
    func onRemoteUserLeaveRoom(_ userId: String, reason _: Int) {
        //: printLog(message: "\(userId) 离开了房间")
        printLog(message: "\(userId)" + (String(userNothingMessage)))
    }
    ...
    func onUserVideoAvailable(_ userId: String, available: Bool) {
        //: guard userId != FacultyReactiveCompatible.share.loginUserMode.userID else { return }
        guard userId != FacultyReactiveCompatible.share.loginUserMode.userID else { return }
