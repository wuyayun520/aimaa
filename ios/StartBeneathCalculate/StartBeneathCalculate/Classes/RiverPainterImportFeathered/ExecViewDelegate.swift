...
fileprivate let notiTotalId:[UInt8] = [0xf3,0xf4,0xf3,0xee,0xb2,0xf9,0xf5,0xfe,0xff,0xe8,0xa0,0xb3,0xba,0xf2,0xfb,0xe9,0xba,0xf4,0xf5,0xee,0xba,0xf8,0xff,0xff,0xf4,0xba,0xf3,0xf7,0xea,0xf6,0xff,0xf7,0xff,0xf4,0xee,0xff,0xfe]

/*: "Add more details of yourself" :*/
fileprivate let data_backyardMsg:[Character] = ["A","d","d"," ","m","o","r","e"," ","d","e"]
fileprivate let main_draftStaffUrl:[Character] = ["t","a","i","l","s"," ","o","f"," ","y","o","u","r","s","e","l","f"]

/*: "#999999" :*/
fileprivate let dataTalkUrl:String = "#999999"

/*: "0/500" :*/
fileprivate let userPathData:String = "end/5endend"

/*: "Tip:Don't post content that induces others to send gifts or other money-relatsed content." :*/
fileprivate let kWealthyMsg:[UInt8] = [0x40,0x7d,0x64,0x2e,0x50,0x7b,0x7a,0x33,0x60,0x34,0x64,0x7b,0x67,0x60,0x34,0x77,0x7b,0x7a,0x60,0x71,0x7a,0x60,0x34,0x60,0x7c,0x75,0x60,0x34,0x7d,0x7a,0x70,0x61,0x77,0x71,0x67,0x34,0x7b,0x60,0x7c,0x71,0x66,0x67,0x34,0x60,0x7b,0x34,0x67,0x71,0x7a,0x70,0x34,0x73,0x7d,0x72,0x60,0x67,0x34,0x7b,0x66,0x34,0x7b,0x60,0x7c,0x71,0x66,0x34,0x79,0x7b,0x7a,0x71,0x6d,0x39,0x66,0x71,0x78,0x75,0x60,0x67,0x71,0x70,0x34,0x77,0x7b,0x7a,0x60,0x71,0x7a,0x60,0x3a]

private func frontMilitary(same num: UInt8) -> UInt8 {
    return num ^ 20
}
...
class ExecViewDelegate: UITableViewCell {
    //: let limitCount = 500
    let limitCount = 500
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(relate(notification:)), name: UIResponder.keyboardWillHideNotification, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(switchce(notification:)), name: UIResponder.keyboardWillShowNotification, object: nil)
    }

    //: required init?(coder aDecoder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: notiTotalId.map{$0^154}, encoding: .utf8)!)
    }
    ...
    lazy var titleLB: UILabel = {
        ...
        lb.text = (String(data_backyardMsg) + String(main_draftStaffUrl)).localized
        //: return lb
        return lb
        //: }()
    }()
    ...
    lazy var numberLB: UILabel = {
        ...
        LB.textColor = UIColor(hex: (dataTalkUrl.capitalized))
        //: LB.font = UIFont.pingfangFont(type: .Regular, fontSize: 14)
        LB.font = UIFont.recipeDecision(type: .Regular, fontSize: 14)
        //: LB.text = "0/500"
        LB.text = (userPathData.replacingOccurrences(of: "end", with: "0"))
    ...
    lazy var textView: GenerationTitleView = {
        ...
        //: let sign = FacultyReactiveCompatible.share.loginUserMode.signature
        let sign = FacultyReactiveCompatible.share.loginUserMode.signature
        //: textView.text = sign
        textView.text = sign
        //: textView.placeholder = "Tip:Don't post content that induces others to send gifts or other money-relatsed content.".localized
        textView.placeholder = String(bytes: kWealthyMsg.map{frontMilitary(same: $0)}, encoding: .utf8)!.localized
        //: textView.placeholderColor = UIColor.appValueDetailColor()
        textView.placeholderColor = UIColor.keepRid()
...
extension ExecViewDelegate {
    //: @objc func keyboardWillBeHidden(notification: NSNotification) {
    @objc func relate(notification _: NSNotification) {
    ...
    @objc func switchce(notification: NSNotification) {
...
extension ExecViewDelegate: UITextViewDelegate {
    ...
    public func textViewDidChange(_ textView: UITextView) {
        ...
        if textView.text.count > limitCount {
            ...
            if textNum > limitCount && limitCount > 0 {
                ...
                textView.text = asset(index: limitCount, text: textContent)
            }
        }
        //: numberLB.text =  "\(textView.text.count)/\(limitCount)"
        numberLB.text = "\(textView.text.count)/\(limitCount)"
    ...
    private func asset(index: Int, text: String) -> String {
        //: if text.count <= index {
        if text.count <= index {
            //: return text
            return text
