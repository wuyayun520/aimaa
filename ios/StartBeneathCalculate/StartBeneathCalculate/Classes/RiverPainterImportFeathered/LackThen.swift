...
class LackThen: NSObject {
    //: static let shared = TalkingVoiceMsgPlayManager()
    static let shared = LackThen()
...
extension LackThen {
    ...
    func spellModel(playModel: CommunicationHandyJSON) {
            ...
            !dbFilePath.isEmpty && dbFilePath.hasSuffix((String(constPortPeopleTitle)))
        {
            ...
            LocationThen.shared.nativeSize([taskModel])
        }
    }
    ...
    func user(msgArr: [CommunicationHandyJSON]) {
        ...
        for model in msgArr {
            ...
            let bindWrap = SpireThen.familyLayer(with: model.db_voiceCacheWrap.msgId)
            //: if bindWrap != nil {
            if bindWrap != nil {
        ...
        LocationThen.shared.nativeSize(tempArray)
    }
    ...
    func window(filePath: String, msgID: String) {
        ...
        SpireThen.deadlineMsg(model!.db_voiceCacheWrap)
    ...
    func doingAliveUsed() {
        ...
        LocationThen.shared.delegate = self
    }
    ...
    func tar() {
        ...
        LocationThen.shared.delegate = nil
    }
}

//: extension TalkingVoiceMsgPlayManager: AVAudioPlayerDelegate {
extension LackThen: AVAudioPlayerDelegate {
...
extension LackThen: RatioManagerDelegate {
    ...
    func receive(taskModel: RecentTaskModel) {
        ...
        if self.waitPlayID == taskModel.taskId {
            ...
            let warp = SpireThen.familyLayer(with: model.db_voiceCacheWrap.msgId)
            //: if warp != nil {
            if warp != nil {
            ...
            let warp = SpireThen.familyLayer(with: taskModel.taskId)
