...
/*: "Select a date" :*/
fileprivate let const_lawnOptionItsStr:String = "detail mid quit cant inputSele"
fileprivate let kStillPath:String = "thread give comment panel date"
...
// __CLOSE_PRINT__
//
//  StretchCompleteReactiveCompatible.swift
//  AbroadTalking
//
...
protocol GrossObjectProtocol: NSObjectProtocol {
...
class StretchCompleteReactiveCompatible: PeerPickView, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    weak var delegate: GrossObjectProtocol?

    //: override func initPickView() {
    override func handPickLeave() {
        //: super.initPickView()
        super.handPickLeave()

        //: titleString = "Select a date".localized
        titleString = (String(const_lawnOptionItsStr.suffix(4)) + "ct a" + String(kStillPath.suffix(5))).localized
        ...
        before(year: defaultYear!, month: defaultMonth!, day: defaultDay!)
    }

    //: func setDefaultDate(year: Int, month: Int, day: Int) {
    func before(year: Int, month: Int, day: Int) {
        //: currentYear = year
        currentYear = year
        //: if year == defaultYear {
        if year == defaultYear {
...
//        }
    ...
    override func error(button: UIButton) {
        //: super.confirmClick(button: button)
        super.error(button: button)
        //: refreshPickViewData()
        refreshThroughAggregation()

        //: if let day = selectDay {
        if let day = selectDay {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: day)
            delegate?.counter(year: selectYear!, month: selectMonth!, day: day)
            //: } else {
        } else {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: 0)
            delegate?.counter(year: selectYear!, month: selectMonth!, day: 0)
        }
    }
        ...
        if isShowDay {
            ...
            if component == 0 {
                ...
                if currentYear == yearSum + minShowYear - 1 {
                    ...
                    return vantage(year: year, month: month)
                }
                //: } else {
            } else {
                //: return self.yearSum
    ...
    func vantage(year: Int, month: Int) -> Int {
    ...
    func refreshThroughAggregation() {
