...
/*: "#BAFBFF" :*/
fileprivate let main_boxMessage:String = "care here harvest#BAFBF"
fileprivate let showSuspendMsg:String = "decrease"
...
class InterpolateSpineQuery: NSObject {
    ...
    func signature(model: TraceModelType) -> TraceModelType {
        //: self.model = model
        self.model = model
        /// 普通文本消息
        //: if self.model.messageType == 1 {
    ...
    func secondWithinComment() {
        //: let paraStyle = paragraphStyle()
        let paraStyle = airshipTempSetup()
        ...
        let name = objectTarget(text: firstStr, font: UIFont.originally(fontSize: 14), color: UIColor(hex: (String(main_boxMessage.suffix(6)) + showSuspendMsg.replacingOccurrences(of: "decrease", with: "F")))!)
        // 内容
        //: let content = getAttributed(text: self.model.content, font: UIFont.pingfangMediumFont(fontSize: 14), color: UIColor.white)
        let content = objectTarget(text: self.model.content, font: UIFont.originally(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        ...
        notMy(attribText: self.model.msgAttribText)
    }
    ...
    func pitchSuspend() {
        ...
        let content = objectTarget(text: self.model.transContent, font: UIFont.originally(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        //: name.yy_paragraphStyle = paraStyle
        name.yy_paragraphStyle = paraStyle
    ...
    func airshipTempSetup() -> NSMutableParagraphStyle {
    ...
    func objectTarget(text: String, font: UIFont, color: UIColor) -> NSMutableAttributedString {
...
//            attribute.yy_writingDirection = [3]
        ...
        attribute.yy_paragraphStyle = airshipTempSetup()
        //: return attribute
        return attribute
    }
    ...
    func notMy(attribText: NSMutableAttributedString) {
        // 距离左边8  距离右边也为8
        //: let maxWidth = VideoMsgTableViewWidth - 36
        let maxWidth = kRateData - 36
