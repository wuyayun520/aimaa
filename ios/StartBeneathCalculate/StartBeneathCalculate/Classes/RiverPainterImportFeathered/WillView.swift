...
fileprivate let user_packageKey:[UInt8] = [0x46,0x41,0x46,0x5b,0x7,0x4c,0x40,0x4b,0x4a,0x5d,0x15,0x6,0xf,0x47,0x4e,0x5c,0xf,0x41,0x40,0x5b,0xf,0x4d,0x4a,0x4a,0x41,0xf,0x46,0x42,0x5f,0x43,0x4a,0x42,0x4a,0x41,0x5b,0x4a,0x4b]

private func warningEffGreet(fire num: UInt8) -> UInt8 {
    return num ^ 47
}

/*: "img_home_shadow" :*/
fileprivate let noti_edgeName:[Character] = ["i","m","g","_","h","o","m","e","_","s","h"]
fileprivate let show_migrationValue:String = "adtransactionw"

/*: "#9610FF" :*/
fileprivate let showAdjustmentPath:String = "#9610Fpublic thumb daily launch"
fileprivate let noti_extentContextStrongText:String = "may"

/*: "#8566FF" :*/
fileprivate let main_thereKey:[Character] = ["#","8","5","6","6","F","F"]

/*: "Random Video" :*/
fileprivate let main_withoutData:String = "what withinRandom"
fileprivate let notiInjuryInsideMsg:[Character] = ["o"]

/*: "icon_match_call_free" :*/
fileprivate let data_accountingMsg:String = "look trade transfer zzicon_m"
fileprivate let data_hereValue:[Character] = ["a","l","l","_","f","r","e","e"]

/*: "Free" :*/
fileprivate let kLeaveMsg:String = "urban string above flowFree"

/*: "Scroll_upto_discove" :*/
fileprivate let userTheoryId:String = "Scrollorigin incentive read"
fileprivate let notiYetContent:String = "have smootho_dis"

/*: "icon_VectorStroke" :*/
fileprivate let notiSufficientKey:String = "policy slip register thanicon_V"
fileprivate let notiDefineData:String = "eentitle"
fileprivate let showMagnitudeerruptId:String = "torSissue imagine seek rounding"

/*: "get json error" :*/
fileprivate let data_rejectRegularStr:String = "option acceptget j"
fileprivate let user_successfulMsg:String = "etagor"
...
class WillView: UIView {
    ...
    var maxRemoveDistance = main_assetUrl / 4.0 /// 最大移除距离(默认屏幕的1/4)
    //: var reusableCells = [TalkingClubCardViewCell]()
    var reusableCells = [RetainView]() /// 重用卡片数组
    //: var isAlpha = true
    ...
    var dataSource: AllDataSource? /// 数据源
    //: var delegate: TalkingClubCardViewDelegate?
    var delegate: RetainHelpViewDelegate?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: user_packageKey.map{warningEffGreet(fire: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var leftbackImageView: UIImageView = {
        ...
        imag.image = UIImage.adName(name: (String(noti_edgeName) + show_migrationValue.replacingOccurrences(of: "transaction", with: "o")))
    ...
    private lazy var randomBtn: UIButton = {
        ...
        btn.setBackgroundImage(UIImage.announcementSize(colors: [UIColor(hex: (String(showAdjustmentPath.prefix(6)) + noti_extentContextStrongText.replacingOccurrences(of: "may", with: "F")))!.cgColor, UIColor(hex: (String(main_thereKey)))!.cgColor], size: CGSize(width: main_assetUrl - 30, height: 54)), for: .normal)
        //: btn.layer.cornerRadius = 27
        btn.layer.cornerRadius = 27
        //: btn.layer.masksToBounds = true
        btn.layer.masksToBounds = true
        //: btn.setTitle("Random Video".localized, for: .normal)
        btn.setTitle((String(main_withoutData.suffix(6)) + " Vide" + String(notiInjuryInsideMsg)).localized, for: .normal)
        //: btn.addTarget(self, action: #selector(randomBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(springAcross), for: .touchUpInside)
        //: self.addSubview(btn)
        self.addSubview(btn)
    ...
    lazy var freeImag: UIButton = {
        ...
        img.setBackgroundImage(UIImage.adName(name: (String(data_accountingMsg.suffix(6)) + "atch_c" + String(data_hereValue))), for: .normal)
        //: img.setTitle("Free".localized, for: .normal)
        img.setTitle((String(kLeaveMsg.suffix(4))).localized, for: .normal)
...
extension WillView {
    ...
    func provide() {
        //: showGuidePage()
        pageMisguideBringHome()
    }

    //: private func showGuidePage() {
    private func pageMisguideBringHome() {
        ...
        image.image = UIImage.adName(name: (String(userTheoryId.prefix(6)) + "_upt" + String(notiYetContent.suffix(5)) + "cove"))
        ...
        image2.image = UIImage.adName(name: (String(notiSufficientKey.suffix(6)) + notiDefineData.replacingOccurrences(of: "entitle", with: "c") + String(showMagnitudeerruptId.prefix(4)) + "troke"))
        ...
        anyDirection(player: player)
    }

    //: private func likeplayer(player: SVGAPlayer) {
    private func anyDirection(player: SVGAPlayer) {
        //: do {
        do {
            //: let url = SVGAEffectTool.default.getZipEffectPath(type: .Club_guidePage)
            let url = TaEffectTool.default.towardPath(type: .Club_guidePage)
            ...
            printLog(message: (String(data_rejectRegularStr.suffix(5)) + "son " + user_successfulMsg.replacingOccurrences(of: "tag", with: "rr")))
        }
    }

    //: @objc private func randomBtnClick() {
    @objc private func springAcross() {
        //: self.delegate?.clubCardRandowBtnClick?()
        self.delegate?.manageClick?()
    }

    //: func reloadDataAnimated(anim: Bool) {
    func tailTotalerpretationBackyard(anim: Bool) {
        //: moveCount = 0
        moveCount = 0
        //: self.currentIndex = 0
        self.currentIndex = 0
        ...
        let maxCount = dataSource?.doing(cardView: self) ?? 0
    ...
    func decrease(index: Int) {
        ...
        let cell = dataSource?.frank(cardView: self, index: index)
...
//        let showCount = Double(self.visibleCount - 1)
        ...
        containerView.insertSubview(cell ?? RetainView(), at: 0)
    ...
    func outlineNearbySince(anim: Bool) {
        ...
        for i in 0 ... count - 1 {
            ...
            let cell: RetainView = self.visibleCells[i] as! RetainView
            ...
            if isVisbleLast {
                ...
                self.delegate?.usAccessContact?(cardViewCell: cell, forRowAtIndex: self.currentIndex - i)
            }

            //: if anim {
            if anim {
                //: updateConstraintsCell(cell: cell, transform: transform)
                flushFoot(cell: cell, transform: transform)
                //: } else {
            } else {
    ...
    func flushFoot(cell: RetainView, transform: CGAffineTransform) {
    ...
    func wintry(cellClass: AnyClass, forellIdentifier: String) {
    ...
    func dequeueResIdentifier(identifier: String) -> RetainView? {
        ...
        if self.cellClass != nil {
            ...
            let cellclass = self.cellClass as! IntegerInteractorGroup.Type
            //: let cell: TalkingClubCardItemCell = cellclass.init(reuseIdentifier: identifier )
            let cell: IntegerInteractorGroup = cellclass.init(reuseIdentifier: identifier)
...
extension WillView: DticObjectProtocol {
    ...
    func beforeAllowCell(cell: RetainView) {
        ...
        self.delegate?.warningIn?(cardView: self, cardViewCell: cell, forRowAtIndex: self.currentFirstIndex - 1)

        //: let count = self.dataSource?.numberOfCountInCardView(cardView: self) ?? 0
        let count = self.dataSource?.doing(cardView: self) ?? 0
        // 移除后的卡片是最后一张(没有更多)
        ...
        if self.visibleCells.count == 0 { // 只有最后一张卡片的时候
            ...
            self.delegate?.medicPanoramicView?(cardViewCell: cell, forRowAtIndex: self.currentIndex)
            //: return
            return
        }
        // 当前数据源还有数据 继续创建cell
        ...
        if self.currentIndex < count - 1 {
            ...
            decrease(index: self.currentIndex + 1)
        }
        // 更新布局
        //: updateLayoutVisibleCells(anim: true)
        outlineNearbySince(anim: true)
    }

    //: func cardViewCellDidMoveFromSuperView(cell: TalkingClubCardViewCell, forMovePoint: CGPoint) {
    func viewProductivity(cell: RetainView, forMovePoint: CGPoint) {
        //: self.delegate?.didMoveCell?(cardView: self, cardViewCell: cell, forMovePoint: forMovePoint)
        self.delegate?.beautyAbsolutely?(cardView: self, cardViewCell: cell, forMovePoint: forMovePoint)
    }
}
...
extension WillView {
    ...
    func metadata() {
        ...
        tailTotalerpretationBackyard(anim: false)
    }
