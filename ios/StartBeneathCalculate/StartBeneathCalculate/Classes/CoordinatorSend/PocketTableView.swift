...
fileprivate let k_appealMsg:[UInt8] = [0x67,0x60,0x67,0x7a,0x26,0x6d,0x61,0x6a,0x6b,0x7c,0x34,0x27,0x2e,0x66,0x6f,0x7d,0x2e,0x60,0x61,0x7a,0x2e,0x6c,0x6b,0x6b,0x60,0x2e,0x67,0x63,0x7e,0x62,0x6b,0x63,0x6b,0x60,0x7a,0x6b,0x6a]

private func statementInclude(crush num: UInt8) -> UInt8 {
    return num ^ 14
}
...
enum AnnouncementCountLiteral: Int {
    //: case TimeGap = 0
...
@objc protocol SpanObjectProtocol: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func media()

    //: @objc optional func endScroll()
    @objc optional func doingst()
}

//: class TalkingVideoDanmuMsgListTableView: UIView {
class PocketTableView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_appealMsg.map{statementInclude(crush: $0)}, encoding: .utf8)!)
    }
...
extension PocketTableView {
    ...
    func squareCommit() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(naturalEvent), userInfo: nil, repeats: true)
    }
...
extension PocketTableView {
    ...
    func pageTemp(msgModel: TraceModelType?) {
    ...
    func upCornerAppointment() {
        ...
        if AllHeight > appClickName {
            //: if self.msgTableView.height < VideoMsgTableViewHeight {
            if self.msgTableView.height < appClickName {
                ...
                self.msgTableView.height = appClickName
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = appClickName - AllHeight
        ...
        currencySignature(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func currencySignature(animated: Bool) {
...
extension PocketTableView: UITableViewDelegate, UITableViewDataSource {
    ...
    func garnerIdentity(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.doingst?()
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(userMarginReadCountName)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = AReactiveCompatible.foster(tableView: tableView, msg: msgModel, indexPath: indexPath)
...
extension PocketTableView {
    ...
    func insertDesign() {
        ...
        self.msgTableView.frame = CGRect(x: 0, y: 0, width: kRateData, height: appClickName)
    }
}
