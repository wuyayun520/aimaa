...
extension PocketViewController {
    ...
    func trackModel(conversationModel: MultistageRocketConversationModel) {
        ...
        //: ListenLayoutDelivery.shared.func__removeConversation(targetId: conversationModel.targetId, dataType: .moreList)
        ListenLayoutDelivery.shared.nameType(targetId: conversationModel.targetId, dataType: .moreList)
        ...
        PersonAlertShow.customOfConfig(message: String(bytes: dataSomebodyAgencyMFormat.map{$0^173}, encoding: .utf8)!.localized, leftBtnTitle: (String(main_countryBronzeMatchStr.prefix(6))).localized, rightBtnTitle: "OK".localized, leftBlock: {
            ...
            self.manager.moreMsgArr.forEach { model in
                ...
                V2TIMManager.sharedInstance().markC2CMessage(asRead: model.userID) {
                    ...
                    ElasticActiveEffect.pathAge(msg: (String(user_tolerancePath) + dataIngredientKey.replacingOccurrences(of: "make", with: "es") + "es一\u{952e}\u{5df2}读" + const_hoodName + "ode:") + "\(code)" + (String(k_questionPath) + String(noti_productionName)) + "\(String(describing: desc))")
                }
            }
...
extension PocketViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        //: DisplayAperturePresenter.share.func__pushToPriveteChatVC(chatID: model.targetId) { vc in
        DisplayAperturePresenter.share.aboveCompletion(chatID: model.targetId) { vc in
