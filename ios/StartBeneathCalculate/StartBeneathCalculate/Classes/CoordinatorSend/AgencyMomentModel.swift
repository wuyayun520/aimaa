...
struct AgencyMomentModel: HandyJSON {
    //: var uid: String?
    var uid: String?
    //: var sex: String?
    var sex: String?
...
struct InmateTransformable: HandyJSON {
    //: var thumbUrl: String?
    var thumbUrl: String?
    //: var imgUrl: String?
    var imgUrl: String?
...
enum MigrationBasicType: String {
    //: case nones  = "0"
    case nones = "0"
    //: case Photo = "1"
    case Photo = "1"
...
struct CountMeasurable: HandyJSON {
    //: var videoUrl = ""
    var videoUrl = ""
    //: var imgUrl = ""
    var imgUrl = ""
    ...
    var status: SmartRecentTitle?
}

///
//: enum MomentVideoStatus: String {
enum SmartRecentTitle: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension AgencyMomentModel {
    //: mutating func caculateItemHeight(isPost: Bool = false) {
    mutating func language(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            textHeight = Int(CommonDisclaimerList.keyTotal(text: content!, size: CGSize(width: main_assetUrl - 54, height: CGFloat(MAXFLOAT)), font: UIFont.recipeDecision(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == MigrationBasicType.Photo.rawValue || momentType == MigrationBasicType.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
