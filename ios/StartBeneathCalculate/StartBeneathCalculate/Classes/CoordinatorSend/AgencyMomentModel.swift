...
extension AgencyMomentModel {
    ...
    mutating func language(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            textHeight = Int(MoviegoerThen.keyTotal(text: content!, size: CGSize(width: main_assetUrl - 54, height: CGFloat(MAXFLOAT)), font: UIFont.recipeDecision(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }
