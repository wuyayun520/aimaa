...
@objcMembers public class AgoThen: CompleteCellData {
    ...
    override public func write() -> CGSize {
        ...
        //: let rect = self.attributedString.boundingRect(with: CGSize.init(width: MacroBorderDefineThen.getScreenWidth()-50, height: CGFloat(MAXFLOAT)), options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil )
        let rect = self.attributedString.boundingRect(with: CGSize(width: MacroBorderDefineThen.areaWidth() - 50, height: CGFloat(MAXFLOAT)), options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil)
...
//        let dic = NSDictionary(jsonString: extra ?? "")
    ...
    func day(tips: String) -> NSMutableAttributedString {
        ...
        //: self.yyLayout = designYYTextLayout(array: attrbutedArray, displayMaxWidth: MacroBorderDefineThen.getScreenWidth()-50, para: para)
        self.yyLayout = designPara(array: attrbutedArray, displayMaxWidth: MacroBorderDefineThen.areaWidth() - 50, para: para)
