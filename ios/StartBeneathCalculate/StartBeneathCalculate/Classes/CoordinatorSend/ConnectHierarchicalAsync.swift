...
fileprivate let user_normallyName:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "bg_medals_podium" :*/
fileprivate let k_onlyScanAnonymousStr:String = "bg_medanot replacement explore"
fileprivate let notiShakeValue:[Character] = ["l","s","_","p","o","d","i","u","m"]

/*: "icon_rank_first_bg" :*/
fileprivate let main_officialPath:String = "equalcon"
fileprivate let kFrankMsg:String = "journal relieve foundation wealthy_firs"

/*: "icon_rank_first" :*/
fileprivate let notiPinId:String = "widespreadon"
fileprivate let user_listenTitle:String = "RST"

/*: "icon_ammount_points" :*/
fileprivate let const_lagMessage:String = "icon_balloon properly production value him"
fileprivate let const_netShouldStr:[Character] = ["t","_","p","o","i","n","t","s"]

/*: "icon_lounge" :*/
fileprivate let showSomeoneContent:String = "acquire neticon_lo"
fileprivate let appIllegalReceiveData:String = "ungprovide"

/*: "icon_rank_coin" :*/
fileprivate let dataTailValue:[Character] = ["i","c","o","n","_","r","a","n","k","_","c"]
fileprivate let app_docPath:[Character] = ["o","i","n"]

/*: "icon_rank_coin2" :*/
fileprivate let appErrorTitle:String = "icon_rout unable"
fileprivate let constCleanUrl:[Character] = ["c","o","i","n","2"]

/*: "icon_rank_second" :*/
fileprivate let notiHostUrl:String = "icon_rany cross"
fileprivate let main_databaseMsg:[Character] = ["a","n","k"]
fileprivate let main_saleKingPath:[Character] = ["_","s","e","c","o","n","d"]

/*: "icon_rank_third" :*/
fileprivate let const_retirementName:String = "process smart lengthicon_"
fileprivate let noti_chooseMsg:String = "hwhyrd"
...
// __CLOSE_PRINT__
//
//  ConnectHierarchicalAsync.swift
//  AbroadTalking
//
...
class ConnectHierarchicalAsync: UIView {
    //: var  index = 0
    var index = 0
    ...
    var dataArray: [JumpTransformable] = [] {
        //: didSet {
        didSet {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: user_normallyName.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var medalsView = UIImageView().then {
        ...
        $0.image = UIImage.adName(name: (String(k_onlyScanAnonymousStr.prefix(7)) + String(notiShakeValue)))
    }
    ...
    lazy var firstView = HeaderItemView(frame: .zero, rankType: .firstRankType).then {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(fullness))
...
extension ConnectHierarchicalAsync {
    //: private func bindInteraction() {
    private func resourceWest() {}

    //: private func updateSubviewsWithData() {
    private func familyLabData() {
        //: firstView.updateItemWithModel(model: dataArray[0], type: self.index)
        firstView.sinceType(model: dataArray[0], type: self.index)
        //: secondView.updateItemWithModel(model: dataArray.count>1 ? dataArray[1]: nil, type: self.index)
        secondView.sinceType(model: dataArray.count > 1 ? dataArray[1] : nil, type: self.index)
        //: thirdView.updateItemWithModel(model: dataArray.count>2 ? dataArray[2]: nil, type: self.index)
        thirdView.sinceType(model: dataArray.count > 2 ? dataArray[2] : nil, type: self.index)
    }

    //: @objc func registTapAction(tapGesture: UITapGestureRecognizer) {
    @objc func fullness(tapGesture: UITapGestureRecognizer) {
...
extension ConnectHierarchicalAsync {
    ...
    private func effectShoo() {
        //: let firstWidth = (ScreenWidth - actualWidth(w: 68)) / 3 + actualWidth(w: 10)
        let firstWidth = (main_assetUrl - actualWidth(w: 68)) / 3 + actualWidth(w: 10)
        //: let itemWidth = (ScreenWidth - actualWidth(w: 68)) / 3 - actualWidth(w: 10)
        let itemWidth = (main_assetUrl - actualWidth(w: 68)) / 3 - actualWidth(w: 10)
...
class HeaderItemView: UIView {
    ...
    enum AttentionAvailableType: Int {
        //: case firstRankType
        case firstRankType
        //: case secondRankType
        case secondRankType
    ...
    public init(frame: CGRect, rankType: AttentionAvailableType) {
        //: super.init(frame: frame)
        super.init(frame: frame)
        //: self.setupItemViews(type: rankType)
        self.homeType(type: rankType)
    }
    ...
    lazy var headPicBgView = UIImageView().then {
        ...
        $0.image = UIImage.adName(name: (main_officialPath.replacingOccurrences(of: "equal", with: "i") + "_rank" + String(kFrankMsg.suffix(5)) + "t_bg"))
    }
    ...
    lazy var crownImgView = UIImageView().then {
        ...
        $0.image = UIImage.adName(name: (notiPinId.replacingOccurrences(of: "widespread", with: "ic") + "_rank_fi" + user_listenTitle.lowercased()))
    }
    ...
    lazy var nickLabel = UILabel().then {
        ...
        $0.font = .recipeDecision(type: .Medium, fontSize: 18)
        //: $0.textAlignment = .center
        $0.textAlignment = .center
    }

    //: lazy var coinButton = TalkingButton().then {
    lazy var coinButton = SeparateControlReactiveCompatible().then {
        //: $0.isUserInteractionEnabled = false
        $0.isUserInteractionEnabled = false
        //: $0.setImage(UIImage.BundleImageNamed(name: "icon_ammount_points"), for: .normal)
        $0.setImage(UIImage.adName(name: (String(const_lagMessage.prefix(5)) + "ammoun" + String(const_netShouldStr))), for: .normal)
        //: $0.titleLabel?.font = .pingfangFont(type: .Regular, fontSize: 13)
        $0.titleLabel?.font = .recipeDecision(type: .Regular, fontSize: 13)
    ...
    lazy var loungeImgV = UIImageView().then {
        ...
        $0.image = UIImage.adName(name: (String(showSomeoneContent.suffix(7)) + appIllegalReceiveData.replacingOccurrences(of: "provide", with: "e")))
    }
}
...
extension HeaderItemView {
    ...
    func sinceType(model: JumpTransformable?, type: Int) {
        ...
        headPic.sortAge(urlStr: model!.headPic, placeImg: UIImage.gingerSnap(sex: model!.sex))
        ...
        if type == 0 {
            ...
            coinButton.setImage(UIImage.adName(name: (String(dataTailValue) + String(app_docPath))), for: .normal)
            //: } else {
        } else {
            //: coinButton.setImage(UIImage.BundleImageNamed(name: "icon_rank_coin2"), for: .normal)
            coinButton.setImage(UIImage.adName(name: (String(appErrorTitle.prefix(6)) + "ank_" + String(constCleanUrl))), for: .normal)
        }
    }
}
...
extension HeaderItemView {
    ...
    func homeType(type: AttentionAvailableType) {
...
//        var topCrownMargin = actualHeight(h: 4)
        ...
        case .firstRankType:
            ...
            crownImgView.image = UIImage.adName(name: (notiPinId.replacingOccurrences(of: "widespread", with: "ic") + "_rank_fi" + user_listenTitle.lowercased()))
            //: headPic.layer.borderColor = UIColor.RGBA(255, 224, 58, 1).cgColor
            headPic.layer.borderColor = UIColor.far(255, 224, 58, 1).cgColor
...
//            topCrownMargin = actualHeight(h: 10)
        ...
        case .secondRankType:
            ...
            crownImgView.image = UIImage.adName(name: (String(notiHostUrl.prefix(6)) + String(main_databaseMsg) + String(main_saleKingPath)))
            //: headPic.layer.borderColor = UIColor.RGBA(182, 199, 254, 1).cgColor
            headPic.layer.borderColor = UIColor.far(182, 199, 254, 1).cgColor
        ...
        case .thirdRankType:
            ...
            crownImgView.image = UIImage.adName(name: (String(const_retirementName.suffix(5)) + "rank_t" + noti_chooseMsg.replacingOccurrences(of: "why", with: "i")))
            //: headPic.layer.borderColor = UIColor.RGBA(251, 166, 109, 1).cgColor
            headPic.layer.borderColor = UIColor.far(251, 166, 109, 1).cgColor
            //: headPic.layer.cornerRadius = actualWidth(w: 31)
            headPic.layer.cornerRadius = actualWidth(w: 31)
            //: break
        }
