...
fileprivate let userMutualValue:[UInt8] = [0x22,0x21,0x2d,0x2f,0x3a,0x27,0x21,0x20,0x6e,0x2f,0x3b,0x3a,0x26,0x21,0x3c,0x27,0x34,0x2f,0x3a,0x27,0x21,0x20,0x1d,0x3a,0x2f,0x3a,0x3b,0x3d,0x6e,0x27,0x3d,0x6e]

private func arrayBlind(alive num: UInt8) -> UInt8 {
    return num ^ 78
}

/*: "get location failed. error: :*/
fileprivate let k_literalUpText:[Character] = ["g","e","t"," ","l","o","c","a","t","i","o","n"," ","f","a","i","l","e","d"]
fileprivate let user_mobileData:[Character] = ["."," ","e","r","r","o","r",":"]
...
enum AttentionExpressionConvertible: Int {
    //: case firstEntry = 0
    case firstEntry = 0
...
class SuiteLocationManager: NSObject {
    ...
    var locationType: AttentionExpressionConvertible?

    // singleton
    //: private static var _instance: TalkingLocationManager?
    private static var _instance: SuiteLocationManager?
    //: class func shared() -> TalkingLocationManager {
    class func backBy() -> SuiteLocationManager {
        //: guard _instance != nil else {
        guard _instance != nil else {
            //: _instance = TalkingLocationManager()
            _instance = SuiteLocationManager()
            //: return _instance!
            return _instance!
        }
        //: return _instance!
    ...
    func proposal(block: @escaping Locationblock) {
    ...
    func rightness(block: @escaping Authblock) {
        ...
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.5) { [weak self] in
            ...
            self.darkSameSwitchce()
        }
    }

    /// 设备是否开启了定位服务
    //: func hasLocationService() -> Bool {
    func duringBuild() -> Bool {
    ...
    func cantPermission() -> AttentionExpressionConvertible {
        //: switch locationPermission() {
        switch locationHomePermission() {
        //: case .notDetermined:
        case .notDetermined:
            //: return .firstEntry
            return .firstEntry
    ...
    func locationHomePermission() -> CLAuthorizationStatus {
        ...
        if #available(iOS 14.0, *) {
            ...
            printLog(message: String(bytes: userMutualValue.map{arrayBlind(alive: $0)}, encoding: .utf8)! + "\(status.rawValue)")
            //: return status
            return status
            //: } else {
        } else {
...
extension SuiteLocationManager: CLLocationManagerDelegate {
...
//            guard abs(loction.timestamp.timeIntervalSinceNow) > 5 else { return }
    ...
    func locationManager(_: CLLocationManager, didFailWithError error: Error) {
        ...
        printLog(message: (String(k_literalUpText) + String(user_mobileData)) + "\(error.localizedDescription)")
    }
}
