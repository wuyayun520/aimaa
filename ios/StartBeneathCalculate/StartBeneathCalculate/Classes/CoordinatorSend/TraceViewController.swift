...
class TraceViewController: UIViewController {
    ...
    private lazy var controllerScrollView: UIScrollView = {
        ...
        let count = min(self.controllerItems.count, self.titleItems.count)
        //: var height = MacroBorderDefineThen.getScreenHeight() - (topDistance + kTitle_H)
        var height = MacroBorderDefineThen.eachHeight() - (topDistance + kTitle_H)
        ...
        //: let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: MacroBorderDefineThen.getScreenWidth(), height: height)
        let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: MacroBorderDefineThen.areaWidth(), height: height)
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        //: scrollView.contentSize = CGSize(width: count * Int(MacroBorderDefineThen.getScreenWidth()), height: 0)
        scrollView.contentSize = CGSize(width: count * Int(MacroBorderDefineThen.areaWidth()), height: 0)
        ...
        scrollView.semanticContentAttribute = TaLanguageManager.shared.direction == .leftToRight ? .forceLeftToRight:.forceRightToLeft
        //: return scrollView
        return scrollView
    //: }()
    }()
    ...
    private lazy var titleScrollView: UIScrollView = {
        //: var rect = CGRect(x: 15, y: topDistance, width: MacroBorderDefineThen.getScreenWidth()-rightSpace, height: kTitle_H)
        var rect = CGRect(x: 15, y: topDistance, width: MacroBorderDefineThen.areaWidth()-rightSpace, height: kTitle_H)
        //: if isBisect {
        if isBisect {
            //: rect = CGRect(x: 0, y: topDistance, width: MacroBorderDefineThen.getScreenWidth()-rightSpace, height: kTitle_H)
            rect = CGRect(x: 0, y: topDistance, width: MacroBorderDefineThen.areaWidth()-rightSpace, height: kTitle_H)
        }
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        ...
        var originX: CGFloat = TaLanguageManager.shared.direction == .leftToRight ? 0:25
        ...
        self.titleItems.enumerated().forEach { [weak self] index, obj in
            ...
            //: if isBisect {
            if isBisect {
                //: titleWidth = MacroBorderDefineThen.getScreenWidth() / Double(self.titleItems.count)
                titleWidth = MacroBorderDefineThen.areaWidth() / Double(self.titleItems.count)
            //: } else {
            } else {
                //: titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
                titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
            }
            //: let x = LanguageManager.shared.direction == .leftToRight ? originX:(MacroBorderDefineThen.getScreenWidth()-originX-titleWidth)
            let x = TaLanguageManager.shared.direction == .leftToRight ? originX:(MacroBorderDefineThen.areaWidth()-originX-titleWidth)
...
extension TraceViewController {
    ...
    @objc private func handicappedPure(_ sender: UIButton) {
        ...
        let index = sender.tag - 1000
        //: let offset_x = CGFloat(index) * MacroBorderDefineThen.getScreenWidth()
        let offset_x = CGFloat(index) * MacroBorderDefineThen.areaWidth()
    ...
    private func soup(withIndex index: Int) {
        ...
        //: let origin_x = CGFloat(index) * MacroBorderDefineThen.getScreenWidth()
        let origin_x = CGFloat(index) * MacroBorderDefineThen.areaWidth()
        ...
        //: vc.view.frame = CGRect(x: origin_x, y: 0, width: MacroBorderDefineThen.getScreenWidth(), height: controllerScrollView.frame.size.height)
        vc.view.frame = CGRect(x: origin_x, y: 0, width: MacroBorderDefineThen.areaWidth(), height: controllerScrollView.frame.size.height)
    ...
    func belowPresentation(_ sender: UIButton) {
        //: if titleScrollView.contentSize.width <= MacroBorderDefineThen.getScreenWidth() {
        if titleScrollView.contentSize.width <= MacroBorderDefineThen.areaWidth() {
            //: return
            return
        }
        //: var offset_x = sender.center.x - MacroBorderDefineThen.getScreenWidth() * 0.5
        var offset_x = sender.center.x - MacroBorderDefineThen.areaWidth() * 0.5
        //: if offset_x < 0 {
        if offset_x < 0 {
            //: offset_x = 0
            offset_x = 0
        }
        //: let maxOffset = titleScrollView.contentSize.width - MacroBorderDefineThen.getScreenWidth()
        let maxOffset = titleScrollView.contentSize.width - MacroBorderDefineThen.areaWidth()
...
extension TraceViewController: UIScrollViewDelegate {
    ...
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        //: let index = Int(controllerScrollView.contentOffset.x / MacroBorderDefineThen.getScreenWidth())
        let index = Int(controllerScrollView.contentOffset.x / MacroBorderDefineThen.areaWidth())
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        //: let index = Int(scrollView.contentOffset.x / MacroBorderDefineThen.getScreenWidth())
        let index = Int(scrollView.contentOffset.x / MacroBorderDefineThen.areaWidth())
        ...
        //: let scaleR = scrollView.contentOffset.x / MacroBorderDefineThen.getScreenWidth() - CGFloat(index)
        let scaleR = scrollView.contentOffset.x / MacroBorderDefineThen.areaWidth() - CGFloat(index)
        //: let scaleL = 1 - scaleR
        let scaleL = 1 - scaleR
