...
fileprivate let show_boundaryValue:[UInt8] = [0x3b,0x3c,0x3b,0x26,0x7a,0x31,0x3d,0x36,0x37,0x20,0x68,0x7b,0x72,0x3a,0x33,0x21,0x72,0x3c,0x3d,0x26,0x72,0x30,0x37,0x37,0x3c,0x72,0x3b,0x3f,0x22,0x3e,0x37,0x3f,0x37,0x3c,0x26,0x37,0x36]

/*: "Select mute duration" :*/
fileprivate let const_dealSoundUnderId:String = "Selectthis tool"
fileprivate let data_songName:[Character] = [" ","m","u","t","e"," ","d","u","r","a","t","i","o","n"]

/*: "Cirform" :*/
fileprivate let constStrikePath:String = "out lawn oppositionCirform"

/*: "Mute success" :*/
fileprivate let user_expansionGirlData:String = "quit"
fileprivate let appBehindId:[Character] = ["u","t","e"," ","s","u","c","c","e","s","s"]

/*: "day" :*/
fileprivate let appMainStr:String = "dashadow"

/*: "days" :*/
fileprivate let data_headName:String = "cancelays"
...
class AgencyInmateView: UIView {
    ...
    var viewType: BiroViewType = .live
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_boundaryValue.map{$0^82}, encoding: .utf8)!)
    }
    ...
    private lazy var dismissBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(discreditBehind), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var tipsLb: UILabel = {
        ...
        lb.text = (String(const_dealSoundUnderId.prefix(6)) + String(data_songName)).localized
        //: lb.numberOfLines = 0
        lb.numberOfLines = 0
        //: return lb
        return lb
    ...
    lazy var commitBtn: UIButton = {
        ...
        btn.setTitle((String(constStrikePath.suffix(7))).localized, for: .normal)
...
extension AgencyInmateView {
    /// 消除视图按钮
    //: @objc private func bgDismissButtonClick() {
    @objc private func discreditBehind() {
        //: dismiss()
        presentationIndex()
    }
    ...
    func comeUp() {
        ...
        self.contentView.frame = CGRect(x: (main_assetUrl - contentV_Size.width) / 2, y: kTopUrl, width: contentV_Size.width, height: contentV_Size.height)

        //: UIView.animate(withDuration: 0.3) {
        UIView.animate(withDuration: 0.3) {
            //: self.contentView.frame.origin.y = (ScreenHeight-self.contentV_Size.height)/2
            self.contentView.frame.origin.y = (kTopUrl - self.contentV_Size.height) / 2
        }
    }
    ...
    func presentationIndex() {
        ...
        UIView.animate(withDuration: 0.3) {
            ...
            self.contentView.frame.origin.y = kTopUrl
...
extension AgencyInmateView {
    ...
    @objc private func sign(btn: UIButton) {
    ...
    @objc private func utilize() {
        ...
        if self.viewType == .live {
            ...
            FriendshipThen().figure(targetUid: targetUid, duration: index) { succeed, _, _ in
                //: ProgressHUD.dismiss()
                StrangerProgressHUD.barDismiss()
                //: if succeed {
                if succeed {
                    //: self.func__showStatusBarSuccessMsg(showMsg: "Mute success".localized)
                    self.pointOfReferenceMedia(showMsg: (user_expansionGirlData.replacingOccurrences(of: "quit", with: "M") + String(appBehindId)).localized)
                }
                //: self.dismiss()
...
extension AgencyInmateView {
    ...
    private func conversationConstraint() {
        ...
        for i in 0 ..< 3 {
            ...
            btn.askPath(color: UIColor.crosswise(), forState: .selected)
            ...
            let dayStr = (title == "1") ? (appMainStr.replacingOccurrences(of: "shadow", with: "y")).localized : (data_headName.replacingOccurrences(of: "cancel", with: "d")).localized
