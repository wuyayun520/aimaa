...
class ReportedViewController: SafetyFactorViewController {
    ...
    private let CellID = (String(app_minWrittenMsg) + String(show_subStr))
    ...
    lazy var tableView: UITableView = {
        ...
        tableView.register(EdictChoiceCell.classForCoder(), forCellReuseIdentifier: CellID)
...
extension ReportedViewController {
    ...
    private func home() {
        ...
        let arr = AsyncControllerHandler.share.areaArr
        //: if arr.count > 0 {
        if arr.count > 0 {
        ...
        AsyncControllerHandler.share.reqSuccessBlock = { [weak self] in
...
extension ReportedViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell: EdictChoiceCell = tableView.dequeueReusableCell(withIdentifier: CellID, for: indexPath) as! EdictChoiceCell
