...
fileprivate let mainOrientationMsg:[UInt8] = [0xcc,0xcb,0xcc,0xd1,0x8d,0xc6,0xca,0xc1,0xc0,0xd7,0x9f,0x8c,0x85,0xcd,0xc4,0xd6,0x85,0xcb,0xca,0xd1,0x85,0xc7,0xc0,0xc0,0xcb,0x85,0xcc,0xc8,0xd5,0xc9,0xc0,0xc8,0xc0,0xcb,0xd1,0xc0,0xc1]

/*: "img_loading_lose" :*/
fileprivate let kQuoteInfoResolutionStr:String = "img_loadinstance seat giving sound warning"
fileprivate let k_conveyName:[Character] = ["i","n","g","_","l","o","s","e"]
...
// __CLOSE_PRINT__
//
//  ControllerFlyweightOrientation.swift
//  AbroadTalking
//
...
class ControllerFlyweightOrientation: UICollectionViewCell {
    ...
    override init(frame: CGRect) {
        ...
        self.suspendDoingMagnitudeeraction()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: mainOrientationMsg.map{$0^165}, encoding: .utf8)!)
    }
...
extension ControllerFlyweightOrientation {
    //: private func bindInteraction() {
    private func suspendDoingMagnitudeeraction() {
        //: let singleTap = UITapGestureRecognizer(target: self, action: #selector(singleTapAction))
        let singleTap = UITapGestureRecognizer(target: self, action: #selector(rowCut))

        //: doubleTap = UITapGestureRecognizer(target: self, action: #selector(doubleTapAction)).then {
        doubleTap = UITapGestureRecognizer(target: self, action: #selector(savingAlong)).then {
...
extension ControllerFlyweightOrientation {
    ...
    func outsideStorage(imgModel: ViewGalleryModel) {
        ...
        if imgModel.imagePic != nil {
            ...
            imgView.kf.setImage(with: URL(string: imgModel.url), placeholder: UIImage(named: (String(kQuoteInfoResolutionStr.prefix(8)) + String(k_conveyName))), options: [.keepCurrentImageWhileLoading]) { _ in
                //: self.recoverSubviews()
                self.trafficPattern()
            }
        }
    ...
    func marginQuote(imgUrl: String) {
        ...
        imgView.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage(named: (String(kQuoteInfoResolutionStr.prefix(8)) + String(k_conveyName))), options: [.keepCurrentImageWhileLoading]) { _ in
            //: self.recoverSubviews()
            self.trafficPattern()
        }
    }
...
extension ControllerFlyweightOrientation: UIScrollViewDelegate, UIGestureRecognizerDelegate {
...
extension ControllerFlyweightOrientation {
    ...
    func trafficPattern() {
        ...
        var newW = main_assetUrl
        //: var newH = ScreenWidth * tempImg!.size.height / tempImg!.size.width
        var newH = main_assetUrl * tempImg!.size.height / tempImg!.size.width
        ...
        if newH > kTopUrl {
            ...
            newW = tempImg!.size.width * kTopUrl / tempImg!.size.height
        }
        ...
        self.imgContainerView.origin = CGPoint(x: (main_assetUrl - newW) / 2.0, y: (kTopUrl - newH) / 2)
