...
/*: "live_ended" :*/
fileprivate let noti_lookText:[Character] = ["l","i","v","e","_","e","n","d","e","d"]

/*: "The live broadcast has ended" :*/
fileprivate let appQuickPath:String = "The written currently advertising during distribution"
fileprivate let const_certainName:String = "mean ready topic license foster bro"
fileprivate let constInvitationId:String = " hastriumph gallery domain something degree"
fileprivate let main_quitFormat:String = "cap"

/*: "bth_live_male_phone_nor" :*/
fileprivate let const_fatalUrl:String = "his i interested personal elementbth_l"
fileprivate let dataChooseKey:String = "reward what pill universal networkive_"
fileprivate let noti_petitionStr:[Character] = ["m","a","l","e","_","p","h","o","n","e","_","n","o","r"]

/*: "floatingScreen" :*/
fileprivate let noti_pressMessage:[Character] = ["f","l","o","a","t"]
fileprivate let user_productName:String = "ingScreendraw third"

/*: "You have been muted" :*/
fileprivate let notiBorderTitleMessage:String = "You havefully find"
fileprivate let data_rectoStr:[Character] = [" ","b","e","e","n"," ","m","u","t","e","d"]

/*: "You have entered the other party's blacklist" :*/
fileprivate let user_retValue:[UInt8] = [0x6f,0x85,0x8b,0x36,0x7e,0x77,0x8c,0x7b,0x36,0x7b,0x84,0x8a,0x7b,0x88,0x7b,0x7a,0x36,0x8a,0x7e,0x7b,0x36,0x85,0x8a,0x7e,0x7b,0x88,0x36,0x86,0x77,0x88,0x8a,0x8f,0x3d,0x89,0x36,0x78,0x82,0x77,0x79,0x81,0x82,0x7f,0x89,0x8a]

fileprivate func relatedFrom(recover num: UInt8) -> UInt8 {
...
fileprivate let data_hostStr:[UInt8] = [0x2a,0xf,0x10,0x3,0x46,0x14,0x9,0x9,0xb,0x46,0x3,0x1e,0x5,0x3,0x16,0x12,0xf,0x9,0x8,0x4a,0x46,0x16,0xa,0x3,0x7,0x15,0x3,0x46,0x14,0x3,0x12,0x13,0x14,0x8,0x46,0x12,0x9,0x46,0x14,0x3,0x12,0x14,0x1f,0x4a,0x46,0xf,0x0,0x46,0xb,0x13,0xa,0x12,0xf,0x16,0xa,0x3,0x46,0x0,0x7,0xf,0xa,0x13,0x14,0x3,0x15,0x46,0x16,0xa,0x3,0x7,0x15,0x3,0x46,0x5,0x9,0x8,0x12,0x7,0x5,0x12,0x46,0x12,0xe,0x3,0x46,0x15,0x12,0x7,0x0,0x0]

/*: "isCalling" :*/
fileprivate let dataNegotiateCornerKey:[Character] = ["i","s","C","a","l","l","i"]
fileprivate let noti_foreStr:String = "ncrow"
...
class ReportedViewDelegate: SafetyFactorViewController {
    //: var type = LiveEnterType.userDetail
    var type = AvailableEnterType.userDetail // 入口
    ...
    override func viewDidLoad() {
        ...
        AugmentSliderGroup.program().delegate = self
    ...
    private lazy var videoCallManager: StretchObjectProtocol = {
        ...
        m.uid = String(BeReactiveCompatible.clearExplain().liveRoomModel.streamerInfo.uid)
        //: return m
        return m
        //: }()
    }()
    ...
    private lazy var stopLiveBgView: UIView = {
        ...
        let icon = UIImageView(image: UIImage.adName(name: (String(noti_lookText))))
        //: bg.addSubview(icon)
        bg.addSubview(icon)
        //: icon.snp.makeConstraints { make in
        icon.snp.makeConstraints { make in
            //: make.top.equalTo(StatusBarHeight+actualWidth(w: 205))
            make.top.equalTo(show_textNextFormat + actualWidth(w: 205))
        ...
        lab.text = (String(appQuickPath.prefix(4)) + "live" + String(const_certainName.suffix(4)) + "adcast" + String(constInvitationId.prefix(4)) + " ende" + main_quitFormat.replacingOccurrences(of: "cap", with: "d")).localized
        //: bg.addSubview(lab)
        bg.addSubview(lab)
    ...
    private lazy var bottomView: ContinuePinchableTable = {
        //: let view = TalkingLiveRoomBottomView(frame: .zero)
        let view = ContinuePinchableTable(frame: .zero)
        //: view.refreshBottomView(TalkingLiveManager.shared().liveRoomModel)
        view.fun(BeReactiveCompatible.clearExplain().liveRoomModel)
        //: view.delegate = self
        view.delegate = self
        //: return view
        return view
    ...
    private lazy var commInputView: SculleryView = {
        //: let text = TalkingLiveRoomInputView.init()
        let text = SculleryView()
        //: text.delegate = self
        text.delegate = self
        //: text.isHidden = true
        text.isHidden = true
    ...
    private lazy var danmuView: ArmsManufacturerView = {
        //: let view = TalkingDanmuMsgListTableView.init()
        let view = ArmsManufacturerView()
        //: return view
        return view
        //: }()
    }()
    ...
    private lazy var callBtn: UIButton = {
        ...
        btn.setImage(UIImage.adName(name: (String(const_fatalUrl.suffix(5)) + String(dataChooseKey.suffix(4)) + String(noti_petitionStr))), for: .normal)
    ...
    private lazy var topView: ShackUpReactiveCompatible = {
        //: let v = TalkingLiveRoomTopView()
        let v = ShackUpReactiveCompatible()
        //: v.weakVC = self
        v.weakVC = self
        //: v.refreshTopView(TalkingLiveManager.shared().liveRoomModel)
        v.beaconView(BeReactiveCompatible.clearExplain().liveRoomModel)
    ...
    private lazy var floatScreenView: EncodeMultiplicationDelegate = { // 飘屏
        //: let v = TalkingFloatScreenView()
        let v = EncodeMultiplicationDelegate()
...
extension ReportedViewDelegate {
    /// 播放直播内容
    //: @objc private func playLiveVideo() {
    @objc private func openst() {
        //: needReconnect = false
        needReconnect = false
        ...
        self.player.startLivePlay(BeReactiveCompatible.clearExplain().liveRoomModel.livePlayUrl, type: TX_Enum_PlayType.PLAY_TYPE_LIVE_FLV)
    }
    ...
    func first() {
        ...
        AugmentSliderGroup.direct(groupId: BeReactiveCompatible.clearExplain().liveRoomModel.chatGroupId, announcementMsg: BeReactiveCompatible.clearExplain().liveRoomModel.notification)

        //: self.bottomView.toUid = String(TalkingLiveManager.shared().liveRoomModel.streamerInfo.uid)
        self.bottomView.toUid = String(BeReactiveCompatible.clearExplain().liveRoomModel.streamerInfo.uid)
    }
    ...
    func stage() {
        ...
        V2TIMManager.sharedInstance().quitGroup(BeReactiveCompatible.clearExplain().liveRoomModel.chatGroupId, succ: {
            //: }, fail: { code, str in
        }, fail: { _, _ in
            //: })
        })
...
extension ReportedViewDelegate {
    ...
    @objc func quoteTop() {
        ...
        danmuView.snp.updateConstraints { make in
            ...
            make.bottom.equalTo(bottomView.snp.top).offset(-(kTopUrl - data_windowMsg - bottomView.height - data_cornerMessage))
        }
    }

    //: @objc func danmuMoveBottom() {
    @objc func untilRefuse() {
        //: var ishave = 0
        var ishave = 0
        //: for vc in self.children {
        for vc in self.children {
            //: if vc.isKind(of: TalkingPrivateChatController.self) || vc.isKind(of: TalkingChatListViewController.self) {
            if vc.isKind(of: BorderInsularViewDelegate.self) || vc.isKind(of: TagViewController.self) {
                //: ishave += 1
                ishave += 1
            }
        }
...
extension ReportedViewDelegate {
    ...
    @objc private func thanClick() {
        ...
        var videoStr = BeReactiveCompatible.clearExplain().liveRoomModel.videoPrice
        //: var voiceStr = TalkingLiveManager.shared().liveRoomModel.voicePrice
        var voiceStr = BeReactiveCompatible.clearExplain().liveRoomModel.voicePrice
        ...
        if FacultyReactiveCompatible.share.loginUserMode.loungePlus {
            ...
            videoStr = BeReactiveCompatible.clearExplain().liveRoomModel.videoVIPPrice
            //: voiceStr = TalkingLiveManager.shared().liveRoomModel.voiceVIPPrice
            voiceStr = BeReactiveCompatible.clearExplain().liveRoomModel.voiceVIPPrice
        }
        ...
        case .match:
            ...
            kSaveKey.putAvailable(eventID: const_skipSelectMsg, toUid: String(BeReactiveCompatible.clearExplain().liveRoomModel.streamerInfo.uid))
        ...
        case .userDetail:
            ...
            kSaveKey.putAvailable(eventID: userAdjustPath, toUid: String(BeReactiveCompatible.clearExplain().liveRoomModel.streamerInfo.uid))
    ...
    func route(animated: Bool = true) {
        //: self.topView.stopTimer()
        self.topView.haulUpConversation()
        //: req_quitGroup()
        stage() // 退出群聊
        //: TalkingLiveManager.destroy()
        BeReactiveCompatible.year() // 销毁单例
        //: TalkingDanmuManager.shared().delegate = nil
        AugmentSliderGroup.program().delegate = nil
        ...
        guard let currentVC = field() else { return }
    ...
    @objc private func accountYesterday(notification: NSNotification) {
        //: let userinfo = notification.userInfo as! [String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let dict = userinfo["floatingScreen"]
        let dict = userinfo[(String(noti_pressMessage) + String(user_productName.prefix(9)))]
        //: if let model = TalkingFloatingScreenModel.deserialize(from: dict as? Dictionary) {
        if let model = DigitalRefreshScreenModel.deserialize(from: dict as? Dictionary) {
            //: self.floatScreenView.didReceive(model)
            self.floatScreenView.detail(model)
        }
    }
}
...
extension ReportedViewDelegate: BorderActivityObjectProtocol {
    //: func func__commentBtnClick() {
    func alongBackground() {
        //: if Int(Date().timeIntervalSince1970) < TalkingLiveManager.shared().liveRoomModel.muteExpireAt {
        if Int(Date().timeIntervalSince1970) < BeReactiveCompatible.clearExplain().liveRoomModel.muteExpireAt {
            //: self.func__showStatusBarErrorMsg(showMsg: "You have been muted".localized)
            self.micromicronProcess(showMsg: (String(notiBorderTitleMessage.prefix(8)) + String(data_rectoStr)).localized)
            //: return
            return
        }
        /// 拉起弹幕评论
        //: commInputView.updatePlaceholder()
        commInputView.smartPlaceholder()
    }
}

// MARK: - AsyncBackwardFlex

//: extension TalkingLivePullStreamsViewController: TalkingDanmuManagerDelegate {
extension ReportedViewDelegate: AsyncBackwardFlex {
    //: func func__giftRecvNewModel(Msg: TalkingLiveRoomDanmuModel) {
    func pinTab(Msg: LoadSinePool) {
        //: addGiftEffectModelArr(Msg: Msg)
        dealMsg(Msg: Msg)
    }

    //: func func__DanmuRecvNewModel(Msg: TalkingLiveRoomDanmuModel) {
    func guessUponSimulation(Msg: LoadSinePool) {
        //: danmuView.addNewMsg(msgModel: Msg)
        danmuView.bottomFrank(msgModel: Msg)
    }
    ...
    func region(pushUid: String?) {
        ...
        let view = ProlongReactiveCompatible(frame: self.view.frame, uid: pushUid ?? "")
        //: view.delegate = self
        view.delegate = self
        //: view.show()
        view.discountDraft()
    ...
    func liveSound(nickName: String, atUid: String?) {
        ...
        commInputView.afterCur(toUid: atUid ?? "", nickName: nickName)
    }
...
extension ReportedViewDelegate {
    ...
    func dealMsg(Msg: LoadSinePool) {
        ...
        if giftModel != nil {
            ...
            giftEffectView.close(modelArr: [giftModel!])
        }
    }
}
...
extension ReportedViewDelegate: InsertrViewDelegate {
    ...
    func insert(heightToBottom: CGFloat) {
        ...
        danmuView.snp.updateConstraints { make in
            ...
            let height = heightToBottom == 0 ? 0 : -(heightToBottom - bottomView.height + commInputView.height - data_cornerMessage)
    ...
    func objectificationPriority(msgStr: String, toUid: String?) {
        ...
        AugmentSliderGroup.themeFeedback(groupId: BeReactiveCompatible.clearExplain().liveRoomModel.chatGroupId, message: msgStr, toUid: toUid) { succeed, _, _ in
            //: if succeed {
            if succeed {}
        }
    }
...
extension ReportedViewDelegate: V2TIMGroupListener {
    /// 被主播拉黑后，禁言退出房间
    ...
    func onMemberInfoChanged(_: String!, changeInfoList: [V2TIMGroupMemberChangeInfo]!) {
        //: if changeInfoList.first?.userID == FacultyReactiveCompatible.share.loginUserMode.userID {
        if changeInfoList.first?.userID == FacultyReactiveCompatible.share.loginUserMode.userID {
            //: let toastStr = "You have entered the other party's blacklist".localized
            let toastStr = String(bytes: user_retValue.map{relatedFrom(recover: $0)}, encoding: .utf8)!.localized
...
extension ReportedViewDelegate {
    ...
    @objc private func cutGetMoving() {
...
extension ReportedViewDelegate {
    ...
    @objc func note(note: Notification) {
...
extension ReportedViewDelegate: TXLivePlayListener {
    ...
    func onPlayEvent(_ evtID: Int32, withParam _: [AnyHashable: Any]!) {
        ...
        { // 观众侧拉流失败，退出直播间
            ...
            self.micromicronProcess(showMsg: String(bytes: data_hostStr.map{$0^102}, encoding: .utf8)!.localized)
            //: popCurrentViewController()
            route()
        }
    }
...
extension ReportedViewDelegate {
    ...
    private func source() {
        //: self.view.backgroundColor = .mottleResult()
        self.view.backgroundColor = .mottleResult()
    ...
    private func skinMonth() {
        ...
        danmuView.snp.makeConstraints { make in
            ...
            make.height.equalTo(userNumberMessage)
            //: make.width.equalTo(MsgTableViewWidth)
            make.width.equalTo(const_jumpTitle)
        }
        ...
        giftTrackView.snp.makeConstraints { make in
            ...
            make.bottom.equalTo(bottomView.snp.top).offset(-userNumberMessage)
        }
    }
    ...
    func pingThrough() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.note(note:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(cutGetMoving),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(openst),
                                               //: name: LIVE_RECIVE_STARTLIVE_NOTIFICATION,
                                               name: dataMessageTitle,
                                               //: object: nil)
                                               object: nil)
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(quoteTop),
                                               //: name: LIVE_HALF_VIEW_SHOW,
                                               name: notiAccountName,
                                               //: object: nil)
                                               object: nil)
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(accountYesterday(notification:)),
                                               //: name: LIVE_RECIVE_FLOATSCREEN_NOTIFICATION,
                                               name: userAddSessionData,
                                               //: object: nil)
                                               object: nil)

        // 音视频呼叫时，直播间静音
        //: TalkingSocketManager.shared.rx
        ConfigureAxisService.shared.rx
            //: .observeWeakly(Bool.self, "isCalling")
            .observeWeakly(Bool.self, (String(dataNegotiateCornerKey) + noti_foreStr.replacingOccurrences(of: "crow", with: "g")))
        ...
        //: TalkingSocketManager.shared.rx
        ConfigureAxisService.shared.rx
            .observeWeakly(Bool.self, "isGeneral")
            //: .subscribe(onNext: { [weak self] (value) in
            .subscribe(onNext: { [weak self] value in
                //: guard let self = self else { return }
                guard let self = self else { return }
