...
#import "UIImage+Accuracy.h"

//: @implementation UIImage (Compress)
@implementation UIImage (Accuracy)

//: - (NSData *)compressedImageData {
- (NSData *)quote {

    //: UIImage *image = self.wcTimelineCompress;
    UIImage *image = self.writing;
...
- (UIImage *)widespread {
    ...
    return [UIImage imageWithData:[self quote]];
}


//: - (UIImage *)wcSessionCompress {
- (UIImage *)adoptiveParent {
    //: return [self wcCompres:YES];
    return [self post:YES];
}
...
- (UIImage *)post:(Boolean)isSession {
    //: CGSize size = [self wxImageSize:isSession];
    CGSize size = [self searchInner:isSession];
...
- (CGSize)searchInner:(Boolean)isSession {
...
+ (NSData *)add:(UIImage *)image icon:(NSInteger)fileSize{
    ...
    while (outIMGData.length>(fileSize*1024)) {
        ...
        thumIMG = [thumIMG sumRelation:scalesize generalInScale:NO];

        //: outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
        outIMGData = [thumIMG method:(fileSize*1024)];
    }
...
- (UIImage*)indexSize:(CGSize)targetSize{
...
- (UIImage *)sumRelation:(CGSize)targetSize generalInScale:(BOOL)isScale{
...
- (NSData *)method:(NSInteger)fileSize{
