...
class BreastPocketMsgCell: AsyncMasterObserver {
    //: var transBlock: (() -> Void)?
    var transBlock: (() -> Void)?
...
extension BreastPocketMsgCell {
    ...
    override func lineStreetwise(with data: CommonalityCellData) {
        ...
        //: let maxWidth = AnimateUnaryAllocator.getScreenWidth() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        let maxWidth = AnimateUnaryAllocator.areaWidth() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        // 翻译赋值相关
