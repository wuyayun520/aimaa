...
fileprivate let main_angleResFormat:[UInt8] = [0xd4,0xd3,0xd4,0xc9,0x95,0xde,0xd2,0xd9,0xd8,0xcf,0x87,0x94,0x9d,0xd5,0xdc,0xce,0x9d,0xd3,0xd2,0xc9,0x9d,0xdf,0xd8,0xd8,0xd3,0x9d,0xd4,0xd0,0xcd,0xd1,0xd8,0xd0,0xd8,0xd3,0xc9,0xd8,0xd9]

private func retAgainstSimultaneously(sew num: UInt8) -> UInt8 {
    return num ^ 189
}

/*: "icon_chat_translate_nor" :*/
fileprivate let showEditorMessage:[Character] = ["i","c","o","n","_","c","h","a","t","_","t"]
fileprivate let mainCommendData:String = "rcourseslat"
fileprivate let noti_licenseMessage:String = "e_norto refuse dialog"

/*: "icon_chat_translate_pre" :*/
fileprivate let constMissContinuingPath:String = "icostock"
fileprivate let user_calculatePath:String = "adecrease"
fileprivate let show_meetValue:String = "sltimet"

/*: "icon_chat_miss" :*/
fileprivate let main_boundaryData:[Character] = ["i","c","o","n","_","c","h","a"]
fileprivate let user_paraVoicePremiumPath:String = "T"
fileprivate let const_regularProviderFormat:String = "_missspeaker else"

/*: "icon_chat_yes" :*/
fileprivate let show_portStr:[Character] = ["i","c","o","n","_","c","h","a","t","_","y","e","s"]
...
class BreastPocketMsgCell: EarthThen {
    //: var transBlock: (() -> Void)?
    var transBlock: (() -> Void)?

    //: var textData: WillCellData?
    var textData: WillCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: main_angleResFormat.map{retAgainstSimultaneously(sew: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var transBtn: UIButton = {
        ...
        btn.setImage(UIImage.adName(name: (String(showEditorMessage) + mainCommendData.replacingOccurrences(of: "course", with: "an") + String(noti_licenseMessage.prefix(5)))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_chat_translate_pre"), for: .highlighted)
        btn.setImage(UIImage.adName(name: (constMissContinuingPath.replacingOccurrences(of: "stock", with: "n") + "_chat_tr" + user_calculatePath.replacingOccurrences(of: "decrease", with: "n") + show_meetValue.replacingOccurrences(of: "time", with: "a") + "e_pre")), for: .highlighted)
        //: btn.addTarget(self, action: #selector(transBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(createAcross), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension BreastPocketMsgCell {
    ...
    override func lineStreetwise(with data: CommonalityCellData) {
        ...
        //: let maxWidth = MacroBorderDefineThen.getScreenWidth() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        let maxWidth = MacroBorderDefineThen.areaWidth() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        // 翻译赋值相关
        ...
        if textData.translateStatus == .MsgTranslate_translating {
            ...
            self.transStatusImagV.image = UIImage.adName(name: (String(main_boundaryData) + user_paraVoicePremiumPath.lowercased() + String(const_regularProviderFormat.prefix(5))))
            ...
            self.transStatusImagV.image = UIImage.adName(name: (String(show_portStr)))
        }
    ...
    func premium() {
        ...
        let longPressGr = UILongPressGestureRecognizer(target: self, action: #selector(jump(tap:)))
        //: self.container.addGestureRecognizer(longPressGr)
        self.container.addGestureRecognizer(longPressGr)
    }

    //: @objc func LognCellMessageAction(tap: UILongPressGestureRecognizer) {
    @objc func jump(tap: UILongPressGestureRecognizer) {
        //: if tap.state == .ended {
        if tap.state == .ended {
            //: return
            return
