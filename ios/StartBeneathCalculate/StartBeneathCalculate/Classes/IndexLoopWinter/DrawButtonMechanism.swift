...
public final class DrawButtonMechanism: NSObject, TableCodable {
    /// 消息Id【主键】
    ...
    public enum CodingKeys: String, CodingTableKey {
        ...
        public typealias Root = DrawButtonMechanism
        //: case msgId
        case msgId
        //: case db_voiceUri
        case db_voiceUri
...
extension DrawButtonMechanism {
    ...
    public class func exceptAction(_ dic: [AnyHashable: Any]) -> DrawButtonMechanism {
        //: let cache = WCDBVoiceMsgTable()
        let cache = DrawButtonMechanism()
    ...
    class func tapTransfer(_ voiceMsg: DrawButtonMechanism) {
        //: WCDBVoiceMsgTable.db_insertVoiceMsgs([voiceMsg])
        DrawButtonMechanism.refuseOfficial([voiceMsg])
    }

    /// 插入多条数据
    /// - Parameter voiceMsgs: 数据数组
    //: class func db_insertVoiceMsgs(_ voiceMsgs: [WCDBVoiceMsgTable]) {
    class func refuseOfficial(_ voiceMsgs: [DrawButtonMechanism]) {
        ...
        try? RangeManager.shared.database?.run(transaction: { _ in
            ...
            do {
                ...
                ElasticActiveEffect.pathAge(msg: (dataImmediatePrepareName) + "\(k_recordValue)" + (String(mainDropStr) + String(show_yetMsg)) + "\(error).")
            }
            //: })
        })
    }
    ...
    public class func familyLayer(with msgId: String) -> DrawButtonMechanism? {
        //: if let voiceMsgs = WCDBVoiceMsgTable.db_getVoiceMsgs(with: [msgId]) {
        if let voiceMsgs = DrawButtonMechanism.big(with: [msgId]) {
    ...
    class func big(with msgIds: [String]) -> [DrawButtonMechanism]? {
        //: do {
        do {
            //: let condition = WCDBVoiceMsgTable.Properties.msgId.in(msgIds)
            let condition = DrawButtonMechanism.Properties.msgId.in(msgIds)
            ...
            let voiceMsgs: [DrawButtonMechanism]? = try RangeManager.shared.database?.getObjects(on: DrawButtonMechanism.Properties.all, fromTable: k_recordValue, where: condition)
            //: return voiceMsgs
            return voiceMsgs
            //: } catch {
        } catch {
    ...
    class func deadlineMsg(_ voiceMsg: DrawButtonMechanism) {
        //: try? WCDBManager.shared.database?.run(transaction: { _ in
        try? RangeManager.shared.database?.run(transaction: { _ in
            // 数据库中已存在，则更新数据
            //: if WCDBVoiceMsgTable.db_getVoiceMsg(with: voiceMsg.msgId) != nil {
            if DrawButtonMechanism.familyLayer(with: voiceMsg.msgId) != nil {
                //: do {
                do {
                    //: let condition = WCDBVoiceMsgTable.Properties.msgId == voiceMsg.msgId
                    let condition = DrawButtonMechanism.Properties.msgId == voiceMsg.msgId
                    ...
                    try RangeManager.shared.database?.update(table: k_recordValue,
                                                            ...
                                                            on: DrawButtonMechanism.Properties.all,
                                                            //: with: voiceMsg,
                                                            with: voiceMsg,
                                                            //: where: condition)
                                                            where: condition)
                    ...
                    ElasticActiveEffect.pathAge(msg: (dataImmediatePrepareName) + "\(k_recordValue)" + (String(kYourMsg) + String(main_globalName)) + "\(error).")
                }
                ...
                DrawButtonMechanism.tapTransfer(voiceMsg)
            }
            //: })
        })
    }
    ...
    class func courseWith(msgId: String, status: String = "", filePath: String = "") {
        ...
        guard let dbModel = DrawButtonMechanism.familyLayer(with: msgId) else { return }
        ...
        DrawButtonMechanism.deadlineMsg(dbModel)
    }
    ...
    class func transform(msgId: String) {
        ...
        try? RangeManager.shared.database?.run(transaction: { _ in
            ...
            let condition = DrawButtonMechanism.Properties.msgId == msgId
    ...
    class func cover(_ userId: String, toUid: String) {
        ...
        try? RangeManager.shared.database?.run(transaction: { _ in
            ...
            let condition = DrawButtonMechanism.Properties.db_senduid == userId && DrawButtonMechanism.Properties.db_touid == toUid
            //: try? WCDBManager.shared.database?.delete(fromTable: WCDBVoiceMsgTableName,
            try? RangeManager.shared.database?.delete(fromTable: k_recordValue,
                                                     //: where: condition)
                                                     where: condition)
