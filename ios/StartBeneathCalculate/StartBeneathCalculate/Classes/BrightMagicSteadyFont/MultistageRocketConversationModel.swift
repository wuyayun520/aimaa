...
public class MultistageRocketConversationModel: NSObject {
    ...
    //: var chatType: AboveCellPreviewType = .unDefined
    var chatType: AboveCellPreviewType = .unDefined
    ...
    init(conv: V2TIMConversation) {
        ...
        if conv.type == .C2C {
            ...
            self.chatType = .private
            //: if conv.userID == AnimateUnaryAllocator.getXiaoMiID() {
            if conv.userID == AnimateUnaryAllocator.seethe() {
                //: self.chatType = .system
                self.chatType = .system
                //: } else if conv.userID == AnimateUnaryAllocator.getCustomerServiceID() {
            } else if conv.userID == AnimateUnaryAllocator.receiveRemove() {
...
extension MultistageRocketConversationModel {
    ...
    /// - Returns: model
    //: class func getPlaceholderModel(chatType: AboveCellPreviewType) -> TalkingConversationModel {
    class func nearGet(chatType: AboveCellPreviewType) -> MultistageRocketConversationModel {
        ...
        if chatType == .system { // 系统通知
            //: model.userID = AnimateUnaryAllocator.getXiaoMiID()
            model.userID = AnimateUnaryAllocator.seethe()
            //: model.targetId = AnimateUnaryAllocator.getXiaoMiID()
            model.targetId = AnimateUnaryAllocator.seethe()
            ...
            //: model.userID = AnimateUnaryAllocator.getCustomerServiceID()
            model.userID = AnimateUnaryAllocator.receiveRemove()
            //: model.targetId = AnimateUnaryAllocator.getCustomerServiceID()
            model.targetId = AnimateUnaryAllocator.receiveRemove()
...
extension MultistageRocketConversationModel {
    ...
    private func sinceFail(conv: V2TIMConversation) {
        ...
        if let totalIntimate = msgInfo?[(app_plusKey.replacingOccurrences(of: "charm", with: "t") + String(dataValidProtectionUrl.prefix(8)))]?.int {
            ...
            if let model = DeployMatrixDelegate.biologicalAgent(targetId: self.userID) {
                /// 获取消息为最新时
                ...
                if time > model.intimateTime {
                    ...
                    DeployMatrixDelegate.totalimate(with: userID, intimate: totalIntimate, currTime: time)
                    //: } else {
                } else {
