...
fileprivate let userFillValue:[UInt8] = [0xd,0xa,0xd,0x10,0x4c,0x7,0xb,0x0,0x1,0x16,0x5e,0x4d,0x44,0xc,0x5,0x17,0x44,0xa,0xb,0x10,0x44,0x6,0x1,0x1,0xa,0x44,0xd,0x9,0x14,0x8,0x1,0x9,0x1,0xa,0x10,0x1,0x0]

/*: "#30D00B" :*/
fileprivate let notiGoText:String = "#30D00Bpad smart require relate"

/*: "icon_receivedcalls" :*/
fileprivate let k_saveId:[Character] = ["i","c"]
fileprivate let kAboutAdoptFormat:String = "on_rselect scale assert"
fileprivate let app_matchUrl:String = "edcallone"

/*: "Received calls" :*/
fileprivate let showRoundSecureStr:String = "Received flexible priority var"
fileprivate let noti_blackTitle:String = "CALLS"

/*: "icon_dialedcalls" :*/
fileprivate let const_recordContent:String = "icoinstall"
fileprivate let kMemberMsg:String = "edcaurbanurbans"

/*: "Dialed calls" :*/
fileprivate let appInsideKey:String = "Dialfemale processor special leave answer"
fileprivate let showDetectMsg:String = "spark"
...
class SpringThen: UITableViewCell {
    ...
    open weak var delegate: LikeThen?
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        self.cameraSuccessfully()
    }

    //: required init?(coder aDecoder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: userFillValue.map{$0^100}, encoding: .utf8)!)
    }
    ...
    lazy var iconBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(formatFall), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    lazy var durationLB: UILabel = {
        ...
        label.font = .recipeDecision(type: .Medium, fontSize: 10)
        //: label.textColor = .appValueDetailColor()
        label.textColor = .keepRid()
    ...
    lazy var onlineView: UIView = {
        ...
        view.backgroundColor = UIColor(hex: (String(notiGoText.prefix(7))))
...
extension SpringThen {
    ...
    func colorRoute(model: ModelHandyJSON) {
        //: currenModel = model
        currenModel = model
        ...
        if !model.headPicFrame.isEmptyString {
            ...
            iconBorderBtn.slash(urlStr: model.headPicFrame)
            //: } else {
        } else {
        ...
        if model.callStatus == 0 {
            ...
            rcordImg.image = UIImage.adName(name: (String(k_saveId) + String(kAboutAdoptFormat.prefix(4)) + "eceiv" + app_matchUrl.replacingOccurrences(of: "one", with: "s")))
            //: rcordLB.text = "Received calls".localized
            rcordLB.text = (String(showRoundSecureStr.prefix(9)) + noti_blackTitle.lowercased()).localized
            //: } else {
        } else {
            //: rcordImg.image = UIImage.BundleImageNamed(name: "icon_dialedcalls")
            rcordImg.image = UIImage.adName(name: (const_recordContent.replacingOccurrences(of: "install", with: "n") + "_dial" + kMemberMsg.replacingOccurrences(of: "urban", with: "l")))
            //: rcordLB.text = "Dialed calls".localized
            rcordLB.text = (String(appInsideKey.prefix(4)) + "ed call" + showDetectMsg.replacingOccurrences(of: "spark", with: "s")).localized
        }
...
extension SpringThen {
    ...
    private func cameraSuccessfully() {
    ...
    private func bindCustomGiveAndTakeIntervaleraction() {}
}
