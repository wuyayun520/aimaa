...
protocol AsyncBackwardFlex: NSObject {
    /// 弹幕消息
...
class AugmentSliderGroup: NSObject {
    //: private var maxJoinRoom = 1
    private var maxJoinRoom = 1 // 重试加入弹幕房间次数
    //: private static var _instance: TalkingDanmuManager?
    private static var _instance: AugmentSliderGroup? // singleton
    //: open weak var delegate: TalkingDanmuManagerDelegate?
    open weak var delegate: AsyncBackwardFlex?

    //: override private init() {}
    override private init() {}
    //: class func shared() -> TalkingDanmuManager {
    class func program() -> AugmentSliderGroup {
        //: guard _instance != nil else {
        guard _instance != nil else {
            //: _instance = TalkingDanmuManager()
            _instance = AugmentSliderGroup()
            //: return _instance!
            return _instance!
        }
        //: return _instance!
...
extension AugmentSliderGroup {
    ...
    func estimatedSpeaker(msg: V2TIMMessage) {
        ...
        if var model = LoadSinePool.deserialize(from: extraDic as? Dictionary) {
    ...
    private func previous(danmuModel: LoadSinePool) {
...
extension AugmentSliderGroup {
    ...
    class func chance() {
        ...
        if AugmentSliderGroup._instance != nil {
            //: TalkingDanmuManager._instance = nil
            AugmentSliderGroup._instance = nil
        }
    }
    ...
    class func direct(groupId: String, announcementMsg: String, completion: ((_ succeed: Bool) -> Void)? = nil) {
        ...
        V2TIMManager.sharedInstance().joinGroup(groupId, msg: nil) {
            ...
            if announcementMsg.count > 0 {
                ...
                AugmentSliderGroup.program().oddThan(str: announcementMsg)
            }
            //: completion?(true)
            completion?(true)
            ...
            if AugmentSliderGroup.program().maxJoinRoom == 1 {
                //: TalkingDanmuManager.shared().maxJoinRoom = 0
                AugmentSliderGroup.program().maxJoinRoom = 0
                ...
                AugmentSliderGroup.program().micromicronProcess(showMsg: String(bytes: show_createData.reversed(), encoding: .utf8)!.localized)
            }
            //: completion?(false)
            completion?(false)
        }
