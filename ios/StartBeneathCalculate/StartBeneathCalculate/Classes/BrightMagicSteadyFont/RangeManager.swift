...
class RangeManager: NSObject {
    ...
    func bubble() {
        ...
        guard !FacultyReactiveCompatible.share.loginUserMode.userID.isEmpty else {
            ...
            MigrationThen.pathAge(msg: (String(const_mirrorStr.prefix(6)) + "库打\u{5f00}失败：u" + String(app_favoriteFormat)))
            //: return
            return
        }
        ...
        guard database?.canOpen == true else {
            ...
            MigrationThen.pathAge(msg: (constPermissionKey.replacingOccurrences(of: "scan", with: "W") + "\u{6570}据库打\u{5f00}\u{5931}败：") + "\(fileURL.path)")
            //: return
            return
        }
...
extension RangeManager {
    ...
    private func count() {
        ...
        do {
            ...
            try database?.create(table: show_noPath, of: IonInfoTable.self)
            // 语音消息表
            //: try database?.create(table: WCDBVoiceMsgTableName, of: WCDBVoiceMsgTable.self)
            try database?.create(table: k_recordValue, of: SpireThen.self)
            //: } catch {
        } catch {
            //: UploadLogTool.writeLog(msg: "WCDB数据库：创建表失败。error：\(error).")
            MigrationThen.pathAge(msg: (String(showQuantityroFormat) + String(showProcessKey)) + "\(error).")
        }
    }
}
