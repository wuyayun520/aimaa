...
class ReadyLocationListManager {
    ...
    //: func req_initData() {
    func multiple() {
        //: self.officialArr = IonV2Listener.shared.getOfficialList()
        self.officialArr = IonV2Listener.shared.voiderBroad()
        //: self.topArr = IonV2Listener.shared.topConvList
        self.topArr = IonV2Listener.shared.topConvList
        //: self.normalArr = IonV2Listener.shared.norConvList
        self.normalArr = IonV2Listener.shared.norConvList
        // 更多数据
        //: req_moreMsgInitData()
        item()
    ...
    func fileAgent(completion: @escaping (_ succeed: Bool) -> Void) {
        ...
        guard self.isHalf == false else { return }
        //: IonV2Listener.shared.getChatRoomData { succeed in
        IonV2Listener.shared.extra { succeed in
            //: if succeed {
            if succeed {
                //: self.chatRoomArr = IonV2Listener.shared.chatRoomList
                self.chatRoomArr = IonV2Listener.shared.chatRoomList
                //: completion(true)
                completion(true)
            }
    ...
    func sufficientModel(model: MultistageRocketConversationModel?) -> MultistageRocketConversationModel? {
        ...
        let userInfo: ConsumerPenHandyJSON? = InertGasThen.biologicalAgent(targetId: targetId)
    ...
    func wateredStock(conversationList: [MultistageRocketConversationModel], finish: (() -> Void)?) {
        ...
        InertGasThen.resource(with: tempArr) { _, error in
    ...
    func step(completion: @escaping (_ isFinish: Bool) -> Void) {
        //: IonV2Listener.shared.func__getConversationList { arr, leftMore in
        IonV2Listener.shared.colloquy { _, leftMore in
            //: completion(leftMore)
            completion(leftMore)
        }
    }
...
extension ReadyLocationListManager {
    ...
    func item() {
        //: self.moreMsgArr = IonV2Listener.shared.moreMsgList
        self.moreMsgArr = IonV2Listener.shared.moreMsgList
    }
...
extension ReadyLocationListManager {
    ...
    func exceptEastMenu(isFirst: Bool, completion: @escaping (_ succeed: Bool) -> Void) {
        ...
        InertGasThen.allowAcross(with: data) { resultArr, _ in
...
extension ReadyLocationListManager {
    ...
    func fertilizationAge() {
        ...
        self.intimateTopArr.removeAll()
        //: for model in IonV2Listener.shared.norConvList {
        for model in IonV2Listener.shared.norConvList {
        ...
        //: for model in IonV2Listener.shared.topConvList {
        for model in IonV2Listener.shared.topConvList {
            //: if model.totalIntimate >= FacultyReactiveCompatible.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= FacultyReactiveCompatible.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
    ...
    func countimate(uid: String, intimate: Int) {
        ...
        InertGasThen.totalimate(with: uid, intimate: intimate, currTime: Int(Date().timeIntervalSince1970))
    }
