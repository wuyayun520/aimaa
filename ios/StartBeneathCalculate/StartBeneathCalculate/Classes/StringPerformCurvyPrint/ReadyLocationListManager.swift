...
/*: "uid" :*/
fileprivate let userDiscussionMessage:[Character] = ["u","i","d"]

/*: "onlineStatus" :*/
fileprivate let app_storageData:String = "ONLIN"

/*: "isNewUser" :*/
fileprivate let app_urbanBookContent:String = "isNefollow message observe horrible alongside"

/*: "isTPAuth" :*/
fileprivate let showBoyUrl:String = "isTPAuthandle song sequence ask"
fileprivate let kQuoteGrossValue:[Character] = ["h"]

/*: "totalIntimate" :*/
fileprivate let noti_numberTalkProductivityKey:String = "totalInvisible milk"
fileprivate let appNarrowId:[Character] = ["t","i","m","a","t","e"]

/*: "userStatus" :*/
fileprivate let main_toolAtValue:[Character] = ["u","s","e","r","S","t","a"]
fileprivate let mainPopularContributionMessage:String = "skins"

/*: "intimate" :*/
fileprivate let showFlowJoinText:String = "irowimate"
...
// __CLOSE_PRINT__
//
//  ReadyLocationListManager.swift
//  AbroadTalking
//
...
enum HappeningTransformable: Int {
    //: case offical  = 0
    case offical = 0 // 官方
    //: case chatRoom = 1
    case chatRoom = 1 // 聊天室
...
class ReadyLocationListManager {
    //: var isHalf = false
    ...
    private var moreMsgModel: MultistageRocketConversationModel = //: return MultistageRocketConversationModel.getPlaceholderModel(chatType: .moreMsg)
        .nearGet(chatType: .moreMsg)
    //: }()

    // 初始化数据
    //: func req_initData() {
    func multiple() {
        //: self.officialArr = ListenLayoutDelivery.shared.getOfficialList()
        self.officialArr = ListenLayoutDelivery.shared.voiderBroad()
        //: self.topArr = ListenLayoutDelivery.shared.topConvList
        self.topArr = ListenLayoutDelivery.shared.topConvList
        //: self.normalArr = ListenLayoutDelivery.shared.norConvList
        self.normalArr = ListenLayoutDelivery.shared.norConvList
        // 更多数据
        //: req_moreMsgInitData()
        item()
    ...
    func fileAgent(completion: @escaping (_ succeed: Bool) -> Void) {
        //: guard self.isHalf == false else { return }
        guard self.isHalf == false else { return }
        //: ListenLayoutDelivery.shared.getChatRoomData { succeed in
        ListenLayoutDelivery.shared.extra { succeed in
            //: if succeed {
            if succeed {
                //: self.chatRoomArr = ListenLayoutDelivery.shared.chatRoomList
                self.chatRoomArr = ListenLayoutDelivery.shared.chatRoomList
                //: completion(true)
                completion(true)
            }
    ...
    func remarkIndexDrag(indexPath: IndexPath) -> MultistageRocketConversationModel? {
    ...
    func sufficientModel(model: MultistageRocketConversationModel?) -> MultistageRocketConversationModel? {
        //: guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        //: let userInfo: ConsumerPenHandyJSON? = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId)
        let userInfo: ConsumerPenHandyJSON? = DeployMatrixDelegate.biologicalAgent(targetId: targetId)
    ...
    func wateredStock(conversationList: [MultistageRocketConversationModel], finish: (() -> Void)?) {
        //: var tempArr = [String]()
        var tempArr = [String]()
        ...
        DeployMatrixDelegate.resource(with: tempArr) { _, error in
    ...
    func pushView(conversationModel: MultistageRocketConversationModel) {
    ...
    func step(completion: @escaping (_ isFinish: Bool) -> Void) {
        //: ListenLayoutDelivery.shared.func__getConversationList { arr, leftMore in
        ListenLayoutDelivery.shared.colloquy { _, leftMore in
            //: completion(leftMore)
            completion(leftMore)
        }
    }
    ...
    lazy var officialArr: [MultistageRocketConversationModel] = //: return Array<MultistageRocketConversationModel>()
        .init()
    //: }()

    //: lazy var topArr: [TalkingConversationModel] = {
    lazy var topArr: [MultistageRocketConversationModel] = //: return Array<MultistageRocketConversationModel>()
        .init()
    //: }()

    //: lazy var normalArr: [TalkingConversationModel] = {
    lazy var normalArr: [MultistageRocketConversationModel] = //: return Array<MultistageRocketConversationModel>()
        .init()
    //: }()

    //: lazy var chatRoomArr: [TalkingConversationModel] = {
    lazy var chatRoomArr: [MultistageRocketConversationModel] = //: return Array<MultistageRocketConversationModel>()
        .init()
    //: }()

    //: lazy var intimateNorArr: [TalkingConversationModel] = {
    lazy var intimateNorArr: [MultistageRocketConversationModel] = //: return Array<MultistageRocketConversationModel>()
        .init()
    //: }()

    //: lazy var intimateTopArr: [TalkingConversationModel] = {
    lazy var intimateTopArr: [MultistageRocketConversationModel] = //: return Array<MultistageRocketConversationModel>()
        .init()
    //: }()

    //: lazy var moreMsgArr: [TalkingConversationModel] = {
    lazy var moreMsgArr: [MultistageRocketConversationModel] = //: return Array<MultistageRocketConversationModel>()
        .init()
    //: }()
}
...
extension ReadyLocationListManager {
    /// 检查消息列表中是否已经建立过会话
...
extension ReadyLocationListManager {
    ...
    func item() {
        //: self.moreMsgArr = ListenLayoutDelivery.shared.moreMsgList
        self.moreMsgArr = ListenLayoutDelivery.shared.moreMsgList
    }

    /// 根据索引获取模型（更多消息）
    //: func req_moreMsgConversationModel(indexPath: IndexPath) -> TalkingConversationModel? {
    func toAGreaterExtentFromSunnaModify(indexPath: IndexPath) -> MultistageRocketConversationModel? {
    ...
    func digitizerLevelModel() -> MultistageRocketConversationModel {
        // 更新更多消息最新数据
        //: req_moreMsgInitData()
        item()
        ...
        moreMsgModel.unreadCount = smartHide()
...
extension ReadyLocationListManager {
    ...
    func exceptEastMenu(isFirst: Bool, completion: @escaping (_ succeed: Bool) -> Void) {
        ...
        DeployMatrixDelegate.allowAcross(with: data) { resultArr, _ in
            ...
            if resultArr?.count ?? 0 <= 0 { // 无数据
                ...
                resultArr?.forEach { dict in
                    ...
                    let uid = json[(String(userDiscussionMessage))].stringValue
                    //: let onlineStatus = json["onlineStatus"].intValue
                    let onlineStatus = json[(app_storageData.lowercased() + "eStatus")].intValue
                    //: let isNewUser = json["isNewUser"].boolValue
                    let isNewUser = json[(String(app_urbanBookContent.prefix(4)) + "wUser")].boolValue
                    //: let istpAuth = json["isTPAuth"].boolValue
                    let istpAuth = json[(String(showBoyUrl.prefix(7)) + String(kQuoteGrossValue))].boolValue
                    //: let totalIntimate = json["totalIntimate"].int ?? 0
                    let totalIntimate = json[(String(noti_numberTalkProductivityKey.prefix(7)) + String(appNarrowId))].int ?? 0
                    //: let userStatus = json["userStatus"].int ?? 1
                    let userStatus = json[(String(main_toolAtValue) + mainPopularContributionMessage.replacingOccurrences(of: "skin", with: "tu"))].int ?? 1
                    ...
                    self.countimate(uid: uid, intimate: totalIntimate)
                    //: })
                }
            }
            //: completion(true)
    ...
    func create(userInfo: [String: Any]) -> Bool {
        ...
        let uid = userInfo[(String(userDiscussionMessage))] as! String
        //: let onlineStatus = userInfo["onlineStatus"] as! Int
        let onlineStatus = userInfo[(app_storageData.lowercased() + "eStatus")] as! Int
        //: let isNewUser = userInfo["isNewUser"] as! Bool
        let isNewUser = userInfo[(String(app_urbanBookContent.prefix(4)) + "wUser")] as! Bool

        /// 用户状态更新
        //: let userStatus = userInfo["userStatus"] as? Int ?? 1
        let userStatus = userInfo[(String(main_toolAtValue) + mainPopularContributionMessage.replacingOccurrences(of: "skin", with: "tu"))] as? Int ?? 1
        //: var needuserStatus = false
        var needuserStatus = false
        //: if userInfo.keys.contains("userStatus") {
        if userInfo.keys.contains((String(main_toolAtValue) + mainPopularContributionMessage.replacingOccurrences(of: "skin", with: "tu"))) {
            //: needuserStatus = true
            needuserStatus = true
        }
...
extension ReadyLocationListManager {
    ...
    func fertilizationAge() {
        ...
        self.intimateTopArr.removeAll()
        //: for model in ListenLayoutDelivery.shared.norConvList {
        for model in ListenLayoutDelivery.shared.norConvList {
            //: if model.totalIntimate >= FacultyReactiveCompatible.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= FacultyReactiveCompatible.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
                let userModel = self.sufficientModel(model: model)
                //: self.intimateNorArr.append(userModel!)
                self.intimateNorArr.append(userModel!)
            }
        }
        //: for model in ListenLayoutDelivery.shared.topConvList {
        for model in ListenLayoutDelivery.shared.topConvList {
            //: if model.totalIntimate >= FacultyReactiveCompatible.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= FacultyReactiveCompatible.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
    ...
    func readyData(convModel: MultistageRocketConversationModel, isTop: Bool) {
        //: if isTop {
        if isTop {
    ...
    func aerogrammePath(indexPath: IndexPath) -> MultistageRocketConversationModel? {
        //: switch indexPath.section {
        switch indexPath.section {
    ...
    func countimate(uid: String, intimate: Int) {
        ...
        DeployMatrixDelegate.totalimate(with: uid, intimate: intimate, currTime: Int(Date().timeIntervalSince1970))
    }

    /// 更新消息列表单个用户亲密度
    //: func update_userIntimate(userInfo: [String: Any]) -> Bool {
    func trackDevicePlay(userInfo: [String: Any]) -> Bool {
        //: let uid = userInfo["uid"] as! String
        let uid = userInfo[(String(userDiscussionMessage))] as! String
        //: let intimate = userInfo["intimate"] as! Int
        let intimate = userInfo[(showFlowJoinText.replacingOccurrences(of: "row", with: "nt"))] as! Int
        //: var updateSucceed = false
        var updateSucceed = false
        //: if let topModel = self.topArr.filter({ $0.userID == uid }).first {
        if let topModel = self.topArr.filter({ $0.userID == uid }).first {
