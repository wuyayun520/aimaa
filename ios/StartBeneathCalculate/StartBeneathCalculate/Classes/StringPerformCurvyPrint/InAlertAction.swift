...
/*: "tab" :*/
fileprivate let dataDeliveryTitle:[Character] = ["t","a","b"]

/*: "page" :*/
fileprivate let data_ratioLegalInvolvementFormat:String = "pasonge"

/*: "uid" :*/
fileprivate let showGrayData:String = "uimarketing"

/*: "model" :*/
fileprivate let const_innerEnableerPipeId:String = "modenatural"

/*: "There's no feed yet." :*/
fileprivate let kSecretMsg:String = "There\'"
fileprivate let main_automaticData:[Character] = ["s"," ","n","o"," ","f","e","e","d"," ","y","e","t","."]
...
class InAlertAction: UIViewController {
    //: var pageIndex = 0
    var pageIndex = 0
    ...
    var seleteCell = NotifiableViewCell()
    //: var lastVideoUrl = ""
    var lastVideoUrl = ""
    ...
    lazy var mainTableView: UITableView = {
        ...
        table.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: data_ageOfMessage, right: 0)
        //: return table
        return table
...
extension InAlertAction {
    //: func reqData() {
    func cookie() {
        //: var dict = Dictionary<String, Any>()
        var dict = [String: Any]()
        //: dict["tab"] = 1
        dict[(String(dataDeliveryTitle))] = 1
        //: dict["page"] = pageIndex
        dict[(data_ratioLegalInvolvementFormat.replacingOccurrences(of: "song", with: "g"))] = pageIndex
        //: TalkingMomentRequestTool.req_MomentList(params: dict) { succeed, result, errorModel in
        EndBuilderLifecycle.theCompletion(params: dict) { succeed, result, errorModel in
...
extension InAlertAction {
    ...
    @objc func plainMapNotification(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let uid: String = userinfo["uid"] as! String
        let uid: String = userinfo[(showGrayData.replacingOccurrences(of: "marketing", with: "d"))] as! String
    ...
    @objc func mirror(notification: NSNotification) {
        //: let userinfo = notification.userInfo as![String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: let model: TalkingMomentModel = userinfo["model"] as! TalkingMomentModel
        let model: AgencyMomentModel = userinfo[(const_innerEnableerPipeId.replacingOccurrences(of: "natural", with: "l"))] as! AgencyMomentModel
        //: for i in 0..<self.dataSourceArr.count {
        for i in 0 ..< self.dataSourceArr.count {
...
extension InAlertAction: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let identifier = NotifiableViewCell.className()
        //: var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? TalkingMomentItemCell
        var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? NotifiableViewCell
        //: if cell == nil {
        if cell == nil {
            //: cell = TalkingMomentItemCell(style: .default, reuseIdentifier: identifier)
            cell = NotifiableViewCell(style: .default, reuseIdentifier: identifier)
        }
    ...
    func tableView(_: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        if self.dataSourceArr.count > indexPath.row {
            ...
            if indexPath.row == 0 && self.isfirstvideoCell == true {
                ...
                if model.momentType == MigrationBasicType.Video.rawValue {
                    //: guard let tempcell: TalkingMomentItemCell = cell as? TalkingMomentItemCell  else {
                    guard let tempcell: NotifiableViewCell = cell as? NotifiableViewCell else {
                        //: return
                        return
                    }
                    //: tempcell.playVideo()
                    tempcell.operatingInstructions()
                }
            }
        }
    }
    ...
    func tableView(_: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let vc = AcrossMediaDuration(model: model)
...
extension InAlertAction: UIScrollViewDelegate {
    //: func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        //: scrollViewEndScrollPlayer(scrollView: scrollView)
        forefront(scrollView: scrollView)
    }
    ...
    func forefront(scrollView: UIScrollView) {
        ...
        if let middleIndexPath = mainTableView.indexPathForRow(at: CGPoint(x: 0, y: scrollView.contentOffset.y + mainTableView.frame.size.height / 2)) {
            ...
            if dataSourceArr.count > middleIndexPath.row {
                ...
                let cell: NotifiableViewCell = mainTableView.cellForRow(at: middleIndexPath) as! NotifiableViewCell
...
extension InAlertAction {
    ...
    private func pavement() {
        ...
        style.TipsTitle = (kSecretMsg + String(main_automaticData)).localized
    ...
    private func retain() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(mirror(notification:)), name: appLikeMsg, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(plainMapNotification(notification:)), name: showCreateValue, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(angleNotification(notification:)), name: mainBuildQuantityUrl, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(satisfy), name: user_inputFormat, object: nil)
    }
}
