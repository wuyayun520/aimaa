...
fileprivate let show_freshMessage:[UInt8] = [0x8a,0x8f,0x8a,0x95,0x49,0x84,0x90,0x85,0x86,0x93,0x5b,0x4a,0x41,0x89,0x82,0x94,0x41,0x8f,0x90,0x95,0x41,0x83,0x86,0x86,0x8f,0x41,0x8a,0x8e,0x91,0x8d,0x86,0x8e,0x86,0x8f,0x95,0x86,0x85]

fileprivate func workBoard(mutual num: UInt8) -> UInt8 {
...
/*: "icon_photo_giftbg" :*/
fileprivate let app_everyoneData:String = "rating document usericon_ph"
fileprivate let appTotalContent:String = "sure closed methodoto_g"
fileprivate let user_indexPath:[Character] = ["i","f","t","b","g"]

/*: "Send" :*/
fileprivate let const_lagNeverCapacityData:[Character] = ["S","e","n","d"]

/*: "nav_back_black_nor" :*/
fileprivate let data_pingName:[Character] = ["n","a","v","_","b","a","c","k","_","b","l","a","c","k","_","n","o","r"]

/*: "toUid" :*/
fileprivate let app_mildValue:[Character] = ["t","o","U","i","d"]

/*: "giftId" :*/
fileprivate let user_upAskKey:[Character] = ["g","i","f","t","I","d"]

/*: "giftNum" :*/
fileprivate let notiPlantMsg:String = "giftNumfilter robot formation"

/*: "unlockMsgId" :*/
fileprivate let mainLifeStr:String = "unlockMnet month operating escape place"
fileprivate let kAlongName:String = "issue bigsgId"

/*: "Need a %@ (%@ Gold coins) to unlock" :*/
fileprivate let constMobileName:[UInt8] = [0xc1,0xea,0xea,0xeb,0xaf,0xee,0xaf,0xaa,0xcf,0xaf,0xa7,0xaa,0xcf,0xaf,0xc8,0xe0,0xe3,0xeb,0xaf,0xec,0xe0,0xe6,0xe1,0xfc,0xa6,0xaf,0xfb,0xe0,0xaf,0xfa,0xe1,0xe3,0xe0,0xec,0xe4]

private func successCrop(advocate num: UInt8) -> UInt8 {
    return num ^ 143
}

/*: "icon_photo_lock" :*/
fileprivate let userAttachKey:String = "icon_pitem bottom absolute"
fileprivate let dataModeTitle:String = "filter approach attach_lock"

/*: "Click on the screen to view 15s" :*/
fileprivate let dataCounterName:[UInt8] = [0xc6,0xe9,0xec,0xe6,0xee,0xa5,0xea,0xeb,0xa5,0xf1,0xed,0xe0,0xa5,0xf6,0xe6,0xf7,0xe0,0xe0,0xeb,0xa5,0xf1,0xea,0xa5,0xf3,0xec,0xe0,0xf2,0xa5,0xb4,0xb0,0xf6]

/*: "icon_photo_burn" :*/
fileprivate let mainDemandTitle:String = "happyo"
fileprivate let notiLabelTitle:String = "n_phdecision both"

/*: "Already burned" :*/
fileprivate let user_controlBeatData:[Character] = ["A","l","r","e","a","d","y"]
fileprivate let constTheMessage:String = "await project media exclusive icy burned"
...
// __CLOSE_PRINT__
//
//  ConfirmRecognizerDelegate.swift
//  AbroadTalking
//
...
typealias IntimatePhotoBlock = (_ model: CarryModel) -> Void

//: class TalkingIntimatePhotoPreviewVC: TalkingBaseViewController {
class ConfirmRecognizerDelegate: SafetyFactorViewController {
    ...
    //: private var imageTotalData = ApartCellData(direction: .MsgDirectionOutgoing)
    private var imageTotalData = ApartCellData(direction: .MsgDirectionOutgoing) // 图片消息数据
    //: private var photoModel = AbTalkingChatMsgIntimatePhotoModel()
    private var photoModel = CarryModel() // 当前私密照片模型
    ...
    ///   - imageCellData: 图片模型
    ///   - toUid: 对方Id
    //: init(imageCellData: ApartCellData, toUid: String) {
    init(imageCellData: ApartCellData, toUid: String) {
        ...
        //: targetId = toUid
        targetId = toUid
        //: isMySelf = (imageTotalData.msgModel.user.uid == Int(FacultyReactiveCompatible.share.loginUserMode.userID))
        isMySelf = (imageTotalData.msgModel.user.uid == Int(FacultyReactiveCompatible.share.loginUserMode.userID))
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_freshMessage.map{workBoard(mutual: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var giftBgView: UIImageView = {
        ...
        img.image = UIImage.adName(name: (String(app_everyoneData.suffix(7)) + String(appTotalContent.suffix(5)) + String(user_indexPath)))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var giftIcon: UIImageView = {
        ...
        img.notFit(urlStr: photoModel.giftIcon)
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var giftSendBtn: UIButton = {
        ...
        btn.setTitle((String(const_lagNeverCapacityData)).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(sendNear), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    lazy var backBtn: UIButton = {
        ...
        let img = UIImage.adName(name: (String(data_pingName))).withRenderingMode(.alwaysTemplate)
...
extension ConfirmRecognizerDelegate {
    /// 调用送礼接口
    /// - Parameters:
    ///   - giftId: 礼物Id
    ///   - giftNum: 礼物数量
    //: func req_sendGiftMsg(giftId: String, giftNum: String) {
    func compute(giftId: String, giftNum: String) {
        ...
        params[(String(app_mildValue))] = self.targetId
        //: params["giftId"] = giftId
        params[(String(user_upAskKey))] = giftId
        //: params["giftNum"] = giftNum
        params[(String(notiPlantMsg.prefix(7)))] = giftNum
        //: params["unlockMsgId"] = self.imageTotalData.msgModel.msgInfo.msgId
        params[(String(mainLifeStr.prefix(7)) + String(kAlongName.suffix(4)))] = self.imageTotalData.msgModel.msgInfo.msgId
...
extension ConfirmRecognizerDelegate {
    ...
    @objc private func sendNear() {
        //: req_sendGiftMsg(giftId: photoModel.giftId, giftNum: "1")
        compute(giftId: photoModel.giftId, giftNum: "1")
    }
    ...
    @objc private func argument() {
        ...
        self.para(urlStr: photoModel.originUrl, isMosaic: false) { [weak self] isSucceed in
            //: ProgressHUD.dismiss()
            ...
            self.countdownTimer = Timer.scheduledTimer(withTimeInterval: 0.1, block: { [weak self] _ in
                ...
                self.progressView.forwarding(totalTime: 150, currTime: time)
                //: time -= 1
                time -= 1
                //: }, repeats: true)
            }, repeats: true)
...
extension ConfirmRecognizerDelegate {
    ...
    private func swallowConstraint() {
    ...
    private func canFireBy() {
        ...
        if photoModel.lockStatus == 1 { // 锁定状态，需要送礼解锁
            ...
            self.desLab.text = String(bytes: constMobileName.map{successCrop(advocate: $0)}, encoding: .utf8)!.extraPropertyLetter(photoModel.giftName, photoModel.giftPrice)
            ...
            if photoModel.burnStatus == 0 { // 未销毁
                ...
                self.intimateStatusImgV.image = UIImage.adName(name: (String(userAttachKey.prefix(6)) + "hoto" + String(dataModeTitle.suffix(5))))
                //: self.desLab.text = "Click on the screen to view 15s".localized
                self.desLab.text = String(bytes: dataCounterName.map{$0^133}, encoding: .utf8)!.localized
                // 添加手势
                //: let tap = UITapGestureRecognizer(target: self, action: #selector(burnPhotoTapGesture))
                let tap = UITapGestureRecognizer(target: self, action: #selector(argument))
                ...
                self.intimateStatusImgV.image = UIImage.adName(name: (mainDemandTitle.replacingOccurrences(of: "happy", with: "ic") + String(notiLabelTitle.prefix(4)) + "oto_burn"))
                //: self.desLab.text = "Already burned".localized
                self.desLab.text = (String(user_controlBeatData) + String(constTheMessage.suffix(7))).localized
            }
        ...
        self.para(urlStr: imgUrlStr, isMosaic: imgIsMosaic)
    }
    ...
    private func para(urlStr: String, isMosaic: Bool, completionHandler: ((_ isSucceed: Bool) -> Void)? = nil) {
