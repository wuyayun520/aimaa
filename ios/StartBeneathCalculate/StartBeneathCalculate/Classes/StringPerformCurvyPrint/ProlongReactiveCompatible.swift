...
extension ProlongReactiveCompatible {
    ...
    @objc func commentLimited(sender: UIButton) {
        ...
        if FacultyReactiveCompatible.share.loginUserMode.sex == self.userModel.sex {
            ...
            if self.userModel.isAttention {
                ...
                //: case 1:
                case 1:
                    //: DisplayAperturePresenter.share.func__pushToPriveteChatVC(chatID: self.userModel.uid, isFrom: .LiveRoom)
                    DisplayAperturePresenter.share.aboveCompletion(chatID: self.userModel.uid, isFrom: .LiveRoom)
                    //: self.dismiss()
                    self.substitutionShow()
                //: break
                //: default:
    ...
    @objc private func diorama() {
        //: DisplayAperturePresenter.share.func__pushToUserDetailVC(uid: self.userModel.uid)
        DisplayAperturePresenter.share.numbererest(uid: self.userModel.uid)
        //: self.dismiss()
        self.substitutionShow()
    }
}
...
extension ProlongReactiveCompatible {
    ...
    @objc func nonremittal() {
        ...
        let dropMenu = ControllerSystemScale.gameArray(anchorView: moreBtn, titleArray: titleArray, imageArray: [])
    ...
    func discountDraft() {
        ...
        //: popView?.showInView(view: self.currentViewController()?.view ?? AnimateUnaryAllocator.getWindow())
        popView?.ad(view: self.field()?.view ?? AnimateUnaryAllocator.frameworkWindow())
        ...
        if index == 0 {
            ...
            if self.userModel.isMute {
                ...
                PersonAlertShow.panAngleKind(title: nil, message: String(bytes: main_methodKey.map{$0^110}, encoding: .utf8)!.localized, leftBtnTitle: (String(show_endStageText.suffix(6))).localized, rightBtnTitle: (String(dataAccessName) + String(k_singleData)).localized) {
                    ...
                    AcrossLabelMend().travel(targetUid: self.userModel.uid, completion: { succeed, _, _ in
                        //: ProgressHUD.dismiss()
