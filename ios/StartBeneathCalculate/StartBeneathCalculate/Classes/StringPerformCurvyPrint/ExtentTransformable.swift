...
/*: "num" :*/
fileprivate let show_ownerPath:String = "sortm"
...
@objc public enum EmigrationMeasurable: Int {
    //: case normal  = 1
    case normal = 1 // 普通礼物
    //: case myStery = 2
    case myStery = 2 // 盲盒礼物
...
struct ExtentTransformable: HandyJSON {
    //: init() {
    init() {}
    ...
    var animationTimes = 0
    //: var showType = 0
    var showType = 0 /// EmigrationMeasurable
    ...
    mutating func resultSmooth(giftNum: Int) {
        //: self.num = giftNum
        self.num = giftNum
    }
}
...
struct HeMeasurable: HandyJSON {
    ...
    mutating func mapping(mapper: HelpingMapper) {
        ...
        mapper <<<
            //: self.giftNum <-- "num"
            self.giftNum <-- (show_ownerPath.replacingOccurrences(of: "sort", with: "nu"))
    }
}
