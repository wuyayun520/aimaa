...
fileprivate let const_addMessage:[UInt8] = [0xb6,0xbb,0xb6,0xc1,0x75,0xb0,0xbc,0xb1,0xb2,0xbf,0x87,0x76,0x6d,0xb5,0xae,0xc0,0x6d,0xbb,0xbc,0xc1,0x6d,0xaf,0xb2,0xb2,0xbb,0x6d,0xb6,0xba,0xbd,0xb9,0xb2,0xba,0xb2,0xbb,0xc1,0xb2,0xb1]

fileprivate func constraintPut(person num: UInt8) -> UInt8 {
...
/*: "icon_chat_lock" :*/
fileprivate let constRangeBearPlatData:String = "icon_ccream awake"

/*: "icon_chat_burned" :*/
fileprivate let constSeaValue:[Character] = ["i","c","o","n","_","c"]
fileprivate let main_likePath:String = "hat_source slip fast before multiple"

/*: "icon_chat_burned_shan" :*/
fileprivate let constRunningValue:[Character] = ["i","c","o","n","_","c","h","a","t","_","b","u","r","n","e","d","_","s","h","a","n"]

/*: "Already burned" :*/
fileprivate let dataEditionUrl:String = "Already shrink end ex"
fileprivate let noti_spaceName:String = "entryurned"
...
class ImageViewCell: AsyncMasterObserver {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: const_addMessage.map{constraintPut(person: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var lockImagV: UIImageView = {
        ...
        imag.image = UIImage.adName(name: (String(constRangeBearPlatData.prefix(6)) + "hat_lock"))
    ...
    lazy var burnedImagV: SeparateControlReactiveCompatible = {
        ...
        imgV.setBackgroundImage(UIImage.adName(name: (String(constSeaValue) + String(main_likePath.prefix(4)) + "burned")), for: .normal)
        //: imgV.setImage(UIImage.BundleImageNamed(name: "icon_chat_burned_shan"), for: .normal)
        imgV.setImage(UIImage.adName(name: (String(constRunningValue))), for: .normal)
        //: imgV.imageAlignment = .top
        imgV.imageAlignment = .top
        //: imgV.setTitle("Already burned".localized, for: .normal)
        imgV.setTitle((String(dataEditionUrl.prefix(8)) + noti_spaceName.replacingOccurrences(of: "entry", with: "b")).localized, for: .normal)
...
extension ImageViewCell {
    ...
    override func lineStreetwise(with data: CommonalityCellData) {
        ...
        //: guard let imageCellData = data as? ApartCellData else { return }
        guard let imageCellData = data as? ApartCellData else { return }
        ...
        let intimatePhoto: CarryModel? = imageCellData.msgModel.msgInfo.intimatePhoto
        ...
        if intimatePhoto != nil || imageCellData.msgModel.msgInfo.customSend.isBurn == "1" || Int(imageCellData.msgModel.msgInfo.customSend.giftId) ?? 0 > 0 { // 私密照片
            ...
            passingTakeMosaic(data: imageCellData, urlStr: imgUrlStr, isMosaic: imgIsMosaic)

            //: } else {
        } else { // 普通图片
            //: showImageView(data: imageCellData, urlStr: nil, isMosaic: false)
            passingTakeMosaic(data: imageCellData, urlStr: nil, isMosaic: false)
        }
    }
    ...
    ///   - urlStr: 图片网络地址
    ///   - isMosaic: 是否需要马赛克
    //: func showImageView(data: ApartCellData, urlStr: String?, isMosaic: Bool) {
    func passingTakeMosaic(data: ApartCellData, urlStr: String?, isMosaic: Bool) {
        ...
        if data.imageUrl!.count > 0, isExists {
            ...
            DispatchQueue.global().async {
                ...
                DispatchQueue.main.async {
                    ...
                    //: if isMosaic {
                    if isMosaic {
                        //: self.photoView.image = UIImage.aboveVideoPosition(image, to: 90)
                        self.photoView.image = UIImage.aboveVideoPosition(image, to: 90)
                        //: } else {
                    } else {
            ...
            have(urlStr: urlStr ?? "", isMosaic: isMosaic)
            //: } else {
        } else { // 本地图片请求网络
            ...
            if data.imageData != nil {
                ...
                self.photoView.notFit(urlStr: data.imageUrl!)
            }
        }
    }
    ...
    func have(urlStr: String, isMosaic: Bool) {
        //: self.photoView.setCookieToUrlImage(urlStr: urlStr) {[weak self] image in
        self.photoView.dismissGreen(urlStr: urlStr) { [weak self] image in
