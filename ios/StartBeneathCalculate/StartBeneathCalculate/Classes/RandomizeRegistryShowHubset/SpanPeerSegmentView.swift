...
fileprivate let data_expressionPath:[Character] = ["设","置","\u{666e}","通","状","\u{6001}","的","\u{6587}","字","颜","色","时"," ","请","使","用","R","G","B","空","\u{95f4}","的","颜","色","\u{503c}"]

/*: "设置选中状态的文字颜色时 请使用RGB空间的颜色值" :*/
fileprivate let app_rearTitle:String = "设置选phase"
fileprivate let app_modifyData:String = "字颜色时"
fileprivate let data_rawUrl:String = " \u{8bf7}使\u{7528}R"
fileprivate let noti_oatKey:String = "GB\u{7a7a}间"

/*: "init(coder:) has not been implemented" :*/
fileprivate let data_incentiveId:[UInt8] = [0x9,0xe,0x9,0x14,0x48,0x3,0xf,0x4,0x5,0x12,0x5a,0x49,0x40,0x8,0x1,0x13,0x40,0xe,0xf,0x14,0x40,0x2,0x5,0x5,0xe,0x40,0x9,0xd,0x10,0xc,0x5,0xd,0x5,0xe,0x14,0x5,0x4]

private func starDistance(former num: UInt8) -> UInt8 {
    return num ^ 96
}

/*: "设置的下标不合法!!" :*/
fileprivate let notiScenarioData:String = "设\u{7f6e}的下标不合"
fileprivate let notiCommentData:String = "\u{6cd5}!!"

/*: "Bags" :*/
fileprivate let mainLessName:String = "Bagstransaction currently happy triumph"
...
open class SpanPeerSegmentView: UIView {
    //: open var segmentStyle: SegmentStyle
    open var segmentStyle: LayoutColorExtension
    /// 点击响应
    ...
    fileprivate lazy var normalColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let normalRgb = self.black(self.segmentStyle.normalTitleColor) {
            //: return normalRgb
            return normalRgb
            //: } else {
        } else {
            //: fatalError("设置普通状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((String(data_expressionPath)))
        }
        //: }()
    }()
    ...
    fileprivate lazy var selectedTitleColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let selectedRgb = self.black(self.segmentStyle.selectedTitleColor) {
            //: return selectedRgb
            return selectedRgb
            //: } else {
        } else {
            //: fatalError("设置选中状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((app_rearTitle.replacingOccurrences(of: "phase", with: "中") + "状\u{6001}的文" + app_modifyData.capitalized + data_rawUrl + noti_oatKey + "的颜色值"))
        }
    ...
    fileprivate func black(_ color: UIColor) -> (r: CGFloat, g: CGFloat, b: CGFloat)? {
    ...
    public init(frame: CGRect, segmentStyle: LayoutColorExtension, titles: [String]) {
    ...
    @objc func remoteDetect(_ tapGes: UITapGestureRecognizer) {
        //: guard let currentLabel = tapGes.view as? CustomLabel else { return }
        guard let currentLabel = tapGes.view as? BuilderBridgeOrigin else { return }
    ...
    public required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: data_incentiveId.map{starDistance(former: $0)}, encoding: .utf8)!)
    }
}
...
public extension SpanPeerSegmentView {
    ///  对外界暴露设置选中的下标的方法 可以改变设置下标滚动后是否有动画切换效果
    //: public func selectedIndex(_ selectedIndex: Int, animated: Bool) {
    func bag(_ selectedIndex: Int, animated: Bool) {
        //: assert(!(selectedIndex < 0 || selectedIndex >= titles.count), "设置的下标不合法!!")
        assert(!(selectedIndex < 0 || selectedIndex >= titles.count), (notiScenarioData + notiCommentData))
...
//: extension ScrollSegmentView {
extension SpanPeerSegmentView {
    // 根据Titles添加相应的控件
    //: fileprivate func setupTitles() {
    fileprivate func equalNames() {
        //: for (index, title) in titles.enumerated() {
        for (index, title) in titles.enumerated() {
            //: let label = CustomLabel(frame: CGRect.zero)
            let label = BuilderBridgeOrigin(frame: CGRect.zero)
            //: label.tag = index
            label.tag = index
            //: label.text = title
            label.text = title
            ...
            let tapGes = UITapGestureRecognizer(target: self, action: #selector(self.remoteDetect(_:)))
            ...
            if title == (String(mainLessName.prefix(4))).localized {
    ...
    fileprivate func edgePosition() {
        ...
        if let firstLabel = labelsArray[0] as? BuilderBridgeOrigin {
    ...
    func occur(_: Bool) {
        ...
        if currentIndex == titles.firstIndex(of: (String(mainLessName.prefix(4))).localized) {
            //: redBagsLab.isHidden = true
            redBagsLab.isHidden = true
            //: TalkingChatGiftManager.share.showBagsRed = false
            RecognizeReactiveCompatible.share.showBagsRed = false
        }

        //: guard currentIndex != oldIndex else { return }
        guard currentIndex != oldIndex else { return }
        //: let oldLabel = labelsArray[oldIndex] as! CustomLabel
        let oldLabel = labelsArray[oldIndex] as! BuilderBridgeOrigin
        //: let currentLabel = labelsArray[currentIndex] as! CustomLabel
        let currentLabel = labelsArray[currentIndex] as! BuilderBridgeOrigin
        // 让选中标签居中显示
    ...
    public func document(_ currentIndex: Int) {
...
open class BuilderBridgeOrigin: UILabel {
    /// 用来记录当前label的缩放比例
