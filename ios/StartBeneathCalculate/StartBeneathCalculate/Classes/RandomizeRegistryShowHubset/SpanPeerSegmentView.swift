...
open class SpanPeerSegmentView: UIView {
    ...
    open var segmentStyle: TaSegmentStyle
    /// 点击响应
    ...
    public init(frame: CGRect, segmentStyle: TaSegmentStyle, titles: [String]) {
    ...
    @objc func remoteDetect(_ tapGes: UITapGestureRecognizer) {
        ...
        guard let currentLabel = tapGes.view as? StrangerThen else { return }
...
extension SpanPeerSegmentView {
    ...
    fileprivate func equalNames() {
        ...
        for (index, title) in titles.enumerated() {
            ...
            let label = StrangerThen(frame: CGRect.zero)
            //: label.tag = index
            label.tag = index
            //: label.text = title
            label.text = title
    ...
    fileprivate func edgePosition() {
        ...
        if let firstLabel = labelsArray[0] as? StrangerThen {
    ...
    func occur(_: Bool) {
        ...
        let oldLabel = labelsArray[oldIndex] as! StrangerThen
        //: let currentLabel = labelsArray[currentIndex] as! CustomLabel
        let currentLabel = labelsArray[currentIndex] as! StrangerThen
        // 让选中标签居中显示
...
open class StrangerThen: UILabel {
    /// 用来记录当前label的缩放比例
