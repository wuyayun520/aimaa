...
/*: "&sr=6" :*/
fileprivate let show_usedPath:String = "season recommendation sample&sr=6"
...
enum EveryContentMagnitudeerpolation: String, CaseIterable {
...
class HandleOptionInstance: NSObject {
    //: class func getFullUrl(urlType: WebUrlType)-> String {
    class func hire(urlType: BorderBasicType) -> String {
        //: var url = FacultyReactiveCompatible.share.appConfigMode.urlH5Domain
        var url = FacultyReactiveCompatible.share.appConfigMode.urlH5Domain
        //: switch urlType {
        switch urlType {
        //: case .RechargeFullPage:
        case .RechargeFullPage:
            //: url.append(FacultyReactiveCompatible.share.appUserConfigMode.fullRechargePage.isEmptyString ? urlType.rawValue : FacultyReactiveCompatible.share.appUserConfigMode.fullRechargePage)
            url.append(FacultyReactiveCompatible.share.appUserConfigMode.fullRechargePage.isEmptyString ? urlType.rawValue : FacultyReactiveCompatible.share.appUserConfigMode.fullRechargePage)

        //: break
        //: case .RechargeHalfPage:
        case .RechargeHalfPage:
            //: url.append(FacultyReactiveCompatible.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : FacultyReactiveCompatible.share.appUserConfigMode.halfRechargePage)
            url.append(FacultyReactiveCompatible.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : FacultyReactiveCompatible.share.appUserConfigMode.halfRechargePage)

        //: case .RechargeToVideoInitivHalfPage:
        case .RechargeToVideoInitivHalfPage:
            //: let newstr = FacultyReactiveCompatible.share.appUserConfigMode.halfRechargePage.appending("&sr=6")
            let newstr = FacultyReactiveCompatible.share.appUserConfigMode.halfRechargePage.appending((String(show_usedPath.suffix(5))))
            //: url.append(FacultyReactiveCompatible.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : newstr)
            url.append(FacultyReactiveCompatible.share.appUserConfigMode.halfRechargePage.isEmptyString ? urlType.rawValue : newstr)
        //: default:
        default:
