...
fileprivate let user_tunValue:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_chats_banner_subtract" :*/
fileprivate let showExecuteReasonName:[Character] = ["i","c","o","n","_","c","h","a","t","s","_","b","a","n","n","e","r","_","s","u","b","t","r","a","c","t"]

/*: "icon_chats_banner_subtract_pre" :*/
fileprivate let k_instanceLabelKey:String = "icon_joint narrow"
fileprivate let kBulletName:String = "_bannedirect editor maximum fun clot"
fileprivate let dataRangeName:[Character] = ["b"]
fileprivate let show_todayTitle:[Character] = ["t","r","a","c","t","_","p","r","e"]

/*: "bannerList" :*/
fileprivate let data_demonstrateFormat:[Character] = ["b","a","n","n"]
fileprivate let data_assessmentStr:[Character] = ["e","r","L","i","s","t"]

/*: "JXBannerCellID" :*/
fileprivate let mainOperationContent:[Character] = ["J","X","B","a","n","n","e"]
fileprivate let data_chamberName:String = "require hold categoryrC"
fileprivate let dataPunishMessage:[Character] = ["e","l","l","I","D"]

/*: "#FFFFFF" :*/
fileprivate let appLogicalFormat:[Character] = ["#"]
fileprivate let main_greenCommonText:[Character] = ["F","F","F","F","F","F"]
...
class PrivateDataSource: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: user_tunValue.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var bannerListData: [LackTransformable] = //: return Array<LackTransformable>()
        .init()
    //: }()
    ...
    private lazy var closeBtn: UIButton = {
        ...
        btn.setImage(UIImage.adName(name: (String(showExecuteReasonName))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_chats_banner_subtract_pre"), for: .selected)
        btn.setImage(UIImage.adName(name: (String(k_instanceLabelKey.prefix(5)) + "chats" + String(kBulletName.prefix(6)) + "r_su" + String(dataRangeName) + String(show_todayTitle))), for: .selected)
...
extension PrivateDataSource {
    /// banner
    //: func reqBanner() {
    func anyPrivacy() {
        //: SocialRequestManager.init().req_adBanner(position: 3) { [weak self] succeed, result, errorModel in
        AudiogramRequestManager().radish(position: 3) { [weak self] _, result, _ in
            ...
            let array = JSON(result[(String(data_demonstrateFormat) + String(data_assessmentStr))] ?? []).arrayValue
...
extension PrivateDataSource {
    ...
    @objc private func onClick() {
        ...
        self.delegate?.bannerEye(isMiniView: isMiniView)
    }

    //: func resetView(isMiniView: Bool) {
    func checkion(isMiniView: Bool) {
        //: if isMiniView {
        if isMiniView {
...
extension PrivateDataSource: JXBannerDelegate, JXBannerDataSource {
    ...
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        ...
        return JXBannerCellRegister(type: JXBannerCell.self, reuseIdentifier: (String(mainOperationContent) + String(data_chamberName.suffix(2)) + String(dataPunishMessage)))
    }
    ...
    func jxBanner(_: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
        ...
        tempCell.imageView.sortAge(urlStr: url, placeImg: UIImage.scribe())
                  ...
                  builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder
    {
        ...
        pageControl.activeColor = UIColor(hex: (String(appLogicalFormat) + String(main_greenCommonText)))!
