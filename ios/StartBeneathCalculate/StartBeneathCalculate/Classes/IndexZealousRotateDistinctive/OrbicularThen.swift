...
fileprivate let app_spectrumName:[UInt8] = [0xf6,0xf1,0xf6,0xeb,0xb7,0xfc,0xf0,0xfb,0xfa,0xed,0xa5,0xb6,0xbf,0xf7,0xfe,0xec,0xbf,0xf1,0xf0,0xeb,0xbf,0xfd,0xfa,0xfa,0xf1,0xbf,0xf6,0xf2,0xef,0xf3,0xfa,0xf2,0xfa,0xf1,0xeb,0xfa,0xfb]

private func belowYou(partner num: UInt8) -> UInt8 {
    return num ^ 159
}

/*: "move" :*/
fileprivate let userLiveStr:String = "mokeep"
...
class OrbicularThen: UIView {
    ...
    var danumuMsgArray: [DigitalRefreshScreenModel] = []
    //: var danumuItemViewArray: [TalkingFloatScreenItemView] = []
    var danumuItemViewArray: [ExamReactiveCompatible] = []

    //: let TapAction: PublishSubject<TalkingFloatingScreenModel?> = PublishSubject()
    let TapAction: PublishSubject<DigitalRefreshScreenModel?> = PublishSubject()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_spectrumName.map{belowYou(partner: $0)}, encoding: .utf8)!)
    }
    ...
    func detail(_ danumuMsgModel: DigitalRefreshScreenModel?) {
    ...
    @objc func visitor(_: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(visitor(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension OrbicularThen: UIGestureRecognizerDelegate {
    ...
    func decision(for danumuMsgModel: DigitalRefreshScreenModel?, isFirst: Bool) {
        //: let itemView = TalkingFloatScreenItemView(model: danumuMsgModel)
        let itemView = ExamReactiveCompatible(model: danumuMsgModel)
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (userLiveStr.replacingOccurrences(of: "keep", with: "ve")))
    ...
    func outThan(_ isFirst: Bool) {
        ...
        if danumuMsgArray.count > 0 {
            ...
            if let model {
                ...
                decision(for: model, isFirst: isFirst)
...
extension OrbicularThen {
    // 添加视图
    //: private func setupSubviews() {
    private func passingArray() {
