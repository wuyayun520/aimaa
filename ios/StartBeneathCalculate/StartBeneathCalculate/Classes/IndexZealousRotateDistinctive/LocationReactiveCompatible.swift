...
/*: "#ffffff" :*/
fileprivate let app_systemName:String = "#ffffffarray shake processing"

/*: "earned" :*/
fileprivate let mainAdoptMsg:String = "earcityd"

/*: "Coins in" :*/
fileprivate let data_motionFormat:[Character] = ["C","o","i"]
fileprivate let noti_borderKey:String = "ns indistant public"

/*: "Error:  :*/
fileprivate let showBounceFormat:String = "shrink currentlyError:"
fileprivate let show_orientationData:String = " "

/*: "#C946D2" :*/
fileprivate let userQuoteFormat:String = "#C946D2mill project coal never and"

/*: "btn_chat_floatS_userBK_nor" :*/
fileprivate let constPreviousStr:[Character] = ["b","t","n","_","c","h","a","t","_","f","l","o","a","t","S","_","u","s","e","r","B","K","_","n","o","r"]
...
// __CLOSE_PRINT__
//
//  LocationReactiveCompatible.swift
//  AbroadTalking
//
...
class LocationReactiveCompatible: UIView {
    ...
    private let TextColor = (String(app_systemName.prefix(7)))

    //: var clickBlock: (() -> Void)?
    var clickBlock: (() -> Void)?

    //: convenience init(model: TalkingGameFloatingScreenModel?) {
    convenience init(model: NameTheoryMeasurable?) {
        //: self.init()
        self.init()
    ...
    private func album() {
        ...
        if danumuMsgModel.bgPic.count > 0 {
            ...
            terrace(withUrl: danumuMsgModel.bgPic)
        }
        ...
        if danumuMsgModel.content.count > 0 {
            ...
            do {
                ...
                if let range = attrStr.string.range(of: (mainAdoptMsg.replacingOccurrences(of: "city", with: "ne"))) {
                ...
                if let range = attrStr.string.range(of: (String(data_motionFormat) + String(noti_borderKey.prefix(5)))) {
                    //: attrStr.addAttributes([.foregroundColor: UIColor.white], range: NSRange(range, in: attrStr.string))
                    attrStr.addAttributes([.foregroundColor: UIColor.white], range: NSRange(range, in: attrStr.string))
                }
                //: attrStr.addAttributes([.font: UIFont.pingfangFont(type: .Medium, fontSize: 12)], range: NSRange(location: 0, length: attrStr.length))
                attrStr.addAttributes([.font: UIFont.recipeDecision(type: .Medium, fontSize: 12)], range: NSRange(location: 0, length: attrStr.length))
                ...
                printLog(message: (String(showBounceFormat.suffix(6)) + show_orientationData.capitalized) + "\(error)")
            }
        }
    }

    //: func updateBackground(withUrl url: String) {
    func terrace(withUrl url: String) {
        ...
        self.backgroundImageView.sortAge(urlStr: url) { [weak self] _, image in
            ...
            if let image {
                ...
                if image.size.height > self.HeightOfBackgroundImageView {
                    ...
                    newImage = image.dataQuality(with: CGSize(width: targetWidth, height: self.HeightOfBackgroundImageView))
                }
            }
    ...
    var danumuMsgModel: NameTheoryMeasurable? {
        //: didSet {
        didSet {
            //: configModel()
            album()
    ...
    var totalTime: CGFloat {
        ...
        return (main_assetUrl + widthValue) * SpendTime / main_assetUrl
    }

    //: var totalTimeOfLine: CGFloat {
    var totalTimeOfLine: CGFloat {
        //: return widthValue * LineSpendTime / ScreenWidth
        return widthValue * LineSpendTime / main_assetUrl
    }

    //: var hide2ShowTime: CGFloat {
    var hide2ShowTime: CGFloat {
        //: return widthValue * SpendTime / ScreenWidth
        return widthValue * SpendTime / main_assetUrl
    }
    ...
    lazy var headAnimatedImageView: UIImageView = {
        ...
        h.layer.borderColor = UIColor(hex: (String(userQuoteFormat.prefix(7))))?.cgColor
        //: return h
        return h
        //: }()
    }()
    ...
    lazy var headBackImageView: UIImageView = {
        ...
        h.image = UIImage.adName(name: (String(constPreviousStr)))
        //: return h
        return h
        //: }()
    }()
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(isShape(_:)))
        //: return t
        return t
        //: }()
    }()
...
extension LocationReactiveCompatible {
    /// 点击跳转事件
    //: @objc func tapAction(_ tapGesture: UITapGestureRecognizer) {
    @objc func isShape(_: UITapGestureRecognizer) {
        //: guard let gameUrl = self.danumuMsgModel?.gameUrl else { return }
        guard let gameUrl = self.danumuMsgModel?.gameUrl else { return }
        //: let httpsUrl = FacultyReactiveCompatible.share.appConfigMode.urlH5Domain+gameUrl
        let httpsUrl = FacultyReactiveCompatible.share.appConfigMode.urlH5Domain + gameUrl
        ...
        config.clearBgColor = true
        //: DisplayAperturePresenter.share.func__pushToWebVC(urlStr: httpsUrl, webConfig: config)
        DisplayAperturePresenter.share.lifeStyle(urlStr: httpsUrl, webConfig: config)
    }
}
...
extension LocationReactiveCompatible {
    ...
    private func sessionFrameRender() {
    ...
    private func permissionAutomatically() {
