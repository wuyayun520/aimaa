...
class LocationThen: NSObject {
    ...
    static let shared = LocationThen()
...
extension LocationThen {
    ...
    func nativeSize(_ tasksArray: [RecentTaskModel]) {
        ...
        for model in filterArray {
            ...
            SpireThen.courseWith(msgId: model.taskId, status: "\(ReadyBroadTransformable.prepare.rawValue)", filePath: model.zipFilePath)
...
extension LocationThen: URLSessionDataDelegate {
    // 开始
...
extension LocationThen {
    ...
    func rawTa(url: String, isSuspend: Bool) {
        ...
        case .chatRecord:
            ...
            SpireThen.courseWith(msgId: taskModel.taskId, status: status)
            //: if isSuspend == false {
            if isSuspend == false {
    ...
    func again(url: String) {
        ...
        case .chatRecord:
            ...
            SpireThen.transform(msgId: taskModel.taskId)
    ...
    func sparkUrl(url: String) {
        ...
        if size > 0 {
            ...
            if taskModel.taskTypeId == .chatRecord {
                ...
                SpireThen.courseWith(msgId: taskModel.taskId, status: "\(ReadyBroadTransformable.finish.rawValue)")
            }
            ...
            case .chatRecord:
                ...
                SpireThen.courseWith(msgId: taskModel.taskId, status: "\(ReadyBroadTransformable.expired.rawValue)")
            ...
            if let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
                ...
                if let handle = FileHandle(forWritingAtPath: newPath) {
                    ...
                    if typeId == .chatRecord {
                        ...
                        SpireThen.courseWith(msgId: keyId, filePath: newPath)
                    }
