...
fileprivate let app_optionMessage:[UInt8] = [0x96,0x91,0x96,0x8b,0xd7,0x9c,0x90,0x9b,0x9a,0x8d,0xc5,0xd6,0xdf,0x97,0x9e,0x8c,0xdf,0x91,0x90,0x8b,0xdf,0x9d,0x9a,0x9a,0x91,0xdf,0x96,0x92,0x8f,0x93,0x9a,0x92,0x9a,0x91,0x8b,0x9a,0x9b]

private func toolMethod(number num: UInt8) -> UInt8 {
    return num ^ 255
}

/*: "btnJQbEAB_etihw_kcab_shadow" :*/
fileprivate let showCleanHoneyData:String = "btnJQbattention backyard"
fileprivate let userWrittenData:[Character] = ["E","A","B","_","e","t","i","h","w","_","k","c","a","b","_","s"]
fileprivate let userMovementName:[Character] = ["h","a","d","o","w"]

/*: "Edit" :*/
fileprivate let constCounterruptName:[Character] = ["E","d","i","t"]

/*: "Finish" :*/
fileprivate let user_creationMsg:String = "native ink reduce full liteFinish"

/*: "btnYDo2AS_oediv_play" :*/
fileprivate let dataQualityPath:String = "btnYapply invite relieve why"
fileprivate let app_smartPath:String = "div_link price world"

/*: "Cancel" :*/
fileprivate let constLawnExistingMsg:String = "Cancelarea may"

/*: "bgQkRTbF_oediv_toast" :*/
fileprivate let main_grossMsg:String = "bgQkfound recipe"
fileprivate let k_stageData:String = "oediv_tink bronze yesterday stretch"
fileprivate let notiOutputFormat:String = "oasprice"

/*: "The video length should be %@-%@ seconds. Please intercept it" :*/
fileprivate let noti_hiddenFormat:[UInt8] = [0x74,0x69,0x20,0x74,0x70,0x65,0x63,0x72,0x65,0x74,0x6e,0x69,0x20,0x65,0x73,0x61,0x65,0x6c,0x50,0x20,0x2e,0x73,0x64,0x6e,0x6f,0x63,0x65,0x73,0x20,0x40,0x25,0x2d,0x40,0x25,0x20,0x65,0x62,0x20,0x64,0x6c,0x75,0x6f,0x68,0x73,0x20,0x68,0x74,0x67,0x6e,0x65,0x6c,0x20,0x6f,0x65,0x64,0x69,0x76,0x20,0x65,0x68,0x54]
...
typealias VideoEditBlock = (_ videoInfo: MotionMeasurable?) -> Void

//: let kMinVideoDuration: CGFloat = 5.0
let notiNameStr: CGFloat = 5.0

//: class TalkingVideoEditorController: TalkingBaseViewController {
class SmartViewController: SafetyFactorViewController {
    ...
    private var slider: DenseCubitThroughput?
    //: private var videoInfo = TalkingVideoInfoModel()
    private var videoInfo = MotionMeasurable() // 编辑结果模型
    //: private var currStyle = TalkingVideoEditorStyle.normal
    private var currStyle = AEditorStyle.normal // 当前样式
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        //: if TalkingSocketManager.shared.isGeneral == true {
        if ConfigureAxisService.shared.isGeneral == true { // 正在音视频通话，静音播放
    ...
    init(style: AEditorStyle = .normal, maxDuration: CGFloat, videoURL: URL, completionHandler: @escaping VideoEditBlock) {
        ...
        imgWidth = (main_assetUrl - 2 * padding) / (framesInScreen * 1.0)
        //: imgHeight = 50
        imgHeight = 50
        //: duration = 0
        duration = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_optionMessage.map{toolMethod(number: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var backBtn: UIButton = {
        ...
        v.setImage(UIImage.adName(name: (String(showCleanHoneyData.prefix(6)) + String(userWrittenData) + String(userMovementName))), for: .normal)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = LocalRouterDescription.shared.direction == .rightToLeft ? .right : .left
        //: v.addTarget(self, action: #selector(backAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(medium), for: .touchUpInside)
    ...
    private lazy var editBtn: UIButton = {
        ...
        v.setTitle((String(constCounterruptName)).localized, for: .normal)
        //: v.setTitleColor(UIColor.appThemeColor(), for: .normal)
        v.setTitleColor(UIColor.crosswise(), for: .normal)
        //: v.titleLabel?.font = UIFont.fontRaw(fontSize: 14)
        v.titleLabel?.font = UIFont.fontRaw(fontSize: 14)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = LocalRouterDescription.shared.direction == .rightToLeft ? .right : .left
        //: v.isHidden = true
...
//        v.addTarget(self, action: #selector(editAction(sender:)), for: .touchUpInside)
    ...
    private lazy var bottomDoneBtn: UIButton = {
        ...
        v.setTitle((String(user_creationMsg.suffix(6))).localized, for: .normal)
        //: v.setTitleColor(UIColor.white, for: .normal)
        v.setTitleColor(UIColor.white, for: .normal)
        //: v.titleLabel?.font = UIFont.pingfangMediumFont(fontSize: 14)
        v.titleLabel?.font = UIFont.originally(fontSize: 14)
        //: v.layer.cornerRadius = 14.0
        v.layer.cornerRadius = 14.0
        //: v.layer.masksToBounds = true
        v.layer.masksToBounds = true
        //: v.backgroundColor = UIColor.appThemeColor()
        v.backgroundColor = UIColor.crosswise()
        //: v.addTarget(self, action: #selector(bottomDoneAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(timeImmemorial), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: return v
        return v
    ...
    private lazy var playBtn: UIButton = {
        ...
        v.setImage(UIImage.adName(name: (String(dataQualityPath.prefix(4)) + "Do2AS_oe" + String(app_smartPath.prefix(4)) + "play")), for: .normal)
        //: v.addTarget(self, action: #selector(playAction(sender:)), for: .touchUpInside)
        v.addTarget(self, action: #selector(paddyThroughFreed(sender:)), for: .touchUpInside)
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        //: return v
        return v
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        v.setTitle((String(constLawnExistingMsg.prefix(6))).localized, for: .normal)
    ...
    private lazy var confirmBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(confrim), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: v.snp.makeConstraints { make in
        v.snp.makeConstraints { make in
    ...
    private lazy var toastImageView: UIImageView = {
        ...
        let v = UIImageView(image: UIImage.adName(name: (String(main_grossMsg.prefix(4)) + "RTbF_" + String(k_stageData.prefix(7)) + notiOutputFormat.replacingOccurrences(of: "price", with: "t"))))
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        ...
        lab.text = String(bytes: noti_hiddenFormat.reversed(), encoding: .utf8)!.extraPropertyLetter(minDuration, Int(maxduration))
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.numberOfLines = 0
        lab.numberOfLines = 0
    ...
    lazy var useVideoView: LocationInfoView = {
        //: let v = TalkingSendIntimateInfoView(frame: CGRect.zero, type: .intimateVideo)
        let v = LocationInfoView(frame: CGRect.zero, type: .intimateVideo)
        //: v.isHidden = true
        v.isHidden = true
        //: self.view.addSubview(v)
        self.view.addSubview(v)
...
extension SmartViewController {
    ...
    private func animated(animated _: Bool = true) {
    ...
    @objc func subedit() {
        ...
        self.previewLayer?.frame = CGRect(x: 26, y: 26, width: main_assetUrl - 2 * 26, height: kTopUrl - 46 - 127 - data_cornerMessage)
        //: imgWidth = ScreenWidth / framesInScreen
        imgWidth = main_assetUrl / framesInScreen

        //: if slider == nil {
        if slider == nil {
            //: let frame = CGRect(x: 0, y: ScreenHeight, width: ScreenWidth, height: CGFloat(imgHeight))
            let frame = CGRect(x: 0, y: kTopUrl, width: main_assetUrl, height: CGFloat(imgHeight))
            // 创建预览区域
            ...
            slider = DenseCubitThroughput(frame: frame, url: self.videoURL!, imgWidth: imgWidth, maxDuration: maxduration, minVideoDuration: notiNameStr)
            ...
            slider?.snp.makeConstraints { make in
                ...
                make.bottom.equalToSuperview().offset(-60 - data_cornerMessage)
...
//                self.playing = false
            ...
            slider?.slidingBlock = { [weak self] (direction: ItinerarySlideDirection) in
                //: guard let self = self else { return }
                guard let self = self else { return }
...
//                        self.playBtn.isHidden = true
    ...
    @objc func timeImmemorial() {
        ...
        if self.videoEditHandler != nil {
            ...
            ActivityBuildListener.shared.signalWork(videoInfo: videoInfo, containerView: self.view, saveDocuments: true) { [weak self] outputPath, coverPath in
                //: guard let self = self else { return }
                guard let self = self else { return }
                //: self.popCurrentViewController()
                self.animated()
    ...
    @objc func paddyThroughFreed(sender: UIButton) {
...
extension SmartViewController {
    ...
    private func tandoor() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(playbackTo), name: NSNotification.Name.AVPlayerItemDidPlayToEndTime, object: item)
        ...
        self.playerTimer = self.player?.addPeriodicTimeObserver(forInterval: step, queue: DispatchQueue.main, using: { [weak self] time in
            ...
            if self.slider != nil, self.slider!.isHidden == false {
                ...
                self.slider?.overlay.exceptFamilyXposition(xposition)
            }
            //: }) as AnyObject?
        }) as AnyObject?
    ...
    private func formatInstall() {
        ...
        self.botBar.snp.makeConstraints { make in
            ...
            make.height.equalTo(48 + data_cornerMessage)
        }
    ...
    private func funcNotification() {
        // 监听应用进入后台
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(didEnterBackground),
                                               selector: #selector(taskWithOffst),
