...
class MigrationThen: NSObject {
    /// 上传7天内
    //: private let uploadDay = 7
    private let uploadDay = 7
    //: static let shared = UploadLogTool()
    static let shared = MigrationThen()
    ...
    func leaveWithLog(_ showTips: Bool = true) {
        ...
        if let tempPath = MigrationThen.hospital(), let zipPath = MigrationThen.pastMirror() {
    ...
    private func fashion(logPath: String) {
        ...
        subpaths.forEach { fileName in
            ...
            if fileDate > daysAgo,
               ...
               let tempPath = MigrationThen.hospital()
            {
