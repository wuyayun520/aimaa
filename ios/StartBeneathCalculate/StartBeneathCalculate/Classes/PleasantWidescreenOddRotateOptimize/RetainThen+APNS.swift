    ...
    class func ageAttach(didRegisterForRemoteNotificationsWithDeviceToken deviceToken: Data) {
        ...
        Messaging.messaging().token { token, error in
            ...
            if let error = error {
                ...
                MigrationThen.pathAge(msg: (String(k_tempData.suffix(5)) + "Token " + mainUponUrl.replacingOccurrences(of: "lat", with: "E") + String(dataBlindUrl.prefix(6))) + "\(error).")
        ...
        if UIApplication.shared.applicationState != .active {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 1) {
                ...
                if userInfo.keys.contains((kCorrectValue.replacingOccurrences(of: "window", with: "a"))) {
                    ...
                    PrecedencyThen.share.APNSUserInfoStr = extraStr
                    //: TalkingAPNSManager.share.launchFromApns = true
                    PrecedencyThen.share.launchFromApns = true
                }
            }
        }
    }
