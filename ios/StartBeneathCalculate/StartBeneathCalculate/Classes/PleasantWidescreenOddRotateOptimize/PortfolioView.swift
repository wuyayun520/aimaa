...
extension PortfolioView {
    ...
    private func headSubviews() {
        ...
        var buttonX: CGFloat = LocalRouterDescription.shared.direction == .leftToRight ? -5 : 20
        ...
        for (i, item) in items.enumerated() {
            ...
            addSubview(itemButton)
            //: let x = LanguageManager.shared.direction == .leftToRight ? buttonX:(AnimateUnaryAllocator.getScreenWidth()-buttonX-buttonW)
            let x = LocalRouterDescription.shared.direction == .leftToRight ? buttonX : (AnimateUnaryAllocator.areaWidth() - buttonX - buttonW)
