...
extension RetainThen {
    ...
    func pointEvent(_: UIApplication) {
        ...
        //: IonV2Listener.shared.func__TXSDKInit()
        IonV2Listener.shared.sh()
...
extension RetainThen: TXUGCBaseDelegate {
    ...
    public func onLicenceLoaded(_ result: Int32, reason: String!) {
        ...
        MigrationThen.pathAge(msg: (String(userPassingTitle) + k_earlyPath.replacingOccurrences(of: "evaluate", with: "r") + String(notiAcceptId.suffix(7))) + "\(result)" + (String(k_redStr) + String(main_halfDeliveryStr)) + "\(String(describing: reason)).")
    }
}
