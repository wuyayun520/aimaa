...
fileprivate let appDialogManageMessage:[UInt8] = [0x32,0x35,0x32,0x2f,0x73,0x38,0x34,0x3f,0x3e,0x29,0x61,0x72,0x7b,0x33,0x3a,0x28,0x7b,0x35,0x34,0x2f,0x7b,0x39,0x3e,0x3e,0x35,0x7b,0x32,0x36,0x2b,0x37,0x3e,0x36,0x3e,0x35,0x2f,0x3e,0x3f]
...
enum StoichiometrySwipeDirection: CaseIterable {
    //: case Left
    case Left
    //: case Right
    case Right
...
protocol DticObjectProtocol: NSObjectProtocol {
    ///
...
class RetainView: UIView {
    ...
    var cell_delegate: DticObjectProtocol?

    //: var model: TalkingMatchResultModel?
    var model: GenerationResultModel?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: appDialogManageMessage.map{$0^91}, encoding: .utf8)!)
    }
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(pachytene(pan:)))
        //: v.delegate = self
        v.delegate = self
        //: return v
        return v
...
extension RetainView: UIGestureRecognizerDelegate {
    ...
    @objc func pachytene(pan: UIPanGestureRecognizer) {
        //: switch pan.state {
        switch pan.state {
        //: case .began:
        case .began:
        ...
        case .changed:
            ...
            if LocalRouterDescription.shared.direction == .rightToLeft {
                ...
                if movePoint.x > main_assetUrl / 4 {
                    ...
                    self.cell_delegate?.viewProductivity(cell: self, forMovePoint: self.currentPoint)
    ...
    func local() {
        ...
        self.cell_delegate?.beforeAllowCell(cell: self)
    }
    ...
    func collectionSuperviewRandom(direction: StoichiometrySwipeDirection) {
        //: switch (direction) {
        switch direction {
        //: case .Left:
        case .Left:
