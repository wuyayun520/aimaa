...
public class RetainThen: NSObject {
    // 后台下载
    ...
    public static let shared = RetainThen()
...
public extension RetainThen {
    ...
    func kitCommit(_ application: UIApplication, didFinishLaunchingWithOptions _: [UIApplication.LaunchOptionsKey: Any]?, window: UIWindow?) -> Bool {
        ...
        RetainThen.shared.window = window
        ...
        RetainThen.shared.agnize()
        //: AppDelegateHelper.shared.initGetCache()
        RetainThen.shared.packageNest()
        ...
        RetainThen.shared.currApplication = application
        // 支持后台播放音乐
        //: do {
        do {
            ...
            MigrationThen.pathAge(msg: (showKindValue + "乐模式异常: ") + "\(error).")
        }
    ...
    class func productivity(_: UIApplication) {
        ...
        RetainThen.shared.mated()
        // 进入前台，结束后台任务
        //: AppDelegateHelper.shared.checkAndEndBackgroundTask()
        RetainThen.shared.lengthByTask()
    }
    ...
    class func partnerAlbum(_: UIApplication, handleEventsForBackgroundURLSession _: String, completionHandler: @escaping () -> Void) {
        ...
        RetainThen.shared.backgroundSessionCompletionHandler = completionHandler
    }
}
...
extension RetainThen {
    ...
    @objc private func noneNetwork() {
        ...
        if currApplication != nil {
            ...
            RetainThen.shared.pointEvent(currApplication!)
        }
    ...
    private func regularApplication() {
        ...
        config.isHideBackBtn = true
        //: SmartThen.share.func__pushToWebVC(webViewType: .ForcedTransfer, webConfig: config)
        SmartThen.share.constituent(webViewType: .ForcedTransfer, webConfig: config)
    }
    ...
    private func agnize() {
        ...
        NotificationCenter.default.rx
            ...
            .subscribe(onNext: { _ in
                ...
                AlsoThen.shared.photographicCamera(userLoginOut: true)
        ...
        NotificationCenter.default.rx.notification(app_textValue)
            ...
            .subscribe(onNext: { _ in
                ...
                TotalThen.sortPending { succeed, _, _ in
