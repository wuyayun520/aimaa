...
/*: "gifts" :*/
fileprivate let k_temporaryMsg:[Character] = ["g","i","f","t","s"]

/*: "vipGifts" :*/
fileprivate let noti_equallyMessage:String = "vipGiftsbig face brother my"

/*: "packGifts" :*/
fileprivate let userDetailMsg:String = "cap character former sectionpackGift"
fileprivate let user_featureAdjustmentPath:String = "business"

/*: "giftSendItem" :*/
fileprivate let mainTranslationFormat:[Character] = ["g","i","f","t","S"]
fileprivate let constSignalId:String = "endItemcloud say phone"
...
// __CLOSE_PRINT__
//
//  RecognizeReactiveCompatible.swift
//  AbroadTalking
//
...
class RecognizeReactiveCompatible: NSObject {
    /// 是否展示背包红点
    ...
    private var cacheDict = [String: [AvailableAfterInsetTarget: [Any]]]()
    // 数据锁
    ...
    private var giftType = LikewiseClusterLiteral.normal

    //: static let share = TalkingChatGiftManager()
    static let share = RecognizeReactiveCompatible()
...
extension RecognizeReactiveCompatible {
    /// 发送礼物面板数据
    /// - Parameters:
    ...
    func connectView(type: LikewiseClusterLiteral = .normal, isRefresh: Bool = false, completion: @escaping () -> Void) {
        //: TalkingChatGiftManager.share.giftType = type
        RecognizeReactiveCompatible.share.giftType = type
        //: if isRefresh == false {
        if isRefresh == false {
            //: let dataDict = cacheDict[type.rawValue]
            let dataDict = cacheDict[type.rawValue]
            //: let hotArr = dataDict?[GiftDataType.Hot] ?? [Any]()
            let hotArr = dataDict?[AvailableAfterInsetTarget.Hot] ?? [Any]()
        ...
        if type == .groupChat {
            ...
            RecognizeReactiveCompatible.share.rowBy(completion: { isSucceed in
            ...
            RecognizeReactiveCompatible.share.frontBurner(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            RecognizeReactiveCompatible.share.suppress(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
    ...
    private func suppress(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = RecognizeReactiveCompatible.share.giftType
        //: TalkingChatRequestTool.req_ChatGiftList(type: type.rawValue) { succeed, result, errorModel in
        PouchReactiveCompatible.chatPerformCompletion(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
                //: var dataDict = [GiftDataType: [Any]]()
                var dataDict = [AvailableAfterInsetTarget: [Any]]()
                // hot礼物
                //: if let array: Array = data["gifts"] as? Array<Any> {
                if let array: Array = data[(String(k_temporaryMsg))] as? [Any] {
                    //: var hotArr = [Any]()
                    var hotArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<ReportedMeasurable>.deserialize(from: array as? Array) {
                        //: hotArr.append(contentsOf: datas as [Any])
                        hotArr.append(contentsOf: datas as [Any])
                    }
                    //: dataDict[GiftDataType.Hot] = hotArr
                    dataDict[AvailableAfterInsetTarget.Hot] = hotArr
                }

                // vip礼物
                //: if let array: Array = data["vipGifts"] as? Array<Any> {
                if let array: Array = data[(String(noti_equallyMessage.prefix(8)))] as? [Any] {
                    //: var vipArr = [Any]()
                    var vipArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<ReportedMeasurable>.deserialize(from: array as? Array) {
                    ...
                    dataDict[AvailableAfterInsetTarget.Vip] = vipArr
                }

                // Bags礼物
                //: if let array: Array = data["packGifts"] as? Array<Any> {
                if let array: Array = data[(String(userDetailMsg.suffix(8)) + user_featureAdjustmentPath.replacingOccurrences(of: "business", with: "s"))] as? [Any] {
                    //: var bagsArr = [Any]()
                    var bagsArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<ReportedMeasurable>.deserialize(from: array as? Array) {
                    ...
                    dataDict[AvailableAfterInsetTarget.Bags] = bagsArr
                }
                ...
                let sendItem: Array = data[(String(mainTranslationFormat) + String(constSignalId.prefix(7)))] as! [Any]
                ...
                for i in 0 ..< sendItem.count {
                    ...
                    if let configModel = JSONDeserializer<EaseModelType>.deserializeFrom(dict: sendItem[i] as? [String: Any], designatedPath: nil) {
    ...
    private func rowBy(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = RecognizeReactiveCompatible.share.giftType
        //: TalkingChatRequestTool.req_ChatRoomGiftList(type: type.rawValue) { succeed, result, errorModel in
        PouchReactiveCompatible.storageConversation(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func birthdayCompletion(params: [String: Any], completion: @escaping (_ data: [Any]?) -> Void) {
        //: TalkingChatRequestTool.req_getChatRoomMembers(params: params, completion: { succeed, result, errorModel in
        PouchReactiveCompatible.playAgree(params: params, completion: { succeed, result, _ in
            //: if succeed {
            if succeed {
                ...
                for dict in data {
                    ...
                    if let memberModel = JSONDeserializer<RocketTransformable>.deserializeFrom(dict: dict as? [String: Any], designatedPath: nil) {
    ...
    private func frontBurner(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.live
        let type = LikewiseClusterLiteral.live
        //: TalkingChatRequestTool.req_LiveGiftList(type: type.rawValue) { succeed, result, errorModel in
        PouchReactiveCompatible.headingCompletion(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func assemblage(perPageCount: Int, dataType: AvailableAfterInsetTarget) -> Int {
    ...
    func extractTimeKind(dataType: AvailableAfterInsetTarget) -> [Any] {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
    ...
    func silken() -> [Any] {
        ...
        if RecognizeReactiveCompatible.share.giftType == .groupChat {
            //: return chatRoomSendItems
            return chatRoomSendItems
            //: } else if TalkingChatGiftManager.share.giftType == .live {
        } else if RecognizeReactiveCompatible.share.giftType == .live {
    ...
    func removePitch(dataType: AvailableAfterInsetTarget, dataArr: [Any]) {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
...
extension RecognizeReactiveCompatible {
    ...
    @objc private func scope() {
        //: gift_clearMemoryCache(showRed: true)
        promptRed(showRed: true)
    }
}
...
extension RecognizeReactiveCompatible {
    ...
    func promptRed(showRed: Bool = false) {
        //: TalkingChatGiftManager.share.showBagsRed = showRed
        RecognizeReactiveCompatible.share.showBagsRed = showRed
        //: cacheDict.removeAll()
        cacheDict.removeAll()
    }
}
