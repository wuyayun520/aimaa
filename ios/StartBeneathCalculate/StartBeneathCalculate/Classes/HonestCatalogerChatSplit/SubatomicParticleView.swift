...
fileprivate let noti_buildInjuryValue:[UInt8] = [0x43,0x44,0x43,0x5e,0x2,0x49,0x45,0x4e,0x4f,0x58,0x10,0x3,0xa,0x42,0x4b,0x59,0xa,0x44,0x45,0x5e,0xa,0x48,0x4f,0x4f,0x44,0xa,0x43,0x47,0x5a,0x46,0x4f,0x47,0x4f,0x44,0x5e,0x4f,0x4e]

private func eyelessCoordinate(balance num: UInt8) -> UInt8 {
    return num ^ 42
}

/*: "FFD855" :*/
fileprivate let showFirstFormat:String = "FFDthan"
fileprivate let k_siblingStr:String = "5"
...
class SubatomicParticleView: UIView {
    ...
    var defaultWidth: CGFloat = 0.0 {
        ...
        didSet {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.2) {
                ...
                let minX = self.leftSilder.frame.origin.x + self.defaultWidth - userTimeStr
                ...
                self.displayIn(startX: self.videoStartX)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_buildInjuryValue.map{eyelessCoordinate(balance: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var indicatorView: UIView = {
        ...
        indicator.backgroundColor = UIColor(hex: (showFirstFormat.replacingOccurrences(of: "than", with: "85") + k_siblingStr.capitalized))
        //    indicator.alpha = 0.5
    ...
    private lazy var leftSilder: UIView = {
        ...
        leftSilder.frame = CGRect(x: data_conversationName, y: 0, width: userTimeStr, height: self.frame.size.height)
    ...
    private lazy var rightSlider: UIView = {
        ...
        rightSlider.frame = CGRect(x: self.frame.size.width - userTimeStr, y: 0, width: userTimeStr, height: self.frame.size.height)
    ...
    private lazy var leftCoverView: UIView = {
        ...
        leftCoverView.backgroundColor = UIColor.far(0, 0, 0, 0.6)
        //: leftCoverView.frame = CGRect(x: 0, y: 0, width: LeftMargin, height: self.frame.size.height)
        leftCoverView.frame = CGRect(x: 0, y: 0, width: data_conversationName, height: self.frame.size.height)
    ...
    private lazy var rightCoverView: UIView = {
        ...
        rightCoverView.backgroundColor = UIColor.far(0, 0, 0, 0.6)
...
extension SubatomicParticleView {
    ...
    func displayIn(startX: CGFloat) {
        ...
        rightScope = CGVector(dx: leftSilder.frame.minX + minDurationGap - userTimeStr,
                              //: dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kSliderWidth)
                              dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - userTimeStr)
    }
}
...
extension SubatomicParticleView {
    ...
    @objc func publishGesture(gesture: UIPanGestureRecognizer) {
        ...
        case .ended:
            ...
            self.displayIn(startX: videoStartX)
    ...
    @objc func willGesture(gesture: UIPanGestureRecognizer) {
        //: let thisTimePoint = gesture.location(in: self)
        let thisTimePoint = gesture.location(in: self)
        //: let state = gesture.state
        let state = gesture.state
...
extension SubatomicParticleView {
    ...
    private func alongCookie() {
        ...
        let leftPanGesture = UIPanGestureRecognizer(target: self, action: #selector(publishGesture(gesture:)))
        ...
        let rightPanGesture = UIPanGestureRecognizer(target: self, action: #selector(willGesture(gesture:)))
    ...
    func exceptFamilyXposition(_ xposition: CGFloat) {
