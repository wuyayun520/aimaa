...
fileprivate let show_seasonKey:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "move" :*/
fileprivate let app_handKey:String = "movpeer"
...
class CompareSlashChannel: UIView {
    ...
    var danumuMsgArray: [NameTheoryMeasurable] = []
    //: var danumuItemViewArray: [TalkingGameFloatingScreenItemView] = []
    var danumuItemViewArray: [LocationReactiveCompatible] = []

    //: let TapAction: PublishSubject<TalkingGameFloatingScreenModel?> = PublishSubject()
    let TapAction: PublishSubject<NameTheoryMeasurable?> = PublishSubject()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_seasonKey.reversed(), encoding: .utf8)!)
    }
    ...
    func strip(_ danumuMsgModel: NameTheoryMeasurable?) {
    ...
    @objc func outsidePending(_: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(outsidePending(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension CompareSlashChannel: UIGestureRecognizerDelegate {
    ...
    func createThroughFirst(for danumuMsgModel: NameTheoryMeasurable?, isFirst: Bool) {
        //: let itemView = TalkingGameFloatingScreenItemView(model: danumuMsgModel)
        let itemView = LocationReactiveCompatible(model: danumuMsgModel)
        ...
        var frame = CGRect(x: main_assetUrl, y: isFirst ? 7 : 50, width: itemView.widthValue, height: 24)
        //: var endFrame = frame
        var endFrame = frame
        ...
        if LocalRouterDescription.shared.direction == .rightToLeft {
            ...
            endFrame.origin.x = main_assetUrl
        }
        //: addSubview(itemView)
        addSubview(itemView)
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (app_handKey.replacingOccurrences(of: "peer", with: "e")))
    ...
    func nearby(_ isFirst: Bool) {
        ...
        if danumuMsgArray.count > 0 {
            ...
            if let model {
                ...
                createThroughFirst(for: model, isFirst: isFirst)
