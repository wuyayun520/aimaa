...
fileprivate let app_convertStr:[UInt8] = [0xd7,0xdc,0xd7,0xe2,0x96,0xd1,0xdd,0xd2,0xd3,0xe0,0xa8,0x97,0x8e,0xd6,0xcf,0xe1,0x8e,0xdc,0xdd,0xe2,0x8e,0xd0,0xd3,0xd3,0xdc,0x8e,0xd7,0xdb,0xde,0xda,0xd3,0xdb,0xd3,0xdc,0xe2,0xd3,0xd2]

fileprivate func emptyMic(application num: UInt8) -> UInt8 {
...
/*: "WantView" :*/
fileprivate let dataTemporaryExceptKey:[Character] = ["D","r","o","p","M","e","m","b","e","r","M","e","n","u","C","e","l","l"]

/*: "roomId" :*/
fileprivate let userBlendRegainFormat:String = "roomIdsecurity any tip effective destroy"

/*: "type" :*/
fileprivate let notiBetweenPath:String = "tywell"

/*: "page" :*/
fileprivate let appBehindContent:[Character] = ["p","a","g","e"]

/*: "icon_chats_pcr" :*/
fileprivate let userWithinKey:[Character] = ["i","c","o","n","_","c","h","a","t"]
fileprivate let data_underMessage:String = "manager estimated untils_pcr"

/*: "All Numbers" :*/
fileprivate let k_memberId:[Character] = ["A","l","l"," "]
fileprivate let mainSucceedGiftId:String = "Numbersplayer formation commend drop both"

/*: "all" :*/
fileprivate let data_splitTitle:String = "ALL"

/*: "Recent Gifts" :*/
fileprivate let userDepthKey:String = "import custom decreaseRecent Gi"
fileprivate let mainStandardText:[Character] = ["f","t","s"]
...
// __CLOSE_PRINT__
//
//  ReferencePointViewDelegate.swift
//  AbroadTalking
//
...
protocol RocketAlsoViewDelegate {
...
class ReferencePointViewDelegate: UIView {
    //: var pageIndex = 0
    var pageIndex = 0
    //: var roomID = ""
    var roomID = ""
    //: var menuDelegate: DropDownMemberMenuViewDelegate?
    var menuDelegate: RocketAlsoViewDelegate?
    ...
    public var triangleHeight = Config.ALaCarteExecCreater.triangleHeight
    /**
     *  调整使下拉优先 当向下偏转屏幕距离足够，优先向下偏转
     */
    //: public var adjustPullDown = Config.DropMenuConfig.adjustPullDown
    public var adjustPullDown = Config.ALaCarteExecCreater.adjustPullDown
    ...
    fileprivate lazy var contentView: UIView = self.text()
    //: fileprivate lazy var mTable: UITableView = self.makeTableView()
    fileprivate lazy var mTable: UITableView = self.belowCircle()
    //: fileprivate var anchorRect = CGRect.zero
    fileprivate var anchorRect = CGRect.zero

    //: static let shared = DropDownMemberMenuView(frame: .zero)
    static let shared = ReferencePointViewDelegate(frame: .zero)
    ...
    init(frame: CGRect, anchorView: UIView, roomID: String) {
        ...
        super.init(frame: frame)
        //: let window = MacroBorderDefineThen.getWindow()
        let window = MacroBorderDefineThen.frameworkWindow()
        //: self.roomID = roomID
        self.roomID = roomID
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_convertStr.map{emptyMic(application: $0)}, encoding: .utf8)!)
    }

    //: private func makeContentView() -> UIView {
    private func text() -> UIView {
        //: let view  = UIView()
        let view = UIView()
        //: return view
        return view
    ...
    private func belowCircle() -> UITableView {
    ...
    private func forwardingUi() {
        ...
        mTable.register(WantView.self, forCellReuseIdentifier: (String(dataTemporaryExceptKey)))
    ...
    func databaseData() {
        ...
        var dict = [String: Any]()
        //: dict["roomId"] = roomID
        dict[(String(userBlendRegainFormat.prefix(6)))] = roomID
        //: dict["type"] = "3"
        dict[(notiBetweenPath.replacingOccurrences(of: "well", with: "pe"))] = "3"
        //: dict["page"] = String(pageIndex)
        dict[(String(appBehindContent))] = String(pageIndex)
        ...
        RecognizeReactiveCompatible.share.birthdayCompletion(params: dict, completion: { array in
            ...
            if self.pageIndex == 0 {
                ...
                model.headPic = (String(userWithinKey) + String(data_underMessage.suffix(5)))
                //: model.nickname = "All Numbers".localized
                model.nickname = (String(k_memberId) + String(mainSucceedGiftId.prefix(7))).localized
                //: model.uid = "all"
                model.uid = (data_splitTitle.lowercased())
...
extension ReferencePointViewDelegate: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: (String(dataTemporaryExceptKey)), for: indexPath) as! WantView
        ...
        if recentmenuArray.count > 0 && indexPath.section == 0 {
            ...
            cellModel = recentmenuArray[indexPath.row] as! RocketTransformable
            //: }else {
        } else {
            //: cellModel = menuArray[indexPath.row] as! TalkingChatRoomMemberModel
            cellModel = menuArray[indexPath.row] as! RocketTransformable
        }
        //: cell.setDropMemberMenuCell(model: cellModel)
        cell.asGameShow(model: cellModel)
        //: return cell
        return cell
    }
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        menuDelegate?.processor(model: cellModel)
    }

    //: func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
    func tableView(_: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        //: let headeView = UILabel.init(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: 23))
        let headeView = UILabel(frame: CGRect(x: 0, y: 0, width: main_assetUrl, height: 23))
        //: headeView.backgroundColor = UIColor.appBgColor()
        headeView.backgroundColor = UIColor.changeSibling()
        //: let titleLB = UILabel.init()
        let titleLB = UILabel()
        //: titleLB.frame = CGRect(x: 3, y: 5, width: 100, height: 14)
        titleLB.frame = CGRect(x: 3, y: 5, width: 100, height: 14)
        //: titleLB.font = .pingfangFont(type: .Medium, fontSize: 12)
        titleLB.font = .recipeDecision(type: .Medium, fontSize: 12)
        //: titleLB.textColor = .appValueDetailColor()
        titleLB.textColor = .keepRid()
        //: if recentmenuArray.count > 0 && section == 0 {
        if recentmenuArray.count > 0 && section == 0 {
            //: titleLB.text = "Recent Gifts".localized
            titleLB.text = (String(userDepthKey.suffix(9)) + String(mainStandardText)).localized
            //: }else {
        } else {
            //: titleLB.text = "All Numbers".localized
            titleLB.text = (String(k_memberId) + String(mainSucceedGiftId.prefix(7))).localized
        }
