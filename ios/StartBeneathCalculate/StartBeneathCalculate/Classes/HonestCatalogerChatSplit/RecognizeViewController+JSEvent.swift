...
extension RecognizeViewController {
    ...
    func limitedMessage(_ message: WKScriptMessage) {
        ...
        case .MethodType_customerService:
            //: SmartThen.share.func__pushToPriveteChatVC(chatID: MacroBorderDefineThen.getCustomerServiceID())
            SmartThen.share.aboveCompletion(chatID: MacroBorderDefineThen.receiveRemove())
        ...
        case .MethodType_goMessageDetail:
            //: SmartThen.share.func__pushToPriveteChatVC(chatID: json["uid"].stringValue)
            SmartThen.share.aboveCompletion(chatID: json[(const_regainMessage.replacingOccurrences(of: "growth", with: "i"))].stringValue)

        //: case .MethodType_goPersonInfo:
        case .MethodType_goPersonInfo:
            //: SmartThen.share.func__pushToUserDetailVC(uid: json["uid"].stringValue)
            SmartThen.share.numbererest(uid: json[(const_regainMessage.replacingOccurrences(of: "growth", with: "i"))].stringValue)
    ...
    private func duringAppear(productId: String, source: Int = -1, payType: AttentionType) {
        ...
        CloutThen.share.acquire(name: (String(userNeedScoreData.suffix(5)) + "aseC" + dataMilitaryFormat.lowercased()))
    ...
    private func computeEncouraging(price: Double, payMode: String, scene: String) {
        ...
        CloutThen.share.tidings(payType: payMode, price: price, currency: (String(userDowntownUrl)))
