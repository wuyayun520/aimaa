...
public class InertGasThen: NSObject {
    ...
    private static var _instance: InertGasThen?
    //: class func shared() -> DBUserInfoManager {
    class func arcShared() -> InertGasThen {
        //: guard _instance != nil else {
        guard _instance != nil else {
            //: _instance = DBUserInfoManager()
            _instance = InertGasThen()
            // 添加退出账号通知
...
extension InertGasThen {
    ...
    class func resource(with userIdsArr: [String], completion: @escaping ([ConsumerPenHandyJSON]?, Error?) -> Void) {
        ...
        if InertGasThen.arcShared().needReqUidsSet != nil { // 合并去重
            //: newUserIdsArr = Array(DBUserInfoManager.shared().needReqUidsSet!.union(userIdsArr))
            newUserIdsArr = Array(InertGasThen.arcShared().needReqUidsSet!.union(userIdsArr))
            //: DBUserInfoManager.shared().needReqUidsSet = nil
            InertGasThen.arcShared().needReqUidsSet = nil
        }
    ...
    public class func molarityMessage(message: V2TIMMessage) -> SpireThen {
        ...
        var cacheWrap = SpireThen.familyLayer(with: key)
        //: if cacheWrap == nil {
        if cacheWrap == nil {
            ...
            cacheWrap = SpireThen.exceptAction(info)
        }
        //: return cacheWrap!
        return cacheWrap!
    }
...
extension InertGasThen {
    ...
    class func biologicalAgent(targetId: String) -> ConsumerPenHandyJSON? {
        ...
        return IonInfoTable.afterNet(with: targetId)
    }
    ...
    class func blocked(with userInfo: ConsumerPenHandyJSON?) {
        ...
        let model = InertGasThen.biologicalAgent(targetId: userInfo.uid)
        // 自定义字段需要赋值，防止被覆盖
        ...
        IonInfoTable.soulInfo(userInfo)
        //: DispatchQueue.main.async {
        DispatchQueue.main.async {
    ...
    class func micWith(with targetUid: String) {
        ...
        IonInfoTable.bodyMedium(with: targetUid)
        //: DBUserInfoManager.shared().reqedUidsSet.remove(targetUid)
        InertGasThen.arcShared().reqedUidsSet.remove(targetUid)
    }
}
...
extension InertGasThen {
    ...
    private class func privacyChannel(with userIdsArr: [String], completion: @escaping ([ConsumerPenHandyJSON]?, Error?) -> Void) {
        ...
        let userModels = IonInfoTable.borderTotal(with: userIdsArr)
        ...
        for uid in userIdsArr where !userModels.contains(where: { $0.uid == uid }) {
            ...
            if InertGasThen.arcShared().reqedUidsSet.contains(uid) == false {
        ...
        if !noDBUidArr.isEmpty {
            ...
            InertGasThen.be(with: noDBUidArr)
        }
    }
    ...
    private class func be(with userIdsArr: [String]) {
        ...
        InertGasThen.arcShared().reqedUidsSet = InertGasThen.arcShared().reqedUidsSet.union(userIdsArr)
        ...
        PouchReactiveCompatible.maximumRequireListener(param: [String(bytes: k_milkData.reversed(), encoding: .utf8)!: requestUidsStr]) { succeed, dataArr in
            ...
            guard succeed else { // 失败还原数据
                ...
                InertGasThen.arcShared().reqedUidsSet.subtract(userIdsArr)
                //: return
                return
            }
            // 更新数据库
            //: DBUserInfoManager.updateMsgUserInfoTable(userInfoArr: dataArr)
            InertGasThen.numberelligenceOn(userInfoArr: dataArr)
            // 发送通知
