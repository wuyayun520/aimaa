...
fileprivate let userApplicationId:[Character] = ["T","S","y","s","t","e","m","M","e","s","s","a","g"]
fileprivate let k_computerMovieClothesData:[Character] = ["e","C","e","l","l","_","R","e","u","s","e","I","d"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let kSpendNearbyData:[UInt8] = [0x24,0x23,0x24,0x39,0x65,0x2e,0x22,0x29,0x28,0x3f,0x77,0x64,0x6d,0x25,0x2c,0x3e,0x6d,0x23,0x22,0x39,0x6d,0x2f,0x28,0x28,0x23,0x6d,0x24,0x20,0x3d,0x21,0x28,0x20,0x28,0x23,0x39,0x28,0x29]

/*: "icon_talk_notic" :*/
fileprivate let k_indexName:String = "icon_tshall blink put bolt"
fileprivate let notiAttachId:[Character] = ["a","l","k","_","n","o","t","i","c"]
...
@objc public protocol SmartPerson: NSObjectProtocol {
    /**
     *  长按消息回调
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onLongPressMessage(cell: RatioViewCell)
    func sessionMonth(cell: RatioViewCell)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onRetryMessage(cell: RatioViewCell)
    func phoneMessage(cell: RatioViewCell)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessage(cell: RatioViewCell)
    func renderFlash(cell: RatioViewCell)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessageAvatar(cell: RatioViewCell)
    func roundBass(cell: RatioViewCell)

    /**
     *  点击消息单元中引用消息
     *  @param cell 委托者，消息单元
     */
    //: func onReplyMessage(cell: RatioViewCell)
    func upgradeModify(cell: RatioViewCell)
}

//: @objcMembers public class RatioViewCell: TCommonTableViewCell {
@objcMembers public class RatioViewCell: PanelWillViewCell {
    /**
     *  信息数据类。
     *  存储了该massageCell中所需的信息。包括发送者 ID，发送者头像、信息发送状态、信息气泡图标等。
     *  messageData 详细信息请参考：Section\Chat\CellData\AgoneCellData.h
     */
    //: public var messageData: AgoneCellData?
    public var messageData: AgoneCellData?

    /// 复用cell标识符
    //: @objc public static let TSystemMessageCell_ReuseId = "TSystemMessageCell_ReuseId"
    public static let TSystemMessageCell_ReuseId = (String(userApplicationId) + String(k_computerMovieClothesData))

    /**
     *  协议委托
     *  负责实现 SmartPerson 协议中的功能。
     */
    //: public weak var delegate: TMessageCellDelegate?
    public weak var delegate: SmartPerson?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kSpendNearbyData.map{$0^77}, encoding: .utf8)!)
    }
    ...
    override public func lineStreetwise(with data: CommonalityCellData) {
        //: super.fill(with: data)
        super.lineStreetwise(with: data)
        //: let newData: AgoneCellData = data as! AgoneCellData
        let newData: AgoneCellData = data as! AgoneCellData
    ...
    override public func updateConstraints() {
        ...
        let csize = self.messageData?.write()
    ...
    lazy var avatarView: UIImageView = {
        ...
        let tap1 = UITapGestureRecognizer(target: self, action: #selector(conOn(recognizer:)))
    ...
    lazy var nameLabel: UILabel = {
        ...
        lb.font = UIFont.originally(fontSize: 13)
        //: lb.textColor = .gray
        lb.textColor = .gray
    ...
    public lazy var container: UIView = {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(manual(recognizer:)))
        ...
        let longPress = UILongPressGestureRecognizer(target: self, action: #selector(recover(recognizer:)))
    ...
    lazy var retryView: UIImageView = {
        ...
        imagV.image = UIImage.adName(name: (String(k_indexName.prefix(6)) + String(notiAttachId)))
        ...
        let resendTap = UITapGestureRecognizer(target: self, action: #selector(recognizerRefer(recognizer:)))
...
extension RatioViewCell {
    ...
    @objc func recognizerRefer(recognizer _: UIGestureRecognizer) {
        //: if self.messageData?.status == .Msg_Status_Fail {
        if self.messageData?.status == .Msg_Status_Fail {
            //: self.delegate?.onRetryMessage(cell: self)
            self.delegate?.phoneMessage(cell: self)
        }
    }

    //: @objc func onSelectMessage(recognizer: UIGestureRecognizer) {
    @objc func manual(recognizer _: UIGestureRecognizer) {
