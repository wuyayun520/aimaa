...
fileprivate let appCommandKey:[UInt8] = [0x1c,0x1b,0x1c,0x1,0x5d,0x16,0x1a,0x11,0x10,0x7,0x4f,0x5c,0x55,0x1d,0x14,0x6,0x55,0x1b,0x1a,0x1,0x55,0x17,0x10,0x10,0x1b,0x55,0x1c,0x18,0x5,0x19,0x10,0x18,0x10,0x1b,0x1,0x10,0x11]

private func agendaSocial(equally num: UInt8) -> UInt8 {
    return num ^ 117
}

/*: "icon_lounge" :*/
fileprivate let noti_agentValue:String = "multi bothicon_lou"
fileprivate let constBoardScriptName:[Character] = ["n","g","e"]

/*: "icon_home_v" :*/
fileprivate let data_managerUrl:[Character] = ["i","c","o","n","_","h","o","m","e","_","v"]

/*: "My album" :*/
fileprivate let dataNestMsg:String = "electric people post reportedMy alb"
fileprivate let notiMoonPath:[Character] = ["u","m"]

/*: "0A071F" :*/
fileprivate let data_identifyPath:String = "limited sort go out0A071F"

/*: "#C5BEFF" :*/
fileprivate let dataFrontText:[Character] = ["#","C","5","B","E","F","F"]

/*: "🎉congratulations! She may be very interested in you. She's not far from you. \nWhy don't you say hello to her?" :*/
fileprivate let mainListTitle:[UInt8] = [0xd8,0xb7,0xa6,0xa1,0x4b,0x47,0x46,0x4f,0x5a,0x49,0x5c,0x5d,0x44,0x49,0x5c,0x41,0x47,0x46,0x5b,0x9,0x8,0x7b,0x40,0x4d,0x8,0x45,0x49,0x51,0x8,0x4a,0x4d,0x8,0x5e,0x4d,0x5a,0x51,0x8,0x41,0x46,0x5c,0x4d,0x5a,0x4d,0x5b,0x5c,0x4d,0x4c,0x8,0x41,0x46,0x8,0x51,0x47,0x5d,0x6,0x8,0x7b,0x40,0x4d,0xf,0x5b,0x8,0x46,0x47,0x5c,0x8,0x4e,0x49,0x5a,0x8,0x4e,0x5a,0x47,0x45,0x8,0x51,0x47,0x5d,0x6,0x8,0x22,0x7f,0x40,0x51,0x8,0x4c,0x47,0x46,0xf,0x5c,0x8,0x51,0x47,0x5d,0x8,0x5b,0x49,0x51,0x8,0x40,0x4d,0x44,0x44,0x47,0x8,0x5c,0x47,0x8,0x40,0x4d,0x5a,0x17]

private func ingredientIcon(belly num: UInt8) -> UInt8 {
    return num ^ 40
}

/*: "Why don't you say hello to her?" :*/
fileprivate let const_frameworkWrittenUrl:[UInt8] = [0x2d,0x12,0x3,0x5a,0x1e,0x15,0x14,0x5d,0xe,0x5a,0x3,0x15,0xf,0x5a,0x9,0x1b,0x3,0x5a,0x12,0x1f,0x16,0x16,0x15,0x5a,0xe,0x15,0x5a,0x12,0x1f,0x8,0x45]

private func ginAdvice(play num: UInt8) -> UInt8 {
    return num ^ 122
}

/*: "🎉congratulations! He may be very interested in you. He's not far from you. \nWhy don't you say hello to him?" :*/
fileprivate let k_exceptMessage:[UInt8] = [0x3f,0x6d,0x69,0x68,0x20,0x6f,0x74,0x20,0x6f,0x6c,0x6c,0x65,0x68,0x20,0x79,0x61,0x73,0x20,0x75,0x6f,0x79,0x20,0x74,0x27,0x6e,0x6f,0x64,0x20,0x79,0x68,0x57,0xa,0x20,0x2e,0x75,0x6f,0x79,0x20,0x6d,0x6f,0x72,0x66,0x20,0x72,0x61,0x66,0x20,0x74,0x6f,0x6e,0x20,0x73,0x27,0x65,0x48,0x20,0x2e,0x75,0x6f,0x79,0x20,0x6e,0x69,0x20,0x64,0x65,0x74,0x73,0x65,0x72,0x65,0x74,0x6e,0x69,0x20,0x79,0x72,0x65,0x76,0x20,0x65,0x62,0x20,0x79,0x61,0x6d,0x20,0x65,0x48,0x20,0x21,0x73,0x6e,0x6f,0x69,0x74,0x61,0x6c,0x75,0x74,0x61,0x72,0x67,0x6e,0x6f,0x63,0x89,0x8e,0x9f,0xf0]

/*: "Why don't you say hello to him?" :*/
fileprivate let app_tapValue:[UInt8] = [0x13,0x2c,0x3d,0x64,0x20,0x2b,0x2a,0x63,0x30,0x64,0x3d,0x2b,0x31,0x64,0x37,0x25,0x3d,0x64,0x2c,0x21,0x28,0x28,0x2b,0x64,0x30,0x2b,0x64,0x2c,0x2d,0x29,0x7b]

/*: "#FFD13A" :*/
fileprivate let data_exactlyPath:[Character] = ["#","F","F","D","1","3","A"]

/*: "icon_male_default" :*/
fileprivate let notiParaData:[UInt8] = [0xdc,0xd6,0xda,0xdb,0xea,0xd8,0xd4,0xd9,0xd0,0xea,0xd1,0xd0,0xd3,0xd4,0xc0,0xd9,0xc1]

private func atomicTheory(build num: UInt8) -> UInt8 {
    return num ^ 181
}

/*: "icon_female_default" :*/
fileprivate let showUsedMessage:String = "icoappeal"
fileprivate let mainMaxArenaExitContent:String = "e_defbackground medium"
fileprivate let k_tireRefId:[Character] = ["a","u","l","t"]

/*: "My interests：" :*/
fileprivate let main_partUniformKey:[Character] = ["M","y"," "]
fileprivate let kLensPath:String = "intinterruptr"

/*: "btn_me_edit" :*/
fileprivate let show_minValue:String = "lose question assert herebtn_"
...
class PeerSaverMsgCell: EarthThen {
    //: var matchData: TalkingChatMatchMsgCellData?
    var matchData: SubjectCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: appCommandKey.map{agendaSocial(equally: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.adName(name: (String(noti_agentValue.suffix(8)) + String(constBoardScriptName)))
        //: img.isHidden = true
        img.isHidden = true
        //: return img
        return img
    ...
    private lazy var authImgview: UIImageView = {
        ...
        imgV.image = UIImage.adName(name: (String(data_managerUrl)))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    private lazy var imgTitleLab: UILabel = {
        ...
        label.font = UIFont.originally(fontSize: 15)
        //: label.textColor = UIColor.mottleResult()
        label.textColor = UIColor.mottleResult()
        //: label.text = "My album".localized
        label.text = (String(dataNestMsg.suffix(6)) + String(notiMoonPath)).localized
        //: return label
        return label
        //: }()
    }()
    ...
    private lazy var bgView: UIView = {
        ...
        view.backgroundColor = UIColor(hex: (String(data_identifyPath.suffix(6))))?.withAlphaComponent(0.3)
        //: view.layer.cornerRadius = 12
        view.layer.cornerRadius = 12
        //: view.layer.masksToBounds = true
        view.layer.masksToBounds = true
        ...
        view.layer.borderColor = UIColor(hex: (String(dataFrontText)))?.cgColor
        //: return view
        return view
        //: }()
    }()
    ...
    private lazy var matchLab: UILabel = {
        ...
        var str = String(bytes: mainListTitle.map{ingredientIcon(belly: $0)}, encoding: .utf8)!.localized
        //: var matchStr = "Why don't you say hello to her?".localized
        var matchStr = String(bytes: const_frameworkWrittenUrl.map{ginAdvice(play: $0)}, encoding: .utf8)!.localized
        ...
        if FacultyReactiveCompatible.share.loginUserMode.sex == "2" {
            ...
            str = String(bytes: k_exceptMessage.reversed(), encoding: .utf8)!.localized
            //: matchStr = "Why don't you say hello to him?".localized
            matchStr = String(bytes: app_tapValue.map{$0^68}, encoding: .utf8)!.localized
        }
        ...
        var attriString = NSMutableAttributedString(string: str as String, attributes: [.font: UIFont.originally(fontSize: 15), .foregroundColor: UIColor.white])
        ...
        if let range = str.range(of: matchStr) {
            ...
            attriString.addAttributes([NSAttributedString.Key.foregroundColor: UIColor(hex: (String(data_exactlyPath)))!], range: nsRange)
        }
...
extension PeerSaverMsgCell {
    ...
    override func lineStreetwise(with data: CommonalityCellData) {
        ...
        self.matchData = data as? SubjectCellData
        ...
        // 为空时必须初始化
        //: let userInfoModel = self.matchData?.msgModel.userInfoModel ?? ConsumerPenHandyJSON.init()
        let userInfoModel = self.matchData?.msgModel.userInfoModel ?? ConsumerPenHandyJSON()
        ...
        if self.vipBgView.isHidden == false {
            //: if let vipModel = FacultyReactiveCompatible.share.appUserConfigMode.vipSetting.filter({$0.vipSkinId == userInfoModel.vipSkinId}).first {
            if let vipModel = FacultyReactiveCompatible.share.appUserConfigMode.vipSetting.filter({ $0.vipSkinId == userInfoModel.vipSkinId }).first {
                //: self.vipBgView.setUrlImage(urlStr: vipModel.vipChatSkin, placeImg: nil)
                self.vipBgView.notFit(urlStr: vipModel.vipChatSkin, placeImg: nil)
            }
        }
        //: self.headImgView.setUrlImage(urlStr: userInfoModel.headPic)
        self.headImgView.sortAge(urlStr: userInfoModel.headPic)
        //: self.infoNameLab.textColor = userInfoModel.loungePlus ? .areaEmphasis():.mottleResult()
        self.infoNameLab.textColor = userInfoModel.loungePlus ? .areaEmphasis() : .mottleResult()
        ...
        let imgStr = userInfoModel.sex == ATextLiteral.male.rawValue ? String(bytes: notiParaData.map{atomicTheory(build: $0)}, encoding: .utf8)! : (showUsedMessage.replacingOccurrences(of: "appeal", with: "n") + "_femal" + String(mainMaxArenaExitContent.prefix(5)) + String(k_tireRefId))
        //: sexBtn.setBackgroundImage(UIImage.BundleImageNamed(name: imgStr), for: .normal)
        sexBtn.setBackgroundImage(UIImage.adName(name: imgStr), for: .normal)
        ...
        if userInfoModel.interest.count > 0 {
            ...
            let matchStr = (String(main_partUniformKey) + kLensPath.replacingOccurrences(of: "interrupt", with: "e") + "ests：").localized
            ...
            let attributes = [.font: UIFont.originally(fontSize: 15), .foregroundColor: UIColor.crosswise()] as [NSAttributedString.Key: Any]
            ...
            //: attrString.addAttributes([NSAttributedString.Key.foregroundColor: UIColor.mottleResult()], range: str.range(of: matchStr))
            attrString.addAttributes([NSAttributedString.Key.foregroundColor: UIColor.mottleResult()], range: str.range(of: matchStr))
    ...
    func current() {
        ...
        let maxCount = Int((main_assetUrl - 60) / 62)
        ...
        for index in 0 ..< min(maxCount, matchData.msgModel.userInfoModel.picture.count) {
            ...
            imgV.sortAge(urlStr: matchData.msgModel.userInfoModel.picture[index])
        ...
        if matchData.msgModel.userInfoModel.picture.count > maxCount {
            ...
            imgV.image = UIImage.adName(name: (String(show_minValue.suffix(4)) + "me_edit"))
            //: self.pictureView.addSubview(imgV)
            self.pictureView.addSubview(imgV)
            //: imgV.snp.remakeConstraints { make in
            imgV.snp.remakeConstraints { make in
    ...
    override public func updateConstraints() {
        ...
        if userInfoModel.uid.isEmptyString {
            ...
            self.infoView.snp.remakeConstraints { make in
                ...
                make.width.equalTo(main_assetUrl - 30)
            ...
            if userInfoModel.interest.count > 0 {
                ...
                let inTheight = self.interestsLab.sizeThatFits(CGSize(width: main_assetUrl - 54, height: CGFLOAT_MAX)).height
                ...
                self.interestsLab.snp.remakeConstraints { make in
                    ...
                    make.width.equalTo(main_assetUrl - 54)
        ...
        let labelSize = self.matchLab.sizeThatFits(CGSize(width: main_assetUrl - 30 - 24, height: CGFLOAT_MAX))
...
extension PeerSaverMsgCell {
    ...
    private func margin() {
        ...
        let tapGes = UITapGestureRecognizer(target: self, action: #selector(tapDoingMessage(_:)))
    ...
    @objc func tapDoingMessage(_: UITapGestureRecognizer) {
