...
fileprivate let showProductStr:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_userdetail_header_shadow" :*/
fileprivate let kOwnerFatherContentKey:[Character] = ["i","c","o","n","_","u","s","e","r","d","e","t","a","i","l"]
fileprivate let user_taMetalKey:String = "adopt than_h"
fileprivate let app_reserveMessage:String = "tingadting"

/*: "icon_lounge" :*/
fileprivate let constHappyMessage:[Character] = ["i","c","o","n","_","l","o","u","n","g"]
fileprivate let notiExtentUrl:String = "estimated"

/*: "Official" :*/
fileprivate let app_myAngleFormat:String = "Officialframework none span unique convert"

/*: "Reviewing" :*/
fileprivate let dataSiblingPath:[Character] = ["R"]
fileprivate let app_resultTitle:String = "eviewidirector"

/*: "#FF506D" :*/
fileprivate let appGreetUrl:String = "draw discussion#FF506D"

/*: "icon_user_detail_intimate_header" :*/
fileprivate let noti_endId:[UInt8] = [0x56,0x5c,0x50,0x51,0x60,0x4a,0x4c,0x5a,0x4d,0x60,0x5b,0x5a,0x4b,0x5e,0x56,0x53,0x60,0x56,0x51,0x4b,0x56,0x52,0x5e,0x4b,0x5a,0x60,0x57,0x5a,0x5e,0x5b,0x5a,0x4d]

private func todayReach(stream num: UInt8) -> UInt8 {
    return num ^ 63
}

/*: "?toUid= :*/
fileprivate let dataPartnerId:[Character] = ["?","t","o","U","i","d","="]

/*: "AttentionViewCell" :*/
fileprivate let constGumReadingMsg:String = "TUserDmanual carpet shore required capture"
fileprivate let showActionLabMsg:String = "administrative statin control laterHeade"
fileprivate let mainAvailableValue:[Character] = ["r"]
fileprivate let main_assertName:[Character] = ["I","t","e","m","C","e","l","l"]

/*: "#FFFFFF" :*/
fileprivate let constForwardData:String = "#FFFFFFprevious pop legal require"

/*: "#BDB8B5" :*/
fileprivate let user_remindRoutineText:String = "#BDB8B5square until green"

/*: "head_detail_bottom" :*/
fileprivate let kDescendFormat:[Character] = ["h","e","a","d","_","d","e","t","a","i","l","_","b","o","t","t","o","m"]

/*: "#2DF2FF" :*/
fileprivate let k_minimizeContent:String = "never reader guide join observe#2DF2FF"

/*: "Live" :*/
fileprivate let mainProcedureStr:[Character] = ["L","i","v","e"]

/*: "#48FF1E" :*/
fileprivate let mainUnableText:[Character] = ["#","4","8","F","F","1","E"]

/*: "Online" :*/
fileprivate let notiSecureTitle:String = "Onlinesuper inventory lay appropriate"

/*: "#FF4B7F" :*/
fileprivate let const_passingSomethingKey:[Character] = ["#","F","F","4","B","7","F"]

/*: "busy" :*/
fileprivate let showMainUrl:String = "buunder"
...
let dataSystemName = (302.0 + show_textNextFormat)

//: class TalkingUserDetailHeaderView: UIView {
class TrueDataSource: UIView {
    //: var VideoCallBlock: (() -> Void)?
    var VideoCallBlock: (() -> Void)?
    //: private var userInfoModel = TalkingUserInfoModel()
    private var userInfoModel = SeekGoMeasurable()
    ...
    override init(frame: CGRect) {
        ...
        constraintSubstituteSufficientVisible()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: showProductStr.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var bannerView: JXBanner = {
        ...
        let frame = CGRect(x: 0, y: 0, width: main_assetUrl, height: dataSystemName)
        ...
        view.placeholderImgView.image = UIImage.demur()
    ...
    private lazy var shadowBgView: UIImageView = {
        ...
        let v = UIImageView(image: UIImage.adName(name: (String(kOwnerFatherContentKey) + String(user_taMetalKey.suffix(2)) + app_reserveMessage.replacingOccurrences(of: "ting", with: "e") + "r_shadow")))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var vipBgView: UIImageView = {
        ...
        v.frame = CGRect(x: 0, y: dataSystemName - 15, width: main_assetUrl, height: 15)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var nicknameLab: UILabel = {
        ...
        lab.font = UIFont.recipeDecision(type: .Medium, fontSize: 19)
    ...
    private lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.adName(name: (String(constHappyMessage) + notiExtentUrl.replacingOccurrences(of: "estimated", with: "e")))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var officalImgV: UIButton = {
        ...
        img.setTitle((String(app_myAngleFormat.prefix(8))).localized, for: .normal)
    ...
    private lazy var onlineImgV: SeparateControlReactiveCompatible = {
    ...
    private lazy var liveView: ViewReactiveCompatible = {
        ...
        v.tapGestureBlock = { [weak self] in
            ...
            guard let self = self else { return }
            //: DisplayAperturePresenter.share.func_audiencePushToLiveRoomVC(uid: self.userInfoModel.uid, enterType: .userDetail)
            DisplayAperturePresenter.share.simpleAlong(uid: self.userInfoModel.uid, enterType: .userDetail)
        }
        //: return v
        return v
        //: }()
    ...
    private lazy var reViewingLB: UILabel = {
        ...
        lb.text = (String(dataSiblingPath) + app_resultTitle.replacingOccurrences(of: "director", with: "ng")).localized
        //: lb.textColor = .white
        lb.textColor = .white
        //: lb.backgroundColor = UIColor.init(hex: "#FF506D")
        lb.backgroundColor = UIColor(hex: (String(appGreetUrl.suffix(7))))
    ...
    lazy var lastLoginLab: UILabel = {
        ...
        lb.font = .recipeDecision(type: .Medium, fontSize: 12)
        //: lb.textColor = .white
        lb.textColor = .white
        //: return lb
        return lb
    ...
    private lazy var intimateBtn: SeparateControlReactiveCompatible = {
        ...
        btn.setImage(UIImage.adName(name: String(bytes: noti_endId.map{todayReach(stream: $0)}, encoding: .utf8)!), for: .normal)
        //: btn.isHidden = true
        btn.isHidden = true
        //: btn.addTarget(self, action: #selector(intimateBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(destinationClick), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension TrueDataSource {
    /// 查看视频
    //: private func goToVideo() {
    private func gestureEye() {
        //: let vc = TalkingMomentVideoVC.init(videoPath: self.userInfoModel.videoUrl)
        let vc = AssociatedDirectProject(videoPath: self.userInfoModel.videoUrl)
        //: if FacultyReactiveCompatible.share.loginUid != self.userInfoModel.uid {
        if FacultyReactiveCompatible.share.loginUid != self.userInfoModel.uid {
    ...
    private func voice(_ index: Int) {
        ...
        let vc = SpanScreenVc(momentModel: currenModel, index: index, type: .normal)
        //: self.currentViewController()?.navigationController?.pushViewController(vc, animated: true)
        self.field()?.navigationController?.pushViewController(vc, animated: true)
    }

    //: @objc func intimateBtnClick() {
    @objc func destinationClick() {
        //: let url = TalkingWebManager.getFullUrl(urlType: .Intimate) + "?toUid=\(self.userInfoModel.uid)"
        let url = HandleOptionInstance.hire(urlType: .Intimate) + (String(dataPartnerId)) + "\(self.userInfoModel.uid)"
...
extension TrueDataSource: JXBannerDataSource {
    ...
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        ...
        return JXBannerCellRegister(type: AttentionViewCell.self, reuseIdentifier: (String(constGumReadingMsg.prefix(6)) + "etail" + String(showActionLabMsg.suffix(5)) + String(mainAvailableValue) + String(main_assertName)))
    }

    //: func jxBanner(_ banner: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
    func jxBanner(_: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
        //: let bannerCell = cell as! TUserDetailHeaderItemCell
        let bannerCell = cell as! AttentionViewCell
        //: let urlStr = self.dataArray[index]
        let urlStr = self.dataArray[index]

        //: if index == 0 && userInfoModel.sex == Gender.female.rawValue && userInfoModel.videoUrl.count>1 {
        if index == 0 && userInfoModel.sex == ATextLiteral.female.rawValue && userInfoModel.videoUrl.count > 1 { // 视频
            //: bannerCell.setVideoView()
            bannerCell.computePrepare()
            //: bannerCell.configModel(videoUrl: userInfoModel.videoUrl, imgUrl: userInfoModel.cover)
            bannerCell.creation(videoUrl: userInfoModel.videoUrl, imgUrl: userInfoModel.cover)
            ...
            bannerCell.bgImgView.sortAge(urlStr: urlStr, placeImg: UIImage.ridMale())
        }
    ...
    func jxBanner(_: JXBannerType, centerIndex: Int, centerCell _: UICollectionViewCell) {
        //: guard userInfoModel.uid == FacultyReactiveCompatible.share.loginUid else {
        guard userInfoModel.uid == FacultyReactiveCompatible.share.loginUid else {
            //: return
            return
        }
    ...
    func jxBanner(pageControl _: JXBannerType, numberOfPages _: Int, coverView: UIView, builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder {
        ...
        pageControl.activeColor = UIColor(hex: (String(constForwardData.prefix(7))))!
        //: pageControl.inactiveColor = UIColor(hex: "#BDB8B5")!
        pageControl.inactiveColor = UIColor(hex: (String(user_remindRoutineText.prefix(7))))!
...
extension TrueDataSource: JXBannerDelegate {
    ...
    public func jxBanner(_: JXBannerType, didSelectItemAt index: Int) {
        //: if index == 0 && userInfoModel.sex == Gender.female.rawValue && userInfoModel.videoUrl.count>1 {
        if index == 0 && userInfoModel.sex == ATextLiteral.female.rawValue && userInfoModel.videoUrl.count > 1 {
            //: goToVideo()
            gestureEye()
            ...
            if userInfoModel.sex == ATextLiteral.female.rawValue && userInfoModel.videoUrl.count > 1 && curren > 0 {
                //: curren -= 1
                curren -= 1
            }
            //: goToPhoto(curren)
...
extension TrueDataSource {
    ...
    func operateView(_ userModel: SeekGoMeasurable) {
        ...
        if userModel.sex == ATextLiteral.female.rawValue && userModel.videoUrl.count > 1 {
        ...
        let normalImg = UIImage.adName(name: (String(kDescendFormat))).withTintColor(.white)
        //: if userModel.loungePlus {
        if userModel.loungePlus {
            //: if let vipModel = FacultyReactiveCompatible.share.appUserConfigMode.vipSetting.filter({$0.vipSkinId == userModel.vipSkinId}).first {
            if let vipModel = FacultyReactiveCompatible.share.appUserConfigMode.vipSetting.filter({ $0.vipSkinId == userModel.vipSkinId }).first {
                //: vipBgView.setUrlImage(urlStr: vipModel.vipSkinTop, placeImg: normalImg)
                vipBgView.sortAge(urlStr: vipModel.vipSkinTop, placeImg: normalImg)
            }
            //: } else {
        } else {
        ...
        //: self.nicknameLab.textColor = userModel.loungePlus ? .areaEmphasis():.white
        self.nicknameLab.textColor = userModel.loungePlus ? .areaEmphasis() : .white
        ...
        if userModel.intimateUrl.count > 0 {
            ...
            intimateImgV.notFit(urlStr: userModel.intimateUrl)
        }
        // 在线状态
        ...
        if userModel.livePlayUrl.count > 0 {
            ...
            let image = UIImage.scrape(color: UIColor(hex: (String(k_minimizeContent.suffix(7))))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.oval(), for: .normal)
            onlineImgV.setImage(image.oval(), for: .normal)
            //: onlineImgV.setTitle("Live".localized, for: .normal)
            onlineImgV.setTitle((String(mainProcedureStr)).localized, for: .normal)
            ...
            let image = UIImage.scrape(color: UIColor(hex: (String(mainUnableText)))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.oval(), for: .normal)
            onlineImgV.setImage(image.oval(), for: .normal)
            //: onlineImgV.setTitle("Online".localized, for: .normal)
            onlineImgV.setTitle((String(notiSecureTitle.prefix(6))).localized, for: .normal)
            ...
            let image = UIImage.scrape(color: UIColor(hex: (String(const_passingSomethingKey)))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.oval(), for: .normal)
            onlineImgV.setImage(image.oval(), for: .normal)
            //: onlineImgV.setTitle("busy".localized, for: .normal)
            onlineImgV.setTitle((showMainUrl.replacingOccurrences(of: "under", with: "sy")).localized, for: .normal)
            //: } else {
        } else {
        ...
        // 直播
        //: if FacultyReactiveCompatible.share.loginUid != userModel.uid &&
        if FacultyReactiveCompatible.share.loginUid != userModel.uid,
        ...
        if liveView.isHidden == false {
            ...
            liveView.transformUrl(liveUrl: userModel.livePlayUrl)
        }
    }
    ...
    private func constraintSubstituteSufficientVisible() {
        //: liveView.snp.makeConstraints { make in
        liveView.snp.makeConstraints { make in
            //: make.trailing.equalToSuperview()
            make.trailing.equalToSuperview()
            //: make.top.equalTo(StatusBarHeight+80)
            make.top.equalTo(show_textNextFormat + 80)
        ...
        reViewingLB.snp.makeConstraints { make in
            ...
            make.top.equalToSuperview().offset(show_textNextFormat + 15)
