...
final class LazyPriorityAdapter: TableCodable {
    /// 用户Id【主键】
    //: var uid: String = ""
    var uid: String = ""
    /// 昵称
    ...
    enum CodingKeys: String, CodingTableKey {
        ...
        typealias Root = LazyPriorityAdapter
        //: case uid
        case uid
        //: case nickname
        case nickname
...
extension LazyPriorityAdapter {
    /// ConsumerPenHandyJSON  =>  LazyPriorityAdapter
    //: private class func mapToDBUserInfoData(_ userInfo: ConsumerPenHandyJSON) -> WCDBUserInfoTable {
    private class func root(_ userInfo: ConsumerPenHandyJSON) -> LazyPriorityAdapter {
        //: let dbData = WCDBUserInfoTable()
        let dbData = LazyPriorityAdapter()
    ...
    /// [LazyPriorityAdapter] => [ConsumerPenHandyJSON]
    //: private class func mapToUserInfoModels(_ dbDatas: [WCDBUserInfoTable]) -> [ConsumerPenHandyJSON] {
    private class func date(_ dbDatas: [LazyPriorityAdapter]) -> [ConsumerPenHandyJSON] {
...
extension LazyPriorityAdapter {
    ...
    class func bodyMedium(with userId: String) {
        ...
        try? RangeManager.shared.database?.run(transaction: { _ in
            ...
            let condition = LazyPriorityAdapter.Properties.uid.in(userId)
    ...
    class func afterNet(with userId: String) -> ConsumerPenHandyJSON? {
        ...
        let userInfos = LazyPriorityAdapter.borderTotal(with: [userId])
    ...
    class func borderTotal(with userIds: [String]) -> [ConsumerPenHandyJSON] {
        ...
        do {
            ...
            let condition = LazyPriorityAdapter.Properties.uid.in(userIds)
            ...
            let userInfos: [LazyPriorityAdapter]? = try RangeManager.shared.database?.getObjects(on: LazyPriorityAdapter.Properties.all, fromTable: show_noPath, where: condition)
            //: if userInfos?.isEmpty == false {
            if userInfos?.isEmpty == false {
                //: let models = WCDBUserInfoTable.mapToUserInfoModels(userInfos!)
                let models = LazyPriorityAdapter.date(userInfos!)
                //: return models
                return models
            }
            //: } catch {
    ...
    class func soulInfo(_ userInfo: ConsumerPenHandyJSON) {
        ...
        let dbData = LazyPriorityAdapter.root(userInfo)
        //: try? WCDBManager.shared.database?.run(transaction: { _ in
        try? RangeManager.shared.database?.run(transaction: { _ in
            // 数据库中已存在，则更新数据
            //: if WCDBUserInfoTable.db_getUserInfo(with: dbData.uid) != nil {
            if LazyPriorityAdapter.afterNet(with: dbData.uid) != nil {
                //: do {
                do {
                    //: let condition = WCDBUserInfoTable.Properties.uid == dbData.uid
                    let condition = LazyPriorityAdapter.Properties.uid == dbData.uid
                    ...
                    try RangeManager.shared.database?.update(table: show_noPath,
                                                            ...
                                                            on: LazyPriorityAdapter.Properties.all,
                                                            //: with: dbData,
                                                            with: dbData,
                                                            //: where: condition)
                                                            where: condition)
                    ...
                    ElasticActiveEffect.pathAge(msg: (String(user_organizationMessage.prefix(5))) + "\(show_noPath)" + (String(show_tableValue) + String(appActiveStr.prefix(3))) + "\(error).")
                }
                //: } else {
            } else {
                // 数据库中不存在，则插入数据
                //: WCDBUserInfoTable.db_insertUserInfo(dbData)
                LazyPriorityAdapter.aboveResistance(dbData)
            }
            //: })
        })
    }
    ...
    private class func aboveResistance(_ userInfo: LazyPriorityAdapter) {
        //: WCDBUserInfoTable.db_insertUserInfos([userInfo])
        LazyPriorityAdapter.colorJump([userInfo])
    }

    /// 插入多条数据
    /// - Parameter userInfos: 数据数组
    //: private class func db_insertUserInfos(_ userInfos: [WCDBUserInfoTable]) {
    private class func colorJump(_ userInfos: [LazyPriorityAdapter]) {
        //: try? WCDBManager.shared.database?.run(transaction: { _ in
        try? RangeManager.shared.database?.run(transaction: { _ in
            //: do {
            do {
                ...
                ElasticActiveEffect.pathAge(msg: (String(user_organizationMessage.prefix(5))) + "\(show_noPath)" + (data_reasonFillMsg + "入数据失" + String(showRemainingStr.prefix(8))) + "\(error).")
            }
            //: })
        })
    }
