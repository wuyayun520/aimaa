...
class QuantityThen: SafetyFactorViewController {
    ...
    lazy var mainTableView: UITableView = {
        ...
        table.register(ItemSeparateThen.self, forCellReuseIdentifier: ItemSeparateThen.className())
...
extension QuantityThen {
    ...
    func towardAdd() {
        ...
        TagThen.contain(params: dict) { succeed, result, errorModel in
    ...
    func following() {
        ...
        TagThen.keepBack(params: dict) { succeed, result, errorModel in
            //: ProgressHUD.dismiss()
    ...
    func misquote(params: [String: Any], index: Int) {
        ...
        TagThen.completion(params: params) { [self] succeed, result, _ in
...
//                            }
    ...
    func brisance(cid: String, lastId: String, limit: Int, index: IndexPath) {
        ...
        TagThen.followWeekly(params: dict) { succeed, result, _ in
            //: ProgressHUD.dismiss()
            StrangerProgressHUD.barDismiss()
            //: if succeed {
            if succeed {
    ...
    @objc func stampPadStream() {
        ...
        if self.momentModel.uid == FacultyReactiveCompatible.share.loginUserMode.userID {
            ...
            vc.munuBlock = { [weak self] _, _ in
                ...
                TagThen.paramsCompletionRemoveCulmination(params: dict) { succeed, _, _ in
                    //: ProgressHUD.dismiss()
                    StrangerProgressHUD.barDismiss()
                    //: if succeed {
                    if succeed {
...
extension QuantityThen: PersonThen {
        ...
        if self.momentModel.uid == FacultyReactiveCompatible.share.loginUserMode.userID {
            ...
            vc.munuBlock = { [weak self] _, _ in
                ...
                PersonAlertShow.threeEscape(title: (String(noti_frameworkKey) + String(noti_existingPath)).localized, attributedMessage: NSMutableAttributedString(), leftBtnTitle: (String(user_statinMessage)).localized, rightBtnTitle: (String(mainDayName)).localized) {
                    ...
                    TagThen.corner(params: dict) { succeed, _, _ in
                        //: ProgressHUD.dismiss()
                        StrangerProgressHUD.barDismiss()
                        //: if succeed {
                        if succeed {
...
extension QuantityThen: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let identifier = ItemSeparateThen.className()
        //: var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? TalkingCommentReplyItemCell
        var cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? ItemSeparateThen
        //: if cell == nil {
        if cell == nil {
            //: cell = TalkingCommentReplyItemCell(style: .default, reuseIdentifier: identifier)
            cell = ItemSeparateThen(style: .default, reuseIdentifier: identifier)
        }
        //: let model = self.commentList[indexPath.section]
        let model = self.commentList[indexPath.section]
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        let tcell: ItemSeparateThen = cell as! ItemSeparateThen
