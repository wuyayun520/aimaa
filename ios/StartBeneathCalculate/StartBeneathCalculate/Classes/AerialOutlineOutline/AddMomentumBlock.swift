...
fileprivate let user_resolutionKey:[UInt8] = [0x70,0x75,0x70,0x7b,0x2f,0x6a,0x76,0x6b,0x6c,0x79,0x41,0x30,0x27,0x6f,0x68,0x7a,0x27,0x75,0x76,0x7b,0x27,0x69,0x6c,0x6c,0x75,0x27,0x70,0x74,0x77,0x73,0x6c,0x74,0x6c,0x75,0x7b,0x6c,0x6b]

fileprivate func everyoneConstruct(shirt num: UInt8) -> UInt8 {
...
/*: "icon_coinxxx" :*/
fileprivate let main_enableName:String = "icon_clater route table path"
fileprivate let appProfileWantGivingValue:[Character] = ["o","i","n","x","x","x"]

/*: "icon_gift_num_bg" :*/
fileprivate let const_woundSArenaName:[Character] = ["i","c","o","n","_","g","i","f","t","_","n"]
fileprivate let constAgendaUrl:[Character] = ["u","m","_","b","g"]
...
class AddMomentumBlock: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: user_resolutionKey.map{everyoneConstruct(shirt: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var nameLB: UILabel = {
        ...
        lb.font = UIFont.recipeDecision(type: .Regular, fontSize: 10)
        //: lb.textColor = .white
        lb.textColor = .white
    ...
    lazy var priceBtn: SeparateControlReactiveCompatible = {
        //: let btn = TalkingButton.init()
        let btn = SeparateControlReactiveCompatible()
        //: btn.setTitleColor(UIColor.white, for: .normal)
        btn.setTitleColor(UIColor.white, for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangFont(type: .Regular, fontSize: 10)
        btn.titleLabel?.font = UIFont.recipeDecision(type: .Regular, fontSize: 10)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_coinxxx"), for: .normal)
        btn.setImage(UIImage.adName(name: (String(main_enableName.prefix(6)) + String(appProfileWantGivingValue))), for: .normal)
    ...
    lazy var numBgView: UIImageView = {
        ...
        let imag = UIImage.adName(name: (String(const_woundSArenaName) + String(constAgendaUrl)))
        //: let sizex = imag.size
        let sizex = imag.size
    ...
    lazy var numLb: UILabel = {
        ...
        lb.font = UIFont.recipeDecision(type: .Regular, fontSize: 9)
        //: return lb
        return lb
        //: }()
    }()
...
extension AddMomentumBlock {
    /// 更改选中状态
    //: func setChecked(checked: Bool) {
    func me(checked: Bool) {
        ...
        if checked {
            ...
            bgView.layer.borderColor = UIColor.crosswise().cgColor
    ...
    func on(currenmodel: ReportedMeasurable, giftType: AvailableAfterInsetTarget) {
        //: model = currenmodel
        model = currenmodel
        //: giftIcon.setUrlImage(urlStr: model.imgPreview ?? "")
        giftIcon.notFit(urlStr: model.imgPreview ?? "")
        //: priceBtn.setTitle("\(model.price)", for: .normal)
        priceBtn.setTitle("\(model.price)", for: .normal)
        //: selectedImageView.setUrlImage(urlStr: model.cornerMark, placeImg: nil)
        selectedImageView.notFit(urlStr: model.cornerMark, placeImg: nil)
        ...
        let attributes = [NSAttributedString.Key.font: UIFont.recipeDecision(type: .Regular, fontSize: 11),
...
extension AddMomentumBlock {
    ...
    func fence() {
        ...
        bgView.backgroundColor = UIColor.sewColor()
