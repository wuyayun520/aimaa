...
fileprivate let kPlantMillUrl:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "#171717" :*/
fileprivate let user_whiteMsg:String = "#171717"

/*: "icon_cameraoff" :*/
fileprivate let userDiscountPath:String = "icon_camrecommend each reader"
fileprivate let constSaltValue:String = "ERAOFF"
...
class WillWindowView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        self.backgroundColor = UIColor.far(51, 51, 51, 1)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kPlantMillUrl.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var bgView: UIImageView = {
        ...
        alphaV.backgroundColor = UIColor(hex: (user_whiteMsg.capitalized))
        //: alphaV.alpha = 0.6
        alphaV.alpha = 0.6
        //: v.addSubview(alphaV)
        v.addSubview(alphaV)
    ...
    private lazy var icon: UIImageView = {
        ...
        v.image = UIImage.adName(name: (String(userDiscountPath.prefix(8)) + constSaltValue.lowercased()))
        //: v.isHidden = true
        v.isHidden = true
        //: return v
        return v
...
extension WillWindowView: UIGestureRecognizerDelegate {
    /// 拖拽位移手势
    /// - Parameter pan: 手势
    //: @objc private func dragWindow(pan: UIPanGestureRecognizer) {
    @objc private func processorOval(pan: UIPanGestureRecognizer) {
        ...
        newCenterX = min(newCenterX, main_assetUrl - self.frame.size.width * 0.5)
        //: newCenterY = max(self.frame.size.height * 0.5, newCenterY)
        newCenterY = max(self.frame.size.height * 0.5, newCenterY)
        //: newCenterY = min(newCenterY, ScreenHeight - self.frame.size.height * 0.5)
        newCenterY = min(newCenterY, kTopUrl - self.frame.size.height * 0.5)
    ...
    @objc private func springContainer() {
        //: if self.block != nil {
        if self.block != nil {
            //: self.block!()
            self.block!()
...
extension WillWindowView {
    ...
    private func counteractionDot() {
        // 切换视角手势
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(exchangeRenderContainer))
        let tap = UITapGestureRecognizer(target: self, action: #selector(springContainer))
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(processorOval(pan:)))
        //: pan.delegate = self
        pan.delegate = self
