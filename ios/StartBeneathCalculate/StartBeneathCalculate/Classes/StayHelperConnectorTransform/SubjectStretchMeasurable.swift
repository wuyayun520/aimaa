...
/*: "reply" :*/
fileprivate let data_meetPath:String = "ralbumply"
...
// __CLOSE_PRINT__
//
//  SubjectStretchMeasurable.swift
//  AbroadTalking
//
...
struct SubjectStretchMeasurable: HandyJSON {
    //: var cid: String?
    ...
    var comment = [DigitalAnalogConverterModelType]()
...
extension SubjectStretchMeasurable {
    ...
    mutating func mark() {
        ...
        if replyUser?.count ?? 0 > 0 {
            ...
            if LocalRouterDescription.shared.direction == .leftToRight {
                ...
                tempContent = (data_meetPath.replacingOccurrences(of: "album", with: "e")).localized + "\(replyUser!): \(content!)"
                //: } else {
            } else {
                //: tempContent = "\(content!) :\(replyUser!)" + "reply".localized
                tempContent = "\(content!) :\(replyUser!)" + (data_meetPath.replacingOccurrences(of: "album", with: "e")).localized
            }
        ...
        let sizeheight = Int(CommonDisclaimerList.keyTotal(text: tempContent, size: CGSize(width: main_assetUrl - 70, height: CGFloat(MAXFLOAT)), font: UIFont.recipeDecision(type: .Regular, fontSize: 16), lineNumber: 0).height)
