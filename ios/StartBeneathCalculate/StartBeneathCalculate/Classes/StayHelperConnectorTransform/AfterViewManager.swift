...
class AfterViewManager: NSObject {
    ...
    private lazy var giftView: ShadeView = {
        //: let v = GiftFloatView()
        let v = ShadeView()
        //: AnimateUnaryAllocator.getWindow().addSubview(v)
        AnimateUnaryAllocator.frameworkWindow().addSubview(v)
        //: return v
        return v
        //: }()
    }()
...
extension AfterViewManager {
    ...
    private func setOutThreadOutlet(_ model: MicromillimeterBroadcastModel) {
        ...
        let isRTL = LocalRouterDescription.shared.direction == .rightToLeft
        // 起始位置：屏幕外
